...
enum list_type { SJL = 0, MJL, PJL, FJL, NJLIST, ZJL, ALLJLIST };
...
#define FORALL_PRMPT_RSRCS(resn)                                               \
    if (pRMOPtr != NULL) {                                                     \
        int _pRMOindex;                                                        \
        for (_pRMOindex = 0; _pRMOindex < pRMOPtr->numPreemptResources;        \
             _pRMOindex++) {                                                   \
...
#define JOB_PEND(s)                                                            \
    (((s)->jStatus & JOB_STAT_PEND) && !((s)->jStatus & JOB_STAT_PSUSP))
...
#define FOR_EACH_JOB_LOCAL_EXECHOST(Host, Job)                                 \
    {                                                                          \
        ...
        for (__hidx__ = 0; __hidx__ < (Job)->numHostPtr; __hidx__++) {         \
            ...
            if (Host == NULL)                                                  \
                break;                                                         \
            if (Host->hStatus & HOST_STAT_REMOTE)                              \
                continue;
...
#define JOB_RSVSLOT_NONPRMPT(Job) ((!(Job)->jStatus & JOB_STAT_PEND))
...
#define USER_GROUP_IS_ALL_USERS(UserGroup)                                     \
    ((UserGroup)->numGroups == 0 && (UserGroup)->memberTab.numEnts == 0)
...
#define FOR_EACH_UGRP_CHILD_USER(User, Grp)                                    \
    if ((User)->children != NULL) {                                            \
        ...
        setIteratorAttach(&__iter__, (User)->children,                         \
                          "FOR_EACH_UGRP_CHILD_USER");                         \
        for (Grp = (struct uData *) setIteratorBegin(&__iter__); Grp != NULL;  \
...
#define FOR_EACH_UGRP_DESCENDANT_USER(Grp, User)                               \
    if ((User)->children != NULL) {                                            \
        ...
        LS_BITSET_ITERATOR_T __iter__;                                         \
        BITSET_ITERATOR_ZERO_OUT(&__iter__);                                   \
        setIteratorAttach(&__iter__, (Grp)->descendants,                       \
                          "FOR_EACH_UGRP_CHILD_USER");                         \
...
#define FOR_EACH_USER_ANCESTOR_UGRP(User, Grp)                                 \
    if ((User)->ancestors != NULL) {                                           \
        ...
        LS_BITSET_ITERATOR_T __iter__;                                         \
        BITSET_ITERATOR_ZERO_OUT(&__iter__);                                   \
        setIteratorAttach(&__iter__, (User)->ancestors,                        \
                          "FOR_EACH_USER_ANCESTOR_UGRP");                      \
        for (Grp = (struct uData *) setIteratorBegin(&__iter__); Grp != NULL;  \
             Grp = (struct uData *) setIteratorGetNextElement(&__iter__)) {
...
extern void getTclHostData(struct tclHostData *, struct hData *,
                           struct hData *);
...
extern int arrayRequeue(struct jData *, struct signalReq *, struct lsfAuth *);

extern int modifyJob(struct modifyReq *, struct submitMbdReply *,
                     struct lsfAuth *);
...
extern void jobStatusSignal(sbdReplyType reply, struct jData *jData,
                            int sigValue, int chkFlags,
...
extern int cntNumPrmptSlots(struct qData *, struct hData *, struct uData *,
                            int *numAvailSUSP);
...
extern void updSwitchJob(struct jData *, struct qData *, struct qData *, int);
...
extern struct hostAcct *addHAcct(struct hTab **, struct hData *, int, int, int,
                                 int);
...
extern void log_switchjob(struct jobSwitchReq *switchReq, int uid,
                          char *userName);
...
extern float getUsablePRHQValue(int, struct hData *, struct qData *,
                                struct resourceInstance **);
extern float getAvailableByPreemptPRHQValue(int, struct hData *,
                                            struct qData *);
extern float getReservedByWaitPRHQValue(int, struct hData *, struct qData *);
extern float takeAvailableByPreemptPRHQValue(int, float, struct hData *,
                                             struct qData *);
extern void addRunJobUsedPRHQValue(int, float, struct hData *, struct qData *);
extern void removeRunJobUsedPRHQValue(int, float, struct hData *,
                                      struct qData *);
extern void addReservedByWaitPRHQValue(int, float, struct hData *,
                                       struct qData *);
extern void removeReservedByWaitPRHQValue(int, float, struct hData *,
                                          struct qData *);
extern void addAvailableByPreemptPRHQValue(int, float, struct hData *,
                                           struct qData *);
extern void removeAvailableByPreemptPRHQValue(int, float, struct hData *,
                                              struct qData *);
...
#define QUEUE_LIMIT(qPtr, i)                                                   \
    ((qPtr)->defLimits[i] > 0 ? (qPtr)->defLimits[i] : (qPtr)->rLimits[i])
#define LIMIT_OF_JOB(jp, i)                                                    \
    ((jp)->shared->jobBill.rLimits[i] > 0                                      \
         ? (jp)->shared->jobBill.rLimits[i]                                    \
         : ((jp)->qPtr->defLimits[i] > 0 ? (jp)->qPtr->defLimits[i]            \
                                         : (jp)->qPtr->rLimits[i]))
...
#define HAS_RUN_WINDOW(qp) (((qp)->windows != NULL) && (qp)->windows[0] != '\0')
    ...
    {                                                                          \
        ...
        if ((job)->groupCands != NULL) {                                       \
            for (jj = 0; jj < (job)->numOfGroups; jj++) {                      \
                FREE_IND_CANDPTR((job)->groupCands[jj].members,                \
                                 (job)->groupCands[jj].numOfMembers);          \
            }                                                                  \
        }                                                                      \
...
int mbd_handle_new_job_reply(struct mbd_client_node *,
                             XDR *,
                             struct packet_header *);
int mbd_handle_job_status(struct mbd_client_node *,
                          XDR *,
                          struct packet_header *);
int mbd_handle_sbd_disconnect(struct mbd_client_node *);
