...
#define LSB_HOST_CLOSED(status)                                                \
    ((status &                                                                 \
      (HOST_STAT_WIND | HOST_STAT_DISABLED | HOST_STAT_LOCKED |                \
       HOST_STAT_LOCKED_MASTER | HOST_STAT_FULL | HOST_STAT_NO_LIM)) != 0)
...
#define LSB_ARRAY_IDX(jobId)                                                   \
    (((jobId) == -1)                                                           \
         ? (0)                                                                 \
         : (int) (((LS_UNS_LONG_INT) jobId >> 32) & LSB_MAX_ARRAY_IDX))
...
extern struct hostConf *lsb_readhost(struct lsConf *, struct lsInfo *, int,
                                     struct clusterConf *);
extern struct queueConf *lsb_readqueue(struct lsConf *, struct lsInfo *, int,
                                       struct sharedConf *);
...
extern struct queueInfoEnt *lsb_queueinfo(char **queues, int *numQueues,
...
extern struct lsbSharedResourceInfo *lsb_sharedresourceinfo(char **, int *,
                                                            char *, int);
