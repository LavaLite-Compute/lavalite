...
struct eventLogHandle *lsb_openelog(struct eventLogFile *ePtr, int *lineNum)
{
    ...
    if (curOpenFile >= 0) {
        ...
        if ((elog_fp = fopen(eventFile, "r")) == NULL) {
            ls_syslog(LOG_ERR, "FUNC_S_FAIL_M", fname, "fopen", eventFile);
            return NULL;
        }
        ...
        if (curOpenFile == 0) {
            ...
            if (fseek(elog_fp, pos, SEEK_SET) != 0)
                ls_syslog(LOG_ERR, "FUNC_D_FAIL_M", fname, "fseek", pos);
        }
    } else {
...
struct eventRec *lsb_getelogrec(struct eventLogHandle *ePtr, int *lineNum)
{
    ...
    if (logRec == NULL && ePtr->lastOpenFile >= 0 &&
        ...
        if ((newfp = fopen(eventFile, "r")) == NULL) {
            ...
            if (fscanf(newfp, "%c%d ", &ch, &pos) != 2 || ch != '#') {
                ls_syslog(LOG_ERR,
                          "%s: fscanf(%s) failed: old event file format", fname,
                          eventFile);
                pos = 0;
            } else {
                *lineNum = 1;
...
void lsb_closeelog(struct eventLogHandle *eventLogHandle)
{
...
struct eventRec *lsb_geteventrec(FILE *log_fp, int *LineNum)
{
    ...
    if ((ccount = stripQStr(line, namebuf)) < 0 ||
        (int) strlen(line) == ccount || (int) strlen(namebuf) >= LL_BUFSIZ_32) {
...
static void freeLogRec(struct eventRec *logRec)
{
    int i;
...
static int readJobNew(char *line, struct jobNewLog *jobNewLog)
{
    int i, cc, ccount;
    int tmpSubmit, tmpBegin, tmpTerm;

    cc = sscanf(line, "%d%d%d%d%d%d%d%d%d%d%n", &(jobNewLog->jobId),
                &(jobNewLog->userId), &(jobNewLog->options),
                &(jobNewLog->numProcessors), &(tmpSubmit), &(tmpBegin),
                &(tmpTerm), &(jobNewLog->sigValue), &(jobNewLog->chkpntPeriod),
                &(jobNewLog->restartPid), &ccount);
    ...
    cc = sscanf(line, "%f%d%n", &jobNewLog->hostFactor, &jobNewLog->umask,
                &ccount);
    if (cc != 2)
        return LSBE_EVENT_FORMAT;
    line += ccount + 1;
    ...
    if (jobNewLog->numAskedHosts > 0) {
        jobNewLog->askedHosts =
            (char **) calloc(jobNewLog->numAskedHosts, sizeof(char *));
    ...
    if (jobNewLog->nxf > 0) {
        jobNewLog->xf =
            (struct xFile *) calloc(jobNewLog->nxf, sizeof(struct xFile));
    ...
    cc = sscanf(line, "%d%d%n", &(jobNewLog->options2), &(jobNewLog->idx),
                &ccount);
    if (cc != 2)
        return LSBE_EVENT_FORMAT;
...
static int readJobMod(char *line, struct jobModLog *jobModLog)
{
    int i, cc, ccount;

    saveQStr(line, jobModLog->jobIdStr);

    cc = sscanf(line, "%d%d%d%d%n", &(jobModLog->options),
                &(jobModLog->options2), &(jobModLog->delOptions),
                &(jobModLog->userId), &ccount);
    if (cc != 4) {
        return LSBE_EVENT_FORMAT;
    }
    line += ccount + 1;
    ...
    cc = sscanf(line, "%d%d%d%d%d%d%d%n", &(jobModLog->submitTime),
                &(jobModLog->umask), &(jobModLog->numProcessors),
                &(jobModLog->beginTime), &(jobModLog->termTime),
                &(jobModLog->sigValue), &(jobModLog->restartPid), &ccount);
    if (cc != 7)
        return LSBE_EVENT_FORMAT;
    line += ccount + 1;
    ...
    if (jobModLog->options & SUB_HOST) {
        ...
        if (jobModLog->numAskedHosts) {
            jobModLog->askedHosts =
                (char **) calloc(jobModLog->numAskedHosts, sizeof(char *));
    ...
    if (jobModLog->options & SUB_OTHER_FILES) {
        ...
        if (jobModLog->nxf > 0) {
            jobModLog->xf =
                (struct xFile *) calloc(jobModLog->nxf, sizeof(struct xFile));
    ...
    cc = sscanf(line, "%d%d%n", &(jobModLog->niosPort),
                &(jobModLog->maxNumProcessors), &ccount);
    if (cc != 2)
        return LSBE_EVENT_FORMAT;
    line += ccount + 1;
...
static int readJobStart(char *line, struct jobStartLog *jobStartLog)
{
    static char fname[] = "readJobStart";
    int i, cc, ccount;

    cc = sscanf(line, "%d%d%d%d%f%d%n", &(jobStartLog->jobId),
                &(jobStartLog->jStatus), &(jobStartLog->jobPid),
                &(jobStartLog->jobPGid), &(jobStartLog->hostFactor),
    ...
    if (jobStartLog->numExHosts == 0) {
        ls_syslog(LOG_ERR,
                  "%s: The number of execution hosts is zero for job <%d>",
                  fname, jobStartLog->jobId);
        return LSBE_EVENT_FORMAT;
    }

    jobStartLog->execHosts =
        (char **) calloc(jobStartLog->numExHosts, sizeof(char *));
...
static int readJobStartAccept(char *line,
                              struct jobStartAcceptLog *jobStartAcceptLog)
{
    int cc, ccount;

    cc = sscanf(line, "%d%d%d%n", &(jobStartAcceptLog->jobId),
                &(jobStartAcceptLog->jobPid), &(jobStartAcceptLog->jobPGid),
                &ccount);
    if (cc != 3)
        return LSBE_EVENT_FORMAT;
    line += ccount + 1;
...
static int readJobExecute(char *line, struct jobExecuteLog *jobExecuteLog)
{
    int cc, ccount;

    cc = sscanf(line, "%d%d%d%n", &(jobExecuteLog->jobId),
...
static int readJobStatus(char *line, struct jobStatusLog *jobStatusLog)
{
    int cc, ccount;
    int tmpEnd;

    cc = sscanf(line, "%d%d%d%d%f%d%d%n", &(jobStatusLog->jobId),
                &(jobStatusLog->jStatus), &(jobStatusLog->reason),
                &(jobStatusLog->subreasons), &(jobStatusLog->cpuTime),
...
static int readSbdJobStatus(char *line, struct sbdJobStatusLog *jobStatusLog)
{
    int cc, ccount;
    int tmpActPeriod;

    cc = sscanf(line, "%d%d%d%d%d%d%d%d%d%d%d%d%n", &(jobStatusLog->jobId),
                &(jobStatusLog->jStatus), &(jobStatusLog->reasons),
                &(jobStatusLog->subreasons), &(jobStatusLog->actPid),
                &(jobStatusLog->actValue), &(tmpActPeriod),
                &(jobStatusLog->actFlags), &(jobStatusLog->actStatus),
                &(jobStatusLog->actReasons), &(jobStatusLog->actSubReasons),
                &(jobStatusLog->idx), &ccount);
    if (cc != 12)
        return LSBE_EVENT_FORMAT;
...
static int readMig(char *line, struct migLog *migLog)
{
    int cc, ccount, i;
    char hName[MAXHOSTNAMELEN];

    cc = sscanf(line, "%d%d%n", &(migLog->jobId), &(migLog->numAskedHosts),
                &ccount);
    if (cc != 2)
        return LSBE_EVENT_FORMAT;
    ...
    if (migLog->numAskedHosts) {
        migLog->askedHosts =
            (char **) calloc(migLog->numAskedHosts, sizeof(char *));
...
static int readJobSigAct(char *line, struct sigactLog *sigactLog)
{
    int cc, ccount;
    int tmpPeriod;
    char sigSymbol[MAXLINELEN];

    cc = sscanf(line, "%d%d%d%d%d%d%d%n", &(sigactLog->jobId), &(tmpPeriod),
                &(sigactLog->pid), &(sigactLog->jStatus), &(sigactLog->reasons),
...
static int readJobRequeue(char *line, struct jobRequeueLog *jobRequeueLog)
{
    int cc, ccount;
...
static int readJobClean(char *line, struct jobCleanLog *jobCleanLog)
{
    int cc, ccount;

    cc = sscanf(line, "%d%d%n", &(jobCleanLog->jobId), &(jobCleanLog->idx),
                &ccount);
    if (cc != 2)
        return LSBE_EVENT_FORMAT;
...
static int readChkpnt(char *line, struct chkpntLog *chkpntLog)
{
    int cc, ccount;
    int tmpPeriod;

    cc = sscanf(line, "%d%d%d%d%d%n", &(chkpntLog->jobId), &(tmpPeriod),
                &(chkpntLog->pid), &(chkpntLog->ok), &(chkpntLog->flags),
                &ccount);
    if (cc != 5)
        return LSBE_EVENT_FORMAT;
...
static int readJobSwitch(char *line, struct jobSwitchLog *jobSwitchLog)
{
    int cc, ccount;

    cc = sscanf(line, "%d%d%n", &(jobSwitchLog->userId), &(jobSwitchLog->jobId),
                &ccount);
    if (cc != 2)
        return LSBE_EVENT_FORMAT;
...
static int readJobMove(char *line, struct jobMoveLog *jobMoveLog)
{
    int cc;
    int ccount;

    cc = sscanf(line, "%d%d%d%d%n", &(jobMoveLog->userId), &(jobMoveLog->jobId),
...
static int readQueueCtrl(char *line, struct queueCtrlLog *queueCtrlLog)
{
    int cc, ccount;
...
static int readHostCtrl(char *line, struct hostCtrlLog *hostCtrlLog)
{
    int cc, ccount;
...
static int readMbdDie(char *line, struct mbdDieLog *mbdDieLog)
{
    int cc;
...
static int readUnfulfill(char *line, struct unfulfillLog *unfulfillLog)
{
    int cc;
    int ccount;
    int tmpChkPeriod;

    cc = sscanf(line, "%d%d%d%d%d%d%d%n", &(unfulfillLog->jobId),
                &(unfulfillLog->notSwitched), &(unfulfillLog->sig),
                &(unfulfillLog->sig1), &(unfulfillLog->sig1Flags),
...
static int readLoadIndex(char *line, struct loadIndexLog *loadIndexLog)
{
    int cc, ccount, i;
    ...
    if ((loadIndexLog->name =
             (char **) calloc(loadIndexLog->nIdx, sizeof(char *))) == NULL) {
        loadIndexLog->nIdx = 0;
        return LSBE_NO_MEM;
    }
...
static int readJobFinish(char *line, struct jobFinishLog *jobFinishLog,
                         time_t eventTime)
{
    int i, cc, ccount;
    ...
    cc = sscanf(line, "%d%d%d%d%d%d%d%d%n", &(jobFinishLog->jobId),
                &(jobFinishLog->userId), &(jobFinishLog->options),
                &(jobFinishLog->numProcessors), &(tmpSubmit), &(tmpBegin),
                &(tmpTerm), &(tmpStart), &ccount);
    if (cc != 8)
        return LSBE_EVENT_FORMAT;
    ...
    if (jobFinishLog->numAskedHosts > 0) {
        jobFinishLog->askedHosts =
            (char **) calloc(jobFinishLog->numAskedHosts, sizeof(char *));
    ...
    if (jobFinishLog->numExHosts > 0) {
        jobFinishLog->execHosts =
            (char **) calloc(jobFinishLog->numExHosts, sizeof(char *));
    ...
    if ((cc = sscanf(line, "%d%n", &jobFinishLog->maxNumProcessors, &ccount)) !=
        1)
        return LSBE_EVENT_FORMAT;
    line += ccount + 1;
...
static int writeJobNew(FILE *log_fp, struct jobNewLog *jobNewLog)
{
    int i;
    ...
    if (fprintf(log_fp, " %d %d %d %d %d %d %d %d %d %d", jobNewLog->jobId,
                jobNewLog->userId, jobNewLog->options, jobNewLog->numProcessors,
                (int) jobNewLog->submitTime, (int) jobNewLog->beginTime,
                (int) jobNewLog->termTime, jobNewLog->sigValue,
...
static int writeJobMod(FILE *log_fp, struct jobModLog *jobModLog)
{
    int i;
    ...
    if (fprintf(log_fp, " %d %d %d", jobModLog->options, jobModLog->options2,
                jobModLog->delOptions) < 0)
        return LSBE_SYS_CALL;

    if ((fprintf(log_fp, " %d", jobModLog->userId) < 0) ||
        (addQStr(log_fp, jobModLog->userName) < 0))
        return LSBE_SYS_CALL;
    if (fprintf(log_fp, " %d %d %d %d %d %d %d", jobModLog->submitTime,
                jobModLog->umask, jobModLog->numProcessors,
                jobModLog->beginTime, jobModLog->termTime, jobModLog->sigValue,
    ...
    if (fprintf(log_fp, " %d %d", jobModLog->niosPort,
                jobModLog->maxNumProcessors) < 0)
        return LSBE_SYS_CALL;
...
static int writeJobStart(FILE *log_fp, struct jobStartLog *jobStartLog)
{
    int i;

    if (fprintf(log_fp, " %d %d %d %d %3.1f %d", jobStartLog->jobId,
                jobStartLog->jStatus, jobStartLog->jobPid, jobStartLog->jobPGid,
...
static int writeJobStartAccept(FILE *log_fp,
                               struct jobStartAcceptLog *jobStartAcceptLog)
{
...
static int writeJobExecute(FILE *log_fp, struct jobExecuteLog *jobExecuteLog)
{
    if (fprintf(log_fp, " %d %d %d", jobExecuteLog->jobId,
...
static int writeJobStatus(FILE *log_fp, struct jobStatusLog *jobStatusLog)
{
    if (fprintf(log_fp, " %d %d %d %d %4.4f %d", jobStatusLog->jobId,
                jobStatusLog->jStatus, jobStatusLog->reason,
                jobStatusLog->subreasons, jobStatusLog->cpuTime,
                (int) jobStatusLog->endTime) < 0)
        return LSBE_SYS_CALL;
...
static int writeSbdJobStatus(FILE *log_fp, struct sbdJobStatusLog *jobStatusLog)
{
...
static int writeJobSwitch(FILE *log_fp, struct jobSwitchLog *jobSwitchLog)
{
    if (fprintf(log_fp, " %d %d", jobSwitchLog->userId, jobSwitchLog->jobId) <
        0)
        return LSBE_SYS_CALL;
...
static int writeMig(FILE *log_fp, struct migLog *migLog)
{
    int i;
...
static int writeJobSigAct(FILE *log_fp, struct sigactLog *sigactLog)
{
    if (fprintf(log_fp, " %d %d %d %d %d %d %d", sigactLog->jobId,
                (int) sigactLog->period, sigactLog->pid, sigactLog->jStatus,
                sigactLog->reasons, sigactLog->flags, sigactLog->actStatus) < 0)
        return LSBE_SYS_CALL;
...
static int writeJobRqueue(FILE *log_fp, struct jobRequeueLog *jobRequeueLog)
{
...
static int writeJobClean(FILE *log_fp, struct jobCleanLog *jobCleanLog)
{
...
static int writeChkpnt(FILE *log_fp, struct chkpntLog *chkpntLog)
{
    if (fprintf(log_fp, " %d %d %d %d %d %d\n", chkpntLog->jobId,
                (int) chkpntLog->period, chkpntLog->pid, chkpntLog->ok,
                chkpntLog->flags, chkpntLog->idx) < 0)
        return LSBE_SYS_CALL;

    return LSBE_NO_ERROR;
}

static int writeJobMove(FILE *log_fp, struct jobMoveLog *jobMoveLog)
{
    if (fprintf(log_fp, " %d %d %d %d %d", jobMoveLog->userId,
                jobMoveLog->jobId, jobMoveLog->position, jobMoveLog->base,
                jobMoveLog->idx) < 0)
        return LSBE_SYS_CALL;
...
static int writeQueueCtrl(FILE *log_fp, struct queueCtrlLog *queueCtrlLog)
{
...
static int writeHostCtrl(FILE *log_fp, struct hostCtrlLog *hostCtrlLog)
{
...
static int writeMbdDie(FILE *log_fp, struct mbdDieLog *mbdDieLog)
{
...
static int writeUnfulfill(FILE *log_fp, struct unfulfillLog *unfulfillLog)
{
    if (fprintf(log_fp, " %d %d %d %d %d %d %d %d\n", unfulfillLog->jobId,
                unfulfillLog->notSwitched, unfulfillLog->sig,
                unfulfillLog->sig1, unfulfillLog->sig1Flags,
                (int) unfulfillLog->chkPeriod, unfulfillLog->notModified,
                unfulfillLog->idx) < 0)
        return LSBE_SYS_CALL;
...
static int writeLoadIndex(FILE *log_fp, struct loadIndexLog *loadIndexLog)
{
    int i;
...
static int writeJobFinish(FILE *log_fp, struct jobFinishLog *jobFinishLog)
{
    int i;

    if (fprintf(log_fp, " %d %d %d %d %d %d %d %d", jobFinishLog->jobId,
                jobFinishLog->userId, jobFinishLog->options,
                jobFinishLog->numProcessors, (int) jobFinishLog->submitTime,
                (int) jobFinishLog->beginTime, (int) jobFinishLog->termTime,
    ...
    if (fprintf(log_fp, " %d %3.1f", jobFinishLog->jStatus,
...
static int writeMbdStart(FILE *log_fp, struct mbdStartLog *mbdStartLog)
{
...
static int readMbdStart(char *line, struct mbdStartLog *mbdStartLog)
{
    int cc;
...
static int readLogSwitch(char *line, struct logSwitchLog *logSwitchLog)
{
    int cc, ccount;
...
static int writeJobSignal(FILE *log_fp, struct signalLog *signalLog)
{
    if (fprintf(log_fp, " %d %d %d", signalLog->jobId, signalLog->userId,
...
static int writeJobMsg(FILE *log_fp, struct jobMsgLog *jobMsgLog)
{
    if (fprintf(log_fp, " %d %d %d %d", jobMsgLog->usrId, jobMsgLog->jobId,
                jobMsgLog->msgId, jobMsgLog->type) < 0)
        return LSBE_SYS_CALL;
...
static int writeJobMsgAck(FILE *log_fp, struct jobMsgLog *jobMsgLog)
{
    if (fprintf(log_fp, " %d %d %d %d", jobMsgLog->usrId, jobMsgLog->jobId,
                jobMsgLog->msgId, jobMsgLog->type) < 0)
        return LSBE_SYS_CALL;
...
static int readJobSignal(char *line, struct signalLog *signalLog)
{
    int cc, ccount;
    char sigSymbol[MAXLINELEN];

    cc = sscanf(line, "%d%d%d%n", &(signalLog->jobId), &(signalLog->userId),
                &(signalLog->runCount), &ccount);
    if (cc != 3)
        return LSBE_EVENT_FORMAT;
    line += ccount + 1;
    if ((ccount = stripQStr(line, sigSymbol)) < 0)
...
static int readJobMsg(char *line, struct jobMsgLog *jobMsgLog)
{
    int cc, ccount;
    char strBuf[MAXLINELEN];

    cc = sscanf(line, "%d%d%d%d%n", &(jobMsgLog->usrId), &(jobMsgLog->jobId),
...
static int readJobMsgAck(char *line, struct jobMsgAckLog *jobMsgAckLog)
{
    int cc, ccount;
    char strBuf[MAXLINELEN];

    cc = sscanf(line, "%d%d%d%d%n", &(jobMsgAckLog->usrId),
                &(jobMsgAckLog->jobId), &(jobMsgAckLog->msgId),
                &(jobMsgAckLog->type), &ccount);
...
static int writeJobForce(FILE *log_fp,
                         struct jobForceRequestLog *jobForceRequestLog)
{
    int i;

    if (fprintf(log_fp, " %d %d %d %d %d ", jobForceRequestLog->jobId,
                jobForceRequestLog->userId, jobForceRequestLog->idx,
...
static int writeLogSwitch(FILE *log_fp, struct logSwitchLog *logSwitchLog)
{
...
static int readJobForce(char *line,
                        struct jobForceRequestLog *jobForceRequestLog)
{
    int i;
    int cc;
    ...
    cc = sscanf(line, "%d%d%d%d%d%n", &(jobForceRequestLog->jobId),
                &(jobForceRequestLog->userId), &(jobForceRequestLog->idx),
                &(jobForceRequestLog->options),
...
static int writeJobAttrSet(FILE *log_fp, struct jobAttrSetLog *jobAttrSetLog)
{
    if (fprintf(log_fp, " %d %d %d %d ", jobAttrSetLog->jobId,
                jobAttrSetLog->idx, jobAttrSetLog->uid,
...
static int readJobAttrSet(char *line, struct jobAttrSetLog *jobAttrSetLog)
{
    int ccount;

    if (sscanf(line, "%d%d%d%d%n", &(jobAttrSetLog->jobId),
               &(jobAttrSetLog->idx), &(jobAttrSetLog->uid),
               &(jobAttrSetLog->port), &ccount) != 4)
        return LSBE_EVENT_FORMAT;
    return LSBE_NO_ERROR;
}
                                    ...
                                    struct jobIdIndexS *indexS)
{
    ...
    while (true) {
        if (ePtr->fp != NULL)
            logRec =
                lsbGetNextJobRecFromFile(ePtr->fp, lineNum, numJobIds, jobIds);
        else
            lsberrno = LSBE_EOF;

        if (logRec == NULL && ePtr->curOpenFile > 0 && lsberrno == LSBE_EOF &&
            ePtr->curOpenFile > ePtr->lastOpenFile) {
            if (ePtr->fp != NULL)
                fclose(ePtr->fp);
            ...
            if (ePtr->curOpenFile == 1) {
                ...
                if (indexS != NULL) {
                    nextFileNumber =
                        getNextFileNumFromIndexS(indexS, numJobIds, jobIds);
                }
...
static lsb_event_t get_event_type(const char *name)
{
...
int getEventTypeAndKind(char *typeStr, int *eventKind)
{
    int eventType;
...
int lsb_readeventrecord(char *line, struct eventRec *logRec)
{
...
void readEventRecord(char *line, struct eventRec *logRec)
{
    switch (logRec->type) {
    case EVENT_JOB_NEW:
    case EVENT_JOB_MODIFY:
    ...
    case EVENT_JOB_START_ACCEPT:
        lsberrno =
            readJobStartAccept(line, &(logRec->eventLog.jobStartAcceptLog));
        break;
    case EVENT_JOB_STATUS:
...
int getJobIdIndexFromEventFile(char *eventFile, struct sortIntList *header,
                               time_t *timeStamp)
{
    ...
    if (cc != 2 || ch != '#') {
        ls_syslog(LOG_INFO, "%s: fscanf(%s) failed: old event file format",
                  fname, eventFile);
        if (fseek(eventFp, 0, SEEK_SET) != 0)
            ls_syslog(LOG_ERR, "FUNC_D_FAIL_M", fname, "fseek", 0);
        *timeStamp = 0;
    }
...
int getJobIdFromEvent(char *line, int eventType)
{
    int cc;
    int eventJobId;
...
int writeJobIdIndexToIndexFile(FILE *indexFp, struct sortIntList *header,
                               time_t timeStamp)
{
    int minJobId;
    int maxJobId;
...
// Bug do we need this?
int updateJobIdIndexFile(char *indexFile, char *eventFile, int totalEventFile)
{
    ...
    if (stat(indexFile, &st) == 0) {
        ...
        if (fscanf(indexFp, "%s %s %d %d\n", nameBuf, indexVersion, &rows,
                   &lastUpdate) != 4 ||
            strcmp(nameBuf, LSF_JOBIDINDEX_FILETAG)) {
            ls_syslog(LOG_ERR, "%s: %s is not an jobid index file", fname,
                      indexFile);
    ...
    if ((stat(indexFile, &st) != 0) || (indexFp == NULL)) {
        ...
        if (fchmod(fileno(indexFp), 0644) != 0) {
            ls_syslog(LOG_ERR, "FUNC_S_FAIL_M", fname, "fchmod", indexFile);
        }
    ...
    if (fprintf(indexFp, "%s %s %d %d", LSF_JOBIDINDEX_FILETAG, indexVersion,
                rows, lastUpdate) < 0) {
        lsberrno = LSBE_SYS_CALL;
    }
                             ...
                             int64_t *jobIds)
{
    ...
    while (true) {
        ...
        if (fscanf(indexS->fp, "#%ld %d %ld %ld\n", &(indexS->timeStamp),
                   &(indexS->totalJobIds), &(indexS->minJobId),
                   &(indexS->maxJobId)) != 4) {
            lsberrno = LSBE_SYS_CALL;
            return -1;
        }
        ...
        while (lsberrno != LSBE_EOF) {
            if ((acctLogPtr = lsb_geteventrec(acctLogFp, &lineNum)) == NULL) {
                if (lsberrno != LSBE_EOF) {
                    if (logclass & LC_EXEC) {
                        ls_syslog(LOG_DEBUG,
                                  "%s: Error in reading event file <%s> at "
                                  "line <%d>\n",
