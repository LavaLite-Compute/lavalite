...
hEnt *addMembStr(struct hTab *tabPtr, char *memberStr)
{
    hEnt *ent;
    int new;
...
char remvMembStr(struct hTab *tabPtr, char *memberStr)
{
    hEnt *ent;
...
hEnt *chekMembStr(struct hTab *tabPtr, char *memberStr)
{
    hEnt *ent = NULL;
...
struct sortIntList *initSortIntList(int increased)
{
    ...
    if ((headerPtr = (struct sortIntList *) malloc(
             sizeof(struct sortIntList))) == NULL) {
        return NULL;
    }
...
int insertSortIntList(struct sortIntList *header, int value)
{
...
struct sortIntList *getNextSortIntList(struct sortIntList *header,
...
int getMinSortIntList(struct sortIntList *header, int *minValue)
{
...
int getMaxSortIntList(struct sortIntList *header, int *maxValue)
{
    if (header == header->forw)
        return -1;
    if (header->value)
...
int getTotalSortIntList(struct sortIntList *header)
{
    struct sortIntList *listPtr;
    int total = 0;
...
void copyJUsage(struct jRusage *to, struct jRusage *from)
{
    ...
    if (from->npids) {
        newPidInfo =
            (struct pidInfo *) calloc(from->npids, sizeof(struct pidInfo));
        if (newPidInfo != NULL) {
            if (to->npids)
                FREEUP(to->pidInfo);
...
char *lsb_splitName(char *str, unsigned int *number)
{
    ...
    for (size_t i = 0; i < l; i++) {
        if (str[i] != '*') {
            ...
            if (nameNum <= 0) {
                nameNum = 1;
                ls_syslog(LOG_ERR, "%s: bad input format.  Assuming 1 host.\n",
                          fname);
            }
        }
    }
...
NAMELIST *lsb_compressStrList(char **strList, int numStr)
{
    ...
    if (!nameList.names || !nameList.counter) {
        ls_syslog(LOG_ERR, "FUNC_FAIL_M", fname, "calloc");
        FREEUP(nameList.names);
        FREEUP(nameList.counter);
    ...
    nameList.names =
        (char **) realloc(nameList.names, nameList.listSize * sizeof(char *));
    nameList.counter =
        (int *) realloc(nameList.counter, nameList.listSize * sizeof(int));

    if (!nameList.names || !nameList.counter) {
        ls_syslog(LOG_ERR, "FUNC_FAIL_M", fname, "realloc");
        for (i = 0; i < nameList.listSize; i++)
            FREEUP(nameList.names[i]);
        FREEUP(nameList.names);
        FREEUP(nameList.counter);
...
char *lsb_printNameList(NAMELIST *nameList, int format)
{
    ...
    if (nameList == NULL) {
        ls_syslog(LOG_ERR, "%s: NULL input.\n", fname);
        return NULL;
    }
...
NAMELIST *lsb_parseLongStr(char *string)
{
    ...
    if (string == NULL || strlen(string) <= 0) {
        ls_syslog(LOG_ERR, "%s: bad input", fname);
        return (NAMELIST *) NULL;
    }
    ...
    if (strlen(prevStr) <= 0) {
        ls_syslog(LOG_ERR, "%s: blank input\n", fname);
        FREEUP(prevStr);
        FREEUP(nameList.names);
        FREEUP(nameList.counter);
        return (NAMELIST *) &nameList;
    ...
    while ((curStr = getNextWord_(&string)) != NULL) {
        if (strcmp(curStr, prevStr)) {
            if (nameList.listSize == numStr) {
                ls_syslog(
                    LOG_ERR,
                    "%s: list exceeded allocated memory (shouldn't happen)\n",
                    fname);
                return (NAMELIST *) NULL;
            }
...
NAMELIST *lsb_parseShortStr(char *string, int format)
{
    ...
    while (curStr != NULL) {
        ...
        if (format == PRINT_MCPU_HOSTS) {
            ...
            if ((curStr = getNextWord_(&string)) == NULL) {
                ls_syslog(LOG_ERR, "%s: LSB_MCPU_HOSTS format error\n", fname);
                FREEUP(nameList.names);
                FREEUP(nameList.counter);
                return (NAMELIST *) NULL;
            }
