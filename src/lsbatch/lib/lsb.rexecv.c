/* $Id: lsb.rexecv.c,v 1.8 2007/08/15 22:18:48 tmizan Exp $
...
static int CopyCommand2(char **from, int len)
{
    int i, size;
...
static int parseLine2(char *line, int *embedArgc, char ***embedArgv, int option)
{
#define INCREASE 40
...
static int parseScript2(FILE *from, int *embedArgc, char ***embedArgv,
                        int option)
{
        ...
        if (option & EMBED_INTERACT) {
            ...
            if (length + lineLen + 20 >= size) {
                ...
                if ((sp = (char *) realloc(buf, size)) == NULL) {
                    free(buf);
                    fprintf(stderr,
                            ("Unable to reallocate memory for commands"));
                    return -1;
                }
                buf = sp;
            }
        ...
        if (firstLine[0] != '\n' && firstLine[0] != '\0') {
            if (notBourne == true) {
                if (length + strlen(szTmpShellCommands) + 1 >= size) {
                    ...
                    if ((sp = (char *) realloc(buf, size)) == NULL) {
                        free(buf);
                        fprintf(stderr, ("Unable to reallocate memory for temp "
                                         "shell file"));
                        return -1;
                    }
                    buf = sp;
...
int fillReq2(int argc, char **argv, int operate, struct submit *req)
{
    ...
    if (operate == CMD_BRESTART) {
        ...
        template = "h|V|O|Tn|T:xn|x|rn|r|Bn|B|Nn|N|En|E:"
                   "wn|w:fn|f:kn|k:Rn|R:mn|m:Jn|J:in|i:en|e:qn|q:bn|b:tn|t:sn|"
                   "s:cn|c:Wn|W:Fn|F:Dn|D:Sn|S:Cn|C:Mn|M:on|o:nn|n:un|u:Pn|P:"
                   "Ln|L:Gn|G:Xn|X:Z:";
    } else {
        req->options = 0;
        template = "E:T:"
                   "w:f:k:R:m:J:L:u:i:o:e:n:q:b:t:s:c:v:p:W:F:D:S:C:M:O:P:Ip|"
                   "Is|I|r|H|x|N|B|h|V|G:X:K|";
    }
    ...
    if (myArgc > 0 && myArgv0 != NULL) {
        ...
        if (!CopyCommand2(argv + optind, argc - optind - 1))
            return -1;
    } else if (parseScript2(stdin, &embedArgc, &embedArgv,
