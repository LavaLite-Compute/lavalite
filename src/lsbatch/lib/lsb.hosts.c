...
struct hostInfoEnt *lsb_hostinfo_ex(char **hosts, int *numHosts, char *resReq,
                                    int options)
{
    ...
    if (numHosts == NULL || numReq == 0) {
        ...
        if ((hostInfoReq.names = (char **) calloc(numReq, sizeof(char *))) ==
            NULL) {
            lsberrno = LSBE_NO_MEM;
            return NULL;
        }
    ...
    if (!xdr_encodeMsg(&xdrs, (char *) &hostInfoReq, &hdr, xdr_infoReq, 0,
                       NULL)) {
        xdr_destroy(&xdrs);
        free(request_buf);
        lsberrno = LSBE_XDR;
