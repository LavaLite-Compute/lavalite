/* $Id: bhist.c,v 1.15 2007/08/15 22:18:43 tmizan Exp $
...
void usage(char *cmd)
{
    ...
    fprintf(stderr, "             [-f logfile_name | -n num_logfiles | -n "
                    "min_logfile, max_logfile]\n");
    fprintf(
        stderr,
...
void fileNumbers(char *inputstr, struct bhistReq *Req)
{
    ...
    if (res == NULL) {
        ...
        if (!isint_(inputstr) || (Req->numLogFile = atoi(inputstr)) < 0) {
            ...
            fprintf(stderr,
                    (_i18n_msg_get(ls_catd, NL_SETN, 3176,
                                   "The number of event log files must be a "
                                   "positive integer.\n"))); /* catgets  3176 */
            exit(-1);
        }
    } else {
        ...
        if (!isint_(first) || !isint_(second) ||
            ...
            fprintf(stderr, "bhist: ");
            fprintf(stderr, (_i18n_msg_get(
                                ls_catd, NL_SETN, 3267,
                                "The numbers of both event log files must be "
                                "positive integers.\n"))); /* catgets  3267  */
            exit(-1);
        }
        ...
        if (Req->numMinLogFile > Req->numLogFile) {
            ...
            fprintf(
                stderr,
                (_i18n_msg_get(ls_catd, NL_SETN, 3297,
                               "The value of min_logfile should not exceed the "
                               "value of max_logfile\n"))); /* catgets  3297  */
            exit(-1);
        }
...
int main(int argc, char **argv)
{
    ...
    if (Req.options & OPT_CHRONICLE) {
        if (!foundJob) {
            printf((_i18n_msg_get(
                ls_catd, NL_SETN, 3160,
                "No matching job/event found\n"))); /* catgets  3160  */
            exit(-1);
        } else
            printf("\n");
        exit(0);
    ...
    if (!foundJob) {
        if (Req.options & OPT_ARRAY_INFO)
            printf((_i18n_msg_get(
                ls_catd, NL_SETN, 3161,
                "No matching job array found\n"))); /* catgets  3161  */
        else
            printf(
                (_i18n_msg_get(ls_catd, NL_SETN, 3162,
...
static int do_options(int argc, char **argv, struct bhistReq *bhistReq)
{
    extern char *optarg;
    char *envHours;
    struct passwd *pwPtr;
    ...
    while ((cc = getopt(argc, argv, "VaAhwlbdepstrf:q:m:u:C:D:S:N:J:P:T:n:")) !=
           EOF) {
        switch (cc) {
        case 'A':
        ...
        case 'f':
            ...
            if (bhistReq->options & OPT_NUMLOGFILE) {
                fprintf(
                    stderr,
                    (_i18n_msg_get(ls_catd, NL_SETN, 3164,
                                   "Cannot use option %s together with %s.\n")),
                    "'-f'", "'-n'"); /* catgets  3164  */
                return (-1);
            }
            if (optarg && strlen(optarg) < MAXFILENAMELEN - 1) {
                ...
                fprintf(stderr,
                        (_i18n_msg_get(ls_catd, NL_SETN, 3166,
                                       "%s : Bad event file name.\n")),
                        optarg); /* catgets  3166  */
                return (-1);
            }
        case 'u':
        ...
        case 'm':
            ...
            if ((officialName = getHostOfficialByName_(optarg)) == NULL) {
                fprintf(
                    stderr,
                    (_i18n_msg_get(
                        ls_catd, NL_SETN, 3168,
                        "Warning: <%s> is not a valid hostname\n")), /* catgets
                                                                        3168  */
                    optarg);
                if (strlen(optarg) < MAXHOSTNAMELEN - 1)
                    ...
                    fprintf(
                        stderr, "%s : %s.\n", optarg,
                        (_i18n_msg_get(ls_catd, NL_SETN, 3169,
                                       "Bad host name"))); /* catgets  3169  */
                    return (-1);
                }
            } else
        ...
        case 'q':
            ...
            if (optarg && strlen(optarg) < MAX_LSB_NAME_LEN - 1) {
                ...
                fprintf(stderr, "%s : %s.\n", optarg,
                        _i18n_msg_get(ls_catd, NL_SETN, 3170,
                                      "Bad queue name")); /* catgets 3170 */
                return (-1);
            }
        case 'C':
            ...
            if ((tempPtr = ls_getmodelfactor(optarg)) == NULL)
                if ((tempPtr = ls_gethostfactor(optarg)) == NULL)
                    if ((!isanumber_(optarg)) ||
                        ...
                        fprintf(stderr, "<%s> %s.\n", optarg,
                                (_i18n_msg_get(
                                    ls_catd, NL_SETN, 3171,
                                    "neither a host model, nor a host name, "
                                    "nor a CPU factor"))); /* catgets  3171  */
                        return (-1);
                    }
            if (tempPtr)
    ...
    if (bhistReq->options & OPT_CHRONICLE) {
        ...
        if (bhistReq->options & ~OPT_CHRONICLE & ~OPT_TIME_INTERVAL &
            ~OPT_ELOGFILE)
            usage(argv[0]);
        if (bhistReq->searchTime[0] == -1) {
            if (defaultTime[0] == 0)
                bhistReq->searchTime[0] =
                    defaultTime[1] - DEFAULT_CHRONICLE_SEARCH_TIME;
            else
    ...
    if (!(bhistReq->options & OPT_TIME_INTERVAL)) {
        if ((bhistReq->numJobs > 0) || (bhistReq->options & OPT_COMPLETE) ||
            (bhistReq->options & OPT_SUBMIT) ||
            (bhistReq->options & OPT_NUMLOGFILE) ||
            (bhistReq->options & OPT_DISPATCH)) {
            if ((lsbMode_ & LSB_MODE_BATCH) &&
                !(bhistReq->options &
                  (OPT_NUMLOGFILE | OPT_TIME_INTERVAL | OPT_SUBMIT |
                   OPT_DISPATCH | OPT_ELOGFILE))) {
                if (defaultTime[0] == 0)
                    bhistReq->searchTime[0] =
                        defaultTime[1] - DEFAULT_CHRONICLE_SEARCH_TIME;
                else
...
static char *getEventStatus(struct eventRecord *event)
{
    ...
    case JOB_STAT_PEND:
        ...
        sprintf(status, (_i18n_msg_get(ls_catd, NL_SETN, 3178, "Pending:%s")),
                lsb_pendreason(1, &event->reasons, /* catgets  3178  */
                               NULL, event->ld));
        if ((event->reasons == PEND_SBD_JOB_REQUEUE ||
             event->reasons == PEND_QUE_PRE_FAIL ||
             event->reasons == PEND_JOB_PRE_EXEC) &&
            WEXITSTATUS(wStatus)) {
            char *sp;
            sp = status;
            while (*sp != '\n')
                sp++;
            sprintf(sp,
                    (_i18n_msg_get(ls_catd, NL_SETN, 3179, "(exit code %d)\n")),
                    WEXITSTATUS(wStatus)); /* catgets  3179  */
        }
        break;
    case JOB_STAT_RUN:
        strcpy(status, (_i18n_msg_get(ls_catd, NL_SETN, 3180,
                                      "Running"))); /* catgets  3180  */
        break;
    ...
    case JOB_STAT_SSUSP:
        ...
        if (event->reasons & SUSP_RES_LIMIT) {
            sprintf(status, "%s: %s",
                    _i18n_msg_get(ls_catd, NL_SETN, 3196,
                                  "Terminated"), /* catgets  3196  */
                    lsb_suspreason((event->reasons & ~SUSP_MBD_LOCK),
                                   event->subreasons, event->ld));
        } else {
            sprintf(status, "%s: %s",
                    _i18n_msg_get(ls_catd, NL_SETN, 3181,
                                  "Suspended"), /* catgets  3181  */
                    lsb_suspreason((event->reasons & ~SUSP_MBD_LOCK),
                                   event->subreasons, event->ld));
        }
        break;
    case JOB_STAT_USUSP:
        strcpy(
            status,
            (_i18n_msg_get(
                ls_catd, NL_SETN, 3182,
                "Suspended by the user or administrator"))); /* catgets  3182 */
        break;
    case JOB_STAT_PSUSP:
        strcpy(status, (_i18n_msg_get(ls_catd, NL_SETN, 3183,
                                      "Suspended by the user or administrator "
                                      "while pending"))); /* catgets
                                                             3183
                                                           */
        break;
    case JOB_STAT_UNKWN:
        strcpy(status,
               (_i18n_msg_get(
                   ...
                   "Unknown; unable to reach the execution host"))); /* catgets
                                                                        3184  */
        break;
    case JOB_STAT_EXIT:
        if (event->reasons & EXIT_ZOMBIE) {
            strcpy(
                status,
                (_i18n_msg_get(
                    ls_catd, NL_SETN, 3185,
                    "Execution host unavailable; job is killed"))); /* catgets
                                                                       3185  */
        } else if (event->reasons & EXIT_KILL_ZOMBIE)
            strcpy(status,
                   (_i18n_msg_get(
                       ls_catd, NL_SETN, 3186,
                       "Termination request issued; but unable to reach the "
                       "execution host"))); /* catgets  3186  */
        else if (event->reasons & EXIT_ZOMBIE_JOB)
            strcpy(
                status,
                (_i18n_msg_get(
                    ls_catd, NL_SETN, 3187,
                    "sbatchd startup; kill ZOMBIE job"))); /* catgets  3187  */
        else if (event->reasons & EXIT_RERUN)
            strcpy(status,
                   (_i18n_msg_get(
                       ls_catd, NL_SETN, 3188,
                       "Exited; job will be requeued and rerun"))); /* catgets
                                                                       3188  */
        else if (event->reasons & EXIT_NO_MAPPING)
            strcpy(status,
                   (_i18n_msg_get(ls_catd, NL_SETN, 3189,
                                  "Exited; remote job has no user mapping for "
                                  "the local cluster"))); /* catgets  3189  */
        else if (event->reasons & EXIT_INIT_ENVIRON)
            strcpy(status, (_i18n_msg_get(
                               ls_catd, NL_SETN, 3191,
                               "Exited; remote job cannot run locally because "
                               "of environment problem"))); /* catgets  3191  */
        else if (event->reasons & EXIT_PRE_EXEC)
            strcpy(status, (_i18n_msg_get(ls_catd, NL_SETN, 3192,
                                          "Exited; remote job's pre-exec "
                                          "command failed"))); /* catgets
                                                                  3192
                                                                */
            ...
            strcpy(
                status,
                (_i18n_msg_get(
                    ls_catd, NL_SETN, 3428,
                    "Exited; job has been forced to exit"))); /* catgets 3428 */
        else
            strcpy(status, (_i18n_msg_get(ls_catd, NL_SETN, 3193,
                                          "Exited"))); /* catgets  3193  */
        break;
    case JOB_STAT_DONE:
        strcpy(status,
               (_i18n_msg_get(ls_catd, NL_SETN, 3194,
                              "Done successfully"))); /* catgets  3194  */
        break;
    default:
        strcpy(status, (_i18n_msg_get(ls_catd, NL_SETN, 3195,
                                      "Unknown"))); /* catgets  3195  */
        break;
    }

succeed_rtn:
    if (logclass & LC_TRACE)
        ls_syslog(LOG_DEBUG2, "%s: Job status is %s/%x", fname, status,
                  event->jStatus);
    return status;
}

static int skip_jobRecord(struct jobRecord *jobRecord, struct bhistReq *Req)
{
...
static void displayhist(struct bhistReq *bhistReq)
{
    ...
    while (jobRecord != jobRecordList) {
        ...
        while (event) {
            ...
            if (logclass & LC_TRACE)
                ls_syslog(LOG_DEBUG2, "%s: (1)event.kind=%x", fname,
                          event->kind);
            if (nextevent && (logclass & LC_TRACE))
                ls_syslog(LOG_DEBUG2, "%s: (1)nextevent.kind=%x", fname,
                          nextevent->kind);
            ...
            if (event->jStatus & (JOB_STAT_DONE | JOB_STAT_EXIT)) {
                if ((bhistReq->options & OPT_NORMALIZECPU) && job->cpuTime > 0)
                    job->cpuTime =
                        job->cpuTime * job->cpuFactor / bhistReq->cpuFactor;
            }
            if (matchJobId(bhistReq, LSB_JOBID((int) job->jobId, event->idx))) {
                printEvent(bhistReq, jobRecord, job, event, timeStamp,
                           timestamp, 0);
            }
            ...
            if (nextevent == NULL) {
                if ((event->jStatus & (JOB_STAT_DONE) ||
                    ...
                    currentOrDoneTime =
                        putstr_(_i18n_ctime(ls_catd, CTIME_FORMAT_a_b_d_T,
                                            (time_t *) &(timeStamp)));
                } else {
                    ...
                    if (stayTime < 0) {
                        ...
                        currentOrDoneTime = putstr_(_i18n_ctime(
                            ls_catd, CTIME_FORMAT_a_b_d_T, (time_t *) &(now)));
                }
            ...
            if (!matchJobId(bhistReq,
                            LSB_JOBID((int) job->jobId, lastevent->idx)) &&
            ...
            if ((event->kind != EVENT_JOB_SWITCH)) {
                ...
                case JOB_STAT_EXIT:
                    if (event->reasons & (EXIT_RERUN | FINISH_PEND) ||
                        nextevent)
                        pendTime += timeinterval;
        ...
        if (bhistReq->options & OPT_DFTFORMAT) {
            if (first == TRUE) {
                printf(
                    (_i18n_msg_get(ls_catd, NL_SETN, 3197,
                                   "Summary of time in seconds spent in "
                                   "various states:\n"))); /* catgets  3197  */
                printf((_i18n_msg_get(ls_catd, NL_SETN, 3198,
                                      "JOBID   USER    JOB_NAME  PEND    PSUSP "
                                      "  RUN     USUSP "))); /* catgets  3198 */
                printf("  ");
                printf((_i18n_msg_get(
                    ls_catd, NL_SETN, 3199,
                    "SSUSP   UNKWN   TOTAL\n"))); /* catgets  3199  */
                first = FALSE;
            }
            if (bhistReq->options & OPT_WIDEFORMAT) {
                ...
                printf("%-7s %-7s %-9s "
                       "%-8.0f%-8.0f%-8.0f%-8.0f%-8.0f%-8.0f%-10.0f\n",
            ...
            printf("\n%s  ",
                   (_i18n_msg_get(ls_catd, NL_SETN, 3200,
                                  "Summary of time in seconds spent in various "
                                  "states by"))); /* catgets  3200  */
            printf("%-12.24s\n", currentOrDoneTime);
            FREEUP(currentOrDoneTime);
            printf(
                "  %s    ",
                (_i18n_msg_get(
                    ls_catd, NL_SETN, 3201,
                    "PEND     PSUSP    RUN      USUSP    SSUSP"))); /* catgets
                                                                       3201  */
            printf((_i18n_msg_get(ls_catd, NL_SETN, 3202,
                                  "UNKWN    TOTAL\n"))); /* catgets  3202  */
...
static char logfile_check(struct bhistReq *Req)
{
    ...
    if (Req->options & OPT_ELOGFILE) {
        ...
        do {
            sprintf(eventFileName, "%s/%s/logdir/lsb.events.%d",
                    bhistParams[LSB_SHAREDIR].paramValue, clusterName,
                    ++maxEventFile);
    ...
    while (TRUE) {
        if ((log = lsbGetNextJobEvent(eLogPtr, &lineNum, Req->numJobs,
                                      Req->jobIds, jobIdIndexSPtr)) != NULL) {
            if ((Req->searchTime[1] == -1) ||
                (log->eventTime >= Req->searchTime[0] &&
                 log->eventTime <= Req->searchTime[1])) {
                parse_event(log, Req);
...
static void readEventFromHead(char *eventDir, struct bhistReq *reqPtr)
{
    ...
    if ((statBuf.st_mode & S_IFREG) != S_IFREG) {
        fprintf(stderr, "%s: %s\n", eventFile,
                (_i18n_msg_get(ls_catd, NL_SETN, 3204,
                               "Not a regular file\n"))); /* catgets  3204  */
        exit(-1);
    }
...
{
    loadIndex =
        (struct loadIndexLog *) bhist_malloc(sizeof(struct loadIndexLog));
    loadIndex->nIdx = 11;
...
#define PRT_FMTSTR(i18nstr)                                                    \
    ...
    sprintf(prline,
            I18N(3357,
                 "%-12.19s: Parameters of Job are changed:") /* catgets 3357 */,
            timestamp);
    prtLine(prline);
    ...
    if (jobModLog->options2 & SUB2_IN_FILE_SPOOL) {
        sprintf(prline,
                I18N(3348,
                     "Input file (Spooled) change to : %s"), /* catgets 3348 */
                jobModLog->inFile);
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->options & SUB_EXCLUSIVE) {
        sprintf(prline,
                I18N(3363, "Job changes to exclusive")); /* catgets 3363 */
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->options & SUB_NOTIFY_END) {
        sprintf(
            ...
            I18N(
                3364,
                "Job will send mail to user when job ends")); /* catgets 3364 */
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->options & SUB_NOTIFY_BEGIN) {
        sprintf(prline,
                I18N(3365,
                     "Job will send mail to user when job begins")); /* catgets
                                                                        3365 */
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->options & SUB_RERUNNABLE) {
        sprintf(prline,
                I18N(3369, "Job changes to rerunnable")); /* catgets 3369 */
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->options & SUB_OTHER_FILES) {
        ...
        for (i = 0; i < jobModLog->nxf; i++) {
            sprintf(prline,
                    I18N(3374, "%3d : subfile name is %s") /* catgets 3374 */,
                    i, jobModLog->xf[i].subFn);
            PRT_FMTSTR(prline);
            sprintf(prline,
                    I18N(3375, "      execfile name is %s") /* catgets 3375 */,
    ...
    if (jobModLog->options & SUB_PRE_EXEC) {
        sprintf(
            prline,
            I18N(3377, "Pre exec command changes to : %s") /* catgets 3377 */,
    ...
    if (jobModLog->options & SUB_PROJECT_NAME) {
        sprintf(prline,
                I18N(3380, "Project name changes to : %s") /* catgets 3380 */,
    ...
    for (i = 0; i < LSF_RLIM_NLIMITS; i++)
        if (jobModLog->rLimits[i] > 0) {
            ...
            case LSF_RLIMIT_CPU:
                sprintf(
                    prline,
                    I18N(3381, "CPU limit changes to : %d (minutes)") /* catgets
                                                                         3381 */
                    ,
            ...
            case LSF_RLIMIT_FSIZE:
                sprintf(
                    prline,
                    I18N(3382,
                         "File limit changes to : %d (KB)") /* catgets 3382 */,
            ...
            case LSF_RLIMIT_DATA:
                sprintf(
                    prline,
                    I18N(3383,
                         "Data limit changes to : %d (KB)") /* catgets 3383 */,
            ...
            case LSF_RLIMIT_STACK:
                sprintf(
                    prline,
                    I18N(3384,
                         "Stack limit changes to : %d (KB)") /* catgets 3384 */,
            ...
            case LSF_RLIMIT_CORE:
                sprintf(
                    prline,
                    I18N(3385,
                         "Core limit changes to : %d (KB)") /* catgets 3385 */,
            ...
            case LSF_RLIMIT_RSS:
                sprintf(
                    ...
                    I18N(
                        3386,
                        "Memory limit changes to : %d (KB)") /* catgets 3386 */,
            ...
            case LSF_RLIMIT_SWAP:
                sprintf(
                    prline,
                    I18N(3387,
                         "Swap limit changes to : %d (KB)") /* catgets 3387 */,
            ...
            case LSF_RLIMIT_RUN:
                sprintf(
                    prline,
                    I18N(3433, "run limit changes to : %d (minutes)") /* catgets
                                                                         3433 */
                    ,
    ...
    if (jobModLog->options2 & SUB2_JOB_CMD_SPOOL) {
        sprintf(
            prline,
            I18N(3151,
                 "Job command (Spooled) changes to : %s"), /* catgets 3151 */
            jobModLog->command);
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->numProcessors != DEFAULT_NUMPRO &&
        ...
        sprintf(
            prline,
            I18N(3391,
                 "New minimal num of processors is : %d") /* catgets 3391 */,
            jobModLog->numProcessors);
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->maxNumProcessors != DEFAULT_NUMPRO &&
        ...
        sprintf(
            prline,
            I18N(3392, "New max num of processors is : %d") /* catgets 3392 */,
    ...
    if (jobModLog->termTime > 0) {
        ...
        sprintf(prline,
                I18N(3394, "Job new end time is : %s") /* catgets 3394 */, sp);
        PRT_FMTSTR(prline);
        free(sp);
    }
    ...
    if (jobModLog->delOptions & SUB_QUEUE) {
        sprintf(
            prline,
            I18N(3395, "Job queue changes back to default") /* catgets 3395 */);
        PRT_FMTSTR(prline);
    }

    if (jobModLog->delOptions & SUB_HOST) {
        sprintf(prline,
                I18N(3396, "Host changes to default: ")); /* catgets 3396 */
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->delOptions2 & SUB2_JOB_CMD_SPOOL) {
        sprintf(
            prline,
            I18N(3153, "Job command spooling is canceled")); /* catgets 3153 */
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->delOptions2 & SUB2_IN_FILE_SPOOL) {
        sprintf(
            prline,
            I18N(3154, "Input file spooling is canceled")); /* catgets 3154 */
        PRT_FMTSTR(prline);
    }

    if (jobModLog->delOptions & SUB_IN_FILE) {
        sprintf(prline,
                I18N(3397, "Input file is disabled")); /* catgets 3397 */
        PRT_FMTSTR(prline);
    }

    if (jobModLog->delOptions & SUB_OUT_FILE) {
        sprintf(prline,
                I18N(3398, "Output file is disabled")); /* catgets 3398 */
        PRT_FMTSTR(prline);
    }

    if (jobModLog->delOptions & SUB_ERR_FILE) {
        sprintf(prline,
                I18N(3399, "Error file is disabled")); /* catgets 3399 */
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->delOptions & SUB_NOTIFY_END) {
        sprintf(prline,
                I18N(3401,
                     "No mail will be sent to user when job ends")); /* catgets
                                                                        3401 */
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->delOptions & SUB_NOTIFY_BEGIN) {
        sprintf(
            prline,
            I18N(3402,
                 "No mail will be to user when job begins")); /* catgets 3402 */
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->delOptions & SUB_CHKPNTABLE) {
        sprintf(
            prline,
            I18N(3404,
                 "Job changes to be NOT checkpointable")); /* catgets 3404 */
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->delOptions & SUB_RERUNNABLE) {
        sprintf(
            prline,
            I18N(3405, "Job changes to be not rerunnable")); /* catgets 3405 */
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->delOptions & SUB_DEPEND_COND) {
        sprintf(prline,
                I18N(3407, "No dependances condition")); /* catgets 3407 */
        PRT_FMTSTR(prline);
    }

    if (jobModLog->delOptions & SUB_RES_REQ) {
        sprintf(prline,
                I18N(3408, "No  resource requirement")); /* catgets 3408 */
        PRT_FMTSTR(prline);
    }

    if (jobModLog->delOptions & SUB_OTHER_FILES) {
        sprintf(prline,
                I18N(3409, "No transfer files specified")); /* catgets 3409 */
        PRT_FMTSTR(prline);
    }

    if (jobModLog->delOptions & SUB_PRE_EXEC) {
        sprintf(prline,
                I18N(3410, "Pre exec command is disabled")); /* catgets 3410 */
        PRT_FMTSTR(prline);
    }

    if (jobModLog->delOptions & SUB_LOGIN_SHELL) {
        sprintf(prline,
                I18N(3411, "Log shell changes to default")); /* catgets 3411 */
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->delOptions & SUB_MAIL_USER) {
        sprintf(prline,
                I18N(3412,
                     "Mail user changes to original user")); /* catgets 3412 */
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->delOptions & SUB_PROJECT_NAME) {
        sprintf(
            prline,
            I18N(3413, "Project name changes to default")); /* catgets 3413 */
        PRT_FMTSTR(prline);
    }
    ...
    for (i = 0; i < LSF_RLIM_NLIMITS; i++)
        if (jobModLog->rLimits[i] == DELETE_NUMBER) {
            ...
            case LSF_RLIMIT_CPU:
                sprintf(prline,
                        I18N(3414, "bmod -cn used. CPU limit set to queue CPU "
                                   "limit")); /* catgets 3414 */
                PRT_FMTSTR(prline);
                break;
            case LSF_RLIMIT_FSIZE:
                sprintf(prline,
                        I18N(3415, "bmod -Fn used. File limit set to queue "
                                   "file limit")); /* catgets 3415 */
                PRT_FMTSTR(prline);
                break;
            case LSF_RLIMIT_DATA:
                sprintf(prline,
                        I18N(3416, "bmod -Dn used. Data limit set to queue "
                                   "data limit")); /* catgets 3416 */
                PRT_FMTSTR(prline);
                break;
            case LSF_RLIMIT_STACK:
                sprintf(prline,
                        I18N(3417, "bmod -Sn used. Stack limit set to queue "
                                   "stack limit")); /* catgets 3417 */
                PRT_FMTSTR(prline);
                break;
            case LSF_RLIMIT_CORE:
                sprintf(prline,
                        I18N(3418, "bmod -Cn used. Core limit set to queue "
                                   "core limit")); /* catgets 3418 */
                PRT_FMTSTR(prline);
                break;
            case LSF_RLIMIT_RSS:
                sprintf(prline,
                        I18N(3419, "bmod -Mn used. Memory limit set to queue "
                                   "memory limit")); /* catgets 3419 */
                PRT_FMTSTR(prline);
                break;
            case LSF_RLIMIT_SWAP:
                sprintf(prline,
                        I18N(3420, "Swap limit is removed")); /* catgets 3420 */
                PRT_FMTSTR(prline);
                break;
            case LSF_RLIMIT_PROCESS:
                sprintf(
                    prline,
                    I18N(3421, "Process limit is removed")); /* catgets 3421 */
                PRT_FMTSTR(prline);
                break;
            case LSF_RLIMIT_RUN:
                sprintf(prline,
                        I18N(3440, "bmod -Wn used. Run limit set to queue run "
                                   "limit")); /* catgets 3440 */
                PRT_FMTSTR(prline);
                break;
            }
    ...
    if (jobModLog->numProcessors == DEL_NUMPRO) {
        sprintf(
            prline,
            I18N(3423,
                 "Minimal num of processors is removed")); /* catgets 3423 */
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->maxNumProcessors == DEL_NUMPRO) {
        sprintf(
            prline,
            I18N(3424, "Max num of processors is removed")); /* catgets 3424 */
        PRT_FMTSTR(prline);
    }

    if (jobModLog->beginTime == DELETE_NUMBER) {
        sprintf(prline,
                I18N(3425, "Job begin time is removed")); /* catgets 3425 */
        PRT_FMTSTR(prline);
    }

    if (jobModLog->termTime == DELETE_NUMBER) {
        sprintf(prline,
                I18N(3426, "Job terminate time is removed")); /* catgets 3426 */
        PRT_FMTSTR(prline);
    }
    ...
    if (jobModLog->delOptions2 & SUB2_JOB_PRIORITY) {
        sprintf(
            prline,
            I18N(3175, "Job priority changes to default")); /* catgets 3175 */
        PRT_FMTSTR(prline);
    }
}

static void prtParameters(struct jobInfoEnt *params, struct bhistReq *bhistReq,
                          char *timestamp)
{
    ...
    if (params->submit.options & SUB_MODIFY_ONCE)
        sprintf(tBuff, "%s: ",
                (_i18n_msg_get(
                    ls_catd, NL_SETN, 3210,
                    "Parameters are modified (only used once) to"))); /* catgets
                                                                         3210 */
    else
        sprintf(
            tBuff, "%s:  ",
            (_i18n_msg_get(ls_catd, NL_SETN, 3211,
                           "Parameters are modified to"))); /* catgets  3211  */

    if (bhistReq->options & OPT_CHRONICLE)
        sprintf(prline, "%-12.19s: %s <%s> %s", timestamp, I18N_Job,
                lsb_jobidinstr(params->jobId), lowFirstChar(tBuff));
    else
        sprintf(prline, "%-12.19s: %s", timestamp, tBuff);

    prtLine(prline);
    sprintf(prline, " %s <%s>, ",
            (_i18n_msg_get(ls_catd, NL_SETN, 3213, "Project")),
            params->submit.projectName); /* catgets  3213  */
    ...
    if ((params->submit.options2 & SUB2_JOB_CMD_SPOOL) &&
        ...
        sprintf(prline, " %s <%s>, ",
                (_i18n_msg_get(ls_catd, NL_SETN, 3152,
                               "Command (Spooled)")), /* catgets 3152 */
                params->submit.command);
        prtLine(prline);
    }
    ...
    if ((params->submit.options2 & SUB2_MODIFY_CMD) &&
        (params->submit.command)) {
        sprintf(prline, " %s <%s>, ",
                (_i18n_msg_get(ls_catd, NL_SETN, 3214, "Command")),
                params->submit.command); /* catgets  3214  */
        prtLine(prline);
    }

    if (params->submit.options & SUB_MAIL_USER) {
        sprintf(prline, " %s <%s>, ",
                (_i18n_msg_get(ls_catd, NL_SETN, 3217, "Mail")),
                params->submit.mailUser); /* catgets  3217  */
        prtLine(prline);
    }
    sprintf(
        prline, "%s <%s>",
        (_i18n_msg_get(ls_catd, NL_SETN, 3218, "Queue")), /* catgets  3218  */
        params->submit.queue);
    prtLine(prline);
    if (params->submit.options & SUB_JOB_NAME) {
        sprintf(prline, ", %s <%s>",
                (_i18n_msg_get(ls_catd, NL_SETN, 3219,
                               "Job Name")), /* catgets  3219  */
...
static char *getUserName(int userId)
{
    ...
    if (getLSFUserByUid_(userId, lsfUserName, MAXLINELEN) == 0)
        ...
        return ((_i18n_msg_get(ls_catd, NL_SETN, 3220,
                               "unknown"))); /* catgets  3220  */
}

static int dispChkpnt(struct eventRecord *event, struct jobRecord *jobRecord)
{
...
char *actionName(char *sigName)
{
    ...
    if ((strcmp(sigName, "CHKPNT") == 0) ||
        ...
        action = _i18n_msg_get(ls_catd, NL_SETN, 3221,
                               "checkpoint action"); /* catgets 3221 */
    } else if (strcmp(sigName, "SIG_SUSP_USER") == 0)
        action = _i18n_msg_get(ls_catd, NL_SETN, 3222,
                               "User suspending action"); /* catgets  3222  */
    else if (strcmp(sigName, "SIG_SUSP_LOAD") == 0)
        action = _i18n_msg_get(ls_catd, NL_SETN, 3223,
                               "Load suspending action"); /* catgets  3223  */
    else if (strcmp(sigName, "SIG_SUSP_WINDOW") == 0)
        action = _i18n_msg_get(ls_catd, NL_SETN, 3225,
                               "Window suspending action"); /* catgets  3225  */
    else if (strcmp(sigName, "SIG_SUSP_OTHER") == 0)
        action = _i18n_msg_get(ls_catd, NL_SETN, 3226,
                               "Other suspending action"); /* catgets  3226  */
    else if (strcmp(sigName, "SIG_RESUME_USER") == 0)
        action = _i18n_msg_get(ls_catd, NL_SETN, 3227,
                               "User resumption action"); /* catgets  3227  */
    else if (strcmp(sigName, "SIG_RESUME_LOAD") == 0)
        action = _i18n_msg_get(ls_catd, NL_SETN, 3228,
                               "Load resumption action"); /* catgets  3228  */
    else if (strcmp(sigName, "SIG_RESUME_WINDOW") == 0)
        action = _i18n_msg_get(ls_catd, NL_SETN, 3230,
                               "Window resumption action"); /* catgets  3230  */
    else if (strcmp(sigName, "SIG_RESUME_OTHER") == 0)
        action = _i18n_msg_get(ls_catd, NL_SETN, 3231,
                               "Other resumption action"); /* catgets  3231  */
    else if (strcmp(sigName, "SIG_TERM_USER") == 0)
        action = _i18n_msg_get(ls_catd, NL_SETN, 3232,
                               "User termination action"); /* catgets  3232  */
    else if (strcmp(sigName, "SIG_TERM_LOAD") == 0)
        action = _i18n_msg_get(ls_catd, NL_SETN, 3233,
                               "Load termination action"); /* catgets  3233  */
    else if (strcmp(sigName, "SIG_TERM_WINDOW") == 0)
        action = _i18n_msg_get(ls_catd, NL_SETN, 3235,
                               "Window termination action"); /* catgets  3235 */
    else if (strcmp(sigName, "SIG_TERM_OTHER") == 0)
        action = _i18n_msg_get(ls_catd, NL_SETN, 3236,
                               "Other termination action"); /* catgets  3236  */
    else if (strcmp(sigName, "SIG_TERM_RUNLIMIT") == 0)
        action =
            _i18n_msg_get(ls_catd, NL_SETN, 3237,
                          "RUNLIMIT termination action"); /* catgets  3237  */
    else if (strcmp(sigName, "SIG_TERM_DEADLINE") == 0)
        action =
            _i18n_msg_get(ls_catd, NL_SETN, 3238,
                          "DEADLINE termination action"); /* catgets  3238  */
    else if (strcmp(sigName, "SIG_TERM_PROCESSLIMIT") == 0)
        action = _i18n_msg_get(
            ls_catd, NL_SETN, 3239,
            "PROCESSLIMIT termination action"); /* catgets  3239  */
        ...
        action = _i18n_msg_get(ls_catd, NL_SETN, 3240,
                               "Unknow action"); /* catgets  3240  */
...
static void printEvent(struct bhistReq *bhistReq, struct jobRecord *jobRecord,
                       ...
                       time_t timeStamp, char *timeStampStr, int option)
{
    ...
    if ((bhistReq->options & OPT_CHRONICLE)) {
        if (event->idx > 0)
            sprintf(tBuff, " %s <%d[%d]>", I18N_Job, (int) job->jobId,
                    event->idx);
        else
    ...
    case EVENT_JOB_START:
        ...
        job->exHosts = (char **) bhist_calloc(job->numExHosts, sizeof(char *));
        for (i = 0; i < job->numExHosts; i++) {
    ...
    case EVENT_JOB_SWITCH:
        if (tBuff[0] == '\0')
            sprintf(
                prline,
                (_i18n_msg_get(ls_catd, NL_SETN, 3248,
                               "%-12.19s: Switched to Queue <%s> by user or "
                               "administrator <%s>")), /* catgets  3248  */
                timeStampStr, event->queue, event->userName);
        else
            sprintf(
                prline,
                (_i18n_msg_get(ls_catd, NL_SETN, 3249,
                               "%-12.19s:%s switched to Queue <%s> by user or "
                               "administrator <%s>")), /* catgets  3249  */
    ...
    case EVENT_JOB_MSG:
        sprintf(prline,
                (_i18n_msg_get(
                    ls_catd, NL_SETN, 3250,
                    "%-12.19s:%s Message <%s>  Message ID<%d> requested by "
                    "user or administrator <%s>")), /* catgets  3250  */
    ...
    case EVENT_JOB_MSG_ACK:
        sprintf(prline,
                (_i18n_msg_get(ls_catd, NL_SETN, 3251,
                               "%-12.19s:%s Message <%s> Message ID<%d>has "
                               "been dispatchd")), /* catgets  3251  */
                timeStampStr, tBuff, event->jmMsg, event->jmMsgId);
        prtLine(prline);
        break;
    ...
    case EVENT_JOB_START_ACCEPT:
        if (tBuff[0] == '\0')
            sprintf(prline, "%-12.19s: %s (Pid %d)", timeStampStr,
                    (_i18n_msg_get(ls_catd, NL_SETN, 3252,
                                   "Starting")), /* catgets  3252  */
                    event->jobPid);
        else
            sprintf(prline, "%-12.19s:%s %s (Pid %d)", timeStampStr, tBuff,
                    (_i18n_msg_get(ls_catd, NL_SETN, 3253,
                                   "starting")), /* catgets  3253  */
                    event->jobPid);
        prtLine(prline);
        break;
    ...
    case EVENT_JOB_SIGNAL:
        if (strcmp(event->sigSymbol, "DELETEJOB") == 0) {
            if (tBuff[0] == '\0')
                sprintf(prline, "%-12.19s: %s <%s>", timeStampStr,
                        (_i18n_msg_get(ls_catd, NL_SETN, 3254,
                                       "Delete requested by user or "
                                       "administrator")), /* catgets
                                                             3254
                                                           */
                        event->userName);
            else
                sprintf(prline, "%-12.19s:%s %s<%s>", timeStampStr, tBuff,
                        (_i18n_msg_get(ls_catd, NL_SETN, 3255,
                                       "delete requested by user or "
                                       "administrator")), /* catgets
                                                             3255
                                                           */
            ...
            if (event->runCount > 0) {
                ...
                sprintf(prline, "%s; %s %d", i18nBuf,
                        (_i18n_msg_get(ls_catd, NL_SETN, 3256,
                                       "Running times is")), /* catgets  3256 */
                        event->runCount);
            }
        } else
            sprintf(prline,
                    _i18n_msg_get(ls_catd, NL_SETN, 3257,
                                  "%-12.19s:%s Signal <%s> requested by user "
                                  "or administrator <%s>"), /* catgets  3257  */
    ...
    case EVENT_MIG:
        if (event->userName && event->userName[0])
            sprintf(
                prline, "%-12.19s:%s %s <%s>", timeStampStr, tBuff,
                _i18n_msg_get(
                    ls_catd, NL_SETN, 3258,
                    "Migration requested by user or administrator"), /* catgets
                                                                        3258  */
                event->userName);
        else
            sprintf(
                prline, "%-12.19s:%s %s", timeStampStr, tBuff,
                _i18n_msg_get(
                    ls_catd, NL_SETN, 3259,
                    "Migration requested by unknown user")); /* catgets  3259 */

        if (event->migNumAskedHosts) {
            sprintf(prline, "%s; %s", prline,
                    (_i18n_msg_get(ls_catd, NL_SETN, 3260,
                                   "Specified Hosts"))); /* catgets  3260  */
        ...
        if ((strcmp(event->sigSymbol, "SIG_CHKPNT") == 0) ||
            ...
            if (IS_PEND(event->jStatus)) {
                if (event->chkPeriod) {
                    sprintf(
                        prline,
                        (_i18n_msg_get(ls_catd, NL_SETN, 3261,
                                       "%-12.19s:%s Checkpoint period is set "
                                       "to %d min.")), /* catgets  3261  */
                        timeStampStr, tBuff, (int) (event->chkPeriod / 60));
                    prtLine(prline);
                }
                break;
            }
            ...
            if (event->actStatus == ACT_START) {
                if (display == TRUE) {
                    char *i18nBuf;
                    i18nBuf = putstr_(
                        (_i18n_msg_get(ls_catd, NL_SETN, 3262,
                                       "initiated"))); /* catgets  3262  */
                    sprintf(
                        prline, "%-12.19s:%s %s %s (actpid %d)", timeStampStr,
                        tBuff,
                        (event->actFlags & LSB_CHKPNT_MIG)
                            ? (_i18n_msg_get(ls_catd, NL_SETN, 3263,
                                             "Migration checkpoint"))
                            : /* catgets  3263  */
                            (_i18n_msg_get(ls_catd, NL_SETN, 3264,
                                           "Checkpoint")), /* catgets  3264  */
                        i18nBuf, event->actPid);
                    if (event->chkPeriod != lastChkPeriod) {
                        lastChkPeriod = event->chkPeriod;
                        if (event->chkPeriod)
                            sprintf(prline,
                                    (_i18n_msg_get(ls_catd, NL_SETN, 3265,
                                                   "%s; checkpoint period is "
                                                   "%d min.")), /* catgets
                                                                   3265
                                                                 */
                    ...
                    sprintf(prline, "%-12.19s:%s %s", timeStampStr, tBuff,
                            _i18n_msg_get(
                                ls_catd, NL_SETN, 3266,
                                "Job is being requeued") /* catgets  3266  */);
                ...
                if (display == TRUE) {
                    ...
                    statusPtr = putstr_(
                        (event->actStatus == ACT_DONE)
                            ? (_i18n_msg_get(ls_catd, NL_SETN, 3270,
                                             "succeeded"))
                            : /* catgets  3270  */
                            (_i18n_msg_get(ls_catd, NL_SETN, 3271,
                                           "failed"))); /* catgets  3271  */
                    ...
                    sprintf(
                        ...
                        tBuff,
                        (event->actFlags & LSB_CHKPNT_MIG)
                            ? (_i18n_msg_get(ls_catd, NL_SETN, 3268,
                                             "Migration checkpoint"))
                            : /* catgets  3268  */
                            (_i18n_msg_get(ls_catd, NL_SETN, 3269,
                                           "Checkpoint")), /* catgets  3269  */
                        statusPtr, event->actPid);
                    free(statusPtr);
                } else
                    sprintf(prline, "%-12.19s:%s %s", timeStampStr, tBuff,
                            _i18n_msg_get(
                                ls_catd, NL_SETN, 3272,
                                "Job has been requeued") /* catgets  3272  */);
            }
            prtLine(prline);
        } else {
            switch (event->actStatus) {
            case ACT_START:
                sprintf(prline, "%-12.19s:%s %s %s (actpid %d)", timeStampStr,
                        tBuff, actionName(event->sigSymbol),
            ...
            case ACT_FAIL:
                sprintf(
                    ...
                    tBuff, actionName(event->sigSymbol),
                    (event->actStatus == ACT_DONE)
                        ? (_i18n_msg_get(ls_catd, NL_SETN, 3275, "completed"))
                        : /* catgets  3275  */
                        (_i18n_msg_get(
                            ls_catd, NL_SETN, 3276,
                            "exited with non-zero value")), /* catgets  3276  */
                    event->actPid);
                break;
            }
            prtLine(prline);
    ...
    case EVENT_JOB_MOVE:
        if (tBuff[0] == '\0')
            sprintf(prline,
                    (_i18n_msg_get(
                        ls_catd, NL_SETN, 3427,
                        "%-12.19s: Job moved to position %d relative to <%s> "
                        "by user or administrator <%s>")), /* catgets 3427 */
                    timeStampStr, event->position, topOrBot(event->base),
                    event->userName);
        else
            sprintf(prline,
                    (_i18n_msg_get(
                        ls_catd, NL_SETN, 3428,
                        "%-12.19s:%s moved to position %d relative to <%s> by "
                        "user or administrator <%s>")), /* catgets 3428 */
                    timeStampStr, tBuff, event->position, topOrBot(event->base),
                    event->userName);
        prtLine(prline);
        break;
    ...
    case EVENT_QUEUE_CTRL:
        ...
        if ((event->jStatus & JOB_STAT_DONE) ||
            ((event->jStatus & JOB_STAT_EXIT) &&
             !(event->reasons &
               (EXIT_ZOMBIE | EXIT_RERUN | EXIT_KILL_ZOMBIE)))) {
            LS_WAIT_T wStatus;

            LS_STATUS(wStatus) = event->exitStatus;
            if ((event->jStatus & JOB_STAT_EXIT) && event->exitStatus &&
                event->cpuTime >= MIN_CPU_TIME) {
                if (WIFEXITED(wStatus))
                    sprintf(
                        prline, " %s %d",
                        (_i18n_msg_get(ls_catd, NL_SETN, 3282,
                                       "with exit code")), /* catgets  3282  */
                        WEXITSTATUS(wStatus));
                else
                    sprintf(prline, " %s %d",
                            (_i18n_msg_get(ls_catd, NL_SETN, 3283,
                                           "by signal")), /* catgets  3283  */
                            WTERMSIG(wStatus));
                prtLine(prline);
            }
            ...
            if ((job->numExHosts > 0) && (!IS_POST_FINISH(event->jStatus))) {
                if (event->cpuTime < MIN_CPU_TIME)
                    sprintf(prline,
                            _i18n_msg_get(
                                ls_catd, NL_SETN, 3284,
                                "The CPU time used is unknown")); /* catgets
                                                                     3284  */
                else {
                    if (bhistReq->options & OPT_NORMALIZECPU)
                        event->cpuTime = event->cpuTime * job->cpuFactor /
                                         bhistReq->cpuFactor;
                    sprintf(
                        prline,
                        _i18n_msg_get(
                            ls_catd, NL_SETN, 3285,
                            "The CPU time used is %1.1f seconds"), /* catgets
                                                                      3285  */
                        event->cpuTime);
                }
                prtLine(". ");
    ...
    case EVENT_JOB_EXECUTE:
        if (tBuff[0] == '\0')
            sprintf(prline,
                    (_i18n_msg_get(
                        ls_catd, NL_SETN, 3286,
                        "%-12.19s: Running with execution home <%s>, Execution "
                        "CWD <%s>, Execution Pid <%d>")), /* catgets  3286  */
                    timeStampStr, event->execHome, event->execCwd,
                    event->jobPid);
        else
            sprintf(
                prline,
                (_i18n_msg_get(
                    ls_catd, NL_SETN, 3287,
                    "%-12.19s:%s running with execution home <%s>, Execution "
                    "CWD <%s>, Execution Pid <%d>")), /* catgets  3287  */
                timeStampStr, tBuff, event->execHome, event->execCwd,
                event->jobPid);
        prtLine(prline);
        if (event->execUsername && strcmp(event->execUsername, "") &&
            strcmp(job->user, event->execUsername)) {
            sprintf(prline, ", %s <%s>",
                    (_i18n_msg_get(ls_catd, NL_SETN, 3288,
                                   "Execution user name")), /* catgets  3288  */
                    event->execUsername);
            prtLine(prline);
        }
        prtLine(";\n");
    ...
    case EVENT_JOB_FORCE:
        if (tBuff[0] == '\0')
            sprintf(
                prline, "%-12.19s: %s <%s>", timeStampStr,
                I18N(3289,
                     "Job is forced to run by user or administrator") /* catgets
                                                                         3289 */
                ,
                event->userName);
        else
            sprintf(
                prline, "%-12.19s: %s %s <%s>", timeStampStr, tBuff,
                I18N(3290,
                     "is forced to run by user or administrator"), /* catgets
                                                                      3290 */
                event->userName);
        prtLine(prline);
        break;
    ...
    case EVENT_JOB_REQUEUE:
        if (tBuff[0] == '\0')
            sprintf(prline, "%-12.19s: %s", timeStampStr,
                    (_i18n_msg_get(
                        ls_catd, NL_SETN, 3291,
                        "Pending: Job has been requeued"))); /* catgets  3291 */

        else
            sprintf(prline, "%-12.19s:%s %s", timeStampStr, tBuff,
                    (_i18n_msg_get(
                        ls_catd, NL_SETN, 3292,
                        "pending: Job has been requeued"))); /* catgets  3292 */
        prtLine(prline);
        break;
    case EVENT_JOB_CLEAN:
        sprintf(prline, "%-12.19s: %s", timeStampStr,
                (_i18n_msg_get(
                    ls_catd, NL_SETN, 3293,
                    "Cleaned: Job has been removed"))); /* catgets  3293  */
        prtLine(prline);
        break;
    }
    return;
...
static void printChronicleEventLog(struct eventRec *log, struct bhistReq *req)
{
    ...
    strcpy(timeStampStr, _i18n_ctime(ls_catd, CTIME_FORMAT_a_b_d_T,
    ...
    case EVENT_PRE_EXEC_START:
        ...
        if (log->type == EVENT_PRE_EXEC_START)
            sprintf(
                prline, "%-12.19s:%s, %s", timeStampStr, tBuff,
                (_i18n_msg_get(
                    ls_catd, NL_SETN, 3298,
                    "the pre-exec command is started on"))); /* catgets  3298 */
        else
            sprintf(prline, "%-12.19s:%s, %s", timeStampStr, tBuff,
                    (_i18n_msg_get(
                        ls_catd, NL_SETN, 3299,
                        "the batch job command is started on"))); /* catgets
                                                                     3299  */

        prtLine(prline);
        if (log->eventLog.jobStartLog.numExHosts > 1) {
            sprintf(prline, " %d %s", log->eventLog.jobStartLog.numExHosts,
                    (_i18n_msg_get(ls_catd, NL_SETN, 3300,
                                   "Hosts/Processors"))); /* catgets  3300  */
            prtLine(prline);
        }
    ...
    case EVENT_JOB_EXECUTE:
        ...
        sprintf(prline,
                (_i18n_msg_get(
                    ls_catd, NL_SETN, 3302,
                    "%-12.19s:%s running with execution home <%s>, Execution "
                    "CWD <%s>, Execution Pid <%d>")), /* catgets  3302  */
                timeStampStr, tBuff, log->eventLog.jobExecuteLog.execHome,
        ...
        if (log->eventLog.jobExecuteLog.execUsername &&
            ...
            sprintf(prline, ", %s <%s>",
                    (_i18n_msg_get(ls_catd, NL_SETN, 3303,
                                   "Execution user name")), /* catgets  3303  */
        ...
        if ((event->jStatus & JOB_STAT_DONE) ||
            ...
            if ((event->jStatus & JOB_STAT_EXIT) && event->exitStatus &&
                ...
                if (WIFEXITED(wStatus))
                    sprintf(
                        prline, " %s %d",
                        (_i18n_msg_get(ls_catd, NL_SETN, 3305,
                                       "with exit code")), /* catgets  3305  */
                        WEXITSTATUS(wStatus));
                else
                    sprintf(prline, " %s %d",
                            (_i18n_msg_get(ls_catd, NL_SETN, 3283,
            ...
            if (event->numExHosts > 0) {
                if (event->cpuTime < MIN_CPU_TIME)
                    sprintf(prline, ". %s",
                            (_i18n_msg_get(
                                ls_catd, NL_SETN, 3307,
                                "The CPU time used is unknown"))); /* catgets
                                                                      3307  */
                else {
                    if (req->options & OPT_NORMALIZECPU)
                        event->cpuTime =
                            event->cpuTime * job->cpuFactor / req->cpuFactor;
                    sprintf(
                        prline,
                        (_i18n_msg_get(
                            ls_catd, NL_SETN, 3308,
                            ". The CPU time used is %1.1f seconds")), /* catgets
                                                                         3308 */
                        event->cpuTime);
                }
    ...
    case EVENT_JOB_SWITCH:
        ...
        sprintf(tBuff, " %s <%s>", I18N_Job, lsb_jobid2str(jobId));
        sprintf(prline,
                (_i18n_msg_get(ls_catd, NL_SETN, 3310,
                               "%-12.19s:%s switched to Queue <%s> by user or "
                               "administrator <%s>")), /* catgets  3310  */
                timeStampStr, tBuff, log->eventLog.jobSwitchLog.queue,
    ...
    case EVENT_JOB_MOVE:
        ...
        sprintf(prline,
                (_i18n_msg_get(
                    ls_catd, NL_SETN, 3312,
                    "%-12.19s:%s moved to position %d relative to <%s> by user "
                    "or administrator <%s>")), /* catgets  3312  */
                timeStampStr, tBuff, log->eventLog.jobMoveLog.position,
    ...
    case EVENT_CHKPNT:
        ...
        sprintf(tBuff, " %s <%s>", I18N_Job, lsb_jobid2str(jobId));
        sprintf(prline,
                (_i18n_msg_get(ls_catd, NL_SETN, 3314,
                               "%-12.19s:%s checkpoint info with period<%ld> "
                               "pid<%d> ok<%d> flags<%d>")), /* catgets  3314 */
                timeStampStr, tBuff, log->eventLog.chkpntLog.period,
                log->eventLog.chkpntLog.pid, log->eventLog.chkpntLog.ok,
                log->eventLog.chkpntLog.flags);
        prtLine(prline);
        prtLine(";\n");
        break;
    case EVENT_MIG:
        jobId = LSB_JOBID(log->eventLog.migLog.jobId, log->eventLog.migLog.idx);
        sprintf(tBuff, " %s <%s>", I18N_Job, lsb_jobid2str(jobId));
        if (log->eventLog.migLog.userName && log->eventLog.migLog.userName[0])
            sprintf(prline,
                    (_i18n_msg_get(ls_catd, NL_SETN, 3316,
                                   "%-12.19s:%s migration requested by user or "
                                   "administrator <%s>")), /* catgets  3316  */
                    timeStampStr, tBuff, log->eventLog.migLog.userName);
        else
            sprintf(prline,
                    (_i18n_msg_get(ls_catd, NL_SETN, 3317,
                                   "%-12.19s:%s migration requested by unknown "
                                   "user")), /* catgets
                                                3317
                                              */
                    timeStampStr, tBuff);
        ...
        if (log->eventLog.migLog.numAskedHosts) {
            sprintf(
                prline,
                (_i18n_msg_get(ls_catd, NL_SETN, 3318, "%s; Specified Hosts")),
                prline); /* catgets  3318  */
    ...
    case EVENT_JOB_SIGNAL:
        ...
        if (strcmp(log->eventLog.signalLog.signalSymbol, "DELETEJOB") == 0) {
            sprintf(
                prline, "%-12.19s:%s, %s <%s>", timeStampStr, tBuff,
                I18N(3321,
                     "delete requested by user or administrator"), /* catgets
                                                                      3321 */
                log->eventLog.signalLog.userName);
            if (log->eventLog.signalLog.runCount > 0)
                sprintf(prline, "%s; %s %d", prline,
                        (_i18n_msg_get(ls_catd, NL_SETN, 3322,
                                       "running times is")), /* catgets  3322 */
                        log->eventLog.signalLog.runCount);
        } else
            sprintf(
                prline,
                (_i18n_msg_get(ls_catd, NL_SETN, 3323,
                               "%-12.19s:%s, signal <%s> requested by user or "
                               "administrator <%s>")), /* catgets  3323  */
                timeStampStr, tBuff, log->eventLog.signalLog.signalSymbol,
                log->eventLog.signalLog.userName);
        prtLine(prline);
        prtLine(";\n");
        break;
    ...
    case EVENT_JOB_MSG:
        ...
        sprintf(prline,
                (_i18n_msg_get(
                    ls_catd, NL_SETN, 3329,
                    "%-12.19s:%s, message <%s>  message ID<%d> requested by "
                    "user or administrator <%s>")), /* catgets  3329  */
                timeStampStr, tBuff, log->eventLog.jobMsgLog.msg,
    ...
    case EVENT_JOB_MSG_ACK:
        ...
        sprintf(tBuff, " %s <%s>", I18N_Job, lsb_jobid2str(jobId));
        sprintf(prline,
                (_i18n_msg_get(ls_catd, NL_SETN, 3331,
                               "%-12.19s:%s, message <%s> message ID<%d>has "
                               "been dispatchd")), /* catgets  3331  */
                timeStampStr, tBuff, log->eventLog.jobMsgAckLog.msg,
        ...
        if ((strcmp(sigactLog->signalSymbol, "SIG_CHKPNT") == 0) ||
            ...
            if (IS_PEND(sigactLog->jStatus)) {
                if (sigactLog->period)
                    sprintf(
                        prline,
                        (_i18n_msg_get(ls_catd, NL_SETN, 3335,
                                       "%-12.19s:%s, checkpoint period is set "
                                       "to %d min.")), /* catgets  3335  */
                        timeStampStr, tBuff, (int) (sigactLog->period / 60));
                prtLine(prline);
                break;
            }
            ...
            if (sigactLog->actStatus == ACT_START) {
                ...
                sprintf(prline, "%-12.19s:%s %s %s (actpid %d)", timeStampStr,
                        ...
                        (sigactLog->flags & LSB_CHKPNT_MIG)
                            ? (_i18n_msg_get(ls_catd, NL_SETN, 3337,
                                             "Migration checkpoint"))
                            : /* catgets  3337  */
                            (_i18n_msg_get(ls_catd, NL_SETN, 3338,
                                           "Checkpoint")), /* catgets  3338  */
                ...
                tmpPtr = putstr_(
                    (sigactLog->actStatus == ACT_DONE)
                        ? (_i18n_msg_get(ls_catd, NL_SETN, 3341, "succeeded"))
                        : /* catgets  3341  */
                        (_i18n_msg_get(ls_catd, NL_SETN, 3342,
                                       "failed"))); /* catgets  3342  */
                sprintf(prline, "%-12.19s:%s %s %s (actpid %d)", timeStampStr,
                        tBuff,
                        (sigactLog->flags & LSB_CHKPNT_MIG)
            ...
            case ACT_START:
                sprintf(prline, "%-12.19s:%s %s %s (actpid %d)", timeStampStr,
                        tBuff, actionName(sigactLog->signalSymbol),
            ...
            case ACT_FAIL:
                sprintf(
                    ...
                    tBuff, actionName(sigactLog->signalSymbol),
                    (sigactLog->actStatus == ACT_DONE)
                        ? (_i18n_msg_get(ls_catd, NL_SETN, 3343, "completed"))
                        : /* catgets  3343  */
                        (_i18n_msg_get(
                            ls_catd, NL_SETN, 3344,
                            "exited with non-zero value")), /* catgets  3344  */
                    sigactLog->pid);
                break;
            }
            prtLine(prline);
    ...
    case EVENT_JOB_START_ACCEPT:
        ...
        sprintf(prline,
                (_i18n_msg_get(
                    ls_catd, NL_SETN, 3347,
                    "%-12.19s:%s, starting (Pid %d)")), /* catgets  3347  */
    ...
    default:
        ls_syslog(LOG_ERR, "%s: %s, log.type=%x", fname,
                  I18N(3356, "unknown type"), /* catgets 3356 */
                  log->type);
        break;
    }
}

static char *lowFirstChar(char *statement)
{
...
static int initJobIdIndexS(struct jobIdIndexS *indexS, char *fileName)
{
    ...
    if ((fscanf(indexS->fp, "%s %s %d %ld", tag, version, &(indexS->totalRows),
                &(indexS->lastUpdate)) != 4) ||
        (strcmp(tag, LSF_JOBIDINDEX_FILETAG))) {
        fprintf(
            stderr,
            I18N(3173, "wrong jobId index file format.\n")); /* catgets 3173 */
        fclose(indexS->fp);
        return (-1);
    }
