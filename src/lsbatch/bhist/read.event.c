/* $Id: read.event.c,v 1.10 2007/08/15 22:18:43 tmizan Exp $
...
#define GET_JOBID(jobId, idx)                                                  \
    ((Req.options & OPT_ARRAY_INFO)) ? (jobId) : LSB_JOBID((jobId), (idx))
...
char *bhist_calloc(int nelem, int esize)
{
    char *sp;

    if (nelem <= 0) {
        fprintf(stderr,
                (_i18n_msg_get(ls_catd, NL_SETN, 3451,
                               "Warning: calloc %d elements; allocating 1 "
                               "element\n")), /* catgets
                                                 3451
                                               */
                nelem);
...
char *bhist_malloc(int len)
{
    ...
    if (len <= 0) {
        fprintf(
            stderr,
            (_i18n_msg_get(
                ls_catd, NL_SETN, 3453,
                "Warning: malloc %d bytes; allocating %d bytes\n")), /* catgets
                                                                        3453  */
            len, sizeof(int));
        len = sizeof(int);
    }
...
void removeJobList(struct jobRecord *jobRecordListHead)
{
...
int addEvent(struct eventRecord *event, struct jobRecord *jobRecord)
{
    ...
    if ((readFromHeadFlag == 0) && (jobRecord->nonNewFromHeadFlag == 1) &&
...
char check_queue(struct bhistReq *Req, char *queue)
{
...
struct jobInfoEnt *read_newjob(struct eventRec *log)
{
    ...
    if (submitPtr->numAskedHosts > 0)
        submitPtr->askedHosts =
            (char **) bhist_calloc(submitPtr->numAskedHosts, sizeof(char *));
    ...
    if (submitPtr->nxf > 0) {
        submitPtr->xf =
            (struct xFile *) bhist_calloc(submitPtr->nxf, sizeof(struct xFile));
...
void copyJobModLog(struct jobModLog *des, struct jobModLog *src)
{
    int i;
...
struct jobInfoEnt *copyJobInfoEnt(struct jobInfoEnt *jobInfo)
{
    struct jobInfoEnt *job;
    struct submit *submitPtr;
    int i;
...
void freeJobInfoEnt(struct jobInfoEnt *jobInfoEnt)
{
    int i;
...
void freeJobRecord(struct jobRecord *jobRecord)
{
...
char check_host(struct bhistReq *Req, struct jobRecord *jobRecord)
{
    int i;
...
struct jobRecord *read_startjob(struct eventRec *log)
{
    ...
    if (log->type == EVENT_JOB_EXECUTE)
        jobId = (Req.options & OPT_ARRAY_INFO)
                    ? log->eventLog.jobExecuteLog.jobId
                    : LSB_JOBID(log->eventLog.jobExecuteLog.jobId,
                                log->eventLog.jobExecuteLog.idx);
    else
        jobId = (Req.options & OPT_ARRAY_INFO)
                    ? log->eventLog.jobStartLog.jobId
                    : LSB_JOBID(log->eventLog.jobStartLog.jobId,
    ...
    if (jobRecord->job->numExHosts > 0) {
        ...
        jobRecord->job->exHosts =
            (char **) bhist_calloc(jobRecord->job->numExHosts, sizeof(char *));

        for (i = 0; i < jobRecord->job->numExHosts; i++) {
            jobRecord->job->exHosts[i] = (char *) bhist_malloc(MAXHOSTNAMELEN);

            sprintf(jobRecord->job->exHosts[i], "%s",
                    jobStartLog->execHosts[i]);
        }
    }
...
char read_newstat(struct eventRec *log)
{
...
char read_sigact(struct eventRec *log)
{
    struct eventRecord *event;
    struct jobRecord *jobRecord;
    ...
    event->sigSymbol =
        (char *) bhist_malloc(strlen(sigactLog->signalSymbol) + 1);
...
char read_jobrequeue(struct eventRec *log)
{
    struct eventRecord *event;
    struct jobRecord *jobRecord;
    hEnt *ent;
...
char read_chkpnt(struct eventRec *log)
{
    struct eventRecord *event;
    struct jobRecord *jobRecord;
...
char read_mig(struct eventRec *log)
{
    struct eventRecord *event;
    struct jobRecord *jobRecord;
    ...
    if (event->migNumAskedHosts > 0) {
        event->migAskedHosts =
            (char **) bhist_calloc(event->migNumAskedHosts, sizeof(char *));
        for (i = 0; i < event->migNumAskedHosts; i++) {
            event->migAskedHosts[i] =
                (char *) bhist_malloc(strlen(migLog->askedHosts[i]) + 1);
...
char read_signal(struct eventRec *log)
{
    struct eventRecord *event;
    struct jobRecord *jobRecord;
    hEnt *ent;
    ...
    event->sigSymbol =
        (char *) bhist_malloc(strlen(log->eventLog.signalLog.signalSymbol) + 1);
...
char read_jobstartaccept(struct eventRec *log)
{
    struct eventRecord *event;
    struct jobRecord *jobRecord;
    hEnt *ent;
...
char read_jobmsg(struct eventRec *log)
{
    struct eventRecord *event;
    struct jobRecord *jobRecord;
    hEnt *ent;
    ...
    event->jmSrc =
        (char *) bhist_malloc(strlen(log->eventLog.jobMsgLog.src) + 1);
    strcpy(event->jmSrc, log->eventLog.jobMsgLog.src);
    event->jmDest =
        (char *) bhist_malloc(strlen(log->eventLog.jobMsgLog.dest) + 1);
    strcpy(event->jmDest, log->eventLog.jobMsgLog.dest);
    event->jmMsg =
        (char *) bhist_malloc(strlen(log->eventLog.jobMsgLog.msg) + 1);
...
char read_jobmsgack(struct eventRec *log)
{
    struct eventRecord *event;
    struct jobRecord *jobRecord;
    hEnt *ent;
    ...
    event->jmSrc =
        (char *) bhist_malloc(strlen(log->eventLog.jobMsgAckLog.src) + 1);
    strcpy(event->jmSrc, log->eventLog.jobMsgAckLog.src);
    event->jmDest =
        (char *) bhist_malloc(strlen(log->eventLog.jobMsgAckLog.dest) + 1);
    strcpy(event->jmDest, log->eventLog.jobMsgAckLog.dest);
    event->jmMsg =
        (char *) bhist_malloc(strlen(log->eventLog.jobMsgAckLog.msg) + 1);
...
char read_switch(struct eventRec *log)
{
    struct eventRecord *event;
    struct jobRecord *jobRecord;
    hEnt *ent;
...
char read_jobmove(struct eventRec *log)
{
    struct eventRecord *event;
    struct jobRecord *jobRecord;
    hEnt *ent;
...
char read_loadIndex(struct eventRec *log)
{
    int i;
    if (!loadIndex)
        return (FALSE);
...
char read_jobforce(struct eventRec *log)
{
    struct eventRecord *event;
    hEnt *ent;
    struct jobRecord *jobRecord;
...
int addJob(struct jobRecord *newjobRecord)
{
    hEnt *ent;
    struct jobRecord *jobRecord;
...
void parse_event(struct eventRec *log, struct bhistReq *Req)
{
        ...
        if (job != NULL) {
            ...
            if (check_queue(Req, submitPtr->queue)) {
                ...
                if ((Req->options & OPT_ARRAY_INFO) || numEles <= 0) {
                    if (!idxList) {
                        if ((idxList = (int *) calloc(1, sizeof(int))) ==
                            NULL) {
                            perror("malloc");
                            exit(-1);
                        }
                ...
                for (i = 0; i < numEles; i++) {
                    job->jobId =
                        LSB_JOBID(LSB_ARRAY_JOBID(job->jobId), idxList[i]);
                    if (!(newjobRecord = (struct jobRecord *) malloc(
                              sizeof(struct jobRecord)))) {
                        char i18nBuf[100];
                    ...
                    if (!(event = (struct eventRecord *) malloc(
    ...
    case EVENT_JOB_MODIFY:
        ...
        if (!(event =
                  (struct eventRecord *) malloc(sizeof(struct eventRecord)))) {
            perror("malloc");
            freeJobInfoEnt(job);
            exit(-1);
        }
            ...
            if (check_host(Req, jobRecord)) {
                ...
                if (log->type == EVENT_JOB_EXECUTE) {
                    ...
                    if (event->numExHosts > 0) {
                        event->exHosts = (char **) bhist_calloc(
                            event->numExHosts, sizeof(char *));
                        for (i = 0; i < event->numExHosts; i++) {
                            event->exHosts[i] =
                                (char *) bhist_malloc(MAXHOSTNAMELEN);
...
static struct jobRecord *createJobRec(int jobId)
{
    ...
    if (!(newjobRecord =
              (struct jobRecord *) malloc(sizeof(struct jobRecord)))) {
        perror("malloc");
        exit(-1);
    }
    newjobRecord->job = job;
