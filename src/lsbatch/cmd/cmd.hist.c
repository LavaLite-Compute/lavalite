...
int searchEventFile(struct histReq *req, int *eventFound)
{
...
void displayEvent(struct eventRec *log, struct histReq *req)
{
    char prline[MSGSIZE];
    ...
    case EVENT_QUEUE_CTRL:
        if ((req->opCode == SYS_HIST) ||
            ...
            case QUEUE_INACTIVATE:
                ...
                break;
            default:
                sprintf(prline, "%s <%d>", "unknown operation code",
...
static int isRequested(char *name, char **nameList)
{
    int i = 0;
