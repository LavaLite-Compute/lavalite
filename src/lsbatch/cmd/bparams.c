...
static void printShort(struct parameterInfo *reply)
{
...
static void printLong(struct parameterInfo *reply)
{
    ...
    if (lsbMode_ & LSB_MODE_BATCH) {
        ...
        printf("    PG_SUSP_IT = %d (%s)\n\n", reply->pgSuspendIt, "seconds");
    }

    printf("%s:\n",
           ("The amount of time during which finished jobs are kept in core"));
    printf("    CLEAN_PERIOD = %d (%s)\n\n", reply->cleanPeriod, "seconds");

    printf("%s:\n", ("The maximum number of finished jobs that can be stored "
                     "in current events file"));
    ...
    if (reply->jobPriorityValue > 0) {
        printf("%s.\n", "Job priority is increased by the system dynamically "
    ...
    if (reply->acctArchiveInDays > 0) {
        ...
        printf(" %19s = %d %s \n\n", "ACCT_ARCHIVE_AGE",
               reply->acctArchiveInDays, "days");
    }

    if (reply->acctArchiveInSize > 0) {
        printf("%s:\n", "Mbatchd Archive threshold");
        printf(" %20s = %d %s\n\n", "ACCT_ARCHIVE_SIZE",
               reply->acctArchiveInSize, "kB");
    }
}
