...
static void do_options(int, char **, int *, char **, char **, char **, char **,
                       float *, int *, char **);
...
#define LSB_SECURE_JOBINFO_USERS 0
    {"LSB_SECURE_JOBINFO_USERS", NULL}, {NULL, NULL}};
...
int main(int argc, char **argv)
{
    ...
    if (jInfoH == NULL) {
        if (numJids >= 1) {
            ...
            jobInfoErr(LSB_ARRAY_JOBID(jobId), jobName, user, queue, host,
                       options);
        }
        exit(-1);
    }
...
static void do_options(int argc, char **argv, int *options, char **user,
                       char **queue, char **host, char **jobName,
                       float *cpuFactor, int *format, char **projectName)
{
    extern char *optarg;
    int cc, Nflag = 0;
    char *norOp = NULL;
...
static void displayJobs(struct jobInfoEnt *job, struct jobInfoHead *jInfoH,
                        int options, int format)
{
    ...
    if (first) {
        ...
        if (job->jType == JGRP_NODE_ARRAY)
            ...
            printf(("JOBID   USER    STAT  QUEUE       FROM_HOST      JOB_NAME "
                    "          SUBMIT_TIME\n"));
            exechostfmt = "  ";
        } else {
    ...
    if (options == PEND_JOB) {
            ...
            if (lsbParams[LSB_SHORT_HOSTLIST].paramValue &&
                ...
                for (i = 1; i < hostList->listSize; i++) {
                    ...
                    if (execHostListUsed >= execHostListSize) {
                        execHostListSize += MAXLINELEN;
                        if ((execHostList = realloc(
                                 execHostList, execHostListSize)) == NULL) {
                            fprintf(stderr, "%s: %s failed\n", fName,
                                    "realloc");
                            exit(-1);
                        }
        ...
        if (Wflag == TRUE) {
            ...
            printf(" %-10s %-10s %-6d %-6d ", job->submit.projectName,
    ...
    if ((options & PEND_JOB) && IS_PEND(job->status)) {
        printf(lsb_pendreason(job->numReasons, job->reasonTb, NULL, loadIndex));
    }
...
static int skip_job(struct jobInfoEnt *job)
{
    int i;
...
static char *Time2String(int timer)
{
    ...
    sprintf(TimeStr, "%02d/%02d-%02d:%02d:%02d", Time->tm_mon + 1,
