...
int main(int argc, char **argv)
{
    ...
    TIMEIT(0, (queueInfo = lsb_queueinfo(queues, &numQueues, host, user, 0)),
           "lsb_queueinfo");
...
static void prtQueuesLong(int numQueues, struct queueInfoEnt *queueInfo)
{
    ...
    for (i = 0; i < numQueues; i++) {
        ...
        sprintf(fomt, "%%%dd %%%dd %%%dd %%%dd %%%dd %%%dd\n",
                QUEUE_NJOBS_LENGTH, QUEUE_PEND_LENGTH, QUEUE_RUN_LENGTH,
                QUEUE_SSUSP_LENGTH, QUEUE_USUSP_LENGTH, QUEUE_RSV_LENGTH);
        printf(fomt, qp->numJobs, qp->numPEND, qp->numRUN, qp->numSSUSP,
               qp->numUSUSP, qp->numRESERVE);
...
static void prtQueuesShort(int numQueues, struct queueInfoEnt *queueInfo)
{
    struct queueInfoEnt *qp;
    char statusStr[64];
    char first = FALSE;
    ...
    for (i = 0; i < numQueues; i++) {
        ...
        printf(fomt, qp->numJobs, qp->numPEND, qp->numRUN,
