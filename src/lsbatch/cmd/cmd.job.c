...
prtJobSubmit(struct jobInfoEnt *job, int prt_q, int tFormat)
{
    ...
    if (tFormat) {
        ...
        sprintf(prline, ("%s: Submitted from host <%s>"),
...
prtFileNames(struct jobInfoEnt *job, int prtCwd)
{
    ...
    if (job->submit.options & SUB_OUT_FILE) {
        sprintf(prline, ", %s <%s>",
	        "Output File", job->submit.outFile);
        prtLine(prline);
    }

    if (job->submit.options & SUB_ERR_FILE) {
        sprintf(prline, ", %s <%s>",
		"Error File", job->submit.errFile);
        prtLine(prline);
    }
...
prtJobRusage(struct jobInfoEnt *job)
{
   ...
   if (job->runRusage.utime > 0 || job->runRusage.stime > 0
	...
	if (job->runRusage.mem > 1024)
	    sprintf(prline, ("                     MEM: %d Mbytes"), job->runRusage.mem/1024);
	else
	    sprintf(prline, ("                     MEM: %d Kbytes"), job->runRusage.mem);
	prtLine(prline);
    }
    ...
    for (i=0; i < job->runRusage.npgids; i++) {
        sprintf(prline, ("                     PGID: %d;  "), job->runRusage.pgid[i]);
