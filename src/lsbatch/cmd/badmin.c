/* $Id: badmin.c,v 1.8 2007/08/15 22:18:43 tmizan Exp $
...
badminDebug (int nargc, char *nargv[], int opCode)
{
    ...
    while ((c = getopt(nargc, nargv, opt )) != EOF) {
		...
		if (!isdigit(optarg[i])) {
                    fprintf(stderr,"Command denied. Invalid level value\n");
		    return(-1);
		}
	    }
	    debug.level = atoi(optarg);
            if (opCode == MBD_DEBUG || opCode == SBD_DEBUG) {
                if (debug.level < 0 || debug.level > 3) {
                    fprintf(stderr,"Command denied. Valid debug level is [0-3] \n");
                    return(-1);
                }
            }
            else if (debug.level < 1 || debug.level > 5) {
                fprintf(stderr,"Command denied. Valid timing level is [1-5]\n");
                return(-1);
	    }
            break;
		...
		fprintf(stderr,"Command denied. Invalid file name\n");
            ...
            if (debug.logFileName[strlen(debug.logFileName)-1] == '/' ||
                ...
                fprintf(stderr,"Command denied. File name is needed after the path\n");
                return(-1);
            }
	    break;
        case 'o':
        ...
        for (i = 0; i < numHosts; i++) {
            ...
            if (hostInfo[i].hStatus
                ...
                if (hostInfo[i].hStatus & HOST_STAT_UNAVAIL)
                    fprintf(stderr, "failed : LSF daemon (LIM) is unavailable on host %s\n",
                            hostInfo[i].host);
                else
		    fprintf(stderr, "failed : Slave batch daemon (sbatchd) is unreachable now on host %s\n",
			    hostInfo[i].host);
                continue;
            }
	...
	return(retCode);
        ...
        numHosts = getNames (nargc, nargv, optind, &hosts, &all, "hostC");
	if (numHosts > 0) {
	    fprintf(stderr, "Host name does not need to be specified, set debug to the host which runs MBD\n");
	}
