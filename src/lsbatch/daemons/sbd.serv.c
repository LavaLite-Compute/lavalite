...
do_switchjob(XDR * xdrs, int chfd, struct packet_header * reqHdr)
{
    ...
    if (!found) {
	reply = ERR_NO_JOB;
	ls_syslog(LOG_ERR, "%s: mbatchd trying to switch a non-existent job <%s>", fname, lsb_jobid2str(jobSpecs.jobId));
	goto sendReply;
    }
...
do_modifyjob(XDR * xdrs, int chfd, struct packet_header * reqHdr)
{
    ...
    if (!found) {
	reply = ERR_NO_JOB;
	ls_syslog(LOG_ERR, "%s: mbatchd trying to modify a non-existent job <%s>", fname, lsb_jobid2str(jobSpecs.jobId));
	goto sendReply;
    }
    if (jp->jobSpecs.jStatus & (JOB_STAT_DONE | JOB_STAT_EXIT)) {
	reply = ERR_JOB_FINISH;
	...
	((jp->jobSpecs.lsfLimits[LSF_RLIMIT_CPU].rlim_maxl
	 ...
	 (jp->jobSpecs.lsfLimits[LSF_RLIMIT_CPU].rlim_curh
	  != jobSpecs.lsfLimits[LSF_RLIMIT_CPU].rlim_curh)
        ...
        ls_syslog(LOG_ERR, "%s, LSB_JOB_CPULIMIT is not set for the host, job <%s>, CPU limit not modified", fname, lsb_jobid2str(jobSpecs.jobId));
    } else {
	...
	ls_syslog(LOG_ERR, "%s, LSB_JOB_MEMLIMIT is not set for the host, job <%s>, memory limit not modified", fname, lsb_jobid2str(jobSpecs.jobId));
    } else {
...
do_sbdDebug(XDR * xdrs, int chfd, struct packet_header * reqHdr)
{
   ...
   if (!xdr_encodeMsg(&xdrs2, (char *) 0, &replyHdr, 0, 0, NULL)) {
    ...
    if (chanWrite_(chfd, reply_buf, XDR_GETPOS(&xdrs2)) <= 0) {
        ls_syslog(LOG_ERR, "%s: Sending  reply to master failed : %m",
	    fname);
        xdr_destroy(&xdrs2);
        return;
     }
...
do_jobSetup(XDR * xdrs, int chfd, struct packet_header * reqHdr)
{
    ...
    if (found == FALSE) {
	ls_syslog(LOG_ERR, "%s: Job <%s> is not found",
...
do_lsbMsg(XDR *xdrs, int s, struct packet_header * reqHdr)
{
    ...
    if (!found) {
	ls_syslog(LOG_ERR, "%s: JobId <%s> not found", fname, lsb_jobid2str(jobId));
	replyHdrWithRC(LSBE_NO_JOB, s, m.header->jobId);
        if (freeMsg && m.msg)
            FREEUP(m.msg);
	return;
...
replyHdrWithRC(int rc, int chfd, int jobId)
{
    ...
    if (chanWrite_(chfd, reply_buf, XDR_GETPOS(&xdrs2)) <= 0) {
        ls_syslog(LOG_ERR, "%s", __func__, lsb_jobid2str(tmpJobId), "chanWrite_");
	xdr_destroy(&xdrs2);
	return -1;
    }
    xdr_destroy(&xdrs2);
...
%s: registering supervisor for job=%d", fname, jp->jobSpecs.jobId);
	...
	if ( (msgStat = msgSupervisor(&pmMsg, jp->client)) < 0 ) {
	    ls_syslog(LOG_ERR, "%s: sending signal message to pm failed for job <%s>: %m", fname, lsb_jobid2str(jp->jobSpecs.jobId));
	}
    }
}
...
do_getPid(int s, struct jobCard *jp, struct lsbMsg *m, char *workMsgBuf)
{
    ...
    if ( (msgStat = msgSupervisor(&pmMsg, jp->client)) < 0 ) {
        ls_syslog(LOG_ERR, "%s: sending signal message to pm failed for job <%s>: %m",
        fname, lsb_jobid2str(jp->jobSpecs.jobId));
    }
}
