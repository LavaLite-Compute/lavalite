...
void do_newjob(XDR *xdrs, int chfd, struct packet_header *reqHdr)
{
    ...
    memcpy((char *) &jp->jobSpecs, (char *) &jobSpecs, sizeof(struct jobSpecs));
    ...
    if ((logclass & LC_TRACE) && jp->jobSpecs.jobSpoolDir[0] != 0) {
        ls_syslog(LOG_DEBUG, "%s: the SpoolDir for  job <%s>  is %s \n", fname,
                  lsb_jobid2str(jp->jobSpecs.jobId), jp->jobSpecs.jobSpoolDir);
    }
...
void do_switchjob(XDR *xdrs, int chfd, struct packet_header *reqHdr)
{
    ...
    if (!found) {
        reply = ERR_NO_JOB;
        ls_syslog(LOG_ERR,
                  "%s: mbatchd trying to switch a non-existent job <%s>", fname,
...
void do_modifyjob(XDR *xdrs, int chfd, struct packet_header *reqHdr)
{
    ...
    if (!found) {
        reply = ERR_NO_JOB;
        ls_syslog(LOG_ERR,
                  "%s: mbatchd trying to modify a non-existent job <%s>", fname,
                  lsb_jobid2str(jobSpecs.jobId));
        goto sendReply;
    }
    if (jp->jobSpecs.jStatus & (JOB_STAT_DONE | JOB_STAT_EXIT)) {
    ...
    if ((lsbJobCpuLimit != 1) &&
        ...
        ls_syslog(LOG_ERR,
                  "%s, LSB_JOB_CPULIMIT is not set for the host, job <%s>, CPU "
                  "limit not modified",
    ...
    if ((lsbJobMemLimit != 1) &&
        ...
        ls_syslog(LOG_ERR,
                  "%s, LSB_JOB_MEMLIMIT is not set for the host, job <%s>, "
                  "memory limit not modified",
                  fname, lsb_jobid2str(jobSpecs.jobId));
    } else {
...
void do_sigjob(XDR *xdrs, int chfd, struct packet_header *reqHdr)
{
    ...
    if (!xdr_jobSig(xdrs, &jobSig, reqHdr)) {
        reply = ERR_BAD_REQ;
        ls_syslog(LOG_ERR, "%s", __func__, "xdr_jobSig");
    ...
    if (!JOB_STARTED(jp)) {
        ...
        if (logclass & LC_EXEC)
            ls_syslog(LOG_DEBUG, "%s: Retry signal %s for job <%s>", fname,
                      getLsbSigSymbol(sigValue),
                      lsb_jobid2str(jp->jobSpecs.jobId));
        goto Reply1;
    }
    ...
    if (jp->jobSpecs.actPid || (jp->jobSpecs.jStatus & JOB_STAT_MIG)) {
        if ((cc = jobSigStart(jp, sigValue, jobSig.actFlags, jobSig.chkPeriod,
                              NO_SIGLOG)) < 0)
            reply = ERR_SIG_RETRY;
        else {
            jp->jobSpecs.jStatus &= ~JOB_STAT_MIG;
               ...
               struct packet_header *reqHdr)
{
    ...
    if (logclass & LC_EXEC)
        ls_syslog(LOG_DEBUG, "%s: jmsg len %d jobId %s msg <%s>", fname,
                  buf->len, lsb_jobid2str(jmsg.header->jobId), jmsg.msg);
...
Reply1:
    ...
    if (replyHdrWithRC(reply, s, jmsg.header->jobId) < 0) {
        ls_syslog(LOG_ERR, "%s", __func__, lsb_jobid2str(jmsg.header->jobId),
                  "replyHdrWithRC");
...
void deliverMsg(struct bucket *bucket)
{
    ...
    if (jp->jobSpecs.jStatus & JOB_STAT_RUN) {
        if (logclass & LC_EXEC)
            ls_syslog(LOG_DEBUG, "%s: jobId <%s> len %d chanfd %d", fname,
...
void do_reboot(XDR *xdrs, int chfd, struct packet_header *reqHdr)
{
    ...
    ls_syslog(LOG_NOTICE, "Slave batch daemon reboot command received");
    relife();
    ls_syslog(LOG_ERR, "Unable to relife during rebooting: %m");
    xdr_destroy(&xdrs2);
    return;
}
...
void do_shutdown(XDR *xdrs, int chfd, struct packet_header *reqHdr)
{
    ...
    xdr_destroy(&xdrs2);
    ls_syslog(LOG_NOTICE, "Slave batch daemon shutdown command received");
    die(SLAVE_SHUTDOWN);
}
...
void do_jobSetup(XDR *xdrs, int chfd, struct packet_header *reqHdr)
{
    ...
    if (!xdr_jobSetup(xdrs, &jsetup, reqHdr)) {
        ls_syslog(LOG_ERR, "%s", __func__, "xdr_jobSetup");
        return;
    }
    for (jp = jobQueHead->forw; (jp != jobQueHead); jp = jp->forw) {
    ...
    if (replyHdrWithRC(LSBE_NO_ERROR, chfd, jsetup.jobId) < 0) {
        ls_syslog(LOG_DEBUG, "%s: Reply header failed for job <%s>", fname,
                  lsb_jobid2str(jsetup.jobId));
    }
    if (logclass & LC_EXEC)
        ls_syslog(LOG_DEBUG1,
                  "%s: JobId %s jstatus %d reason %x  jobPid %d jobPGid %d "
                  "execUid %d execGid <%d> execUser <%s> execHome <%s> execCwd "
                  "<%s> execJobFlag %x cpuTime %f w_status %d",
...
void do_jobSyslog(XDR *xdrs, int chfd, struct packet_header *reqHdr)
{
    ...
    if (!xdr_jobSyslog(xdrs, &sysMsg, reqHdr)) {
        ls_syslog(LOG_ERR, "%s", __func__, "xdr_syslog");
        return;
    }
    if (replyHdrWithRC(LSBE_NO_ERROR, chfd, -1) < 0)
        ls_syslog(LOG_ERR, "%s", __func__, "replyHdrWithRC");
               ...
               struct clientNode *cln)
{
    ...
    jp->client = cln;
    if (jp->client == NULL) {
        ls_syslog(LOG_ERR, "%s: jp->client == NULL for parallel job <%s>",
    ...
    if (replyHdrWithRC(LSBE_NO_ERROR, s, m.header->jobId) < 0)
        ls_syslog(LOG_ERR, "%s", __func__, lsb_jobid2str(m.header->jobId),
                  "replyHdrWithRC");
...
void do_lsbMsg(XDR *xdrs, int s, struct packet_header *reqHdr)
{
    ...
    if (strcmp(header.dest, "sbatchd")) {
        ls_syslog(LOG_ERR, "%s: Ignore message not intended for sbatchd",
                  fname);
    ...
    if (logclass & LC_COMM)
        ls_syslog(LOG_DEBUG, "%s: Got opString <%s> from job <%s>", fname,
                  opString, lsb_jobid2str(jobId));
    ...
    if (!found) {
        ls_syslog(LOG_ERR, "%s: JobId <%s> not found", fname,
                  lsb_jobid2str(jobId));
        replyHdrWithRC(LSBE_NO_JOB, s, m.header->jobId);
        if (freeMsg && m.msg)
            FREEUP(m.msg);
    ...
    if (!strcmp(opString, "REG_OP"))
        ...
        ls_syslog(LOG_ERR, "%s: Got bad opstring <%s>", fname, opString);
        replyHdrWithRC(LSBE_PROTOCOL, s, m.header->jobId);
    }
    if (freeMsg && m.msg)
        FREEUP(m.msg);
...
static void do_syslogMsg(int s, char *buf)
{
    ...
    if ((level = getNextWord_(&buf)) == NULL) {
        ls_syslog(LOG_ERR, "%s: got garbage for level", fname);
        return;
    }
...
static void do_rusageMsg(int s, struct jobCard *jp, struct lsbMsg *m,
                         char *workMsgBuf)
{
...
static void do_regOpMsg(int s, struct jobCard *jp, struct lsbMsg *m,
                        char *workMsgBuf)
{
    ...
    ls_syslog(LOG_INFO, "\
%s: registering supervisor for job=%d",
    ...
    if (jp) {
        while ((op = getNextWord_(&workMsgBuf))) {
            ...
            if (!strcmp(op, "CHKPNT"))
                ...
                ls_syslog(LOG_ERR, "%s: Unknown reg op <%s> for job <%s>",
...
static void do_chkpntEndMsg(int s, struct jobCard *jp, struct lsbMsg *m,
                            char *workMsgBuf)
{
...
static void do_getPid(int s, struct jobCard *jp, struct lsbMsg *m,
                      char *workMsgBuf)
{
