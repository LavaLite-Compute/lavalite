...
extern sbdReplyType start_ajob(struct jData *
                               jDataPtr,
                               struct qData *qp,
...
switch_job (struct jData *jDataPtr, int options)
{
    ...
    if (reply != ERR_NO_ERROR) {
	ls_syslog(LOG_ERR, "%s: Job <%s>: Illegal reply code <%d> from host <%s>, switch job failed",
		  fname,
...
signal_job (struct jData *jp, struct jobSig *sendReq, struct jobReply *jobReply)
{
	...
	break;
      ...
      default:
        ls_syslog(LOG_ERR, "%s: Job <%s>: Illegal reply code <%d> from sbatchd on host <%s>", fname, lsb_jobid2str(jp->jobId), reply, toHost);
	reply = ERR_BAD_REPLY;
     }
...
probe_slave(struct hData *hData, char sendJobs)
{
	...
	break;
    default:
        ls_syslog(LOG_ERR, "%s: Illegal reply code <%d> from sbatchd on host <%s>", fname, reply, toHost);
        break;
    }
...
rebootSbd (char *host)
{
    ...
    if ((hData = getHostData(host)) == NULL) {
        ls_syslog(LOG_ERR, "%s: Failed to get hData of host <%s>",
	    fname,
	    host);
        return ERR_FAIL;
    }
    ...
    if (reply != ERR_NO_ERROR)
        ls_syslog(LOG_ERR, "%s: Illegal reply code <%d> from sbatchd on host <%s>", fname, reply, host);
    ...
    if (*cc < 0) {
		...
		*cnt = *cnt + 1;
		if (*cnt >= 5) {
		    ...
		    ls_syslog(LOG_ERR, "The above failure repeated %d times", *cnt);
		    *cnt = 0;
		}
	    } else if (*cnt >= 0) {
		strcpy(lastHost, toHost);
		ls_syslog(LOG_ERR, "%s: Failed to call sbatchd on host <%s>: %s",
		    caller,
		    toHost,
		    lsb_sysmsg());
		*cnt = 1;
...
callSbdDebug(struct debugReq *pdebug)
{
   ...
   if ((hData = getHostData(pdebug->hostName)) == NULL) {
    ...
    if (reply == ERR_NULL || reply == ERR_FAIL || reply == ERR_UNREACH_SBD)
										        return reply;
										    if (reply != ERR_NO_ERROR)
										        ls_syslog(LOG_ERR, "%s: Illegal reply code <%d> from sbatchd on host <%s>", fname, reply, pdebug->hostName);
    return reply;
