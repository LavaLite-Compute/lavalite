...
main (int argc, char **argv)
{
	...
	if (!strcasecmp(daemonParams[LSB_JOB_CPULIMIT].paramValue, "y"))
	{
	    ...
	    ls_syslog(LOG_ERR, "%s: LSB_JOB_CPULIMIT <%s> in lsf.conf is invalid. Valid values are y|Y or n|N; ignoring the parameter.",
	...
	if (!strcasecmp(daemonParams[LSB_JOB_MEMLIMIT].paramValue, "y"))
	{
	    ...
	    ls_syslog(LOG_ERR, "%s: LSB_JOB_MEMLIMIT <%s> in lsf.conf is invalid. Valid values are y|Y or n|N; ignoring the parameter.",
	...
	if (sbdSleepTime < 0) {
	    ls_syslog(LOG_ERR, "%s: Sleep time <%d> is not a non-negative integer; re-life", fname, sbdSleepTime);
	    relife();
	}
...
processMsg(struct clientNode *client)
{
    ...
    if (chanDequeue_(client->chanfd, &buf) < 0) {
        ls_syslog(LOG_ERR, "%s", __func__, "chanDequeue_",
            cherrno);
	...
	if (daemonParams[LSF_AUTH_DAEMONS].paramValue) {
		...
		ls_syslog(LOG_ERR,
			  "%s: Received request %d from bad port <%s>",
	...
	if (bucket) {
	    ...
	    ls_syslog(LOG_ERR, "%s: BATCH_JOB_MSG newBucket failed: %m",
		fname);
	}
	break;
    case SBD_JOB_SETUP:
...
sinit (void)
{
    ...
    ls_syslog(LOG_INFO, ("\
%s: Cluster %s, master %s"), "sbatchd/main", clusterName, masterHost);
...
authCmdRequest(struct clientNode *client, XDR *xdrs, struct LSFHeader *reqHdr)
{
	...
	ls_syslog(LOG_ERR, "%s", __func__, "hostOk",
	    fromHost,
	    sockAdd2Str_(&client->from));
	return LSBE_NOLSF_HOST;
    }
	...
	ls_syslog(LOG_ERR, "%s", __func__, "userok",
	    fromHost);
