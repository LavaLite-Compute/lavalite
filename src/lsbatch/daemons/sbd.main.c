/* $Id: sbd.main.c,v 1.15 2007/08/15 22:18:46 tmizan Exp $
...
int main(int argc, char **argv)
{
    ...
    sinit();
    ls_syslog(LOG_INFO, ("%s: (re-started")), fname);
    ...
    for (;;) {
        ...
        if (sbdSleepTime < 0) {
            ls_syslog(
                LOG_ERR,
                "%s: Sleep time <%d> is not a non-negative integer; re-life",
                fname, sbdSleepTime);
            relife();
        }
...
static void processMsg(struct clientNode *client)
{
    ...
    if (sbdReqtype == MBD_NEW_JOB || sbdReqtype == MBD_SIG_JOB ||
        ...
        if (lsbParams[LSF_AUTH_DAEMONS].paramValue) {
            ...
            if (authMbdRequest(client, &xdrs, &reqHdr) != LSBE_NO_ERROR) {
                ls_syslog(LOG_ERR, "%s", __func__, "authMbdRequest",
            ...
            if (!portok(&client->from)) {
                ls_syslog(LOG_ERR, "%s: Received request %d from bad port <%s>",
    ...
    } else if (sbdReqtype == CMD_SBD_REBOOT || sbdReqtype == CMD_SBD_SHUTDOWN ||
               sbdReqtype == CMD_SBD_DEBUG) {
        if ((cc = authCmdRequest(client, &xdrs, &reqHdr)) != LSBE_NO_ERROR) {
            ls_syslog(LOG_ERR, "%s", __func__, "authCmdRequest",
    ...
    case BATCH_JOB_MSG:
        ...
        if (bucket) {
            ...
            ls_syslog(LOG_ERR, "%s: BATCH_JOB_MSG newBucket failed: %m", fname);
        }
        break;
    case SBD_JOB_SETUP:
    ...
    case RM_JOB_MSG:
        ...
        break;
    default:
        ls_syslog(LOG_ERR, "%s: Unknown request type <%d>", fname, sbdReqtype);
        break;
    }
...
void start_master(void)
{
    ...
    if (newMbdPid == 0) {
        ...
        ls_syslog(LOG_ERR, "%s", __func__, "execve");
...
static int get_new_master(struct sockaddr_in *from)
{
    ...
    if (hostOk((char *) tmpHost, 0) < 0) {
        ls_syslog(LOG_ERR, "%s: Request from non-LSF host <%s/%s>", fname,
                  officialName, sockAdd2Str_(from));
        return -1;
    }
...
void sinit(void)
{
    ...
    ls_syslog(LOG_ERR, "%s: getBootTime( failed; assuming host was not rebooted while sbatchd was down"), fname);
    bootTime = 0;
    die(SLAVE_FATAL);
    }
    ...
    if (!jobQueHead || !clientList) {
        ls_syslog(LOG_ERR, "%s", __func__, "mkListHeader");
        die(SLAVE_FATAL);
    }
...
%s: Cluster %s, master %s"),
    ...
    if (!debug) {
        if (chdir(LSTMPDIR) < 0) {
            ...
            lsb_mperr(fname, "chdir", LSTMPDIR);
            die(SLAVE_FATAL);
        }
    }
...
static void drainMsgQueue(void)
{
    struct bucket *bucket;
...
static void houseKeeping(void)
{
    ...
    if (now - lastCheckMbdTime >= CHECK_MBD_TIME) {
        ...
        if (updMasterHost == NULL) {
            ...
            if (equal_host(masterHost, myhostnm)) {
                if (mbdPid != 0) {
                    if (kill(mbdPid, 0) != 0) {
                        if ((now - lastStartMbdTime >= 3 * CHECK_MBD_TIME) ||
                            !mbdStartedBySbd) {
                            start_master();
    ...
    if (now - lastTime >= sbdSleepTime / 2) {
        if (ls_servavail(2, 1) < 0)
            ls_syslog(LOG_ERR, "main", "ls_servavail");
        lastTime = now;
    }
}
                          ...
                          struct packet_header *reqHdr)
{
    ...
    if (hostOk(fromHost, 0) < 0) {
        ls_syslog(LOG_ERR, "%s", __func__, "hostOk", fromHost,
                  sockAdd2Str_(&client->from));
        return LSBE_NOLSF_HOST;
    }
    ...
    if (!userok(s, &client->from, fromHost, NULL, &auth, debug)) {
        ls_syslog(LOG_ERR, "%s", __func__, "userok", fromHost);
                          ...
                          struct packet_header *reqHdr)
{
    ...
    if (rc == -1) {
        ls_syslog(LOG_ERR, "%s", __func__, "verifyEAuth", fromHost);
        return LSBE_PERMISSION;
    }
