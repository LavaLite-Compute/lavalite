               ...
               sbdReplyType err)
{
    ...
    if ((logclass & LC_TRACE) && (logclass & LC_SIGNAL))
        ls_syslog(LOG_DEBUG, "%s: Entering ... regType %d jobId %s", fname,
    ...
    case LSBE_LOCK_JOB:
        ...
        if (reply == LSBE_LOCK_JOB) {
            if (IS_SUSP(jp->jobSpecs.jStatus))
                ...
                ls_syslog(LOG_ERR,
                          "%s: Job <%s> is in status <%x> and mbatchd wants to "
                          "lock it, ignored.",
                          fname, lsb_jobid2str(jp->jobSpecs.jobId),
                          jp->jobSpecs.jStatus);
        }
    ...
    case LSBE_NO_JOB:
        ...
        if (!IS_POST_FINISH(jp->jobSpecs.jStatus)) {
            ls_syslog(
                LOG_ERR,
                "%s: Job <%s> is forgotten by mbatchd on host <%s>, ignored.",
                fname, lsb_jobid2str(jp->jobSpecs.jobId), masterHost);
        }
    ...
    case LSBE_SBATCHD:
        ls_syslog(LOG_ERR,
                  "%s: mbatchd on host <%s> doesn't think I'm configured as a "
                  "batch server when I report the status for job <%s>",
                  fname, masterHost, lsb_jobid2str(jp->jobSpecs.jobId));
        return -1;
    default:
        ls_syslog(LOG_ERR,
                  "%s: Illegal reply code <%d> from mbatchd on host <%s> for "
                  "job <%s>",
                  fname, reply, masterHost, lsb_jobid2str(jp->jobSpecs.jobId));
        return -1;
    }
}
...
void getJobsState(struct sbdPackage *sbdPackage)
{
    ...
    for (;;) {
        ...
        if (cc < 0) {
            if (equal_host(masterHost, myhostnm)) {
                ...
                ls_syslog(LOG_ERR,
                          "%s: mbatchd on host <%s> not responding: %s", fname,
                          masterHost, lsb_sysmsg());
        } else {
            break;
        }
    ...
    case LSBE_NO_ERROR:
        ...
        if (!xdr_sbdPackage(&xdrs, sbdPackage, &hdr)) {
            ls_syslog(LOG_ERR, "%s", __func__, "xdr_sbdPackage", masterHost);
            lsb_merr(I18N_FUNC_S_FAIL, fname, "xdr_sbdPackage", masterHost);
            xdr_destroy(&xdrs);
            if (cc)
                free(reply_buf);
            return;
        ...
        for (i = 0; i < numJobs; i++) {
            if (!xdr_array_element(&xdrs, &jobSpecs, NULL, xdr_jobSpecs)) {
                ls_syslog(LOG_ERR, "%s", __func__, "xdr_jobSpecs", masterHost);
                lsb_merr(I18N_FUNC_S_FAIL, fname, "xdr_jobSpecs", masterHost);
                xdr_destroy(&xdrs);
                if (cc)
                    free(reply_buf);
                return;
        ...
        if (!xdr_sbdPackage1(&xdrs, sbdPackage, &hdr)) {
            ...
            lsb_merr(I18N_FUNC_S_FAIL, fname, "xdr_sbdPackage1", masterHost);
        }
...
void jobSetupStatus(int jStatus, int pendReason, struct jobCard *jp)
{
...
void sbdSyslog(int level, char *msg)
{
    ...
    for (retryInterval = 5, cc = -1; cc != 0;) {
        ...
        if (cc < 0) {
            ls_syslog(LOG_DEBUG, "%s: Job <%s> call_server(%s,%d) failed: %s",
                      fname, lsb_jobid2str(jobId), myhostnm, ntohs(sbd_port),
                      lsb_sysmsg());
...
int sendUnreportedStatus(struct chunkStatusReq *chunkStatusReq)
{
    ...
    if ((logclass & LC_TRACE) && (logclass & LC_SIGNAL))
        ls_syslog(LOG_DEBUG, "%s: Entering ... regType %d", fname,
                  BATCH_STATUS_CHUNK);
    ...
    for (i = 0; i < chunkStatusReq->numStatusReqs; i++) {
        ...
        len += ALIGNWORD_(strlen(chunkStatusReq->statusReqs[i]->execHome)) + 4 +
               ...
               ALIGNWORD_(strlen(chunkStatusReq->statusReqs[i]->execUsername)) +
               4;
    ...
    default:
        ls_syslog(LOG_ERR,
                  "%s: Illegal reply code <%d> from mbatchd on host <%s>",
                  fname, reply, masterHost);
        return -1;
    }
}
