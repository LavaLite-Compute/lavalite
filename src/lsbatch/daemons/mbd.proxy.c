/* $Id: mbd.proxy.c,v 1.5 2007/08/15 22:18:46 tmizan Exp $
...
static int proxyUSJLEnter(LIST_T *list, void *extra, LIST_EVENT_T *event)
{
    ...
    pxyPred = (PROXY_LIST_ENTRY_T *) listSearchEntry(
        uPtr->pxySJL, (void *) theJob,
        (LIST_ENTRY_EQUALITY_OP_T) startJobPrioEqual, 0);
    ...
    if (pxyPred == NULL)
        ...
        listInsertEntryAfter(uPtr->pxySJL, (LIST_ENTRY_T *) pxyPred,
                             (LIST_ENTRY_T *) newPxy);
    return 0;
}

static int proxyUSJLLeave(LIST_T *list, void *extra, LIST_EVENT_T *event)
{
    ...
    pxy = (PROXY_LIST_ENTRY_T *) listSearchEntry(
        uPtr->pxySJL, (void *) theJob,
        (LIST_ENTRY_EQUALITY_OP_T) proxyListEntryEqual, 0);
    if (pxy == NULL) {
        ls_syslog(LOG_ERR, "%s: cannot find the proxy for the job <%s>", fname,
                  lsb_jobid2str(theJob->jobId));
        return -1;
    }
...
void proxyUSJLAttachObsvr()
{
    ...
    observer = listObserverCreate(
        "User's Started Job Proxy List Observer", NULL,
        (LIST_ENTRY_SELECT_OP_T) NULL, LIST_EVENT_ENTER, proxyUSJLEnter,
        LIST_EVENT_LEAVE, proxyUSJLLeave, LIST_EVENT_NULL);

    if (observer == NULL) {
        ls_syslog(LOG_ERR,
                  "%s: failed to create an observer for user's started job "
                  "proxy list: %s",
    ...
    if (rc < 0) {
        ls_syslog(LOG_ERR,
                  "%s: failed to attach observer to the started job list",
                  fname);
        mbdDie(MASTER_MEM);
    }
}
...
void proxyUSJLAddEntry(struct jData *job)
{
    ...
    if (rc < 0)
        ls_syslog(LOG_ERR, "%s", __func__, "proxyHSJLEnter");
}

static int proxyHSJLEnter(LIST_T *list, void *extra, LIST_EVENT_T *event)
{
    static char fname[] = "proxyUSJLEnter";
    PROXY_LIST_ENTRY_T *pxyPred;
    PROXY_LIST_ENTRY_T *newPxy;
    ...
    FOR_EACH_JOB_LOCAL_EXECHOST(hPtr, theJob)
    {
        ...
        if (pxyPred == NULL)
            ...
            listInsertEntryAfter(hPtr->pxySJL, (LIST_ENTRY_T *) pxyPred,
                                 (LIST_ENTRY_T *) newPxy);
    }
...
static int proxyHSJLLeave(LIST_T *list, void *extra, LIST_EVENT_T *event)
{
    static char fname[] = "proxyUSJLEnter";
    PROXY_LIST_ENTRY_T *pxy;
    struct jData *theJob;
    ...
    FOR_EACH_JOB_LOCAL_EXECHOST(hPtr, theJob)
    {
        pxy = (PROXY_LIST_ENTRY_T *) listSearchEntry(
            ...
            (LIST_ENTRY_EQUALITY_OP_T) proxyListEntryEqual, 0);
        if (pxy == NULL) {
            ls_syslog(LOG_ERR, "%s: cannot find the proxy for the job <%s>",
...
void proxyHSJLAttachObsvr()
{
    ...
    observer = listObserverCreate(
        "Host's Started Job Proxy List Observer", NULL,
        (LIST_ENTRY_SELECT_OP_T) NULL, LIST_EVENT_ENTER, proxyHSJLEnter,
        LIST_EVENT_LEAVE, proxyHSJLLeave, LIST_EVENT_NULL);
...
void proxyHRsvJLAddEntry(struct jData *job)
{
    ...
    FOR_EACH_JOB_LOCAL_EXECHOST(hPtr, job)
    {
        if (hPtr->pxyRsvJL == NULL) {
            ...
            if (hPtr->pxyRsvJL == NULL) {
                ls_syslog(LOG_ERR,
                          "%s: failed to create host %s's proxy list: %s",
        ...
        pxy = (PROXY_LIST_ENTRY_T *) listSearchEntry(
            hPtr->pxyRsvJL, (LIST_ENTRY_T *) job,
            (LIST_ENTRY_EQUALITY_OP_T) proxyListEntryEqual, 0);
        if (pxy != NULL)
            continue;
...
void proxyRsvJLAddEntry(struct jData *job)
{
    ...
    if (pxyRsvJL == NULL) {
        ...
        if (pxyRsvJL == NULL) {
            ls_syslog(LOG_ERR,
                      "%s: failed to create the system wide proxy list for "
                      "slot-reserving jobs: %s",
                      fname, listStrError(listerrno));
            mbdDie(MASTER_MEM);
        }
    }
    ...
    pxyPred = (PROXY_LIST_ENTRY_T *) listSearchEntry(
        pxyRsvJL, (void *) job, (LIST_ENTRY_EQUALITY_OP_T) pendJobPrioEqual, 0);

    newPxy = proxyListEntryCreate((void *) job);
    if (pxyPred == NULL)
        listInsertEntryAtBack(pxyRsvJL, (LIST_ENTRY_T *) newPxy);
    else
        listInsertEntryAfter(pxyRsvJL, (LIST_ENTRY_T *) pxyPred,
                             (LIST_ENTRY_T *) newPxy);
    ...
    if (logclass & (LC_TRACE)) {
        ...
        ls_syslog(LOG_DEBUG, "%s: the current pxyRsvJL contents: <%s>", fname,
                  strBuf);
    }
}
...
PROXY_LIST_ENTRY_T *proxyListEntryCreate(void *subject)
{
    ...
    if (proxyEntFreeList == NULL) {
        ...
        if (!proxyEntFreeList) {
            ls_syslog(LOG_ERR, "%s: failed to create the proxy entry list: %s",
                      fname, listStrError(listerrno));
            mbdDie(MASTER_MEM);
        }
    }
    ...
    if (pxy == NULL)
        pxy = (PROXY_LIST_ENTRY_T *) my_calloc(1, sizeof(PROXY_LIST_ENTRY_T),
                                               fname);
    else
...
void proxyListEntryDestroy(PROXY_LIST_ENTRY_T *proxy)
{
...
char *jobProxySprintf(PROXY_LIST_ENTRY_T *proxy, void *hint)
{
...
struct jData *jobProxyGetPendJob(PROXY_LIST_ENTRY_T *proxy)
{
