...
proxyUSJLEnter(LIST_T *list, void *extra, LIST_EVENT_T *event)
{
    ...
    if (theJob == NULL) {
	ls_syslog(LOG_ERR, "%s: expected a non-NULL job pointer but got a NULL one",
	    fname);
	return -1;
    }
...
proxyUSJLAttachObsvr()
{
    ...
    if (observer == NULL) {
	ls_syslog(LOG_ERR, "%s: failed to create an observer for user's started job proxy list: %s",
    ...
    if (rc < 0) {
	ls_syslog(LOG_ERR, "%s: failed to attach observer to the started job list",
	    fname);
	mbdDie(MASTER_MEM);
    }
...
proxyHRsvJLAddEntry(struct jData *job)
{
	...
	if (hPtr->pxyRsvJL == NULL) {
	    ...
	    if (hPtr->pxyRsvJL == NULL) {
		ls_syslog(LOG_ERR, "%s: failed to create host %s's proxy list: %s",
...
proxyRsvJLAddEntry(struct jData *job)
{
	...
	if (pxyRsvJL == NULL) {
	    ls_syslog(LOG_ERR, "%s: failed to create the system wide proxy list for slot-reserving jobs: %s",
		fname, listStrError(listerrno));
	    mbdDie(MASTER_MEM);
	}
    }
