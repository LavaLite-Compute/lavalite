...
child_handler (int sig)
{
    ...
    while ((pid=wait3(&status, WNOHANG, &rusage)) > 0) {
		...
		if (mbdExitVal == MASTER_RECONFIG) {
		    ls_syslog(LOG_NOTICE, "mbatchd resigned for reconfiguration");
		    start_master();
		} else
	...
	for (jobCard = jobQueHead->forw; (jobCard != jobQueHead);
		...
		jobCard->exitPid = -1;
		if (logclass & LC_EXEC) {
		    ls_syslog(LOG_DEBUG, "child_handler: Job <%s> exitPid <%d> status <%d> exitcode <%d>",
		...
		if (logclass & LC_EXEC) {
		    ls_syslog(LOG_DEBUG, "child_handler: Job <%s> Pid <%d> status <%d> exitcode <%d>",
