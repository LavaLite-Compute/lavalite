...
static void updUAcct(struct jData *, struct uData *, struct hTab **, int, int,
                     int, int, int, struct hData *,
                     void (*)(struct userAcct *, void *), void *);
static void updHAcct(struct jData *, struct qData *, struct uData *,
                     struct hTab **, int, int, int, int,
                     void (*)(struct hostAcct *, void *), void *);
static void updHostData(char, struct jData *, int, int, int, int, int);
static void updUserData1(struct jData *, struct uData *, int, int, int, int,
                         int, int);
static void addValue(int *currentValue, int num, struct jData *jp, char *fname,
                     char *counter);
static void initUData(struct uData *);
static void addOneAbs(int *, int, int);
static struct userAcct *addUAcct(struct hTab **, struct uData *, int, int, int,
                                 int, int);
extern char *lsfDefaultProject;
extern int getQUsable(struct qData *);

void updCounters(struct jData *jData, int oldStatus, time_t eventTime)
{
    ...
    case JOB_STAT_RUN:
        if ((oldStatus & JOB_STAT_PEND) || (oldStatus & JOB_STAT_PSUSP)) {
            ...
            ls_syslog(LOG_DEBUG2, "%s: Job %s RWAIT to RUN", fname,
                      lsb_jobid2str(jData->jobId));
        } else {
            ls_syslog(LOG_ERR, "%s: Job <%s> transited from %d to JOB_STAT_RUN",
    ...
    case JOB_STAT_SSUSP:
        if (oldStatus & JOB_STAT_RUN) {
            ...
            ls_syslog(LOG_ERR,
                      "%s: Job <%s> transited from %d to JOB_STAT_SSUSP", fname,
    ...
    case JOB_STAT_USUSP:
        if (oldStatus & JOB_STAT_RUN) {
            ...
            ls_syslog(LOG_ERR,
                      "%s: Job <%s> transited from %d to JOB_STAT_USUSP", fname,
                      lsb_jobid2str(jData->jobId), oldStatus);
        }
        break;
    ...
    case JOB_STAT_DONE:
        ...
        if (oldStatus & JOB_STAT_WAIT) {
            ...
            ls_syslog(LOG_ERR, "%s: Job <%s> transited from %x to %x", fname,
                      lsb_jobid2str(jData->jobId), oldStatus, jData->jStatus);
        }
        break;
    ...
    case JOB_STAT_PEND:
        if (oldStatus & JOB_STAT_RUN) {
            ...
            ls_syslog(LOG_ERR, "%s: Job <%s> transited from %d to %d", fname,
                      lsb_jobid2str(jData->jobId), oldStatus, jData->jStatus);
        }
        break;
    case JOB_STAT_RUN | JOB_STAT_WAIT:
        if (oldStatus & JOB_STAT_RUN) {
            ls_syslog(LOG_DEBUG2, "%s: Job %s RUN to RWAIT", fname,
                      lsb_jobid2str(jData->jobId));
        } else if (oldStatus & JOB_STAT_PEND) {
            ls_syslog(LOG_DEBUG2, "%s: Job %s PEND to RWAIT", fname,
                      lsb_jobid2str(jData->jobId));
        } else {
            ls_syslog(LOG_ERR, "%s: Job %s transited from %d to %d", fname,
                      lsb_jobid2str(jData->jobId), oldStatus, jData->jStatus);
...
void updSwitchJob(struct jData *jp, struct qData *qfp, struct qData *qtp,
                  int oldNumReq)
{
...
void updQaccount(struct jData *jp, int numJobs, int numPEND, int numRUN,
                 int numSSUSP, int numUSUSP, int numRESERVE)
{
    ...
    if (logclass & LC_JLIMIT)
        ls_syslog(LOG_DEBUG1,
                  "%s: Entering with job=%s queue=%s numJobs=%d numPEND=%d "
                  "numRUN=%d numSSUSP=%d numUSUSP=%d numRESERVE=%d",
    ...
    if (logclass & LC_JLIMIT)
        ls_syslog(LOG_DEBUG2,
                  "%s: job=%s queue=%s numJobs=%d numPEND=%d numRUN=%d "
                  "numSSUSP=%d numUSUSP=%d numRESERVE=%d",
    ...
    updUAcct(jp, jp->uPtr, &(qp->uAcct), numRUN, numSSUSP, numUSUSP, numRESERVE,
             numPEND, (struct hData *) NULL, NULL, (void *) qp);

    FOR_EACH_USER_ANCESTOR_UGRP(jp->uPtr, grp)
    {
        updUAcct(jp, grp, &(qp->uAcct), numRUN, numSSUSP, numUSUSP, numRESERVE,
                 numPEND, (struct hData *) NULL, NULL, (void *) qp);
    }
    END_FOR_EACH_USER_ANCESTOR_UGRP;

    if (!newJob) {
        updHAcct(jp, qp, NULL, &(qp->hAcct), numRUN, numSSUSP, numUSUSP,
                 numRESERVE, NULL, (void *) qp);
    }

    return;
}

static void updUAcct(struct jData *jData, struct uData *up, struct hTab **uAcct,
                     int numRUN, int numSSUSP, int numUSUSP, int numRESERVE,
                     int numPEND, struct hData *hp,
    ...
    if ((foundU = getUAcct(*uAcct, up)) != NULL) {
        ...
        if (logclass & LC_JLIMIT)
            ls_syslog(LOG_DEBUG3,
                      "%s: job=%s host/queue=%s user=%s RUN=%d SSUSP=%d "
                      "USUSP=%d RESERVE=%d PEND=%d numNonPrmptRsv=%d",
                      fname, lsb_jobid2str(jData->jobId),
             ...
             numPEND != 0) {
        foundU = addUAcct(uAcct, up, numRUN, numSSUSP, numUSUSP, numRESERVE,
                          numPEND);
...
%s: New uAcct for job=%s host/queue=%s user=%s RUN=%d SSUSP=%d USUSP=%d RESERVE=%d, PEND=%d",
                fname, lsb_jobid2str(jData->jobId),
                ((hp == NULL) ? jData->qPtr->queue : hp->host),
                ((jData->uPtr == NULL) ? jData->userName : jData->uPtr->user),
                foundU->numRUN, foundU->numSSUSP, foundU->numUSUSP,
                foundU->numRESERVE, foundU->numPEND);
    } else {
        // Counter Bugs
        ls_syslog(LOG_ERR,
                  "%s: expected one of (numRUN, numSSUSP, "
                  "numUSUSP, numRESERVE and numPEND to be non-ZERO, "
    ...
    if (hp == NULL) {
        ...
        if (numSlots <= 0 && (logclass & LC_JLIMIT))
            ls_syslog(LOG_DEBUG3,
                      "%s: H's JL/U reached; job=%s host=%s user=%s", fname,
                      lsb_jobid2str(jData->jobId), hp->host, jData->uPtr->user);
    }
}

static struct userAcct *addUAcct(struct hTab **uAcct, struct uData *up,
                                 int numRUN, int numSSUSP, int numUSUSP,
                                 int numRESERVE, int numPEND)
{
    struct hEnt *newEnt;
    int new;
    struct userAcct *foundU;
    ...
    foundU =
        (struct userAcct *) my_calloc(1, sizeof(struct userAcct), "addUAcct");
...
struct userAcct *getUAcct(struct hTab *uAcct, struct uData *up)
{
    hEnt *uAcctEnt;
...
static void updHAcct(struct jData *jData, struct qData *qp, struct uData *up,
                     struct hTab **hAcct, int numRUN, int numSSUSP,
                     int numUSUSP, int numRESERVE,
                     void (*onNewEntry)(struct hostAcct *, void *), void *extra)
{
    ...
    for (i = 0; i < jData->numHostPtr; i++) {
        ...
        if ((foundH = getHAcct(*hAcct, hp)) != NULL) {
            ...
            if (logclass & LC_JLIMIT)
                ls_syslog(LOG_DEBUG3,
                          "%s: job=%s user/queue=%s host=%s RUN=%d SSUSP=%d "
                          "USUSP=%d RESERVE=%d NonPrmptRsv=%d",
                          fname, lsb_jobid2str(jData->jobId),
            ...
            foundH =
                addHAcct(hAcct, hp, numRUN, numSSUSP, numUSUSP, numRESERVE);
            ...
            if (logclass & LC_JLIMIT)
                ls_syslog(LOG_DEBUG2,
                          "%s: New hAcct: job=%s user/queue=%s host=%s RUN=%d "
                          "SSUSP=%d USUSP=%d RESERVE=%d",
                          fname, lsb_jobid2str(jData->jobId),
                          ((qp != NULL) ? qp->queue : up->user), hp->host,
                          foundH->numRUN, foundH->numSSUSP, foundH->numUSUSP,
                          foundH->numRESERVE);
        ...
        if (qp != NULL) {
            ...
            if (numSlots <= 0 && !(hp->hStatus & HOST_STAT_UNREACH) &&
                ...
                if (numSlots <= 0 && (logclass & (LC_PEND | LC_JLIMIT)))
                    ls_syslog(LOG_DEBUG2,
                              "%s: Q's JL/H reached. Set reason <%d>; job=%s "
                              "host=%s queue=%s",
                              fname, qp->reasonTb[1][hp->hostId],
                              lsb_jobid2str(jData->jobId), hp->host, qp->queue);
            }
            if (numSlots > 0 && (svReason == PEND_QUE_PROC_JLIMIT ||
            ...
            if (qp->hJobLimit == INFINIT_INT && qp->pJobLimit == INFINITY)
                pJobLimit = INFINITY;
...
struct hostAcct *getHAcct(struct hTab *hAcct, struct hData *hp)
{
    hEnt *hAcctEnt;
...
struct hostAcct *addHAcct(struct hTab **hAcct, struct hData *hp, int numRUN,
                          int numSSUSP, int numUSUSP, int numRESERVE)
{
    struct hEnt *newEnt;
    int new;
    ...
    foundH = (struct hostAcct *) my_malloc(sizeof(struct hostAcct), "addHAcct");
...
static void addOneAbs(int *num, int addedNum, int clean)
{
    if (clean == TRUE)
        *num = 0;
    if (addedNum > 0)
...
void updUserData(struct jData *jData, int numJobs, int numPEND, int numRUN,
                 int numSSUSP, int numUSUSP, int numRESERVE)
{
    ...
    if (grpPtr == NULL && numofugroups > 0) {
        grpPtr = (struct uData **) my_calloc(
            numofugroups, sizeof(struct uData *), "updUserData");
    }
    ...
    up = jData->uPtr;
    up->flags &= ~USER_GROUP;
    updUserData1(jData, up, numJobs, numPEND, numRUN, numSSUSP, numUSUSP,
                 numRESERVE);
    ...
    if (!newJob) {
        ...
        if (up->maxJobs == INFINIT_INT)
            ...
            numSlots = up->maxJobs - up->numRUN - up->numSSUSP - up->numUSUSP -
                       up->numRESERVE;
        ...
        if (logclass & (LC_PEND | LC_JLIMIT)) {
            if (numSlots <= 0) {
                ls_syslog(LOG_DEBUG2,
                          "%s: Set reason <%d> job=%s user=%s numJobs=%d "
                          "maxJobs=%d numPEND=%d",
                ...
                ls_syslog(LOG_DEBUG2,
                          "%s: Clear reason <%d> job=%s user=%s numJobs=%d "
                          "maxJobs=%d numPEND=%d",
                          fname, svReason, lsb_jobid2str(jData->jobId),
    ...
    if (!(up->flags & USER_INIT)) {
        ...
        FREEUP(up->gPtr);
        if (numNew > 0) {
            up->gPtr = (struct uData **) my_calloc(
                numNew, sizeof(struct uData *), "updUserData");
        }
    ...
    for (i = 0; i < up->numGrpPtr; i++) {
        ...
        updUserData1(jData, ugp, numJobs, numPEND, numRUN, numSSUSP, numUSUSP,
                     numRESERVE);
        ...
        if (!newJob) {
            ...
            if (logclass & (LC_PEND | LC_JLIMIT)) {
                if (num <= 0)
                    ls_syslog(LOG_DEBUG2,
                              "%s: Set reason <%d>; job=%s group=%s numJobs=%d "
                              "maxJobs=%d numPEND=%d",
                         ...
                         (logclass & (LC_PEND | LC_JLIMIT)))
                    ls_syslog(LOG_DEBUG2,
                              "%s: Clear reason <%d> job=%s group=%s "
                              "numJobs=%d maxJobs=%d numPEND=%d",
...
static void updUserData1(struct jData *jData, struct uData *up, int numJobs,
...
%s: job=%s user/group=%s PEND=%d RUN=%d SSUSP=%d USUSP=%d RESERVE=%d",
                  fname, lsb_jobid2str(jData->jobId), up->user, up->numPEND,
                  up->numRUN, up->numSSUSP, up->numUSUSP, up->numRESERVE);

    newJob = (numRUN == 0 && numSSUSP == 0 && numUSUSP == 0 && numRESERVE == 0);

    if (!newJob && up->pJobLimit < INFINITY) {
        updHAcct(jData, NULL, up, &(up->hAcct), numRUN, numSSUSP, numUSUSP,
                 numRESERVE, (void (*)(struct hostAcct *, void *)) NULL, NULL);
    }
}

static void updHostData(char updHPart, struct jData *jData, int numJobs,
                        int numRUN, int numSSUSP, int numUSUSP, int numRESERVE)
{
    ...
    for (i = 0; i < jData->numHostPtr; i++) {
        ...
        if (hp->uJobLimit < INFINIT_INT && (numRUN != 0 || numSSUSP != 0 ||
                                            numUSUSP != 0 || numRESERVE != 0)) {
        ...
        if (hp->numJobs >= hp->maxJobs) {
            ...
            if ((logclass & (LC_PEND | LC_JLIMIT)) &&
                hReasonTb[1][hp->hostId] == PEND_HOST_JOB_LIMIT) {
                ls_syslog(LOG_DEBUG2,
                          "%s: Clear reason <%d>; job=%s host=%s numJobs=%d "
                          "maxJobs=%d",
                          fname, hReasonTb[1][hp->hostId],
                          lsb_jobid2str(jData->jobId), hp->host, hp->numJobs,
                          hp->maxJobs);
                     ...
                     char *counter)
{
    *currentValue += num;
    ...
    ls_syslog(LOG_ERR,
              "%s: %s is negative; job=%s queue=%s currentValue=%d, num=%d",
              fname, counter, lsb_jobid2str(jp->jobId), jp->qPtr->queue,
              *currentValue, num);
    *currentValue = 0;
}
...
struct uData *getUserData(char *user)
{
    ...
    if (userEnt != NULL) {
        ...
        ls_syslog(LOG_ERR, "%s", __func__, "addUserData", user);
        return ((struct uData *) NULL);
    }

    uData = addUserData(user, INFINIT_INT, INFINITY, "mbatchd/getUserData",
                        FALSE, FALSE);
    if (uData != NULL)
        return uData;
...
int checkUsers(struct infoReq *req, struct userInfoReply *reply)
{
    sTab hashSearchPtr;
    hEnt *hashEntryPtr;
    struct uData *uData, *defUser;
    ...
    for (i = 0; i < req->numNames; i++) {
        ...
        uInfo->numStartJobs =
            uData->numJobs - uData->numPEND - uData->numRESERVE;
        uInfo->numJobs = uData->numJobs;
        uInfo->numPEND = uData->numPEND;
        uInfo->numRUN = uData->numRUN;
        uInfo->numSSUSP = uData->numSSUSP;
...
struct uData *addUserData(char *username, int maxjobs, float pJobLimit,
                          char *filename, int override, int config)
{
    ...
    if (logclass & (LC_JLIMIT))
        ls_syslog(LOG_DEBUG2,
                  "addUserData: New uData for user/group %s with maxJobs=%d "
                  "pJobLimit=%f",
    ...
    if (new) {
        uData =
            (struct uData *) my_calloc(1, sizeof(struct uData), "addUserData");
        ...
        if (filename)
            ls_syslog(LOG_ERR,
                      "%s: %s: User <%s> is multiply defined; retaining old "
                      "definition",
...
%s: new user <%s> added to the all user set",
        ...
        for (allUserGrp = (struct uData *) setIteratorBegin(&iter);
            ...
            if (uData->parents == NULL) {
                ...
                if (uData->parents == NULL) {
                    ls_syslog(LOG_ERR, "%s", __func__, "setCreate", strBuf,
...
void checkParams(struct infoReq *req, struct parameterInfo *reply)
{
    if (defaultQueues)
...
int isAuthManager(struct lsfAuth *auth)
{
    if (mSchedStage == M_STAGE_REPLAY)
        return TRUE;
...
void updResCounters(struct jData *jData, int newStatus)
{
    ...
    case JOB_STAT_SSUSP:
        if (!(jData->jStatus & JOB_STAT_RESERVE) &&
            ...
            ls_syslog(LOG_ERR, "%s: Job <%s> transited from %x to %x", fname,
                      lsb_jobid2str(jData->jobId), jData->jStatus, newStatus);
        }
        break;
    case JOB_STAT_USUSP:
        if (!(jData->jStatus & JOB_STAT_RESERVE) &&
...
static void initUData(struct uData *uData)
{
...
void updHostLeftRusageMem(struct jData *jobP, int order)
{
        ...
        if (resMem < INFINITY && resMem > -INFINITY) {
            ...
            for (numHost = 0; numHost < jobP->numHostPtr; numHost++) {
                ...
                if (logclass & (LC_TRACE | LC_SCHED))
                    ls_syslog(LOG_DEBUG,
                              "%s: job <%s> reserved mem is %f, execution host "
                              "<%s>, new leftRusageMem is %f, order is %d",
...
int64_t getFileSystemFree(char *path)
{
    struct statvfs buf;
