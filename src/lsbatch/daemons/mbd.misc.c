...
updCounters (struct jData *jData, int oldStatus, time_t eventTime)
{
    ...
    case JOB_STAT_SSUSP:
        if (oldStatus & JOB_STAT_RUN) {
            ...
            ls_syslog(LOG_ERR, "%s: Job <%s> transited from %d to JOB_STAT_SSUSP",
                      fname, lsb_jobid2str(jData->jobId), oldStatus);
        }
        break;
    ...
    case JOB_STAT_USUSP:
        if (oldStatus & JOB_STAT_RUN) {
            ...
            ls_syslog(LOG_ERR, "%s: Job <%s> transited from %d to JOB_STAT_USUSP",
                      fname, lsb_jobid2str(jData->jobId), oldStatus);
        }
        break;
...
%s: New uAcct for job=%s host/queue=%s user=%s RUN=%d SSUSP=%d USUSP=%d RESERVE=%d, PEND=%d",
        ...
        // Counter Bugs
        ls_syslog(LOG_ERR, "%s: expected one of (numRUN, numSSUSP, "
                  "numUSUSP, numRESERVE and numPEND to be non-ZERO, "
                  "but they are all ZERO", __func__);
        abort();
        return;
    }
          ...
          char *counter)
{
    *currentValue += num;
    ...
    ls_syslog(LOG_ERR, "%s: %s is negative; job=%s queue=%s currentValue=%d, num=%d", fname, counter, lsb_jobid2str(jp->jobId), jp->qPtr->queue, *currentValue, num);
    *currentValue = 0;
...
getUserData (char *user)
{
    ...
    if (userEnt != NULL) {
        ...
        ls_syslog(LOG_ERR, "%s", __func__, "addUserData",
                  user);
             ...
             char *filename, int override, int config)
{
    ...
    if (new) {
        ...
        if (filename)
            ls_syslog(LOG_ERR, "%s: %s: User <%s> is multiply defined; retaining old definition",
                      fname,
                      filename,
                      username);
        return (struct uData *)userEnt->hData;
             ...
             allUserGrp = (struct uData *)setIteratorGetNextElement(&iter))
        {
            ...
            if (uData->parents == NULL) {
                ...
                uData->parents = setCreate(MAX_GROUPS,
                                           ...
                                           strBuf);
                if (uData->parents == NULL) {
                    ls_syslog(LOG_ERR, "%s", __func__, "setCreate", strBuf, setPerror(bitseterrno));
                    mbdDie(MASTER_MEM);
                }
            }
