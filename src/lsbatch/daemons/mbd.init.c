...
minit (int mbdInitFlags)
{
    ...
    if (allUsersSet == NULL) {
	ls_syslog(LOG_ERR, "%s: failed to create all user set: %s",
    ...
    if (uGrpAllSet == NULL) {
	ls_syslog(LOG_ERR, "%s: failed to create the set of  'all;' user groups: %s",
	    fname, setPerror(bitseterrno));
	mbdDie(MASTER_MEM);
    }
	...
	ls_syslog(LOG_ERR, "%s: failed to create the set of ancestor user group of 'all;' user groups: %s",
	    fname, setPerror(bitseterrno));
	mbdDie(MASTER_MEM);
    }
    ...
    if ((hPtr = getHostData (masterHost)) == NULL
		...
		ls_syslog(LOG_WARNING, "%s: Host <%s> is not defined in the Host section of the lsb.hosts file", fname, masterHost);
...
readUserConf (int mbdInitFlags)
{
	...
	mbdInitFlags == RECONFIG_CONF) {
        ...
        if (userFileConf == NULL && lserrno == LSE_NO_FILE) {
            ls_syslog(LOG_ERR, "%s: File <%s> can not be found, default user will be used",
		    fname,
		    fileName);
...
readQueueConf (int mbdInitFlags)
{
   ...
   if (mbdInitFlags == FIRST_START || mbdInitFlags == RECONFIG_CONF) {
    ...
    if ((qp = getQueueData("default")) != NULL) {
        ls_syslog(LOG_WARNING, "%s: Using the default queue <default> provided by lsb.queues configuration file", fname);
        qp->qAttrib |= Q_ATTRIB_DEFAULT;
...
createDefQueue (void)
{
    ...
    ls_syslog(LOG_WARNING, "%s: Using the default queue <default> provided by the batch system", fname);
    FREEUP (defaultQueues);
    defaultQueues = safeSave ("default");
    qp = initQData();
    qp->qAttrib |= Q_ATTRIB_DEFAULT;
         ...
         int override)
{
	...
	*(hData->windows) = '\0';
	while ((word = getNextWord_(&sp)) != NULL) {
		...
		ls_syslog(LOG_ERR, "%s: Bad time expression <%s>; ignored.", fname, word);
                lsb_CheckError = WARNING_ERR;
                freeWeek (hData->week);
		free (save);
		continue;
                       ...
                       struct gData *groups[], int *ngroups)
{
        ...
        if (pw != NULL) {
            ...
            if (unixGrp) {
                ...
                subgrpPtr = addUnixGrp (unixGrp, word, filename,
                                       groups, ngroups);
                if (!subgrpPtr) {
                    ls_syslog(LOG_WARNING, "%s: No valid users defined in Unix group <%s>; ignoring", fname, word);
                    lsb_CheckError = WARNING_ERR;
                    return;
                }
	...
	if (isgrp)
            ls_syslog(LOG_ERR, "%s: Group <%s> is multiply defined in group <%s>; ignored", fname, word, groupPtr->group);
        else
            ls_syslog(LOG_ERR, "%s: Member <%s> is multiply defined in group <%s>; ignored", fname, word, groupPtr->group);
	lsb_CheckError = WARNING_ERR;
        return;
    }
...
parseAUids (struct qData *qp, char *line)
{
    ...
    if (numAds) {
        ...
        ls_syslog(LOG_ERR, "%s: No queue's administrators defined; ignored", fname);
        lsb_CheckError = WARNING_ERR;
        return;
    }
    sp = line;
    ...
    if (!admins.nAdmins) {
        ls_syslog(LOG_ERR, "%s: No valid queue's administrators defined in <%s> for queue <%s>; ignored", fname, line, qp->queue);
        lsb_CheckError = WARNING_ERR;
   ...
   if (groupType == USER_GRP)
    ...
    while ((word = getNextWord_(&line)) != NULL) {
        if (isInGrp (word, mygp)) {
            ls_syslog(LOG_ERR, "%s/%s: Member <%s> is multiply defined; ignored", filename, fname, word);
            lsb_CheckError = WARNING_ERR;
            continue;
        }
	if (groupType == USER_GRP) {
		...
		TIMEIT(0, unixGrp = mygetgrnam(word), "parseGroups_getgrnam");
            if (unixGrp != NULL) {
                ...
                if (gp == NULL) {
                    ls_syslog(LOG_WARNING, "%s/%s: No valid users in Unix group <%s>; ignoring", filename, fname, word);
		    lsb_CheckError = WARNING_ERR;
                    continue;
                }
                mygp->gPtr[mygp->numGroups] = gp;
...
setParams(struct paramConf *paramConf)
{
    ...
    if (paramConf == NULL || paramConf->param == NULL) {
        ls_syslog(LOG_ERR, "%s: paramConf or param in paramConf is NULL; default  parameters will be used", fname);
        return;
    }
		...
		int *nTempGroups)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        if (getGrpData (tempGData, gPtr->group, *nTempGroups)) {
            ls_syslog(LOG_WARNING, "%s: Group <%s> is multiply defined in %s; ignoring", fname, gPtr->group, HUgroups);
            lsb_CheckError = WARNING_ERR;
            continue;
        }
	    ...
	    grpPtr->numGroups == 0 &&
            ...
            ls_syslog(LOG_WARNING, "%s: No valid member in group <%s>; ignoring the group", fname, grpPtr->group);
            lsb_CheckError = WARNING_ERR;
	    freeGrp (grpPtr);
            *nTempGroups -= 1;
        }
...
addHostData (int numHosts, struct hostInfoEnt *hosts)
{
    ...
    if (numHosts == 0 || hosts == NULL) {
        ls_syslog(LOG_ERR, "%s: No hosts specified in lsb.hosts; all hosts known by LSF will be used", fname);
        addDefaultHost();
        return;
    }
    for (i = 0; i < numHosts; i++) {
        ...
        if (j == numLsfHosts) {
            ls_syslog(LOG_ERR, "%s: Host <%s> is not used by the batch system; ignored", fname, hosts[i].host);
            continue;
        }
	if (lsfHostInfo[j].isServer != TRUE) {
	    ls_syslog(LOG_ERR, "%s: Host <%s> is not a server; ignoring", fname, hosts[i].host);
            continue;
        }
...
addQData (struct queueConf *queueConf, int mbdInitFlags )
{
    ...
    if (queueConf == NULL || queueConf->numQueues <= 0
        ...
        ls_syslog(LOG_ERR, "%s: No valid queue in queueConf structure",
	    fname);
        lsb_CheckError = WARNING_ERR;
        return;
    }
    ...
    for (i = 0; i < queueConf->numQueues; i++) {
        ...
        if (qp->uGPtr)
            if (qp->uGPtr->memberTab.numEnts == 0 &&
                           qp->uGPtr->numGroups == 0) {
                ls_syslog(LOG_ERR, "%s: No valid value for key USERS in the queue <%s>; ignoring the queue",
	            fname, qp->queue);
                lsb_CheckError = WARNING_ERR;
                badqueue = TRUE;
            }
	if (   qp->hostList != NULL
	    ...
	    && qp->askedOthPrio <= 0)
        {
            ls_syslog(LOG_ERR, "%s: No valid value for key HOSTS in the queue <%s>; ignoring the queue",
	        fname, qp->queue);
            lsb_CheckError = WARNING_ERR;
            badqueue = TRUE;
        }
                ...
                *(qp->windows) = '\0';
                ...
                while ((word = getNextWord_(&sp)) != NULL) {
                    ...
                    if (addWindow(word, qp->weekR, qp->queue) < 0) {
                        ls_syslog(LOG_ERR, "%s: Bad time expression <%s>/<%s> for run windows of queue <%s>; ignored",
            ...
            *(qp->windowsD) = '\0';
            ...
            while ((word = getNextWord_(&sp)) != NULL) {
                ...
                if (addWindow(word, qp->week, qp->queue) <0) {
                    ls_syslog(LOG_ERR, "%s: Bad time expression <%s>/<%s> for dispatch windows of queue <%s>; ignored",
 	...
 	if (!qp->windows && !qp->windowsD
        ...
        if (queue->defaultHostSpec) {
		 ...
		 getHostFactor (queue->defaultHostSpec)) == NULL) {
                ls_syslog(LOG_ERR, "%s: Invalid value <%s> for %s in queue <%s>; ignored",
	            fname,
	...
	if (queue->hostSpec)
        ...
        if (queue->resReq) {
            ...
            {
                ls_syslog(LOG_ERR, "%s: RES_REQ <%s> for the queue <%s> is invalid; ignoring", fname, queue->resReq, qp->queue);
                lsb_CheckError = WARNING_ERR;
            } else
	    ...
	    struct resVal *resValPtr;
            ...
            {
                 ls_syslog(LOG_ERR, "%s: RESUME_COND <%s> for the queue <%s> is invalid; ignoring", fname, queue->resumeCond, qp->queue);
                 lsb_CheckError = WARNING_ERR;
	    } else {
	    ...
	    struct resVal *resValPtr;
            if ((resValPtr =
                 checkResReq (queue->stopCond,
			      USE_LOCAL | CHK_TCL_SYNTAX)) == NULL)
           {
                ls_syslog(LOG_ERR, "%s: STOP_COND <%s> for the queue <%s> is invalid; ignoring", fname, queue->stopCond, qp->queue);
                lsb_CheckError = WARNING_ERR;
            } else {
...
createCondNodes (int numConds, char **conds, char *fileName, int flags)
{
	...
	condNode->flags = flags;
        if ((condNode->rootNode = parseDepCond (conds[i], &auth, &errcode,
		       NULL, &jFlags, 0)) == NULL)
	{
            ls_syslog(LOG_ERR, "%s: Bad condition <%s> in file <%s> :%M; ignored", fname, conds[i], fileName);
	    lsb_CheckError = WARNING_ERR;
	}
...
updCondData (struct lsConf *conf, int fileType)
{
    ...
    if (conf == NULL) {
        ls_syslog(LOG_ERR, "%s: NULL Conf pointer for fileType <%x>", fname, fileType);
        return FALSE;
    }
    ...
    while (hEntPtr) {
        switch (listType) {
            ...
            case UDATA:
                ...
                break;
            default:
                ls_syslog(LOG_ERR, "%s: Bad data list type <%d>",
		    fname,
		    listType);
                return;
         }
...
parseQHosts(struct qData *qp, char *hosts)
{
    ...
    if (hosts == NULL) {
        ls_syslog(LOG_ERR, "%s: NULL pointer in HOSTS for queue <%s>",
	    fname, qp->queue);
	return -1;
    }
...
parseFirstHostErr(int returnErr, char *fname, char *hosts, struct qData *qp, struct askedHost *returnHosts, int numReturnHosts)
{
    ...
    if (   ( returnErr == LSBE_MULTI_FIRST_HOST )
        ...
        if (returnErr == LSBE_MULTI_FIRST_HOST )
            ls_syslog(LOG_ERR, "%s: Multiple first execution hosts specified <%s> in HOSTS section for queue <%s>. Ignored.",
	        fname, hosts, qp->queue);
        else if ( returnErr == LSBE_HG_FIRST_HOST )
            ls_syslog(LOG_ERR, "%s: host group <%s> specified as first execution host in HOSTS section for queue <%s>. Ignored.",
                fname, hosts, qp->queue);
        else if ( returnErr == LSBE_HP_FIRST_HOST )
            ls_syslog(LOG_ERR, "%s: host partition <%s> specified as first execution hosts specified <%s> in HOSTS section for queue <%s>. Ignored.",
		fname, hosts, qp->queue);
