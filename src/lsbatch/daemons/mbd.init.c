...
int mbd_init(int mbdInitFlags)
{
    ...
    if (uGrpAllAncestorSet == NULL) {
        ls_syslog(LOG_ERR,
                  "%s: failed to create the set of ancestor user group of "
                  "'all;' user groups: %s",
                  fname, setPerror(bitseterrno));
        mbdDie(MASTER_MEM);
    }
...
static int readHostConf(int mbdInitFlags)
{
    ...
    if (mbdInitFlags == FIRST_START || mbdInitFlags == RECONFIG_CONF) {
        ...
        if (hostFileConf == NULL && lserrno == LSE_NO_FILE) {
            ls_syslog(LOG_ERR,
                      "%s: File <%s> can not be found, all hosts known by LSF "
                      "will be used",
                      fname, fileName);
            addDefaultHost();
            return 0;
    ...
    if ((hostConf = lsb_readhost(hostFileConf, allLsInfo, CONF_CHECK,
                                 &clusterConf)) == NULL) {
        ls_syslog(LOG_ERR, "%s", __func__, "lsb_readhost");
        if (lsb_CheckMode) {
            lsb_CheckError = FATAL_ERR;
            return -1;
        } else
...
struct hData *initHData(struct hData *hData)
{
    int i;
...
static void initThresholds(float loadSched[], float loadStop[])
{
    int i;
...
static void readUserConf(int mbdInitFlags)
{
    ...
    if (mbdInitFlags == FIRST_START || mbdInitFlags == RECONFIG_CONF) {
        ...
        userConf =
            (struct userConf *) my_calloc(1, sizeof(struct userConf), fname);
        goto defaultUser;
    }
    ...
    if ((userConf = lsb_readuser_ex(userFileConf, CONF_CHECK, &clusterConf,
                                    &sharedConf)) == NULL) {
        ls_syslog(LOG_ERR, "%s", __func__, "lsb_readuser_ex");
        if (lsb_CheckMode) {
            lsb_CheckError = FATAL_ERR;
    ...
    createTmpGData(userConf->ugroups, userConf->numUgroups, USER_GRP,
                   tempUGData, &nTempUGroups);
...
defaultUser:
    ...
    if (!defUser) {
        ...
        if (userEnt == NULL) {
            addUserData("default", INFINIT_INT, INFINITY, "readUserConf", FALSE,
                        TRUE);
        }
        defUser = TRUE;
    }
...
static void readQueueConf(int mbdInitFlags)
{
    ...
    if (mbdInitFlags == FIRST_START || mbdInitFlags == RECONFIG_CONF) {
        ...
        if (queueFileConf == NULL && lserrno == LSE_NO_FILE) {
            ls_syslog(
                LOG_ERR,
                "%s: File <%s> can not be found, default queue will be used",
                fname, fileName);
            createDefQueue();
            return;
        }
    ...
    fillSharedConf(&sharedConf);
    if ((queueConf = lsb_readqueue(queueFileConf, allLsInfo,
                                   CONF_CHECK | CONF_RETURN_HOSTSPEC,
                                   &sharedConf)) == NULL) {
        if (lsberrno == LSBE_CONF_FATAL) {
            ls_syslog(LOG_ERR, "%s", __func__, "lsb_readqueue");
            if (lsb_CheckMode) {
                lsb_CheckError = FATAL_ERR;
                return;
            } else
            ...
            lsb_CheckError = WARNING_ERR;
            ls_syslog(LOG_ERR, "%s: failded, default queue will be used",
                      __func__);
            createDefQueue();
            return;
        }
    ...
    if ((qp = getQueueData("default")) != NULL) {
        ls_syslog(LOG_WARNING,
                  "%s: Using the default queue <default> provided by "
                  "lsb.queues configuration file",
                  fname);
        qp->qAttrib |= Q_ATTRIB_DEFAULT;
...
static void createDefQueue(void)
{
    ...
    ls_syslog(
        LOG_WARNING,
        "%s: Using the default queue <default> provided by the batch system",
        fname);
    FREEUP(defaultQueues);
    defaultQueues = safeSave("default");
    qp = initQData();
    qp->qAttrib |= Q_ATTRIB_DEFAULT;
    qp->description =
        safeSave("This is the default queue provided by the batch system. Jobs "
                 "are scheduled with very loose control.");
...
static void addHost(struct hostInfo *hI, struct hData *hD, char *filename,
                    int override)
{
    ...
    if (hI != NULL) {
        ...
        if (hI->maxCpus > 0)
            ...
            ls_syslog(LOG_DEBUG,
                      "%s: numCPUs <%d> of host <%s> is not greater than 0; "
                      "assuming as 1",
                      fname, hI->maxCpus, hI->hostName);
            hData->numCPUs = 1;
        }
    ...
    hData->lsbLoad =
        (float *) my_calloc(allLsInfo->numIndx, sizeof(float), fname);
    hData->busySched =
        (int *) my_calloc(GET_INTNUM(allLsInfo->numIndx), sizeof(int), fname);
    hData->busyStop =
        (int *) my_calloc(GET_INTNUM(allLsInfo->numIndx), sizeof(int), fname);
...
void freeHData(struct hData *hData, char delete)
{
...
static struct gData *addGroup(struct gData **groups, char *gname, int *ngroups)
{
    groups[*ngroups] =
        (struct gData *) my_malloc(sizeof(struct gData), "addGroup");
...
static void addMember(struct gData *groupPtr, char *word, int grouptype,
                      char *filename, struct gData *groups[], int *ngroups)
{
    ...
    if (grouptype == USER_GRP) {
        ...
        if (pw != NULL) {
            ...
            if (unixGrp) {
                subgrpPtr =
                    addUnixGrp(unixGrp, word, filename, groups, ngroups);
                if (!subgrpPtr) {
                    ls_syslog(LOG_WARNING,
                              "%s: No valid users defined in Unix group <%s>; "
                              "ignoring",
                              fname, word);
                    lsb_CheckError = WARNING_ERR;
                    return;
    ...
    if (grouptype == HOST_GRP) {
        if ((subgrpPtr = getGrpData(tempHGData, word, nTempHGroups)) != NULL) {
            ...
            if (get_host_by_name(word, &hp) < 0) {
                ls_syslog(LOG_ERR,
                          "%s: Bad host/group name <%s> in group <%s>; ignored",
                          fname, word, groupPtr->group);
                lsb_CheckError = WARNING_ERR;
                return;
            }
            strcpy(name, hp.name);
    ...
    if (isInGrp(word, groupPtr)) {
        if (isgrp)
            ls_syslog(
                LOG_ERR,
                "%s: Group <%s> is multiply defined in group <%s>; ignored",
                fname, word, groupPtr->group);
        else
            ls_syslog(
                LOG_ERR,
                "%s: Member <%s> is multiply defined in group <%s>; ignored",
                fname, word, groupPtr->group);
        lsb_CheckError = WARNING_ERR;
        return;
    }
...
static void parseAUids(struct qData *qp, char *line)
{
    ...
    if (numAds) {
        ...
        admins.adminNames = (char **) my_malloc(numAds * sizeof(char *), fname);
        admins.nAdmins = 0;
    } else {
        ls_syslog(LOG_ERR, "%s: No queue's administrators defined; ignored",
                  fname);
        lsb_CheckError = WARNING_ERR;
        return;
    }
    ...
    if (!admins.nAdmins) {
        ls_syslog(LOG_ERR,
                  "%s: No valid queue's administrators defined in <%s> for "
                  "queue <%s>; ignored",
                  fname, line, qp->queue);
        lsb_CheckError = WARNING_ERR;
...
static void setAllusers(struct qData *qp, struct admins *admins)
{
    int i;
...
void freeQData(struct qData *qp, int delete)
{
    free(qp->queue);
    free(qp->description);
    if (qp->uGPtr) {
...
static void parseGroups(int groupType, struct gData **group, char *line,
                        char *filename)
{
    ...
    *group = mygp;
    ...
    while ((word = getNextWord_(&line)) != NULL) {
        if (isInGrp(word, mygp)) {
            ls_syslog(LOG_ERR,
                      "%s/%s: Member <%s> is multiply defined; ignored",
                      filename, fname, word);
            lsb_CheckError = WARNING_ERR;
            continue;
        }
        if (groupType == USER_GRP) {
            ...
            if (unixGrp != NULL) {
                gp = addUnixGrp(unixGrp, grpSl, filename, tempUGData,
                                &nTempUGroups);
                if (gp == NULL) {
                    ls_syslog(
                        LOG_WARNING,
                        "%s/%s: No valid users in Unix group <%s>; ignoring",
                        filename, fname, word);
                    lsb_CheckError = WARNING_ERR;
                    continue;
                }
            ...
            if (cc < 0) {
                ls_syslog(LOG_ERR,
                          "%s/%s: Unknown host or host group <%s>; ignored",
                          filename, fname, word);
                lsb_CheckError = WARNING_ERR;
                continue;
            }
            ...
            if (numofhosts) {
                if (!h_getEnt_(&hDataList, word)) {
                    ls_syslog(LOG_ERR,
                              "%s/%s: Host <%s> is not used by the batch "
                              "system; ignored",
                              filename, fname, word);
                    lsb_CheckError = WARNING_ERR;
                    continue;
            ...
            if (isInGrp(word, mygp)) {
                ls_syslog(LOG_ERR,
                          "%s/%s: Host name <%s> is multiply defined; ignored",
                          filename, fname, word);
                lsb_CheckError = WARNING_ERR;
                continue;
            }
...
static void readParamConf(int mbdInitFlags)
{
...
%s: File <%s> can not be found, default parameters will be used",
    ...
    if ((paramConf = lsb_readparam(paramFileConf)) == NULL) {
        if (lsberrno == LSBE_CONF_FATAL) {
            ls_syslog(LOG_ERR, "%s", __func__, "lsb_readparam");
            if (lsb_CheckMode) {
                lsb_CheckError = FATAL_ERR;
                return;
            } else
...
static int searchAll(char *word)
{
    char *sp, *cp;
...
static int isInGrp(char *word, struct gData *gp)
{
    int i;
...
struct qData *lostFoundQueue(void)
{
    ...
    qp = initQData();
    qp->description =
        safeSave("This queue is created by the system to hold the jobs that "
                 "were in the queues being removed from the configuration by "
                 "the LSF Administrator.  Jobs in this queue will not be "
                 "started unless they are switched to other queues.");
    ...
    qp->uGPtr =
        (struct gData *) my_malloc(sizeof(struct gData), "lostFoundQueue");
    qp->uGPtr->group = "";
...
struct hData *lostFoundHost(void)
{
    struct hData *lost;
    struct hData hp;
    ...
    if (lost == NULL) {
        ls_syslog(LOG_ERR, "%s", __func__, "getHostData", LOST_AND_FOUND);
        if (!lsb_CheckMode)
            mbdDie(MASTER_RESIGN);
        else
            lsb_CheckError = FATAL_ERR;
...
void queueHostsPF(struct qData *qp, int *allPoll)
{
    int j;
    struct hData *hData;
...
static struct gData *addUnixGrp(struct group *unixGrp, char *grpName,
                                char *filename, struct gData *groups[],
                                int *ngroups)
{
    int i = -1;
    struct gData *gp;
    ...
    while (unixGrp->gr_mem[++i] != NULL) {
        if ((pw = getpwnam2(unixGrp->gr_mem[i])))
            addMember(gp, unixGrp->gr_mem[i], USER_GRP, filename, groups,
                      ngroups);
        else {
            ls_syslog(LOG_WARNING, "%s: Unknown user <%s> in group <%s>",
...
static void setParams(struct paramConf *paramConf)
{
    ...
    if (paramConf == NULL || paramConf->param == NULL) {
        ls_syslog(LOG_ERR,
                  "%s: paramConf or param in paramConf is NULL; default  "
...
static void addUData(struct userConf *userConf)
{
    ...
    for (i = 0; i < userConf->numUsers; i++) {
        ...
        addUserData(uPtr->user, uPtr->maxJobs, uPtr->procJobLimit, fname, TRUE,
                    TRUE);
...
static void createTmpGData(struct groupInfoEnt *groups, int num, int groupType,
                           struct gData *tempGData[], int *nTempGroups)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        if (groupType == HOST_GRP && gPtr && gPtr->group) {
            ls_syslog(
                LOG_WARNING,
                "%s: Host group name <%s> conflicts with host name; ignoring",
                fname, gPtr->group);
            lsb_CheckError = WARNING_ERR;
            continue;
        }
        ...
        if (getGrpData(tempGData, gPtr->group, *nTempGroups)) {
            ls_syslog(LOG_WARNING,
                      "%s: Group <%s> is multiply defined in %s; ignoring",
                      fname, gPtr->group, HUgroups);
            lsb_CheckError = WARNING_ERR;
            continue;
        }
        ...
        if (grpPtr->memberTab.numEnts == 0 && grpPtr->numGroups == 0 &&
            ...
            ls_syslog(LOG_WARNING,
                      "%s: No valid member in group <%s>; ignoring the group",
                      fname, grpPtr->group);
            lsb_CheckError = WARNING_ERR;
            freeGrp(grpPtr);
            *nTempGroups -= 1;
...
static void addHostData(int numHosts, struct hostInfoEnt *hosts)
{
    ...
    if (numHosts == 0 || hosts == NULL) {
        ls_syslog(LOG_ERR,
                  "%s: No hosts specified in lsb.hosts; all hosts known by LSF "
                  "will be used",
                  fname);
        addDefaultHost();
        return;
...
static void copyHostInfo(struct hostInfoEnt *from, struct hData *to)
{
    to->host = from->host;
...
static void addQData(struct queueConf *queueConf, int mbdInitFlags)
{
    ...
    if (queueConf == NULL || queueConf->numQueues <= 0 ||
        ...
        ls_syslog(LOG_ERR, "%s: No valid queue in queueConf structure", fname);
        lsb_CheckError = WARNING_ERR;
        return;
    }
    ...
    for (i = 0; i < queueConf->numQueues; i++) {
        ...
        if (qp->uGPtr)
            if (qp->uGPtr->memberTab.numEnts == 0 &&
                qp->uGPtr->numGroups == 0) {
                ls_syslog(LOG_ERR,
                          "%s: No valid value for key USERS in the queue <%s>; "
                          "ignoring the queue",
                          fname, qp->queue);
                lsb_CheckError = WARNING_ERR;
                badqueue = TRUE;
            }
        if (qp->hostList != NULL && strcmp(qp->hostList, "none") &&
            qp->numAskedPtr <= 0 && qp->askedOthPrio <= 0) {
            ls_syslog(LOG_ERR,
                      "%s: No valid value for key HOSTS in the queue <%s>; "
                      "ignoring the queue",
                      fname, qp->queue);
            lsb_CheckError = WARNING_ERR;
            badqueue = TRUE;
    ...
    for (i = 0; i < queueConf->numQueues; i++) {
                ...
                *(qp->windows) = '\0';
                ...
                while ((word = getNextWord_(&sp)) != NULL) {
                    ...
                    if (addWindow(word, qp->weekR, qp->queue) < 0) {
                        ls_syslog(LOG_ERR,
                                  "%s: Bad time expression <%s>/<%s> for run "
                                  "windows of queue <%s>; ignored",
                    ...
                    for (i = 0; i < 8; i++) {
                        for (wp = qp->weekR[i], j = 0; wp != NULL;
                            ...
                            ls_syslog(LOG_DEBUG2,
                                      "%s: queue <%s> weekR[%d] window #%d "
                                      "opentime <%f> closetime <%f>",
                                      fname, qp->queue, i, j, wp->opentime,
                                      wp->closetime);
                        }
                    }
                }
            ...
            *(qp->windowsD) = '\0';
            ...
            while ((word = getNextWord_(&sp)) != NULL) {
                ...
                if (addWindow(word, qp->week, qp->queue) < 0) {
                    ls_syslog(LOG_ERR,
                              "%s: Bad time expression <%s>/<%s> for dispatch "
                              "windows of queue <%s>; ignored",
        ...
        if (!qp->windows && !qp->windowsD && !(qp->qStatus & QUEUE_STAT_RUN)) {
            qp->qStatus |= QUEUE_STAT_RUN;
        }

        if (queue->defaultHostSpec) {
            float *cpuFactor;
            if ((cpuFactor = getModelFactor(queue->defaultHostSpec)) == NULL &&
                (cpuFactor = getHostFactor(queue->defaultHostSpec)) == NULL) {
                ls_syslog(
                    LOG_ERR,
                    "%s: Invalid value <%s> for %s in queue <%s>; ignored",
                    fname, queue->defaultHostSpec, "DEFAULT_HOST_SPEC",
                    qp->queue);
                lsb_CheckError = WARNING_ERR;
            }
            if (cpuFactor != NULL)
        ...
        if (queue->resReq) {
            if ((qp->resValPtr = checkResReq(
                     queue->resReq, USE_LOCAL | PARSE_XOR | CHK_TCL_SYNTAX)) ==
                NULL) {
                ls_syslog(
                    LOG_ERR,
                    "%s: RES_REQ <%s> for the queue <%s> is invalid; ignoring",
                    fname, queue->resReq, qp->queue);
                lsb_CheckError = WARNING_ERR;
            } else
        ...
        if (queue->resumeCond) {
            ...
            if ((resValPtr = checkResReq(queue->resumeCond,
                ...
                ls_syslog(LOG_ERR,
                          "%s: RESUME_COND <%s> for the queue <%s> is invalid; "
                          "ignoring",
                          fname, queue->resumeCond, qp->queue);
                lsb_CheckError = WARNING_ERR;
            } else {
        ...
        if (queue->stopCond) {
            struct resVal *resValPtr;
            if ((resValPtr = checkResReq(queue->stopCond,
                                         USE_LOCAL | CHK_TCL_SYNTAX)) == NULL) {
                ls_syslog(LOG_ERR,
                          "%s: STOP_COND <%s> for the queue <%s> is invalid; "
                          "ignoring",
                          fname, queue->stopCond, qp->queue);
                lsb_CheckError = WARNING_ERR;
            } else {
...
static void copyGroups(int copyHGroups)
{
    int i;
...
static void createCondNodes(int numConds, char **conds, char *fileName,
                            int flags)
{
    ...
    for (i = 0; i < numConds; i++) {
        ...
        if (h_getEnt_(&condDataList, conds[i])) {
            ls_syslog(LOG_DEBUG,
                      "%s: Condition <%s> in file <%s> is multiply "
                      "specified;retaining old definition",
        ...
        if ((condNode->rootNode = parseDepCond(conds[i], &auth, &errcode, NULL,
                                               &jFlags, 0)) == NULL) {
            ls_syslog(LOG_ERR,
                      "%s: Bad condition <%s> in file <%s> :%M; ignored", fname,
                      conds[i], fileName);
            lsb_CheckError = WARNING_ERR;
        }
...
static int updCondData(struct lsConf *conf, int fileType)
{
    ...
    if (conf == NULL) {
        ls_syslog(LOG_ERR, "%s: NULL Conf pointer for fileType <%x>", fname,
                  fileType);
        return FALSE;
    }
    ...
    for (i = 0; i < conf->numConds; i++) {
        ...
        if (logclass & LC_TRACE)
            ls_syslog(LOG_DEBUG3,
                      "%s: Condition name <%s> for %dth condition status <%d> "
                      "lastStatus <%d>",
...
static struct lsConf *getFileConf(char *fileName, int fileType)
{
    struct lsConf *confPtr = NULL;

    confPtr = ls_getconf(fileName);
    if (confPtr == NULL) {
        ls_syslog(LOG_ERR, "%s", __func__, "ls_getconf");
...
static void copyQData(struct qData *fromQp, struct qData *toQp)
{
    int i;
...
static void removeFlags(struct hTab *dataList, int flags, int listType)
{
    ...
    while (hEntPtr) {
        ...
        case UDATA:
            ...
            break;
        default:
            ls_syslog(LOG_ERR, "%s: Bad data list type <%d>", fname, listType);
            return;
        }
...
static int needPollHost(struct hData *oldHp, struct hData *newHp)
{
    int i;
...
static int needPollQHost(struct qData *newQp, struct qData *oldQp)
{
...
static void updHostList(void)
{
    ...
    if (logclass & LC_TRACE) {
        for (i = 1; i <= numofhosts; i++)
            ls_syslog(LOG_DEBUG,
                      "updHostList: Host <%s> now is still used by the batch "
                      "system and put into hDataPtrTb[%d]",
                      hDataPtrTb[i]->host, hDataPtrTb[i]->hostId);
        ls_syslog(LOG_DEBUG,
                  "updHostList: Current batch system has numofhosts<%d>, "
                  "numofprocs <%d>",
                  numofhosts, numofprocs);
    }
}
...
static void updQueueList(void)
{
    ...
    if ((newqDataList = (struct qData *) listCreate("Queue List")) == NULL) {
        ls_syslog(LOG_ERR, "updQueueList", "listCreate");
        mbdDie(MASTER_FATAL);
    }
...
static void updUserList(int mbdInitFlags)
{
    ...
    FOR_EACH_HTAB_ENTRY(key, hashTableEnt, &uDataList)
    {
        ...
        if (uData->flags & USER_UPDATE) {
            if ((gPtr = getUGrpData(uData->user)) != NULL) {
                if (mbdInitFlags != FIRST_START && uData->numJobs == 0 &&
                    uData->numPEND == 0 && uData->numRUN == 0 &&
                    uData->numSSUSP == 0 && uData->numUSUSP == 0 &&
                    uData->numRESERVE == 0)
...
static void addUGDataValues(struct uData *gUData, struct gData *gPtr)
{
    ...
    if (numMembers == 1 && strcmp(gUData->user, groupMembers[0]) == 0) {
        ls_syslog(LOG_ERR, "%s: User group <%s> with no members is ignored",
                  fname, gUData->user);
    } else {
...
static void addUGDataValues1(struct uData *gUData, struct uData *uData)
{
    sTab hashSearchPtr;
    hEnt *hashEntryPtr;
...
static int parseQHosts(struct qData *qp, char *hosts)
{
    ...
    if (qp == NULL) {
        ls_syslog(LOG_ERR, "%s: NULL pointer for queue", fname);
        return -1;
    }
    if (hosts == NULL) {
        ls_syslog(LOG_ERR, "%s: NULL pointer in HOSTS for queue <%s>", fname,
                  qp->queue);
        return -1;
    }
    ...
    while ((word = getNextWord_(&sp)) != NULL)
        numAskedHosts++;
    if (numAskedHosts == 0) {
        ls_syslog(LOG_ERR, "%s: No host specified in HOSTS for queue <%s>",
                  fname, qp->queue);
        return -1;
    }
    ...
    returnErr =
        chkAskedHosts(numAskedHosts, askedHosts, numofprocs, &numReturnHosts,
                      &returnHosts, &badReqIndx, &others, 0);
    if (returnErr == LSBE_NO_ERROR) {
        if (numReturnHosts > 0) {
            qp->askedPtr = (struct askedHost *) my_calloc(
                numReturnHosts, sizeof(struct askedHost), fname);
    ...
    } else if (parseFirstHostErr(returnErr, fname, hosts, qp, returnHosts,
                                 numReturnHosts)) {
        ls_syslog(LOG_ERR,
                  "%s: No valid host used by the batch system is specified in "
                  "HOSTS <%s> for queue <%s>",
...
static void freeGrp(struct gData *grpPtr)
{
    if (grpPtr == NULL)
        return;
    ...
    FOR_EACH_HTAB_ENTRY(key, hashTableEnt, &uDataList)
    {
        ...
        if (!(uPtr->flags & USER_ALL)) {
            if (uPtr->children != NULL) {
                ...
                uPtr->children = setCreate(MAX_GROUPS, getIndexByuData,
            ...
            if (uPtr->descendants != NULL) {
                ...
                uPtr->descendants = setCreate(MAX_GROUPS, getIndexByuData,
                                              getuDataByIndex, strBuf);
            }
        } else {
            uPtr->children = NULL;
            ...
            if ((jp->shared->jobBill.options2 & SUB2_USE_DEF_PROCLIMIT) &&
                ...
                jp->shared->jobBill.numProcessors =
                    jp->shared->jobBill.maxNumProcessors =
                        (jp->qPtr->defProcLimit > 0 ? jp->qPtr->defProcLimit
                                                    : 1);
            }
...
static int parseFirstHostErr(int returnErr, char *fname, char *hosts,
                             struct qData *qp, struct askedHost *returnHosts,
                             int numReturnHosts)
{
    int i;
    ...
    if ((returnErr == LSBE_MULTI_FIRST_HOST) ||
        ...
        if (returnErr == LSBE_MULTI_FIRST_HOST)
            ls_syslog(LOG_ERR,
                      "%s: Multiple first execution hosts specified <%s> in "
                      "HOSTS section for queue <%s>. Ignored.",
                      fname, hosts, qp->queue);
        else if (returnErr == LSBE_HG_FIRST_HOST)
            ls_syslog(LOG_ERR,
                      "%s: host group <%s> specified as first execution host "
                      "in HOSTS section for queue <%s>. Ignored.",
                      fname, hosts, qp->queue);
        else if (returnErr == LSBE_HP_FIRST_HOST)
            ls_syslog(
                LOG_ERR,
                "%s: host partition <%s> specified as first execution hosts "
                "specified <%s> in HOSTS section for queue <%s>. Ignored.",
                fname, hosts, qp->queue);
        else if (returnErr == LSBE_OTHERS_FIRST_HOST)
            ls_syslog(LOG_ERR,
                      "%s: \"others\" specified as first execution host "
                      "specified in HOSTS section for queue <%s>. Ignored.",
                      fname, qp->queue);
...
static int resetJData()
{
    ...
    for (jp = jDataList[PJL]->back; jp != jDataList[PJL]; jp = jp->back) {
        jobBill = &(jp->shared->jobBill);

        jp->priority = -1;
        jp->jFlags &= ~(JFLAG_READY | JFLAG_DEPCOND_REJECT | JFLAG_READY1 |
                        JFLAG_READY2 | JFLAG_WILL_BE_PREEMPTED);
...
static int resetQData(struct qData *qp)
{
    ...
    sprintf(description,
            "This queue <%s> becomes LOST_AND_FOUND because it is removed from "
            "the configuration by LSF Administrator.",
            qp->queue);
    if (qp->description) {
    ...
    freeGrp(qp->uGPtr);
    qp->uGPtr =
        (struct gData *) my_malloc(sizeof(struct gData), "resetQueueStatus");
    qp->uGPtr->group = "";
    h_initTab_(&qp->uGPtr->memberTab, 16);
    qp->uGPtr->numGroups = 0;
    h_addEnt_(&qp->uGPtr->memberTab, "nobody", 0);
...
static void checkAskedHostList(struct jData *jp)
{
    ...
    returnErr = chkAskedHosts(subReq->numAskedHosts, subReq->askedHosts,
    ...
    if ((returnErr == LSBE_NO_ERROR) && (numAskedHosts > 0)) {
        jp->askedPtr = (struct askedHost *) my_calloc(
...
static void checkReqHistory(struct jData *jp)
{
    ...
    tmpReqHistory = (struct rqHistory *) my_calloc(
        jp->reqHistoryAlloc, sizeof(struct rqHistory), fname);
        ...
        if (!(uData->flags & USER_GROUP) &&
            ...
            for (allUserGrp = (struct uData *) setIteratorBegin(&iter);
                ...
                if (uData->parents == NULL) {
                    ...
                    uData->parents = setCreate(MAX_GROUPS, getIndexByuData,
                                               getuDataByIndex, strBuf);
                    if (uData->parents == NULL) {
                        ls_syslog(LOG_ERR, "%s", __func__, "\
setCreate",
                                  strBuf, setPerror(bitseterrno));
                        mbdDie(MASTER_MEM);
                    }
                }
                ...
                if (uData->ancestors == NULL) {
                    ...
                    uData->ancestors = setCreate(MAX_GROUPS, getIndexByuData,
                                                 getuDataByIndex, strBuf);
                    if (uData->ancestors == NULL) {
                        ls_syslog(LOG_ERR, "%s", __func__, "\
setCreate",
