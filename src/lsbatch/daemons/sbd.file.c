...
stdoutDirectSymLink(char *jobFile, char *ext, struct jobSpecs *jobSpecsPtr)
{
    ...
    if ((symlink (fullpath, fileLink) == -1) && (errno != EEXIST)) {
        sprintf(errMsg, "%s:Job <%s> Unable to creat a symbolic link from <%s> to <%s>: %s",
               fname,
...
openStdFiles(char *lsbDir, char *chkpntDir, struct jobCard *jobCardPtr, struct hostent *hp)
{
	...
	if ( (rc < 0) && (errno == ENOENT) ) {
		...
		ls_syslog(LOG_ERR,"Can not create spool sub-directories <%s> on execution host",
			  spooledExecName);
		return -1;
            }
	    ...
	    if (rcpFile(jobSpecsPtr, &xf,
                        ...
                        rcpMsg) < 0) {
		ls_syslog(LOG_ERR,"Can not copy spooled executable <%s> from submission host <%s> to execution host",
		...
		ls_syslog(LOG_ERR,
			 "Can not allocate memory to save  spooled executable file name <%s>\n please: remove it manually", spooledExecName );
	    }
	} else {
    ...
    for (i = 0; i < jobSpecsPtr->nxf; i++) {
        if (jobSpecsPtr->xf[i].options & XF_OP_SUB2EXEC) {
            ...
            if (rcpFile(jobSpecsPtr, jobSpecsPtr->xf+i,
                        ...
                        rcpMsg) < 0) {
                sprintf(xMsg, "%sLSF: Failed to copy file <%s> from submission host <%s> to file <%s> on execution host: %s\n",
                    xMsg, jobSpecsPtr->xf[i].subFn, jobSpecsPtr->fromHost,
                jobSpecsPtr->xf[i].execFn, rcpMsg);
            } else {
                if (rcpMsg[0] != '\0')
                    sprintf(xMsg, "%sLSF: Copy file <%s> from submission host <%s> to file <%s> on execution host: %s\n",
  ...
  OpenStdin:
    ...
    if (i < 0) {
	    ...
	    if ( i == 0 ) {
                if (rcpMsg[0] != '\0')
                    sprintf(xMsg, "%sLSF: Copy stdin file <%s> on submissi on host <%s> to <%s> on execution host: %s\n",
                        xMsg,
			xf.subFn,
            ...
            sprintf(xMsg, "%sLSF: Unable to copy stdin file <%s> on submission host <%s> to <%s> on execution host: %s\n",
