/* $Id: sbd.file.c,v 1.11 2007/08/15 22:18:46 tmizan Exp $
...
static void determineFilebufStdoutDirect(char *filebuf,
                                         ...
                                         int flag);
static int stdoutDirectSymLink(char *jobFile, char *ext,
                               struct jobSpecs *jobSpecsPtr);

int rcpFile(struct jobSpecs *jp, struct xFile *xf, char *host, int op,
            char *errMsg)
{
    ...
    if (pid == 0) {
        ...
        if (logclass & LC_FILE) {
            sprintf(debugMsg,
                    "rcpFile: Job %s rcpCmd <%s> subFn <%s> execFn <%s> rcpArg "
                    "<%s> options %x XF_OP_SUB2EXEC %x XF_OP_EXEC2SUB_APPEND "
                    "%x op %x",
    ...
    if (LS_STATUS(status)) {
        if (errMsg[0] == '\0')
            sprintf(errMsg, "lsrcp exited with non-zero status %x",
                    LS_STATUS(status));
        return -1;
    }
...
int rmJobBufFiles(struct jobCard *jp)
{
    ...
    if ((jp->jobSpecs.options2 & SUB2_JOB_CMD_SPOOL) &&
        ...
        *p = '\0';
        ...
        if (retVal < 0) {
            ls_syslog(LOG_ERR,
                      "Can not remove spooled executable directory <%s>\n "
...
static int unlinkBufFiles(char *lsbDir, char *jobFile, struct jobCard *jp,
                          struct hostent *hp)
{
...
int initPaths(struct jobCard *jp, struct hostent *fromHp, struct lenData *jf)
{
    ...
    if (logclass & LC_EXEC) {
        sprintf(errMsg,
                "initPaths: Enter ... Job <%s> subHomeDir <%s> cwd <%s> "
                "execHome <%s> execCwd <%s> jobFile <%s> execUid %d userId %d",
                lsb_jobidinstr(jp->jobSpecs.jobId), jp->jobSpecs.subHomeDir,
                jp->jobSpecs.cwd, jp->jobSpecs.execHome, jp->jobSpecs.execCwd,
    ...
    if (jp->jobSpecs.execCwd[0] == '\0') {
        ...
        sprintf(errMsg, "%s: Job <%s> chdir(%s failed, errno=<%s>. Use <%s> as execCwd"),
		   fname,
    ...
    if (jp->jobSpecs.execHome[0] != '\0') {
        if (!isAbsolutePathExec(jp->jobSpecs.jobFile)) {
            ...
            if (logclass & LC_EXEC) {
                sprintf(errMsg,
                        "%s: job <%s> jp->jobSpecs.jobFile is %s goodspooldir "
                        "is %d",
    ...
    if (logclass & LC_EXEC) {
        if (jp->jobSpecs.jobSpoolDir[0] == '\0') {
            sprintf(errMsg,
                    "%s: job <%s>, lsbDir is %s, jp->jobSpecs.jobFile is <%s>, "
                    "goodspooldir %d",
            ...
            sprintf(errMsg,
                    "%s: job <%s>, spooldir is %s, lsbDir is %s, "
                    "jp->jobSpecs.jobFile is %s, goodSpoolDir %d",
                    fname, lsb_jobidinstr(jp->jobSpecs.jobId),
    ...
    if (logclass & LC_EXEC) {
        sprintf(errMsg,
                "initPaths: Leave ... Job <%s> subHomeDir <%s> cwd <%s> "
                "execHome <%s> execCwd <%s> jobFile <%s> execUid %d userId %d",
                lsb_jobidinstr(jp->jobSpecs.jobId), jp->jobSpecs.subHomeDir,
                jp->jobSpecs.cwd, jp->jobSpecs.execHome, jp->jobSpecs.execCwd,
                jp->jobSpecs.jobFile, jp->jobSpecs.execUid,
...
static int cwdJob(struct jobCard *jp, char *cwd, struct hostent *fromHp)
{
    struct passwd *pw;
    char errMsg[MAXLINELEN];
    ...
    if (isAbsolutePathSub(jp, jp->jobSpecs.cwd)) {
        ...
        if (logclass & LC_EXEC) {
            sprintf(errMsg, "cwdJob: mychdir_(%s) failed for job <%s>: %s",
                    chosenPath, lsb_jobidinstr(jp->jobSpecs.jobId),
                    strerror(errno));
            sbdSyslog(LOG_DEBUG, errMsg);
        }

        if ((pw = getpwdirlsfuser_(jp->execUsername)) && pw->pw_dir &&
            isAbsolutePathExec(pw->pw_dir)) {
            if (chdir(pw->pw_dir) == 0) {
                ...
                if (getcwd(homePath, sizeof(homePath))) {
                    ...
                    for (i = 0; homePath[i] != '\0' && subCwd[i] == homePath[i];
                         i++)
                        ;
                    if ((i != 0) && (homePath[i] == '\0')) {
                        if (subCwd[i] == '\0') {
    ...
    if ((pw = getpwdirlsfuser_(jp->execUsername)) && pw->pw_dir &&
        ...
        if (logclass & LC_EXEC) {
            sprintf(errMsg, "cwdJob: chdir(%s) failed for job <%s>: %s", cwd,
                    lsb_jobidinstr(jp->jobSpecs.jobId), strerror(errno));
            sbdSyslog(LOG_DEBUG, errMsg);
        }
    }
    ...
    strcpy(cwd, LSTMPDIR);
    if (chdir(cwd) == -1) {
        sprintf(errMsg, "cwdJob: chdir tmp (%s) failed for job <%s>: %s", cwd,
                lsb_jobidinstr(jp->jobSpecs.jobId), strerror(errno));
        sbdSyslog(LOG_DEBUG, errMsg);
        return -1;
    }
...
static int lsbatchDir(char *lsbDir, struct jobCard *jp, struct hostent *fromHp,
                      struct lenData *jf)
...
static int lsbDirOk(char *lsbDir, struct jobCard *jp, struct hostent *fromHp,
                    struct lenData *jf)
{
    ...
    if (jf) {
        if ((jp->jobSpecs.options & SUB_CHKPNTABLE) == 0) {
            ...
            if (createChkpntJobFile(lsbDir, jp, jf, fromHp, chkpntDir,
                                    restartDir) < 0) {
...
static int createChkpntJobFile(char *lsbDir, struct jobCard *jp,
                               struct lenData *jf, struct hostent *fromHp,
        ...
        if (jp->jobSpecs.chkpntDir[0] == '/' &&
            ...
            *p = '/';
            ...
            if ((mymkdir_(chkDir, 0777, fromHp) < 0 && errno != EEXIST) ||
                ...
                sprintf(errMsg, I18N_FUNC_S_FAIL_EMSG_S, fname, "mymkdir_",
                        jp->jobSpecs.chkpntDir, strerror(errno));
                *p = '/';
                sbdSyslog(LOG_ERR, errMsg);
                umask(mode);
                jobSetupStatus(JOB_STAT_PEND, PEND_CHKPNT_DIR, jp);
            ...
            if ((mkdir(chkDir, 0777) < 0 && errno != EEXIST) ||
                ...
                sprintf(errMsg, I18N_FUNC_S_FAIL_EMSG_S, fname, "mkdir",
                        jp->jobSpecs.chkpntDir, strerror(errno));
                sbdSyslog(LOG_ERR, errMsg);
                umask(mode);
                jobSetupStatus(JOB_STAT_PEND, PEND_CHKPNT_DIR, jp);
        ...
        if (strcmp(oldJobId, newJobId) != 0) {
            if (((dirp1 = opendir(chkpntDir)) == NULL) && (errno == ENOENT)) {
                if (((dirp2 = opendir(restartDir)) == NULL) &&
                    (errno == ENOENT)) {
                    sprintf(errMsg, I18N_FUNC_S_FAIL_EMSG_S, fname, "opendir",
                ...
                if (dirp1) {
                    ...
                    sprintf(errMsg, "Chkpnt directory %s isn't accessable: %s.",
                            chkpntDir, strerror(errno));
                }
                if (((dirp2 = opendir(restartDir)) == NULL) &&
                    (errno == ENOENT)) {
...
int myRename(char *fromDir, char *toDir)
{
    char errMsg[MAXLINELEN];

    if (rename(fromDir, toDir) == -1) {
        sprintf(errMsg, "Unable to rename checkpoint directory %s to %s: %s.",
                               ...
                               struct jobSpecs *jobSpecsPtr)
{
    ...
    if ((symlink(fullpath, fileLink) == -1) && (errno != EEXIST)) {
        sprintf(
            errMsg,
            "%s:Job <%s> Unable to creat a symbolic link from <%s> to <%s>: %s",
            fname, lsb_jobidinstr(jobSpecsPtr->jobId), fileLink, fullpath,
            strerror(errno));
        sbdSyslog(LOG_ERR, errMsg);
        return -1;
    }
...
static int openStdFiles(char *lsbDir, char *chkpntDir,
                        struct jobCard *jobCardPtr, struct hostent *hp)
{
    ...
    if (chkpntDir == NULL) {
        ...
        if (logclass & LC_EXEC) {
            sprintf(errMsg, "%s, jobFile is <%s> not chkpntDir", fname,
                    jobFile);
            sbdSyslog(LOG_DEBUG, errMsg);
        }
    } else {
    ...
    if (jobCardPtr->jobSpecs.options2 & SUB2_JOB_CMD_SPOOL) {
        ...
        if (fp == NULL) {
            ls_syslog(LOG_ERR, "Can not open job file %s", jobFileExt);
            return -1;
        }
        ...
        if (feof(fp) || ferror(fp) || (strlen(line) == 0)) {
            ls_syslog(LOG_ERR, "Can not find user input in job file %s",
                      jobFile);
            fclose(fp);
            return -1;
        }
        ...
        if ((rc < 0) && (errno == ENOENT)) {
            ...
            if (createSpoolSubDir(spooledExecName) != 0) {
                ls_syslog(LOG_ERR,
                          "Can not create spool sub-directories <%s> on "
                          "execution host",
                          spooledExecName);
                return -1;
            }

            if (rcpFile(jobSpecsPtr, &xf, spoolingHost, XF_OP_SUB2EXEC,
                        rcpMsg) < 0) {
                ls_syslog(LOG_ERR,
                          "Can not copy spooled executable <%s> from "
                          "submission host <%s> to execution host",
            ...
            if (jobCardPtr->spooledExec != NULL) {
                ...
                ls_syslog(LOG_ERR,
                          "Can not allocate memory to save  spooled executable "
                          "file name <%s>\n please: remove it manually",
                          spooledExecName);
            }
        } else {
            ...
            if (!S_ISREG(statbuf.st_mode)) {
                ls_syslog(LOG_ERR,
                          "spooled command file <%s> is not a regular file",
                          spooledExecName);
                return -1;
            }
        }
    ...
    for (i = 0; i < jobSpecsPtr->nxf; i++) {
        if (jobSpecsPtr->xf[i].options & XF_OP_SUB2EXEC) {
            ...
            if (rcpFile(jobSpecsPtr, jobSpecsPtr->xf + i,
                        ...
                        rcpMsg) < 0) {
                sprintf(xMsg,
                        "%sLSF: Failed to copy file <%s> from submission host "
                        "<%s> to file <%s> on execution host: %s\n",
                        xMsg, jobSpecsPtr->xf[i].subFn, jobSpecsPtr->fromHost,
                        jobSpecsPtr->xf[i].execFn, rcpMsg);
            } else {
                if (rcpMsg[0] != '\0')
                    sprintf(xMsg,
                            "%sLSF: Copy file <%s> from submission host <%s> "
                            "to file <%s> on execution host: %s\n",
...
OpenStdin:
    if (jobSpecsPtr->options2 & SUB2_IN_FILE_SPOOL) {
        if ((pStrTmp = (char *) getSpoolHostBySpoolFile(jobSpecsPtr->inFile)) !=
            ...
            ls_syslog(LOG_ERR, "jobSpecs.inFile format error:<%s>",
                      jobSpecsPtr->inFile);
            return -1;
        }
    } else {
    ...
    if (i < 0) {
        ...
        if (errno != ENOENT) {
            ...
            if (jobSpecsPtr->options2 & SUB2_IN_FILE_SPOOL) {
                i = rcpFile(jobSpecsPtr, &xf, spoolingHost, XF_OP_SUB2EXEC,
                            rcpMsg);
            } else {
            ...
            if (i == 0) {
                if (rcpMsg[0] != '\0')
                    sprintf(xMsg,
                            "%sLSF: Copy stdin file <%s> on submissi on host "
                            "<%s> to <%s> on execution host: %s\n",
                            xMsg, xf.subFn, jobSpecsPtr->fromHost, xf.execFn,
                            rcpMsg);
                goto OpenStdin;
            }

            sprintf(xMsg,
                    "%sLSF: Unable to copy stdin file <%s> on submission host "
                    "<%s> to <%s> on execution host: %s\n",
                    xMsg, xf.subFn, jobSpecsPtr->fromHost, xf.execFn, rcpMsg);
        }
    } else {
        if (dup2(i, 0) == -1) {
            sprintf(errMsg, "%s: Job <%s> dup2(%d,0 stdin <%s>: %s"),
		fname,
    ...
    if ((i < 0) && outFlag && lsbStdoutDirect) {
        ...
        sbdSyslog(LOG_DEBUG, errMsg);
        sprintf(filebuf, "%s.out", jobFile);
        sprintf(errMsg, "%s: job <%s> filebuf/stdout file <%s>", fname,
    ...
    if ((outFlag) && !(chkpntDir == NULL)) {
        ...
        if ((symlink(filebuf, filebufLink) == -1) && (errno != EEXIST)) {
            sprintf(errMsg,
                    "%s:Job <%s> Unable to creat a symbolic link from <%s> to "
                    "<%s>: %s",
                    fname, lsb_jobidinstr(jobSpecsPtr->jobId), filebufLink,
    ...
    if (dup2(i, 1) == -1) {
        sprintf(errMsg, "%s: Job <%s> dup(%d,1 stdout <%s> failed: %s"),
	    fname,
    ...
    if (!(jobSpecsPtr->options & SUB_ERR_FILE) ||
        ...
        if (dup2(1, 2) == -1) {
            sprintf(errMsg, "%s: Job <%s> dup(1,2 stderr failed: %s"),
		fname,
        ...
        if ((i < 0) && errFlag && lsbStdoutDirect) {
            ...
            sbdSyslog(LOG_DEBUG, errMsg);
            sprintf(filebuf, "%s.err", jobFile);
            sprintf(errMsg, "%s: job <%s> filebuf/stderr file <%s>", fname,
                    lsb_jobidinstr(jobSpecsPtr->jobId), filebuf);
            sbdSyslog(LOG_DEBUG, errMsg);
        ...
        if ((errFlag) && !(chkpntDir == NULL)) {
            ...
            if ((symlink(filebuf, filebufLink) == -1) && (errno != EEXIST)) {
                sprintf(errMsg,
                        "%s:Job <%s> Unable to creat a symbolic link from <%s> "
                        "to <%s>: %s",
                        fname, lsb_jobidinstr(jobSpecsPtr->jobId), filebufLink,
                        filebuf, strerror(errno));
                sbdSyslog(LOG_ERR, errMsg);
        ...
        if (i < 0) {
            sprintf(errMsg, "%s: Job <%s> open(%s stderr failed: %s"),
		fname,
        ...
        if (dup2(i, 2) == -1) {
            sprintf(errMsg, "%s: Job <%s> dup(%d,2 stderr <%s> failed: %s"),
		    fname,
...
int appendJobFile(struct jobCard *jobCard, char *header, struct hostent *hp,
                  char *errMsg)
{
...
static int createJobFile(char *lsbDir, char *chkpntDir, struct jobCard *jp,
                         struct lenData *jf, struct hostent *hp)
{
    ...
    if ((fd = myopen_(jobFile, O_CREAT | O_TRUNC | O_WRONLY, 0700, hp)) == -1) {
        sprintf(errMsg,
                "createJobFile: Job <%s> open jobfile %s failed: lsbDir is %s, "
                "jp->jobSpecs.jobFile is %s,  %s",
    ...
    if (logclass & LC_TRACE) {
        sprintf(errMsg,
                "createJobFile: Job <%s> open jobfile %s success: lsbDir is "
                "%s, jp->jobSpecs.jobFile is %s",
                lsb_jobidinstr(jobSpecsPtr->jobId), jobFile, lsbDir,
                jp->jobSpecs.jobFile);
        sbdSyslog(LOG_DEBUG, errMsg);
    }
    ...
    if ((cc = write(fd, shellLine, strlen(shellLine))) != strlen(shellLine)) {
        sprintf(
            ...
            lsb_jobidinstr(jobSpecsPtr->jobId), jobFile, strlen(shellLine) + 1,
            cc, strerror(errno));
        sbdSyslog(LOG_DEBUG, errMsg);
        close(fd);
        return -1;
    ...
    if ((sp = strstr(jf->data, TRAPSIGCMD)) == NULL) {
        sprintf(errMsg,
                "createJobFile: No TRAPSIGCMD <$LSB_TRAPSIGS...> is found in "
                "jobfile data for job <%s>",
                lsb_jobidinstr(jobSpecsPtr->jobId));
        sbdSyslog(LOG_DEBUG, errMsg);
        sp = jf->data;
    }
    ...
    if ((cc = write(fd, sp, len)) != len) {
        sprintf(errMsg, "%s: Job <%s> write jobfile %s len=%d cc=%d failed: %s",
                fname, lsb_jobidinstr(jobSpecsPtr->jobId), jobFile, len, cc,
                strerror(errno));
        sbdSyslog(LOG_DEBUG, errMsg);
        close(fd);
        return -1;
    ...
    if (close(fd) == -1) {
        sprintf(errMsg, "createJobFile: Job <%s> close jobfile %s failed: %s",
                lsb_jobidinstr(jobSpecsPtr->jobId), jobFile, strerror(errno));
        sbdSyslog(LOG_DEBUG, errMsg);
        return -1;
    }
    ...
    if (chkpntDir != NULL) {
        if ((symlink(jobFile, jobFileLink) == -1) && (errno != EEXIST)) {
            sprintf(errMsg,
                    "createJobFile:Job <%s> Unable to creat a symbolic link "
                    "from <%s> to <%s>: %s",
        ...
        if ((symlink(shellFile, shellFileLink) == -1) && (errno != EEXIST)) {
            sprintf(errMsg,
                    "createJobFile:Job <%s> Unable to creat shellfile symbolic "
                    "link from <%s> to <%s>: %s",
...
int isAbsolutePathSub(struct jobCard *jp, const char *path)
{
    if (path[0] == '/' ||
...
void jobFileExitStatus(struct jobCard *jobCard)
{
    ...
    if ((hp = (struct hostent *) getHostEntryByName_(
             jobCard->jobSpecs.fromHost)) == NULL) {
        ls_syslog(LOG_ERR, "%s", __func__,
...
static int restartFiles(char *lsbDir, char *restartDir, struct jobCard *jp,
                        struct hostent *hp)
{
    int returnCode = 0;
...
static int localJobRestartFiles(char *lsbDir, char *restartDir,
                                struct jobCard *jp, struct hostent *hp)
{
