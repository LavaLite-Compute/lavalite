/* $Id: mbd.requeue.c,v 1.2 2007/08/15 22:18:46 tmizan Exp $
...
int requeueEParse(struct requeueEStruct **rquest, char *reqstr, int *error)
{
    int numStruct, exitV;
    char mode, *word;
    ...
    *error = 0;
    ...
    if (!(*rquest = (struct requeueEStruct *) malloc(
              numAlloc * sizeof(struct requeueEStruct))))
        return 0;
    ...
    if (numStruct)
        *rquest = (struct requeueEStruct *) realloc(
            *rquest, (numStruct + 1) * sizeof(struct requeueEStruct));
    return numStruct;
}
                       ...
                       char mode)
{
    ...
    if (seqnum >= numAlloc - 1) {
        numAlloc <<= 1;
        if (!(*rquest = (struct requeueEStruct *) realloc(
                  *rquest, numAlloc * sizeof(struct requeueEStruct)))) {
            numAlloc >>= 1;
            return -1;
        }
    }
...
int match_exitvalue(struct requeueEStruct *reqE, int exit_value)
{
                     ...
                     struct hData *host)
{
    ...
    if (i + 1 >= *reqHistAlloc) {
        *reqHistAlloc <<= 1;
        if (!(temp = (struct rqHistory *) realloc(
                  *reqHistory, (*reqHistAlloc) * sizeof(struct rqHistory)))) {
            *reqHistAlloc >>= 1;
            return -1;
        } else
            (*reqHistory) = temp;
...
void clean_requeue(struct qData *qp)
{
