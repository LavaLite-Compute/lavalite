/* $Id: mbd.resource.c,v 1.8 2007/08/15 22:18:46 tmizan Exp $
...
static float checkOrTakeAvailableByPreemptPRHQValue(int index, float value,
                                                    struct hData *hPtr,
                                                    struct qData *qPtr,
                                                    int update);
...
static void addSharedResource(struct lsSharedResourceInfo *lsResourceInfo)
{
    ...
    if (getResource(lsResourceInfo->resourceName) != NULL) {
        if (logclass & LC_TRACE)
            ls_syslog(LOG_DEBUG3,
                      "%s: More than one resource <%s> is found in "
                      "lsResourceInfo got from lim; ignoring later",
                      fname, lsResourceInfo->resourceName);
        return;
    }
    resource = (struct sharedResource *) my_malloc(
        sizeof(struct sharedResource), fname);
    ...
    if (numResources == 0)
        temp = (struct sharedResource **) my_malloc(
            sizeof(struct sharedResource *), fname);
    else
        temp = (struct sharedResource **) realloc(
            sharedResources,
...
static void addInstances(struct lsSharedResourceInfo *lsResourceInfo,
    ...
    for (i = 0; i < lsResourceInfo->nInstances; i++) {
        ...
        for (j = 0; j < lsResourceInfo->instances[i].nHosts; j++) {
            if ((hPtr = getHostData(
                ...
                if (logclass & LC_TRACE)
                    ls_syslog(LOG_DEBUG3,
                              "%s: Host <%s> is not used by the batch "
                              "system;ignoring",
...
static void initHostInstances(int num)
{
    sTab hashSearchPtr;
    hEnt *hashEntryPtr;
    struct hData *hData;
...
struct sharedResource *getResource(char *resourceName)
{
    int i;
...
int checkResources(struct resourceInfoReq *resourceInfoReq,
                   struct lsbShareResourceInfoReply *reply)
{
    ...
    reply->numResources = 0;
    reply->resources = (struct lsbSharedResourceInfo *) my_malloc(
...
static int copyResource(struct lsbShareResourceInfoReply *reply,
                        struct sharedResource *resource, char *hostName)
{
    ...
    reply->resources[num].instances =
        (struct lsbSharedResourceInstance *) my_malloc(
            resource->numInstances * sizeof(struct lsbSharedResourceInstance),
            fname);
...
float getHRValue(char *resName, struct hData *hPtr,
                 struct resourceInstance **instance)
{
    int i;
...
void updSharedResourceByRUNJob(const struct jData *jp)
{
    ...
    if (logclass & LC_TRACE)
        ls_syslog(LOG_DEBUG, "\
%s: Updating shared resource by job=%s",
    ...
    for (i = 0; i < jp->numHostPtr; i++) {
        ...
        adjustLoadValue:
            ...
            if ((load = getHRValue(allLsInfo->resTable[ldx].name, jp->hPtr[i],
                                   &instance)) < 0.0) {
                continue;
            } else {
...
%s: jobId=<%s>, hostName=<%s>, resource name=<%s>, the specified rusage of the load or instance <%f>, current lsbload or instance value <%f>, duration <%f>, decay <%f>",
                          fname, lsb_jobid2str(jp->jobId), jp->hPtr[i]->host,
                          allLsInfo->resTable[ldx].name, jackValue, load,
                          duration, decay);
...
%s: JobId=<%s>, hostname=<%s>, resource name=<%s>, the amount by which the load or the instance has been adjusted <%f>, original load or instance value <%f>, runTime=<%d>, value of the load or the instance after the adjustment <%f>, factor <%f>",
                          fname, lsb_jobid2str(jp->jobId), jp->hPtr[i]->host,
                          allLsInfo->resTable[ldx].name, jackValue,
                          originalLoad, jp->runTime, load, factor);
        }
    }
}

static void initQPRValues(struct qPRValues *qPRValuesPtr, struct qData *qPtr)
{
...
static void freePreemptResourceInstance(struct preemptResourceInstance *pRIPtr)
{
...
static void freePreemptResource(struct preemptResource *pRPtr)
{
    int i;
...
void newPRMO(char *nameList)
{
    ...
    pRMOPtr->pResources = (struct preemptResource *) my_malloc(
        nameListPtr->listSize * sizeof(struct preemptResource), fname);

    for (i = 0; i < nameListPtr->listSize; i++) {
        if (nameListPtr->counter[i] > 1) {
            ls_syslog(LOG_WARNING,
                      "Duplicate preemptable resource names (%s) defined, take "
                      "one only.",
                      nameListPtr->names[i]);
        ...
        if ((index = resName2resIndex(nameListPtr->names[i])) < 0) {
            ls_syslog(
                LOG_ERR,
                "Unknown preemptable resource name (%s) defined, ignored.",
                nameListPtr->names[i]);
            lsberrno = LSBE_CONF_WARNING;
            continue;
        }
        ...
        if (index < allLsInfo->numIndx) {
            if (allLsInfo->resTable[index].flags & RESF_BUILTIN) {
                ls_syslog(LOG_ERR,
                          "Built-in load index (%s) can't be defined as a "
                          "preemptable resource, ignored.",
                          nameListPtr->names[i]);
            } else {
                ls_syslog(LOG_ERR,
                          "Host load index (%s) can't be defined as a "
                          "preemptable resource, ignored.",
                          nameListPtr->names[i]);
            }
            lsberrno = LSBE_CONF_WARNING;
        ...
        if (NOT_NUMERIC(allLsInfo->resTable[index])) {
            ls_syslog(LOG_ERR,
                      "Non-numeric resource (%s) can't be defined as a "
                      "preemptable resource, ignored.",
                      nameListPtr->names[i]);
            lsberrno = LSBE_CONF_WARNING;
            continue;
        ...
        if (!(allLsInfo->resTable[index].flags & RESF_RELEASE)) {
            ls_syslog(LOG_ERR,
                      "Non-releasable resource (%s) can't be defined as a "
                      "preemptable resource, ignored.",
                      nameListPtr->names[i]);
            lsberrno = LSBE_CONF_WARNING;
            continue;
        ...
        if (allLsInfo->resTable[index].orderType != DECR) {
            ls_syslog(LOG_ERR,
                      "Increasing resource (%s) can't be defined as a "
                      "preemptable resource, ignored.",
                      nameListPtr->names[i]);
            lsberrno = LSBE_CONF_WARNING;
            continue;
...
float getUsablePRHQValue(int index, struct hData *hPtr, struct qData *qPtr,
                         struct resourceInstance **instance)
{
    float currentValue;
...
float takeAvailableByPreemptPRHQValue(int index, float value,
                                      struct hData *hPtr, struct qData *qPtr)
{
    return checkOrTakeAvailableByPreemptPRHQValue(index, value, hPtr, qPtr, 1);
}

float checkOrTakeAvailableByPreemptPRHQValue(int index, float value,
                                             struct hData *hPtr,
...
void addRunJobUsedPRHQValue(int index, float value, struct hData *hPtr,
                            struct qData *qPtr)
{
...
void removeRunJobUsedPRHQValue(int index, float value, struct hData *hPtr,
                               struct qData *qPtr)
{
    struct qPRValues *qPRVPtr;
...
void addReservedByWaitPRHQValue(int index, float value, struct hData *hPtr,
                                struct qData *qPtr)
{
    struct qPRValues *qPRVPtr;
...
void removeReservedByWaitPRHQValue(int index, float value, struct hData *hPtr,
                                   struct qData *qPtr)
{
    struct qPRValues *qPRVPtr;
...
float getReservedByWaitPRHQValue(int index, struct hData *hPtr,
                                 struct qData *qPtr)
{
    struct qPRValues *qPRVPtr;
...
void addAvailableByPreemptPRHQValue(int index, float value, struct hData *hPtr,
                                    struct qData *qPtr)
{
    struct qPRValues *qPRVPtr;
...
void removeAvailableByPreemptPRHQValue(int index, float value,
                                       struct hData *hPtr, struct qData *qPtr)
{
    struct qPRValues *qPRVPtr;
...
float getAvailableByPreemptPRHQValue(int index, struct hData *hPtr,
                                     struct qData *qPtr)
{
    struct qPRValues *qPRVPtr;
...
int resName2resIndex(char *resName)
{
    int i;
...
int isItPreemptResourceName(char *resName)
{
    return (isItPreemptResourceIndex(resName2resIndex(resName)));
}

int isItPreemptResourceIndex(int index)
{
    int i;
...
int isReservePreemptResource(struct resVal *resValPtr)
{
    int resn;
...
static int addPreemptableResource(int index)
{
    ...
    for (i = 0; i < pRMOPtr->numPreemptResources; i++) {
        if (pRMOPtr->pResources[i].index == index) {
            ls_syslog(
                LOG_INFO,
                "Duplicated preemptable resource name (%s) defined, ignored.",
    ...
    if (sharedIndex < 0) {
        ls_syslog(
            LOG_ERR,
            "Preemptable resource name (%s) is not a shared resource, ignored.",
            allLsInfo->resTable[index].name);
        lsberrno = LSBE_CONF_WARNING;
        return -1;
    }
    ...
    pRMOPtr->pResources[loc].pRInstance =
        (struct preemptResourceInstance *) my_malloc(
            sharedResources[sharedIndex]->numInstances *
                sizeof(struct preemptResourceInstance),
            fname);
...
static struct preemptResourceInstance *findPRInstance(int index,
                                                      struct hData *hPtr)
{
    int i, j;
...
static struct qPRValues *findQPRValues(int index, struct hData *hPtr,
                                       struct qData *qPtr)
{
    struct preemptResourceInstance *pRIPtr;
    int i;
...
static struct qPRValues *addQPRValues(int index, struct hData *hPtr,
                                      struct qData *qPtr)
{
...
static float roundFloatValue(float old)
{
...
void printPRMOValues()
{
    ...
    for (i = 0; i < pRMOPtr->numPreemptResources; i++) {
        ...
        ls_syslog(LOG_DEBUG2,
                  "%s: preemptable resource(%s, %d) has %d instances:", fname,
                  allLsInfo->resTable[pRPtr->index].name, pRPtr->index,
                  pRPtr->numInstances);
        ...
        for (j = 0; j < pRPtr->numInstances; j++) {
            ...
            pRIPtr = &(pRPtr->pRInstance[j]);
            ls_syslog(LOG_DEBUG2, "%s: instance(%d) has %d queues:", fname, j,
                      pRIPtr->nQPRValues);

            for (k = 0; k < pRIPtr->nQPRValues; k++) {
                ls_syslog(LOG_DEBUG2,
                          "%s: queue(%s) has UBRJ-%f, RBPW-%f, ABP-%f, PRJ-%f.",
                          fname, pRIPtr->qPRValues[k].qData->queue,
...
int isHostsInSameInstance(int index, struct hData *hPtr1, struct hData *hPtr2)
{
    int i;
    int j;
