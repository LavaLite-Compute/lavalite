...
newPRMO(char *nameList)
{
	...
	if (nameListPtr->counter[i] > 1) {
            ls_syslog(LOG_WARNING, "Duplicate preemptable resource names (%s) defined, take one only.",
		nameListPtr->names[i]);
            lsberrno = LSBE_CONF_WARNING;

	}
	if ((index = resName2resIndex(nameListPtr->names[i])) < 0) {
            ls_syslog(LOG_ERR, "Unknown preemptable resource name (%s) defined, ignored.",
		nameListPtr->names[i]);
            lsberrno = LSBE_CONF_WARNING;
	    continue;
	}
        ...
        if (index < allLsInfo->numIndx) {
            if (allLsInfo->resTable[index].flags & RESF_BUILTIN) {
                ls_syslog(LOG_ERR, "Built-in load index (%s) can't be defined as a preemptable resource, ignored.",
		    nameListPtr->names[i]);
	    } else {
                ls_syslog(LOG_ERR, "Host load index (%s) can't be defined as a preemptable resource, ignored.",
		    nameListPtr->names[i]);
	    }
            lsberrno = LSBE_CONF_WARNING;
	    continue;
	...
	if (NOT_NUMERIC(allLsInfo->resTable[index])) {
            ls_syslog(LOG_ERR, "Non-numeric resource (%s) can't be defined as a preemptable resource, ignored.",
		nameListPtr->names[i]);
            lsberrno = LSBE_CONF_WARNING;
	    continue;
	}
        ...
        if (!(allLsInfo->resTable[index].flags & RESF_RELEASE)) {
            ls_syslog(LOG_ERR, "Non-releasable resource (%s) can't be defined as a preemptable resource, ignored.",
		nameListPtr->names[i]);
            lsberrno = LSBE_CONF_WARNING;
	    continue;
	}
	...
	if (allLsInfo->resTable[index].orderType != DECR) {
            ls_syslog(LOG_ERR, "Increasing resource (%s) can't be defined as a preemptable resource, ignored.",
		nameListPtr->names[i]);
            lsberrno = LSBE_CONF_WARNING;
	    continue;
	}
...
addPreemptableResource(int index)
{
    ...
    for (i=0; i<pRMOPtr->numPreemptResources; i++) {
        if (pRMOPtr->pResources[i].index == index) {
            ls_syslog(LOG_INFO, "Duplicated preemptable resource name (%s) defined, ignored.",
    ...
    if (sharedIndex < 0) {
        ls_syslog(LOG_ERR, "Preemptable resource name (%s) is not a shared resource, ignored.",
	    allLsInfo->resTable[index].name);
        lsberrno = LSBE_CONF_WARNING;
	return -1;
    }
