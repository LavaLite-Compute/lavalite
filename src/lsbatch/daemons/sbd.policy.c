	...
	if (jobCard->runTime >
	    jobCard->jobSpecs.lsfLimits[LSF_RLIMIT_RUN].rlim_curl) {
			...
			continue;
                        ...
                        ls_syslog(LOG_INFO,"%s: warning period expired killing the job=%d",
	    ...
	    struct jobCard *jobCard, int *reasons, int *subreasons, int num, int *stopmore)
{
    ...
    *subreasons = 0;
	...
	load = NULL;
        ...
        if (load == NULL) {
	    ls_syslog(LOG_ERR, "%s: Can not find load information for host <%s>", fname, jobCard->jobSpecs.toHosts[i]);
            return FALSE;
        }
...
shouldResume (struct hostLoad *loadV, struct jobCard *jp, int num)
{
    ...
    for (j = 0; j <jp->jobSpecs.numToHosts; j++) {
        ...
        if (found != TRUE) {
            ...
            ls_syslog(LOG_ERR, "%s: Can not find load information for host <%s> to check resume condiftions for job <%s>", fname, jp->jobSpecs.toHosts[j], lsb_jobid2str(jp->jobSpecs.jobId));
	...
	ls_syslog(LOG_ERR, "%s: No valid load information is found for job <%s>", fname, lsb_jobid2str(jp->jobSpecs.jobId));
    }
...
sigActEnd (struct jobCard *jobCard)
{
    ...
    switch (jobCard->jobSpecs.actValue) {
        ...
        case SIG_TERM_MEMLIMIT:
            ...
            break;
        default:
            ls_syslog(LOG_ERR, "sigActEnd: unknown sigValue <%d> for job <%s> at the job status <%d> with actPid <%d>",
...
cleanupMigJob(struct jobCard *jp)
{
	...
	lsb_merr2("Unable to setup the environment for job <%s> to run the queue's post exec.  Please run <%s> manually if necessary.\n",
	    lsb_jobid2str(jp->jobSpecs.jobId),
	    jp->jobSpecs.postCmd);
	exit(-1);
    }
