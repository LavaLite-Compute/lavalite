/* $Id: mbd.queue.c,v 1.8 2007/08/15 22:18:46 tmizan Exp $
                ...
                if (logclass & LC_SCHED) {
                    if (qp->runWinCloseTime != 0) {
                        ls_syslog(
                            ...
                            fname, qp->queue, ctime2(&qp->runWinCloseTime));
                    } else {
                        ls_syslog(LOG_DEBUG2,
                                  "%s: run window of queue <%s> is open but "
                                  "runWinCloseTime is 0, reset queue's run "
                                  "window to close",
                                  fname, qp->queue);
                    }
                }
...
struct qData *getQueueData(char *queueName)
{
    struct qData *qp;
    if (qDataList->forw == qDataList)
...
int checkQueues(struct infoReq *queueInfoReqPtr,
    ...
    for (j = 0; j < queueInfoReqPtr->numNames; j++) {
        for (qp = qDataList->back; (qp != qDataList); qp = next) {
            ...
            for (i = 0; i < queueInfoReplyPtr->numQueues; i++) {
                if (strcmp(qp->queue, queueInfoReplyPtr->queues[i].queue) ==
                    0) {
            ...
            if (qp->hostSpec == NULL)
                ...
                if ((cpuFactor = getModelFactor(qp->hostSpec)) == NULL) {
                    if ((cpuFactor = getHostFactor(qp->hostSpec)) == NULL) {
                        ...
                        ls_syslog(LOG_ERR,
                                  "%s: Cannot find cpu factor for hostSpec "
                                  "<%s> in queue <%s>; cpuFactor is set to 1.0",
...
static int isDefQueue(char *qname)
{
    char *cp, *queue;
...
int ctrlQueue(struct controlReq *qcReq, struct lsfAuth *auth)
{
    ...
    if (!isAuthManager(auth) && auth->uid != 0 && !isAuthQueAd(qp, auth)) {
        ls_syslog(
            LOG_CRIT,
            "ctrlQueue: uid <%d> not allowed to perform control operation",
            auth->uid);
    ...
    if (((qp->qStatus & QUEUE_STAT_OPEN) && (qcReq->opCode == QUEUE_OPEN)) ||
        (!(qp->qStatus & QUEUE_STAT_OPEN) && (qcReq->opCode == QUEUE_CLOSED)) ||
        ((qp->qStatus & QUEUE_STAT_ACTIVE) &&
...
char hostQMember(char *host, struct qData *qp)
{
    int i;
...
static int checkHU(char *hostList, char *userList, struct qData *qp)
{
    char *sp;
...
int isQueAd(struct qData *qp, char *lsfUserName)
{
    char *admins, *user;
...
int isInQueues(char *queue, char **queues, int num)
{
    int i;
...
void freeQueueInfoReply(struct queueInfoReply *reply, char *freeAll)
{
    int i;
...
int createQueueHostSet(struct qData *qp)
{
    ...
    qp->hostInQueue =
        setCreate(allHosts, gethIndexByhData, gethDataByhIndex, fname);
    if (!qp->hostInQueue) {
        ls_syslog(LOG_ERR, "%s", __func__, "setCreate", setPerror(bitseterrno));
        mbdDie(MASTER_FATAL);
    }
        ...
        for (i = 1; i < numofhosts; i++) {
            if (setAddElement(qp->hostInQueue, hDataPtrTb[i]) < 0) {
                if (bitseterrno == LS_BITSET_ERR_BADARG) {
                    ls_syslog(LOG_ERR, "%s: Invalid memory detected", fname);
                    mbdDie(MASTER_FATAL);
                } else {
                    ls_syslog(LOG_ERR, "%s: Hosts %s not in mask", fname,
                              hDataPtrTb[i]->host);
                }
            }
        }
        ...
        for (i = 0; i < allHosts; i++)
            if (setAddElement(qp->hostInQueue, qp->askedPtr[i].hData) < 0) {
                if (bitseterrno == LS_BITSET_ERR_BADARG) {
                    ...
                    ls_syslog(LOG_ERR, "%s Hosts %s not in mask", fname,
                              qp->askedPtr[i].hData->host);
                }
            }
    }
    return 0;
}
int gethIndexByhData(void *userData)
{
    struct hData *host;
...
void *gethDataByhIndex(int index)
{
...
int getIndexByQueue(void *queue)
{
    struct qData *qp;
...
void *getQueueByIndex(int queueIndex)
{
    struct qData *qp;
...
static time_t runWindowCloseTime(struct qData *qp)
{
    ...
    if (qp->weekR[dayhour.day] == NULL) {
        ls_syslog(LOG_ERR, "%s: run window for queue <%s> on day <%d> is NULL",
