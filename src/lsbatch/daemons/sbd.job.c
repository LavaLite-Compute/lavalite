/* $Id: sbd.job.c,v 1.15 2007/08/15 22:18:46 tmizan Exp $
...
static void shouldCopyFromLsbatch(struct jobCard *jp, int *cpyStdoutFromLsbatch,
...
static char *getLoginShell(char *jfDada, char *jobFile, struct hostent *hp,
                           int readFile);
static int createTmpJobFile(struct jobSpecs *jobSpecsPtr, struct hostent *hp,
                            char *stdinFile);
...
extern int resumeJob(struct jobCard *jp, int sigValue, int suspendReasons,
                     logType logFlag);
...
static void sbdChildCloseChan(int exceptChan)
{
...
sbdReplyType job_exec(struct jobCard *jobCardPtr, int chfd)
{
    ...
    if (pid < 0) {
        ls_syslog(LOG_ERR, "%s", __func__, lsb_jobid2str(jobSpecsPtr->jobId),
                  "fork");
...
static int sendNotification(struct jobCard *jobCardPtr)
{
    ...
    fprintf(mail, "Sender: LSF System <%s@%s>\n", lsbManager, myhostnm);
    fprintf(mail, "Subject: Job %s: <%s> started.\n",
            lsb_jobid2str(jobSpecsPtr->jobId), jobSpecsPtr->jobName);

    fprintf(mail, "\n");
    fprintf(mail, "Job <%s> was submitted from host <%s> by user <%s>. ",
    ...
    fprintf(mail, "Started at %sunder user <%s>, ",
            jobSpecsPtr->startTime ? ctime2(&jobSpecsPtr->startTime) : temp1,
    ...
    now = time(0);
    fprintf(mail, "Notification reported at %s", ctime2(&now));
...
static int selectPreExecEnvVariables(char *line, char **variables)
{
    ...
    j = 0;
    while ((variables[j] != NULL) && (j <= MAX_PREEXEC_ENVS)) {
        j++;
    }

    while ((pBeg != NULL) && (strlen(pBeg) != 0)) {
        ...
        if ((pBeg = strchr(pBeg, '=')) != NULL) {
            pVarBeg = pBeg;
            while ((pVarBeg[0] != ' ') && (pVarBeg[0] != '\t') &&
                   (pVarBeg[0] != '\r') && (pVarBeg[0] != '\n') &&
                   (pVarBeg[0] != '\"') && (pVarBeg[0] != '\'') &&
                   (pVarBeg[0] != ';') && (pVarBeg[0] != ',') &&
                   (pVarBeg != (char *) &buf)) {
                pVarBeg--;
            ...
            if (pBeg[0] == '"') {
                ...
                pVarEnd = pBeg;
                while ((strlen(pVarEnd) != 0) && (pVarEnd[0] != ' ') &&
                       (pVarEnd[0] != '\t') && (pVarEnd[0] != '\r') &&
                       (pVarEnd[0] != '\n') && (pVarEnd[0] != '\"') &&
                       (pVarEnd[0] != '\'') && (pVarEnd[0] != ';') &&
                       (pVarEnd[0] != ',')) {
                    pVarEnd++;
...
static int editCmntBackSlash(char *line)
{
    ...
    while (commentPtr != NULL) {
        if ((commentPtr == line) || (commentPtr[-1] == '\r') ||
            (commentPtr[-1] == '\n')) {
            commentPtr[0] = '\0';
            pRest = (char *) &commentPtr[0];
            pRest++;

            while ((strlen(pRest) > 0) && (pRest[0] != '\r') &&
                   (pRest[0] != '\n')) {
                pRest++;
            }
    ...
    while (prolongPtr != NULL) {
        if (strlen(prolongPtr) > 0) {
            ...
            while ((strlen(prolongPtr) != 0) && (prolongPtr[0] != '\r') &&
                   (prolongPtr[0] != '\n')) {
                prolongPtr++;
            }

            if ((strlen(prolongPtr) != 0) &&
                ((prolongPtr[0] == '\r') || (prolongPtr[0] == '\n'))) {
                prolongPtr++;
            }
            if ((strlen(prolongPtr) != 0) &&
                ((prolongPtr[0] == '\r') || (prolongPtr[0] == '\n'))) {
...
static void createJobTmpDir(struct jobCard *jobCardPtr)
{
    ...
    if ((mkdir(tmpDirName, 0700) == -1) && (errno != EEXIST)) {
        ls_syslog(LOG_ERR,
                  "%s: Unable to create the job level tmp directory <%s> for "
                  "job <%s>",
...
static void execJob(struct jobCard *jobCardPtr, int chfd)
{
    ...
    if (logclass & LC_EXEC)
        ls_syslog(LOG_DEBUG, "%s: Got job start ok from mbatchd for job <%s>",
                  fname, lsb_jobid2str(jobSpecsPtr->jobId));
    ...
    if (setJobEnv(jobCardPtr) < 0) {
        ls_syslog(LOG_DEBUG, "%s: setJobEnv() failed for job <%s>", fname,
    ...
    if (nice(jobCardPtr->jobSpecs.nice) == -1 && errno != 0) {
        ls_syslog(LOG_ERR, "%s", __func__, "nice", jobSpecsPtr->nice);
    }
    ...
    if ((logclass & LC_EXEC) && jobSpecsPtr->jobSpoolDir[0] != '\0') {
        ls_syslog(LOG_DEBUG, "%s: the JobSpoolDir for the job is %s \n", fname,
                  jobSpecsPtr->jobSpoolDir);
    }
    ...
    if ((jobSpecsPtr->options & SUB_LOGIN_SHELL) || UID_MAPPED(jobCardPtr)) {
        if (createTmpJobFile(jobSpecsPtr, &fromHpBuf, jobCardPtr->stdinFile) <
            0)
            jobSetupStatus(JOB_STAT_PEND, PEND_JOB_NO_FILE, jobCardPtr);
    }
        ...
        if (!(jobSpecsPtr->options & SUB_LOGIN_SHELL) &&
            ...
            if (logclass & (LC_TRACE | LC_EXEC))
                ls_syslog(LOG_DEBUG,
                          ...
                          fname, lsb_jobid2str(jobSpecsPtr->jobId), execArgv[0],
                          execArgv[1]);
            ...
            if (jobSpecsPtr->options & SUB_LOGIN_SHELL) {
                if (jobSpecsPtr->loginShell != NULL &&
                    ...
                    shellPath = getLoginShell(NULL, jobSpecsPtr->jobFile,
                                              &fromHpBuf, 1);
            } else
                shellPath = "/bin/sh";
            ...
            putEnv("PATH",
                   "/bin:/usr/bin/:/usr/mbin:/local/bin:/usr/local:/usr/ucb");
            ...
            if (shellPath != NULL) {
                ...
                fprintf(stderr,
                        "LSF: Unable to execute login shell <%s> for job <%s>: "
                        "%s\n",
...
int setJobEnv(struct jobCard *jp)
{
    ...
    if ((eauthAuxData = getenv("LSF_EAUTH_AUX_DATA")) != NULL) {
        if (logclass & LC_TRACE) {
            ls_syslog(LOG_DEBUG, "%s: LSF_EAUTH_AUX_DATA=%s", fname,
                      eauthAuxData);
        }
    ...
    if (strlen(jp->jobSpecs.chkpntDir) == 0) {
        if (jp->jobSpecs.jobSpoolDir[0] == '\0') {
            sprintf(shellFile, "%s/.lsbatch/%s", jp->jobSpecs.execHome,
                    jp->jobSpecs.jobFile);
        } else {
            sprintf(shellFile, "%s/%s", jp->jobSpecs.jobSpoolDir,
                    jp->jobSpecs.jobFile);
        }
    } else {
        ...
        if (((strPtr = strrchr(jp->jobSpecs.chkpntDir, '/')) != NULL) &&
            ...
            if (jp->jobSpecs.chkpntDir[0] == '/') {
                if (jp->jobSpecs.options & SUB_RESTART) {
                    *strPtr = '\0';
                    sprintf(chkDir, "%s/%s", jp->jobSpecs.chkpntDir,
                ...
                if (jp->jobSpecs.options & SUB_RESTART) {
                    *strPtr = '\0';
                    sprintf(chkDir, "%s/%s/%s", cwd, jp->jobSpecs.chkpntDir,
                            lsb_jobidinstr(jp->jobSpecs.jobId));
                    *strPtr = '/';
                } else {
                    sprintf(chkDir, "%s", jp->jobSpecs.chkpntDir);
            ...
            sprintf(shellFile, "%s/%s", jp->jobSpecs.chkpntDir,
                    jp->jobSpecs.jobFile);
            ls_syslog(LOG_ERR,
                      "Failed to set LSB_CHKPNT_DIR, error in the chkpnt "
                      "directory: %s",
    ...
    sprintf(val, "trap # %d %d %d %d %d", SIGTERM, SIGUSR1, SIGUSR2, SIGINT,
            SIGHUP);
...
#define MAX_LSB_HOSTS_LEN 4096
    if (jp->jobSpecs.numToHosts <= (MAX_LSB_HOSTS_LEN / 2)) {
        hosts = (char *) my_malloc(jp->jobSpecs.numToHosts * MAXHOSTNAMELEN,
                                   "setJobEnv");
        hosts[0] = '\0';
...

    {
        NAMELIST *hostList;
        hostList =
            lsb_compressStrList(jp->jobSpecs.toHosts, jp->jobSpecs.numToHosts);
...
static int mysetLimits(struct jobSpecs *jobSpecsPtr)
{
    ...
    if ((lsbJobCpuLimit != 1) ||
        ...
        rlimitDecode_(&jobSpecsPtr->lsfLimits[LSF_RLIMIT_CPU], &rlimit,
                      LSF_RLIMIT_CPU);

        if (setrlimit(RLIMIT_CPU, &rlimit) < 0) {
            if (!debug)
            ls_syslog(LOG_ERR, "%s: setrlimit(RLIMIT_CPU failed for job <%s>: %m: soft %ld hard %ld"),
			      fname,
			      lsb_jobid2str(jobSpecsPtr->jobId),
...
#ifdef RLIMIT_FSIZE
    rlimitDecode_(&jobSpecsPtr->lsfLimits[LSF_RLIMIT_FSIZE], &rlimit,
                  LSF_RLIMIT_FSIZE);
    if (setrlimit(RLIMIT_FSIZE, &rlimit) < 0) {
        if (!debug)
        ls_syslog(LOG_ERR, "%s: setrlimit(RLIMIT_FSIZE failed for job <%s>: %m: soft %ld hard %ld"),
		fname,
		lsb_jobid2str(jobSpecsPtr->jobId),
		(long)rlimit.rlim_cur,
		rlimit.rlim_max);
...
#ifdef RLIMIT_DATA
    rlimitDecode_(&jobSpecsPtr->lsfLimits[LSF_RLIMIT_DATA], &rlimit,
                  LSF_RLIMIT_DATA);
    if (setrlimit(RLIMIT_DATA, &rlimit) < 0) {
        if (!debug)
        ls_syslog(LOG_ERR, "%s: setrlimit(RLIMIT_DATA failed for job <%s>: %m: soft %ld hard %ld"),
		fname,
		lsb_jobid2str(jobSpecsPtr->jobId),
		(long)rlimit.rlim_cur,
		rlimit.rlim_max);
...
#ifdef RLIMIT_STACK
    rlimitDecode_(&jobSpecsPtr->lsfLimits[LSF_RLIMIT_STACK], &rlimit,
                  LSF_RLIMIT_STACK);
    if (setrlimit(RLIMIT_STACK, &rlimit) < 0) {
        if (!debug)
        ls_syslog(LOG_ERR, "%s: setrlimit(RLIMIT_STACK failed for job <%s>: %m: soft %ld hard %ld"),
		fname,
		lsb_jobid2str(jobSpecsPtr->jobId),
		(long)rlimit.rlim_cur,
		rlimit.rlim_max);
...
#ifdef RLIMIT_CORE
    rlimitDecode_(&jobSpecsPtr->lsfLimits[LSF_RLIMIT_CORE], &rlimit,
                  LSF_RLIMIT_CORE);
    if (setrlimit(RLIMIT_CORE, &rlimit) < 0) {
        if (!debug)
        ls_syslog(LOG_ERR, "%s: setrlimit(RLIMIT_CORE failed for job <%s>: %m: soft %ld hard %ld"),
		fname,
		lsb_jobid2str(jobSpecsPtr->jobId),
		(long)rlimit.rlim_cur,
		rlimit.rlim_max);
    ...
    if (lsbJobMemLimit != 1) {
        rlimitDecode_(&jobSpecsPtr->lsfLimits[LSF_RLIMIT_RSS], &rlimit,
                      LSF_RLIMIT_RSS);
        if (setrlimit(RLIMIT_RSS, &rlimit) < 0) {
            if (!debug)
            ls_syslog(LOG_ERR, "%s: setrlimit(RLIMIT_RSS failed for job <%s>: %m: soft %ld hard %ld"),
		fname,
		lsb_jobid2str(jobSpecsPtr->jobId),
		(int)rlimit.rlim_cur,
		rlimit.rlim_max);
...
int job_finish(struct jobCard *jobCard, int report)
{
    ...
    if (logclass & LC_EXEC)
        ls_syslog(LOG_DEBUG, "%s: Entering ... jobId<%s> status<0x%x>", fname,
...
void unlockHosts(struct jobCard *jp, int num)
{
...
static int finishJob(struct jobCard *jobCard)
{
    ...
    if (rmDirAndFilesEx(tmpDirName, 1) != 0) {
        ls_syslog(LOG_DEBUG,
                  "%s: Fail to  delete TMPDIR=%s directory for job <%s>", fname,
                  tmpDirName, lsb_jobid2str(jobCard->jobSpecs.jobId));
    }

    if (doSendResults) {
        if (send_results(jobCard) != 0) {
            hasError = -1;
            ls_syslog(LOG_ERR, "Has error send result for job<%s>",
                      lsb_jobid2str(jobCard->jobSpecs.jobId));
        }
    }
    if (runQPost(jobCard) != 0) {
        hasError = -1;
        ls_syslog(LOG_ERR, "Has error run queue post for job<%s>",
                  lsb_jobid2str(jobCard->jobSpecs.jobId));
    }
    ...
    if ((!jobCard->mbdRestarted) &&
        ...
        if (rmJobBufFiles(jobCard) != 0) {
            hasError = -1;
            ls_syslog(LOG_ERR, "Has error to remove buffer file for job<%s>",
                      lsb_jobid2str(jobCard->jobSpecs.jobId));
        }
    }
...
void status_report(void)
{
    ...
    for (jp = jobQueHead->back; (jp != jobQueHead); jp = next) {
        ...
        if (logclass & LC_TRACE)
            ls_syslog(
                LOG_DEBUG3,
                "status_report: checking job %s notReproted=%d missing=%d "
                "needReportRU=%d now=%d startTime=%d status=%x",
                lsb_jobid2str(jp->jobSpecs.jobId), jp->notReported, jp->missing,
                jp->needReportRU, now, jp->jobSpecs.startTime,
                jp->jobSpecs.jStatus);
...
static int send_results(struct jobCard *jp)
{
    ...
    if (!(jp->jobSpecs.options & SUB_OUT_FILE)) {
        ...
        if (stat(mailSizeStr, &outfileStat) < 0) {
            ls_syslog(LOG_ERR, "%s", __func__, "stat", mailSizeStr);
            outfileStat.st_size = -1;
        }
    }
    ...
    if (daemonParams[LSB_MAILSIZE_LIMIT].paramValue != NULL) {
        ...
        if (mailSizeLimit <= 0) {
            ls_syslog(LOG_ERR,
                      "%s: Illegal value for LSB_MAILSIZE_LIMIT, ignoring",
                      fname);
        }
    }
    ...
    if (jp->jobSpecs.options & SUB_OUT_FILE) {
        ...
        if (output == NULL) {
            ...
            if (copyStdoutFromLsbatch) {
                ...
                sprintf(line, "Fail to open output file %s: %s.\n",
                        jp->jobSpecs.outFile, strerror(errno));
            }
        ...
        if (jp->jobSpecs.options & SUB_MAIL_USER)
            output =
                (mail = smail(jp->jobSpecs.mailUser, jp->jobSpecs.fromHost));
        else
            output =
                (mail = smail(jp->jobSpecs.userName, jp->jobSpecs.fromHost));
    }
    ...
    if (!copyStdoutFromLsbatch && (output == notif) &&
    ...
    if (jp->jobSpecs.numToHosts <= 1)
        fprintf(notif, "Job was executed on host(s <%s>, in queue <%s>, as user <%s>.\n"),
    ...
    fprintf(notif, "Started at %s", sp);
    free(sp);
    now = time(0);
    fprintf(notif, "Results reported at %s", ctime2(&now));
    ...
    if (cpuTime > MIN_CPU_TIME) {
        if (WIFEXITED(w_status) && !WEXITSTATUS(w_status))
            ...
            if (WIFSIGNALED(w_status)) {
                ...
                if (sig > LSF_NSIG)
                    fprintf(notif, "Exited with unknown status 0%o",
                            jp->w_status);
                else

                {
                    ...
                    temp1 = putstr_((char *sys_siglist[sig]));

                    fprintf(notif, "Exited with signal termination: %s", temp1);
                    free(temp1);
                }
                ...
                if (jp->jobSpecs.reasons & EXIT_REQUEUE) {
                    fprintf(notif, "Job was killed by user or admin, and will "
                                   "be re-queued with the same jobid.");
                } else {
                    fprintf(notif, "Exited with exit code %d",
    ...
    if (!((jp->jobSpecs.options & SUB_OUT_FILE) &&
        ...
        if ((fp = myfopen_(fileName, "r", hp)) == NULL) {
            sprintf(line,
                    "Unable to read output data from the stdout buffer file "
            ...
            if ((output == mail) && (mailSizeLimit > 0) &&
                ...
                fprintf(output, "\n");
                fprintf(output,
                        "Output is larger than limit of %ld KB set by "
                        "administrator.\n",
                        mailSizeLimit);
                fprintf(output, "Output will be saved at %s.out.\n",
                        jp->jobSpecs.jobFile);
            ...
            fprintf(output, "%s\n\n", "The output (if any follows:"));
                ...
                while ((nItems = fread(line, sizeof(char), MAXLINELEN, fp))) {
                    if (fwrite(line, sizeof(char), nItems, output) == 0) {
                        sprintf(line,
                                "\nWARNING: writing output file %s failed for "
                                "job %s\nError message: %s",
        ...
        if (jp->jobSpecs.xf[i].options & XF_OP_EXEC2SUB) {
            if (ofileHost[0] != '\0' && ofIdx == -1) {
                ...
                if ((host = ls_getmnthost(jp->jobSpecs.xf[i].execFn)) &&
                    ostat.st_dev == st.st_dev && ostat.st_ino == st.st_ino &&
            ...
            sprintf(line,
                    "Unable to copy file <%s> to <%s> on submission host <%s>: "
                    "%s.\n",
                    jp->jobSpecs.xf[i].execFn, jp->jobSpecs.xf[i].subFn,
                    jp->jobSpecs.fromHost, rcpMsg);
            hasError = -1;
    ...
    if (!errorOpeningOutputFile && (output == notif) && ferror(output)) {
        sprintf(line,
                ...
                "message: %s",
                jp->jobSpecs.outFile, lsb_jobid2str(SUBMIT_JID(jp)),
                strerror(errno));

        if (jp->jobSpecs.options & SUB_MAIL_USER) {
            merr_user(jp->jobSpecs.mailUser, jp->jobSpecs.fromHost, line,
                      "Warning");
        } else {
            merr_user(jp->jobSpecs.userName, jp->jobSpecs.fromHost, line,
                      "Warning");
        }
    }
    ...
    if (ofIdx >= 0) {
        if (rcpFile(&jp->jobSpecs, jp->jobSpecs.xf + ofIdx,
                    jp->jobSpecs.fromHost, XF_OP_EXEC2SUB, rcpMsg) < 0) {
            sprintf(ps,
                    "We are unable to copy your output file <%s> to <%s> on "
                    "submission host <%s> for job <%s>: %s.\n\n",
                    jp->jobSpecs.xf[ofIdx].execFn, jp->jobSpecs.xf[ofIdx].subFn,
                    jp->jobSpecs.fromHost, lsb_jobid2str(SUBMIT_JID(jp)),
                    rcpMsg);
            hasError = -1;

            if (jp->jobSpecs.options & SUB_MAIL_USER)
                merr_user(jp->jobSpecs.mailUser, jp->jobSpecs.fromHost, ps,
                          "Warning");
            else
                merr_user(jp->jobSpecs.userName, jp->jobSpecs.fromHost, ps,
                          "Warning");
        } else {
            if (rcpMsg[0] != '\0') {
                sprintf(line,
                        "%s: Copy output file <%s> to <%s> on submission host "
                        "<%s> for job <%s>: %s",
                        fname, jp->jobSpecs.xf[ofIdx].execFn,
                        jp->jobSpecs.xf[ofIdx].subFn, jp->jobSpecs.fromHost,
                        lsb_jobid2str(SUBMIT_JID(jp)), rcpMsg);
...
struct jobCard *addJob(struct jobSpecs *jobSpecs, int mbdVersion)
{
    ...
    if (jobSpecs->execUsername[0] == '\0') {
        ...
        if ((pw = getpwlsfuser_(jobSpecs->execUsername)) == NULL ||
            pw->pw_name == NULL) {
            ls_syslog(LOG_ERR, "%s", __func__, lsb_jobid2str(jobSpecs->jobId),
                      "getpwlsfuser_", jobSpecs->execUsername);
            FREEUP(jp);
            return NULL;
        }
    ...
    if (jp->jobSpecs.jAttrib & Q_ATTRIB_EXCLUSIVE)
        if (lockHosts(jp) < 0) {
            ls_syslog(LOG_ERR, "%s: lockHosts( failed for job <%s>; Host used by the job will not be locked"), fname, lsb_jobid2str(jp->jobSpecs.jobId));
        }
    renewJobStat(jp);
...
static int sbdStartupStopJob(struct jobCard *jp, int reasons, int subReasons)
{
    ...
    return (jRunSuspendAct(
        jp, sigValue,
        ((reasons & SUSP_USER_STOP) ? JOB_STAT_USUSP : JOB_STAT_SSUSP), reasons,
        subReasons, NO_SIGLOG));
}
...
void refreshJob(struct jobSpecs *jobSpecs)
{
    ...
    for (jp = jobQueHead->forw; jp != jobQueHead; jp = jp->forw) {
        ...
        if (jobSpecs->resumeCond && jobSpecs->resumeCond[0] != '\0') {
            if ((jp->resumeCondVal =
                     checkThresholdCond(jobSpecs->resumeCond)) == NULL)
                ls_syslog(LOG_ERR, "%s", __func__,
                          lsb_jobid2str(jp->jobSpecs.jobId),
                    ...
                    while (memp != NULL && *memp != NULL) {
                        if (strcmp(*memp, jobCardPtr->execUsername) == 0) {
                            if (setgid(grEntry->gr_gid) < 0) {
                                ls_syslog(LOG_DEBUG,
                                          "setIds: Job <%s> setgid(%d) for "
                                          "LSB_UNIXGROUP=%s  failed: %m",
                                          lsb_jobid2str(jobSpecsPtr->jobId),
...
void deallocJobCard(struct jobCard *jobCard)
{
    ...
    sprintf(fileBuf, "%s/.%s.%d.stt", LSTMPDIR,
            lsb_jobidinstr(jobCard->jobSpecs.jobId), jobCard->jobSpecs.jobPid);
    if (unlink(fileBuf) < 0 && errno != ENOENT)
        ls_syslog(LOG_ERR, "%s", __func__,
                  lsb_jobid2str(jobCard->jobSpecs.jobId), "unlink", fileBuf);
...
void saveSpecs(struct jobSpecs *jobSpecs, struct jobSpecs *specs)
{
    ...
    jobSpecs->toHosts =
        (char **) my_calloc(specs->numToHosts, sizeof(char *), fname);
    ...
    if (specs->nxf)
        jobSpecs->xf =
            (struct xFile *) my_calloc(specs->nxf, sizeof(struct xFile), fname);
    else
        jobSpecs->xf = NULL;
    ...
    if (specs->numEnv > 0) {
        jobSpecs->env =
            (char **) my_calloc(specs->numEnv, sizeof(char *), fname);
...
static int setPGid(struct jobCard *jc)
{
    ...
    if (setpgid(0, getpid()) < 0) {
        ls_syslog(LOG_ERR, "%s", __func__, lsb_jobid2str(jc->jobSpecs.jobId),
                  "setpgid");
        return -1;
    }
...
static char *getLoginShell(char *jfData, char *jobFile, struct hostent *hp,
                           int readFile)
{
    ...
    if ((fp = myfopen_(jobFile, "r", hp)) == NULL) {
        fprintf(stderr, "Cannot open your job file: %s\n", jobFile);
        exit(-1);
    }
    ...
    if (line == NULL) {
        fprintf(stderr, "Corrupted jobfile <%s>\n", jobFile);
        exit(-1);
    }
...
{
    // Bug no support for account mapping
    return 0;
}
...
static void runQPre(struct jobCard *jp, char **variables)
{
    ...
    if ((pid = fork()) == 0) {
        ...
        if (chPrePostUser(jp) < 0) {
            ls_syslog(LOG_ERR,
                      "%s: queue's pre-exec chPrePostUser failed for job <%s>",
                      fname, lsb_jobid2str(jp->jobSpecs.jobId));
            exit(-1);
        }
        ...
        if (fd[1] != STDOUT_FILENO) {
            if (dup2(fd[1], STDOUT_FILENO) != STDOUT_FILENO) {
                ls_syslog(LOG_ERR, "dup2 error to stdout\n");
            } else {
                close(fd[1]);
            }
        }
        ...
        sprintf(errMsg,
                ...
                fname, jp->jobSpecs.preCmd, lsb_jobid2str(jp->jobSpecs.jobId),
                strerror(errno));
    ...
    if (pid < 0) {
        ls_syslog(LOG_ERR, "%s: Fork to run pre-exec for job <%s> failed: %m",
                  fname, lsb_jobid2str(jp->jobSpecs.jobId));
    ...
    fp = fdopen(fd[0], "r");
    if (fp == NULL) {
        ls_syslog(LOG_ERR, "  fp = fdopen( fd[0] returned NULL!");
    }

    lineFromChild[0] = '\0';

    while ((sp = fgets(readBuf, sizeof(readBuf), fp)) != NULL) {
        if (strlen(readBuf) + strlen(lineFromChild) < sizeof(lineFromChild)) {
        ...
        if (selectPreExecEnvVariables((char *) &lineFromChild, variables) ==
            -2) {
            ls_syslog(LOG_ERR, "%s: mismatch in \" pair, job <%s>:\n<%s>",
    ...
    if (ferror(fp)) {
        ls_syslog(LOG_ERR,
                  "runQPre: error in getting pre_exec output from child");
    }
...
int runQPost(struct jobCard *jp)
{
    ...
    if (logclass & LC_TRACE) {
        chuser(batchId);
        ls_syslog(LOG_DEBUG,
                  "runQPost: queue's post-command <%s> pre-command <%s> "
                  "execJobFlag <%x> for job <%s> status %x",
    ...
    if (jp->jobSpecs.preCmd && jp->jobSpecs.preCmd[0] != '\0') {
        if (!(jp->execJobFlag & JOB_EXEC_QPRE_KNOWN) &&
            ...
            if (logclass & LC_EXEC)
                ls_syslog(LOG_DEBUG,
                          "runQPost: qpost is not run for job <%s> because "
                          "status of qpre is not known",
                          lsb_jobid2str(jp->jobSpecs.jobId));
            lsb_merr2("The queue's post-exec command is not run for job <%s> "
                      "because the exit status of the queue's pre-exec is not "
                      "known.  Please run <%s> manually if necessary.\n",
    ...
    if ((pid = fork()) == -1) {
        ...
        ls_syslog(LOG_ERR, "%s", __func__, lsb_jobid2str(jp->jobSpecs.jobId),
                  "fork");
        chuser(jp->jobSpecs.execUid);
        lsb_merr2("Unable to fork a child to run the queue's post-exec command "
                  "for job <%s>.  Please run <%s> manually if necessary.\n",
                  lsb_jobid2str(jp->jobSpecs.jobId), jp->jobSpecs.postCmd);
        return -1;
    }
    ...
    myargv[3] = NULL;
    ls_syslog(LOG_DEBUG, "%s: going to run queue post job %s", fname,
              myargv[2]);

    lsfExecvp("/bin/sh", myargv);

    sprintf(val, "%s: queue's post-exec command <%s> failed for job <%s>: %s",
            fname, jp->jobSpecs.postCmd, lsb_jobid2str(jp->jobSpecs.jobId),
            strerror(errno));
...
static int chPrePostUser(struct jobCard *jp)
{
    ...
    if (initgroups(userName, gid) < 0) {
        ls_syslog(LOG_ERR, "%s", __func__, "initgroups", userName, gid);
    }

    if (setgid(gid) < 0) {
        ls_syslog(LOG_ERR, "%s", __func__, "setgid", gid);
    }

    if (seteuid(uid) < 0) {
        ls_syslog(LOG_ERR, "%s", __func__, "setuid", uid);
        return -1;
    }
...
int postJobSetup(struct jobCard *jp)
{
    ...
    if (jp->jobSpecs.execUsername[0] == '\0') {
        if (acctMapTo(jp) < 0) {
            ls_syslog(LOG_DEBUG, "%s: acctMapTo() failed for job <%s>", fname,
                      lsb_jobid2str(jp->jobSpecs.jobId));
            return -1;
        }
        ...
        if (acctMapOk(jp) < 0) {
            ls_syslog(LOG_DEBUG, "%s: acctMapOk() failed for job <%s>", fname,
                      lsb_jobid2str(jp->jobSpecs.jobId));
            return -1;
        }
    ...
    if ((hp = (struct hostent *) getHostEntryByName_(jp->jobSpecs.fromHost)) ==
        NULL)
        ls_syslog(LOG_ERR, "%s", __func__, lsb_jobid2str(jp->jobSpecs.jobId),
                  "getHostEntryByName_", jp->jobSpecs.fromHost);
    ...
    if (!debug) {
        ...
        if (setgid(jp->execGid) < 0)
            ls_syslog(LOG_ERR, "%s", __func__, fname,
                      lsb_jobid2str(jp->jobSpecs.jobId), "setgid",
                      (int) jp->execGid);
    }
...
static void collectPreStatus(struct jobCard *jp, int pid, char *context)
{
    ...
    sprintf(errMsg, "%s: %s: wait3( got %d, not pid %d for job <%s>"),
...
static int requeueJob(struct jobCard *jp)
{
    ...
    if (logclass & LC_TRACE)
        ls_syslog(LOG_DEBUG,
                  "%s: Exit status for job <%s> is <%d>, normal termination is "
                  "<%d>, jp->w_status <%d>, requeue exit status is <%s>",
...
int reniceJob(struct jobCard *jp)
{
    ...
    if (logclass & LC_TRACE)
        ls_syslog(LOG_DEBUG, "%s: Job <%s> nice value %d", fname,
                  lsb_jobid2str(jp->jobSpecs.jobId), jp->jobSpecs.nice);
...
int updateRUsageFromSuper(struct jobCard *jp, char *mbuf)
{
    ...
    ret = sscanf(mbuf, "%d%d%d%d%d%n", &jusage.mem, &jusage.swap, &jusage.utime,
                 &jusage.stime, &jusage.npids, &cnt);

    if (ret != 5) {
    ls_syslog(LOG_ERR, "%s: sscanf( got %d values from [%s], continuing"),
	    fname, ret, mbuf);
    ...
    if (jusage.npids > 0) {
        ...
        jusage.pidInfo = (struct pidInfo *) my_malloc(
            jusage.npids * sizeof(struct pidInfo), fname);

        for (i = 0; i < jusage.npids; i++) {
            if ((ret = sscanf(mbuf, "%d%d%d%d%n", &jusage.pidInfo[i].pid,
                              &jusage.pidInfo[i].ppid, &jusage.pidInfo[i].pgid,
                              &jusage.pidInfo[i].jobid, &cnt)) != 4) {
                ls_syslog(LOG_ERR, "%s: sscanf( %s for job <%s> i=%d, ret=%d failed: %m"),
    ...
    if (logclass & (LC_TRACE | LC_EXEC)) {
        ls_syslog(LOG_DEBUG,
                  ...
                  fname, lsb_jobid2str(jp->jobSpecs.jobId), jp->runRusage.mem,
                  jp->runRusage.swap, jp->runRusage.utime, jp->runRusage.stime,
                  jp->runRusage.npgids);
        for (i = 0; i < jp->runRusage.npgids; i++)
...
static void updateJUsage(struct jobCard *jPtr, const struct jRusage *jRusage)
{
...
%s: Update rusage for job=%d from supervisor (%x/%d) newutime=%d newstime=%d newmem=%d newswap=%d wrkutime=%d wrkstime=%d prevutime=%d prevstime=%d prevmem=%d prevswap=%d",
                  fname, jPtr->jobSpecs.jobId, jPtr->client, jPtr->newPam,
                  jRusage->utime, jRusage->stime, jRusage->mem, jRusage->swap,
                  jPtr->wrkRusage.utime, jPtr->wrkRusage.stime,
                  jPtr->runRusage.utime, jPtr->runRusage.stime,
...
static void copyPidInfo(struct jobCard *jPtr, const struct jRusage *jRusage)
{
    ...
    if (jRusage->npids > 0) {
        jPtr->runRusage.pidInfo = (struct pidInfo *) my_calloc(
            jRusage->npids, sizeof(struct pidInfo), fname);
        if (jPtr->runRusage.pidInfo == NULL) {
            ls_syslog(LOG_ERR, "%s: failed to malloc (%d bytes.\n"),
		fname,
        ...
        memcpy(jPtr->runRusage.pidInfo, jRusage->pidInfo,
               (jRusage->npids) * (sizeof(struct pidInfo)));
    }
    ...
    if (jRusage->npgids > 0) {
        ...
        memcpy(jPtr->runRusage.pgid, jRusage->pgid,
               (jRusage->npgids) * (sizeof(int)));
    }
}

static void writePidInfoFile(const struct jobCard *jPtr,
                             const struct jRusage *jRusage)
{
    ...
    fp = fopen(buf, "w");
    if (fp == NULL) {
        ls_syslog(LOG_ERR, "%s: Unable to fopen( pidInfo file (%s), %m.\n"),
                fname, buf);
        return;
    }
...
static void jobFinishRusage(struct jobCard *jp)
{
    ...
    if (fp == NULL) {
        ...
        if ((rec = ls_getacctrec(fp, &lineNum)) == NULL) {
            ls_syslog(LOG_ERR, "%s", __func__, "ls_getacctrec", rufn);
        } else {
            if (logclass & LC_EXEC) {
                int w_status;
                LS_STATUS(w_status) = rec->exitStatus;
                ls_syslog(LOG_DEBUG, "%s: Job <%s> status <%d> exitcode <%d>",
                          fname, lsb_jobid2str(jp->jobSpecs.jobId),
...
int sbdParent(char *mode, struct jobCard *jCard, int chfd)
{
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, wrapPipe) < 0) {
        ls_syslog(LOG_ERR, "%s", __func__, "socketpair", "wrapPipe");
        return -1;
    }
    ...
    if (jobEnvFile != NULL) {
        ls_syslog(LOG_DEBUG, "jobEnvFile is:%s", jobEnvFile);

        jobEnvfp = fopen(jobEnvFile, "w");
        if (jobEnvfp == NULL) {
            ls_syslog(LOG_WARNING, "Cannot dump environment to file %s.",
                      jobEnvFile);
        } else {
        ...
        ls_syslog(LOG_WARNING, "Could not create temporary file in /tmp");
    }
    ...
    if (logclass & LC_EXEC)
        ls_syslog(LOG_DEBUG,
                  "%s: Before fork/exec hndlbuf=%s sbdChildArgs[0]=%s", fname,
                  hndlbuf, sbdChildArgs[0]);
    ...
    if (pid == 0) {
        ...
        if (dup2(wrapPipe[1], 0) == -1) {
            ls_syslog(LOG_ERR, "%s", __func__, "dup2");
            exit(-1);
        }
        close(wrapPipe[1]);
        ...
        ls_syslog(LOG_ERR, "%s", __func__, "execv");
        exit(-1);
    }

    if (logclass & LC_TRACE)
        ls_syslog(LOG_DEBUG, "%s: job <%s> forked sbd child pid=%d", fname,
                  lsb_jobidinstr(jCard->jobSpecs.jobId), pid);
    ...
    if (jCard->jobSpecs.eexec.len > 0) {
        ...
        if ((cc1 = b_write_fix(wrapPipe[0], jCard->jobSpecs.eexec.data,
                               jCard->jobSpecs.eexec.len)) !=
            jCard->jobSpecs.eexec.len) {
            ls_syslog(LOG_ERR, "%s", __func__,
                      lsb_jobid2str(jCard->jobSpecs.jobId), "b_write_fix",
                      "wrapPipe");
            close(wrapPipe[0]);
            close(hpipe[0]);
            return -1;
    ...
    if (!xdr_jobCard(&xdrs, jCard, &hdr)) {
        ls_syslog(LOG_ERR, "%s", __func__, "xdr_jobCard");
        close(hpipe[0]);
        free(buf);
        return -1;
    }
    ...
    if (logclass & LC_TRACE)
        ls_syslog(LOG_DEBUG2, "%s: writing len=%d on handle=%d", fname, len,
                  hpipe[0]);
...
void sbdChild(char *mode, char *arg)
{
    ...
    if (logclass & LC_TRACE)
        ls_syslog(LOG_DEBUG, "%s: mode <%s> sbdHandle=%d mbdHandle=%d", fname,
                  mode, sbdHandle, mbdHandle);

    if (getenv("LSB_MANAGER") == NULL) {
        ls_syslog(LOG_ERR,
                  "%s: LSB_MANAGER environment variable not set, protocol "
                  "problem between parent and child sbd",
                  fname);
        exit(-1);
    }
    ...
    if ((jobEnvFile = getenv("LSF_JOBENV_FILE")) != NULL) {
        if (unlink(jobEnvFile) < 0) {
            ls_syslog(LOG_WARNING, "Cannot remove the temp job file %s.",
                      jobEnvFile);
        }
    }
    ...
    if (logclass & LC_EXEC)
        ls_syslog(LOG_DEBUG, "%s: sbdHandle=%d mbdHandle=%d", fname, sbdHandle,
                  mbdHandle);

    if (b_read_fix(sbdHandle, (char *) &len, sizeof(len)) != sizeof(len)) {
        ls_syslog(LOG_ERR, "%s", __func__, "b_read_fix_len");
        exit(-1);
    }
    ...
    if (b_read_fix(sbdHandle, (char *) buf, len) != len) {
        ls_syslog(LOG_ERR, "%s", __func__, "b_read_fix_buf");
        exit(-1);
    }
    ...
    if (logclass & LC_EXEC) {
        ...
        ls_syslog(LOG_DEBUG, "sbdChild: the JOB_SPOOL_DIR is %s",
                  jCard.jobSpecs.jobSpoolDir);
    }
    ...
    if (strcmp(mode, "-s") == 0) {
        ...
        ls_syslog(LOG_ERR, "%s: invalid mode", fname);
    }
...
int initJobCard(struct jobCard *jp, struct jobSpecs *jobSpecs, int *reply)
{
    ...
    if (jobSpecs->resumeCond && jobSpecs->resumeCond[0] != '\0') {
        if ((jp->resumeCondVal = checkThresholdCond(jobSpecs->resumeCond)) ==
            NULL) {
            ls_syslog(LOG_ERR, "%s", __func__,
                      lsb_jobid2str(jp->jobSpecs.jobId), "checkThresholdCond",
                      "resumeCond");
            freeWeek(jp->week);
            *reply = ERR_BAD_REQ;
            return -1;
...
void saveThresholds(struct jobSpecs *jobSpecs, struct thresholds *thresholds)
{
    ...
    jobSpecs->thresholds.loadSched =
        (float **) my_calloc(thresholds->nThresholds, sizeof(float *), fname);
    jobSpecs->thresholds.loadStop =
        (float **) my_calloc(thresholds->nThresholds, sizeof(float *), fname);
    for (i = 0; i < thresholds->nThresholds; i++) {
        jobSpecs->thresholds.loadSched[i] =
            (float *) my_calloc(thresholds->nIdx, sizeof(float), fname);
        jobSpecs->thresholds.loadStop[i] =
            (float *) my_calloc(thresholds->nIdx, sizeof(float), fname);
    }
    for (i = 0; i < thresholds->nThresholds; i++) {
...
static void initJRusage(struct jRusage *jRusage)
{
    jRusage->mem = -1;
    jRusage->swap = -1;
    jRusage->utime = -1;
...
static int getJobVersion(struct jobSpecs *jobSpecs)
{
    int i, version;
    char *sp;
...
int lockHosts(struct jobCard *jp)
{
    ...
    for (i = 0; i < jp->jobSpecs.numToHosts; i++) {
        if (i > 0 &&
            strcmp(jp->jobSpecs.toHosts[i], jp->jobSpecs.toHosts[i - 1]) == 0)

            continue;
        if (lockHost_(0, jp->jobSpecs.toHosts[i]) < 0 &&
            lserrno != LSE_LIM_ALOCKED) {
            ls_syslog(LOG_ERR, "%s", __func__, "lockHost_",
                      "jp->jobSpecs.toHosts[i]");
            unlockHosts(jp, i);
            return -1;
        }
...
static int REShasPTYfix(char *resPath)
{
    FILE *fp;
...
static void setJobArrayEnv(char *jobName, int jobIndex)
{
    ...
    if (idxparse(&idxList, &maxJLimit)) {
        ls_syslog(LOG_ERR, "%s", __func__, "idxparse", index);
        return;
    }
    ...
    if (found) {
        ...
        ls_syslog(LOG_ERR, "%s: Job %d not found in job index list", fname,
                  jobIndex);
    while (idxList) {
        idx = idxList->next;
        FREEUP(idxList);
