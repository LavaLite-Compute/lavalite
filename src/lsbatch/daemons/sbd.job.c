...
createJobTmpDir(struct jobCard *jobCardPtr)
{
    ...
    if (   ( mkdir(tmpDirName, 0700) == -1    )
         &&( errno                   != EEXIST) ) {
	ls_syslog(LOG_ERR, "%s: Unable to create the job level tmp directory <%s> for job <%s>",
...
execJob(struct jobCard *jobCardPtr, int chfd)
{
        ...
        if (officialHostNamePtr) {
            ...
            if ( officialHostNamePtr ) {
                ...
                if ( strcmp(localHost, targetHost) != 0 ) {
                    ls_syslog(LOG_ERR, "%s: queue pre-exec migration host <%s> is not a local host <%s>, ignored. Job <%d>"
	...
	ls_syslog(LOG_ERR, "%s", __func__, "nice",
	    jobSpecsPtr->nice);
    }
	...
	*sp = '=';
	...
	char *strPtr;
	    ...
	    putEnv("LSB_CHKPNT_DIR", chkDir);
            ...
            sprintf(shellFile, "%s/%s", jp->jobSpecs.chkpntDir, jp->jobSpecs.jobFile);
            ls_syslog(LOG_ERR, "Failed to set LSB_CHKPNT_DIR, error in the chkpnt directory: %s",
                jp->jobSpecs.chkpntDir);
        }
...
finishJob(struct jobCard *jobCard)
{
	...
	lsb_merr1("Unable to setup user's environment to report results for job %s\n",
	    lsb_jobid2str(jobCard->jobSpecs.jobId));
	exit(-1);
    }
...
status_report (void)
{
	...
	next = jp->back;
	    ...
	    ls_syslog(LOG_ERR, "%s: Illegal job status <%d> of job <%s> found; re-life",
	...
	if ((now >= jp->jobSpecs.startTime)
		...
		if (rep >= 0) {
			...
			mailed = TRUE;
			lsb_merr(_i18n_printf("%s: unable to report job %s status to master; retried %d times\n",
...
send_results (struct jobCard *jp)
{
	...
	if (WIFEXITED(w_status) && !WEXITSTATUS(w_status))
...

		{
		    ...
		    fprintf(notif, "Exited with signal termination: %s",
			temp1);
		    free(temp1);
		}
	...
	if ((fp = myfopen_(fileName, "r", hp)) != NULL) {
	    ...
	    sprintf(line, "Unable to read stderr data from stderr buffer file; your job was probably aborted prematurely.\n");
	    strcat(ps, line);
	    withps = TRUE;
	} else {
	    if (errFileError)
	...
	if (jp->jobSpecs.xf[i].options & XF_OP_EXEC2SUB) {
		...
		if (rcpMsg[0] != '\0') {
		    sprintf(line, "Copy file <%s> to <%s> on submission host <%s>: %s.\n",
	    ...
	    sprintf(line, "Unable to copy file <%s> to <%s> on submission host <%s>: %s.\n",
		jp->jobSpecs.xf[i].execFn,
		jp->jobSpecs.xf[i].subFn,
		jp->jobSpecs.fromHost,
		rcpMsg);
	...
	if (rcpFile(&jp->jobSpecs, jp->jobSpecs.xf+ofIdx,
		    jp->jobSpecs.fromHost, XF_OP_EXEC2SUB, rcpMsg) < 0) {
	    sprintf(ps, "We are unable to copy your output file <%s> to <%s> on submission host <%s> for job <%s>: %s.\n\n",
...
addJob (struct jobSpecs *jobSpecs, int mbdVersion)
{
    ...
    if (cc < 0) {
	lsb_merr1("Got garbage jobSpecs for job <%s> from mbatchd upon start: die\n", lsb_jobid2str(jobSpecs->jobId));
	die(SLAVE_FATAL);
    }
...
acctMapTo(struct jobCard *jobCard)
{
    ...
    if ((myhostnm = ls_getmyhostname()) == NULL) {
        ls_syslog(LOG_ERR, "acctMapTo", "ls_getmyhostname");
        die(SLAVE_FATAL);
    }
...
trySubUser:
    ...
    if ((pw =  my_getpwnam(jobCard->jobSpecs.userName, "acctMapTo/trySubUser")) == NULL) {
	    ...
	    ls_syslog(LOG_INFO, ("%s: User %s is from a PC client host %s, will use uid %d on SBD host"),
...
acctMapOk(struct jobCard *jobCard)
{
	...
	(statbuf.st_mode & 066)) {
	sprintf(errMsg, "%s: Job <%s> file <%s> is not owned or writable only by the user, file uid <%d>, file mode <%03o>, userId <%d>",
	    fname,
	    lsb_jobid2str(jobCard->jobSpecs.jobId),
	    hostfn,
	    (int)statbuf.st_uid,
    ...
    sprintf(errMsg, "%s: Job <%s> no authorization for user name <%s> from cluster/host <%s/%s> in file <%s>",
	fname,
...
runQPre (struct jobCard *jp, char ** variables)
{
	...
	if (chPrePostUser(jp) < 0) {
	    ls_syslog(LOG_ERR, "%s: queue's pre-exec chPrePostUser failed for job <%s>",
	        fname, lsb_jobid2str(jp->jobSpecs.jobId));
	    exit(-1);
	}
	...
	sprintf(errMsg, "%s: queue's pre-exec command <%s> failed for job <%s>: %s",
	    fname,
	    jp->jobSpecs.preCmd,
	    lsb_jobid2str(jp->jobSpecs.jobId),
	    strerror(errno));
    ...
    if ( ferror(fp) ) {
      ls_syslog( LOG_ERR, "runQPre: error in getting pre_exec output from child");
    }
...
runQPost (struct jobCard *jp)
{
	...
	if (!(jp->execJobFlag & JOB_EXEC_QPRE_KNOWN) &&
	   !(daemonParams[LSB_QPOST_EXEC_ENFORCE].paramValue) ) {
		...
		ls_syslog(LOG_DEBUG, "runQPost: qpost is not run for job <%s> because status of qpre is not known",
			  lsb_jobid2str(jp->jobSpecs.jobId));
	    lsb_merr2("The queue's post-exec command is not run for job <%s> because the exit status of the queue's pre-exec is not known.  Please run <%s> manually if necessary.\n",
	...
	chuser(jp->jobSpecs.execUid);
        lsb_merr2("Unable to fork a child to run the queue's post-exec command for job <%s>.  Please run <%s> manually if necessary.\n",
	    lsb_jobid2str(jp->jobSpecs.jobId), jp->jobSpecs.postCmd);
	return -1;
    }
    ...
    if (chPrePostUser(jp) < 0) {
	ls_syslog(LOG_ERR, "%s", __func__, lsb_jobid2str(jp->jobSpecs.jobId), "chPrePostUser");
	exit(-1);
    }
...
jobFinishRusage(struct jobCard *jp)
{
    ...
    if (fp == NULL) {
        ...
        if ((rec = ls_getacctrec(fp, &lineNum)) == NULL) {
            ls_syslog(LOG_ERR, "%s", __func__, "ls_getacctrec",
                rufn);
	} else {
            if (logclass & LC_EXEC) {
    		int w_status;
...
sbdParent(char *mode, struct jobCard *jCard, int chfd)
{
    ...
    if (jobEnvFile != NULL) {
        ls_syslog(LOG_DEBUG, "jobEnvFile is:%s",
                  jobEnvFile);

        jobEnvfp = fopen (jobEnvFile, "w");
        if (jobEnvfp == NULL) {
            ls_syslog(LOG_WARNING,
                      "Cannot dump environment to file %s.",
                      jobEnvFile);
        } else {
        ...
        ls_syslog(LOG_WARNING,
                  "Could not create temporary file in /tmp"
                 );
    }
...
sbdChild(char *mode, char *arg)
{
    ...
    if (getenv("LSB_MANAGER") == NULL) {
	ls_syslog(LOG_ERR, "%s: LSB_MANAGER environment variable not set, protocol problem between parent and child sbd", fname);
	exit(-1);
    }
    ...
    if ((jobEnvFile = getenv("LSF_JOBENV_FILE")) != NULL) {
        if (unlink(jobEnvFile) < 0) {
            ls_syslog(LOG_WARNING,
                      "Cannot remove the temp job file %s.",
                      jobEnvFile);
        }
    }
...
setJobArrayEnv(char *jobName, int jobIndex)
{
	...
	ls_syslog(LOG_ERR, "%s: Job %d not found in job index list",
	    fname, jobIndex);
    while (idxList) {
	idx = idxList->next;
	FREEUP(idxList);
