...
struct builtIn builtInRes[] = {
    {"r15s", "15-second CPU run queue length", LS_NUMERIC, INCR,
     TYPE1 | RESF_RELEASE, 15},
    {"r1m", "1-minute CPU run queue length (alias: cpu)", LS_NUMERIC, INCR,
     TYPE1 | RESF_RELEASE, 15},
    {"r15m", "15-minute CPU run queue length", LS_NUMERIC, INCR,
     TYPE1 | RESF_RELEASE, 15},
    {"ut", "1-minute CPU utilization (0.0 to 1.0)", LS_NUMERIC, INCR, TYPE1,
     15},
    {"pg", "Paging rate (pages/second)", LS_NUMERIC, INCR, TYPE1, 15},
    {"io", "Disk IO rate (Kbytes/second)", LS_NUMERIC, INCR, TYPE1, 15},
    {"ls", "Number of login sessions (alias: login)", LS_NUMERIC, INCR, TYPE1,
     30},
    {"it", "Idle time (minutes) (alias: idle)", LS_NUMERIC, DECR, TYPE1, 30},
    {"tmp", "Disk space in /tmp (Mbytes)", LS_NUMERIC, DECR, TYPE1, 120},
    {"swp", "Available swap space (Mbytes) (alias: swap)", LS_NUMERIC, DECR,
     TYPE1, 15},
...
struct sharedConf *ls_readshared(char *fname)
{
    FILE *fp;
    char *cp;
    char *word;
    ...
    if (sConf == NULL) {
        if ((sConf = malloc(sizeof(struct sharedConf))) == NULL) {
    ...
    fp = fopen(fname, "r");
    if (fp == NULL) {
        ls_syslog(LOG_ERR, ("%s: Can't open configuration file <%s>."),
                  "ls_readshared", fname);
        lserrno = LSE_NO_FILE;
        return NULL;
    }
...
static char do_Index(FILE *fp, int *lineNum, char *fname)
{
    char *linep;
    ...
    if (!linep) {
        ls_syslog(LOG_ERR, "%s: premature EOF while reading %s", "do_Index",
                  fname, *lineNum, "index");
        return false;
    }

    if (isSectionEnd(linep, fname, lineNum, "newindex")) {
        ls_syslog(LOG_WARNING, ("%s: %s(%d: empty section"), "do_Index", fname,
                  *lineNum);
        return false;
    }
    ...
    if (strchr(linep, '=') == NULL) {
        ...
        if (readHvalues(keyList, linep, fp, fname, lineNum, true, "newindex") <
            0)
            return false;
        setIndex(keyList, fname, *lineNum);
        return true;
    ...
    ls_syslog(LOG_ERR, "%s: premature EOF while reading %s", "do_Index", fname,
              *lineNum, "newindex");
    return true;
}

static char setIndex(struct keymap *keyList, char *fname, int linenum)
{
    int resIdx;
    ...
    if (strpbrk(keyList[3].val, ILLEGAL_CHARS) != NULL) {
        ls_syslog(
            LOG_ERR,
            ("%s: %s(%d: illegal character (one of %s), ignoring index %s"),
    ...
    if ((resIdx = resNameDefined(keyList[3].val)) >= 0) {
        ...
        ls_syslog(LOG_ERR,
                  ("%s: %s(%d: Name %s reserved or previously defined; "
                   "overriding previous index definition"),
                  "setIndex", fname, linenum, keyList[3].val);
    } else {
        resIdx = lsinfo.nRes;
    }
...
static char do_HostTypes(FILE *fp, int *lineNum, char *fname)
{
    ...
    linep = getNextLineC_(fp, lineNum, true);
    if (!linep) {
        ls_syslog(LOG_ERR, "%s: premature EOF while reading %s", "do_HostTypes",
    ...
    if (strchr(linep, '=') == NULL) {
        ...
        ls_syslog(LOG_ERR, "%s not implemented", "do_HostTypes", fname,
                  *lineNum, "HostType");
        doSkipSection(fp, lineNum, fname, "HostType");
        return false;
    }
...
static char addHostType(char *type)
{
    ...
    for (i = 0; i < lsinfo.nTypes; i++) {
        if (strcmp(lsinfo.hostTypes[i], type) != 0)
            continue;
        ls_syslog(LOG_ERR, ("%s: host type %s multiply defined"), "addHostType",
                  type);
        return false;
    }
...
static char do_HostModels(FILE *fp, int *lineNum, char *fname)
{
    char *linep;
    ...
    if (strchr(linep, '=') == NULL) {
        ...
        while ((linep = getNextLineC_(fp, lineNum, true)) != NULL) {
            ...
            if (!isanumber_(keyList[1].val) || atof(keyList[1].val) <= 0) {
                ls_syslog(LOG_ERR,
                          ("%s: %s(%d: Bad cpuFactor for host model %s, "
                           "ignoring line"),
        ...
        ls_syslog(LOG_ERR, "%s not implemented", "do_HostModels", fname,
                  *lineNum, "HostModel");
        doSkipSection(fp, lineNum, fname, "HostModel");
        return false;
    }

    ls_syslog(LOG_ERR, "%s: premature EOF while reading %s", "do_HostModels",
...
static char addHostModel(char *model, char *arch, float factor)
{
    ...
    if (lsinfo.nModels == LL_HOSTMODEL_MAX) {
        ls_syslog(
            LOG_ERR,
            ("%s: Too many host models defined in section HostModel. You can "
             "only define up to %d host models; host model %s ignored"),
    ...
    for (i = 0; i < lsinfo.nModels; ++i) {
        ...
        ls_syslog(
            LOG_ERR,
            "%s: Duplicate host architecture type found in section HostModel. "
            "Architecture type must be unique; host model %s ignored",
            "addHostModel", model);
...
static char do_Resources(FILE *fp, int *lineNum, char *fname)
{
    char *linep;
...
#define RKEY_DESCRIPTION 5
    ...
    linep = getNextLineC_(fp, lineNum, true);
    if (!linep) {
        ls_syslog(LOG_ERR, "%s: premature EOF while reading %s", "do_Resources",
    ...
    if (strchr(linep, '=') == NULL) {
        ...
        while ((linep = getNextLineC_(fp, lineNum, true)) != NULL) {
            ...
            if (strlen(keyList[RKEY_RESOURCENAME].val) >= MAXLSFNAMELEN - 1) {
                ls_syslog(LOG_ERR,
                          ("%s: %s(%d: Resource name %s too long in section "
                           "resource. Should be less than %d characters,  "
                           "ignoring line"),
                          "do_Resources", fname, *lineNum,
            ...
            if (resNameDefined(keyList[RKEY_RESOURCENAME].val) >= 0) {
                ls_syslog(LOG_ERR,
                          ("%s: %s(%d: Resource name %s reserved or previously "
            ...
            if (IS_DIGIT(keyList[RKEY_RESOURCENAME].val[0])) {
                ls_syslog(LOG_ERR,
                          ("%s: %s(%d: Resource name <%s> begun with a digit "
                           "is illegal; ignored"),
                          "do_Resources", fname, *lineNum,
                          keyList[RKEY_RESOURCENAME].val);
                freeKeyList(keyList);
            ...
            if (keyList[RKEY_TYPE].val != NULL &&
                ...
                if ((type = validType(keyList[RKEY_TYPE].val)) >= 0)
                    ...
                    ls_syslog(LOG_ERR,
                              ("%s: %s(%d: resource type <%s> for resource "
                               "<%s> is not valid; ignoring resource <%s> in "
                               "section resource"),
                              "do_Resources", fname, *lineNum,
        ...
        ls_syslog(LOG_ERR, "%s not implemented", "do_Resources", fname,
                  *lineNum, "resource");
        return false;
    }
...
static int resNameDefined(char *name)
{
    int i;
...
struct clusterConf *ls_readcluster_ex(char *fname, struct lsInfo *info,
                                      int lookupAdmins)
{
    ...
    if (cConf == NULL) {
        if ((cConf = (struct clusterConf *) malloc(
                 sizeof(struct clusterConf))) == NULL) {
    ...
    if ((fp = fopen(fname, "r")) == NULL) {
        ...
        ls_syslog(LOG_INFO, "%s: %s(%s) failed: %m", "ls_readcluster", "fopen",
                  fname);
        lserrno = LSE_NO_FILE;
        return NULL;
    }
    ...
    for (;;) {
        ...
        if (!word) {
            ...
            if (count1 > 1) {
                ...
                if (!do_Manager(fp, fname, &lineNum, "clustermanager",
                                lookupAdmins) &&
                    aorm != true) {
                    Error = true;
                } else
            ...
            if (count2 > 1) {
                ...
                if (!do_Manager(fp, fname, &lineNum, "clusteradmins",
                                lookupAdmins) &&
                    aorm != true) {
                    Error = true;
                } else
...
struct clusterConf *ls_readcluster(char *fname, struct lsInfo *info)
{
    return ls_readcluster_ex(fname, info, true);
}

static void freeClusterInfo(struct clusterInfo *cls)
{
    int i;
...
static void initClusterInfo(struct clusterInfo *cls)
{
    if (cls != NULL) {
...
static void freeHostInfo(struct hostInfo *host)
{
    int i;
...
static void initHostInfo(struct hostInfo *host)
{
    if (host != NULL) {
...
static char do_Manager(FILE *fp, char *fname, int *lineNum, char *secName,
                       int lookupAdmins)
{
    char *linep;
    struct keymap keyList1[] = {{"MANAGER", NULL, 0}, {NULL, NULL, 0}};
    struct keymap keyList2[] = {{"ADMINISTRATORS", NULL, 0}, {NULL, NULL, 0}};
    struct keymap *keyList;

    linep = getNextLineC_(fp, lineNum, true);
    if (!linep) {
        ls_syslog(LOG_ERR, "%s: premature EOF while reading %s", "do_Manager",
                  fname, *lineNum, secName);
        return false;
    }
    ...
    if (strchr(linep, '=') == NULL) {
        ...
        if ((linep = getNextLineC_(fp, lineNum, true)) != NULL) {
            ...
            if (getClusAdmins(keyList[0].val, fname, lineNum, secName,
                              lookupAdmins) < 0) {
                FREEUP(keyList[0].val);
                return false;
            } else {
...
static int getClusAdmins(char *line, char *fname, int *lineNum, char *secName,
                         int lookupAdmins)
{
...
static struct admins *getAdmins(char *line, char *fname, int *lineNum,
                                char *secName, int lookupAdmins)
{
...
static int setAdmins(struct admins *admins, int mOrA)
{
    ...
    for (i = 0; i < workNAdmins; i++) {
        ...
        if ((tempAdminNames[tempNAdmins] = putstr_(workAdminNames[i])) ==
            NULL) {
            ls_syslog(LOG_ERR, "%s: %s failed: %m", "setAdmins", "malloc");
...
static char do_Hosts(FILE *fp, char *fname, int *lineNum, struct lsInfo *info)
{
    ...
    FREEUP(keyList);
    if (!(keyList = (struct keymap *) malloc((info->numIndx + 11) *
...
#define NUM_ALLOCATED_RESOURCES 64
    ...
    linep = getNextLineC_(fp, lineNum, true);
    if (!linep) {
        ls_syslog(LOG_ERR, "%s: premature EOF while reading %s", "do_Hosts",
                  fname, *lineNum, "host");
        return false;
    }

    if (isSectionEnd(linep, fname, lineNum, "host")) {
        ls_syslog(LOG_ERR, ("%s: %s(%d: empty host section"), "do_Hosts", fname,
                  *lineNum);
        return false;
    }
    ...
    if (strchr(linep, '=') == NULL) {
        ...
        if (keyList[R].position != -1 && keyList[SERVER0].position != -1) {
            ls_syslog(
                ...
                ("%s: %s(%d: keyword line: conflicting keyword definition: you "
                 "cannot define both 'R' and 'SERVER'. 'R' ignored"),
                "do_Hosts", fname, *lineNum);
            ignoreR = true;
        }
        ...
        while ((linep = getNextLineC_(fp, lineNum, true)) != NULL) {
            ...
            if (info->numIndx &&
                (host.busyThreshold = (float *) malloc(
                     info->numIndx * sizeof(float *))) == NULL) {
                ls_syslog(LOG_ERR, "%s: %s(%d) failed: %m", "do_Hosts",
                          "malloc");
                lserrno = LSE_MALLOC;
                freeHostInfo(&host);
                freekeyval(keyList);
            ...
            putThreshold(R15S, &host, keyList[R15S].position, keyList[R15S].val,
                         INFINITY);
            putThreshold(R1M, &host, keyList[R1M].position, keyList[R1M].val,
                         INFINITY);
            putThreshold(R15M, &host, keyList[R15M].position, keyList[R15M].val,
                         INFINITY);
            putThreshold(UT, &host, keyList[UT].position, keyList[UT].val,
                         INFINITY);
            ...
            putThreshold(PG, &host, keyList[PG].position, keyList[PG].val,
                         INFINITY);
            putThreshold(IO, &host, keyList[IO].position, keyList[IO].val,
                         INFINITY);
            putThreshold(LS, &host, keyList[LS].position, keyList[LS].val,
                         INFINITY);
            putThreshold(IT, &host, keyList[IT].position, keyList[IT].val,
                         -INFINITY);
            putThreshold(TMP, &host, keyList[TMP].position, keyList[TMP].val,
                         -INFINITY);
            putThreshold(SWP, &host, keyList[SWP].position, keyList[SWP].val,
                         -INFINITY);
            putThreshold(MEM, &host, keyList[MEM].position, keyList[MEM].val,
                         -INFINITY);

            for (i = NBUILTINDEX; i < NBUILTINDEX + info->numUsrIndx; i++) {
                if (info->resTable[i].orderType == INCR)
                    putThreshold(i, &host, keyList[i].position, keyList[i].val,
                                 INFINITY);
                else
                    putThreshold(i, &host, keyList[i].position, keyList[i].val,
                                 -INFINITY);
            ...
            while ((word = getNextWord_(&sp)) != NULL) {
                ...
                if (i < n) {
                    ls_syslog(LOG_ERR,
                              ("%s: %s(%d: Resource <%s> multiply specified "
                               "for host %s in section host. Ignored."),
                    ...
                    if (n >= numAllocatedResources) {
                        ...
                        resList = (char **) realloc(
                            resList, numAllocatedResources * (sizeof(char *)));
                        if (resList == NULL) {
                            lserrno = LSE_MALLOC;

                            ls_syslog(LOG_ERR, "%s: %s failed: %m", "do_Hosts",
                                      "calloc");
                            freeHostInfo(&host);
                            freekeyval(keyList);
                            doSkipSection(fp, lineNum, fname, "host");
            ...
            resList[n] = NULL;
            host.nRes = n;
            if (n && (host.resources = (char **) malloc(n * sizeof(char *))) ==
                         NULL) {
            ...
            if (keyList[RUNWINDOW].position != -1) {
                if (strcmp(keyList[RUNWINDOW].val, "") == 0)
                    ...
                    host.windows = parsewindow(keyList[RUNWINDOW].val, fname,
                                               lineNum, "Host");
        ...
        ls_syslog(LOG_ERR, "%s not implemented", "do_Hosts", fname, *lineNum,
                  "host");
        doSkipSection(fp, lineNum, fname, "host");
        return false;
    }

    ls_syslog(LOG_ERR, "%s: premature EOF while reading %s", "do_Hosts", fname,
              *lineNum, "host");
    return true;
}

static void putThreshold(int indx, struct hostInfo *host, int position,
                         char *val, float def)
{
    if (host == NULL)
        return;
...
static char addHost(struct hostInfo *host, char *fname, int *lineNum)
{
...
char *parsewindow(char *linep, char *fname, int *lineNum, char *section)
{
    ...
    *windows = '\0';
    while ((word = getNextWord_(&sp)) != NULL) {
        ...
        if (validWindow(word, section) < 0) {
            ls_syslog(LOG_ERR,
                      "File %s section %s at line %d: Bad time expression "
                      "<%s>; ignored.",
                      fname, section, *lineNum, save);
...
static int validWindow(char *wordpair, char *context)
{
    int oday, cday;
    float ohour, chour;
    char *sp;
...
static char do_Cluster(FILE *fp, int *lineNum, char *fname)
{
    char *linep;
    struct keymap keyList[] = {
        {"CLUSTERNAME", NULL, 0}, {"SERVERS", NULL, 0}, {NULL, NULL, 0}};
    char *servers;
    bool_t found = false;

    linep = getNextLineC_(fp, lineNum, true);
    if (!linep) {
        ls_syslog(LOG_ERR, "%s: premature EOF while reading %s", "do_Cluster",
                  fname, *lineNum, "cluster");
        return false;
    }
    ...
    if (strchr(linep, '=') == NULL) {
        ...
        ls_syslog(LOG_ERR, "%s not implemented", "do_Cluster", fname, *lineNum,
                  "cluster");
        doSkipSection(fp, lineNum, fname, "cluster");
        return false;
    }
...
static char do_Clparams(FILE *clfp, char *lsfile, int *LineNum)
{
    char *linep;
    struct keymap keyList[] = {
#define EXINTERVAL 0
...
#define LSF_ELIM_RESTARTS 10
    ...
    if (!linep) {
        ls_syslog(LOG_ERR, "%s: premature EOF while reading %s", "do_Clparams",
                  lsfile, *LineNum, "parameters");
        return false;
    }
...
#define RKEY_LOCATION 1
    ...
    linep = getNextLineC_(fp, LineNum, true);
    if (!linep) {
        ls_syslog(LOG_ERR, "%s: premature EOF while reading %s", fname, lsfile,
                  *LineNum, "resourceMap");
        return -1;
    }
    ...
    if (strchr(linep, '=') == NULL) {
        ...
        while ((linep = getNextLineC_(fp, LineNum, true)) != NULL) {
            ...
            if (mapValues(keyList, linep) < 0) {
                ls_syslog(LOG_ERR,
                          ("%s: %s(%d: values do not match keys for "
                           "resourceMap section, ignoring line"),
                          fname, lsfile, *LineNum);
                continue;
            }
            ...
            if (keyList[RKEY_LOCATION].val != NULL &&
                ...
                if (addResourceMap(keyList[RKEY_RESOURCE_NAME].val,
                                   ...
                                   *LineNum) < 0) {
                    ls_syslog(LOG_ERR,
                              "%s: %s(%d): addResourceMap() failed for "
                              "resource <%s>; ignoring line",
                              fname, lsfile, *LineNum,
                              keyList[RKEY_RESOURCE_NAME].val);
                    freeKeyList(keyList);
                    continue;
                ...
                ls_syslog(LOG_ERR,
                          ("%s: %s(%d: No LOCATION specified for resource "
                           "<%s>; ignoring the line"),
                          fname, lsfile, *LineNum,
                          keyList[RKEY_RESOURCE_NAME].val);
                freeKeyList(keyList);
        ...
        ls_syslog(LOG_ERR, "%s not implemented", fname, lsfile, *LineNum,
                  "resource");
        return -1;
    }
    return 0;
                          ...
                          int LineNum)
{
    ...
    if (resName == NULL || location == NULL) {
        ls_syslog(LOG_ERR, ("%s: %s(%d: Resource name <%s> location <%s>"),
                  fname, lsfile, LineNum, (resName ? resName : "NULL"),
    ...
    if (!strcmp(location, "!")) {
        ...
        hosts = &tempHost;
        if ((resource = addResource(resName, 1, hosts, initValue, lsfile,
                                    LineNum)) == NULL) {
            ls_syslog(LOG_ERR,
                      "%s: %s(%d): %s() failed; ignoring the instance <%s>",
        ...
        if (*sp == '[') {
            ...
            *sp = '\0';
            ...
            if ((numHosts = parseHostList(cp, lsfile, LineNum, &hosts)) <= 0) {
                ls_syslog(
                    LOG_ERR,
                    ("%s: %s(%d: %s(%s) failed; ignoring the instance <%s%s>"),
                    fname, lsfile, LineNum, "parseHostList", cp, instance, "]");
                continue;
            }

            if (resource == NULL) {
                if ((resource = addResource(resName, numHosts, hosts, initValue,
                                            lsfile, LineNum)) == NULL)
                    ls_syslog(
                        LOG_ERR,
                        ("%s: %s(%d: %s() failed; ignoring the instance <%s>"),
                                                ...
                                                char *fileName, int LineNum)
{
    ...
    if ((resInfo = (struct lsSharedResourceInfo *) myrealloc(
             cConf->shareRes, sizeof(struct lsSharedResourceInfo) *
                                  (cConf->numShareRes + 1))) == NULL) {
                           ...
                           int nHosts, char **hostNames, char *value)
{
    ...
    instance = (struct lsSharedResourceInstance *) myrealloc(
        sharedResource->instances, sizeof(struct lsSharedResourceInstance) *
                                       (sharedResource->nInstances + 1));
    ...
    if ((instance[inst].hostList = (char **) malloc(sizeof(char *) * nHosts)) ==
        NULL) {
        ls_syslog(LOG_ERR, "%s: %s(%d) failed: %m", "addHostInstance",
                  "malloc");
    ...
    for (i = 0; i < nHosts; i++) {
        if ((instance[inst].hostList[i] = putstr_(hostNames[i])) == NULL) {
            ls_syslog(LOG_ERR, "%s: %s failed: %m", "addHostInstance",
                      "putstr_");
            for (i--; i >= 0; i--)
...
clean_up:
    ...
    if (lserrno == LSE_MALLOC) {
        ls_syslog(LOG_ERR, "%s: %s failed: %m", "convertNegNotation_",
                  "malloc");
    }
...
void freeSA_(char **list, int num)
{
    int i;
...
static int resolveBaseNegHosts(char *inHosts, char **outHosts,
                               struct HostsArray *array)
{
