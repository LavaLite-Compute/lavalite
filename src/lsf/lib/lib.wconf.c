/* $Id: lib.wconf.c,v 1.3 2007/08/15 22:18:51 tmizan Exp $
...
struct lsConf *ls_getconf(char *fname)
{
    ...
    int i, len, numDefs = 0, defsize;
    long offset;

    lserrno = LSE_NO_ERR;
    if (fname == NULL) {
        ls_syslog(LOG_ERR, "%s: %s.", "ls_getconf", "Null filename");
        lserrno = LSE_NO_FILE;
        return NULL;
    }
    ...
    if (conf == NULL) {
        ls_syslog(LOG_ERR, "%s: %s(%d) failed: %m", "ls_getconf", "malloc",
                  sizeof(struct lsConf));
        lserrno = LSE_MALLOC;
        return NULL;
    }
    ...
    if (fp == NULL) {
        ls_syslog(LOG_ERR, "%s: %s <%s>.", "ls_getconf",
                  "Can't open configuration file", fname);
        lserrno = LSE_NO_FILE;
        goto Error;
    }
    ...
    while ((linep = getNextLineD_(fp, &lineNum, false)) != NULL) {
        ...
        if (word && word[0] == '#') {
            ...
            if (strcasecmp(cp, "define") == 0) {
                ...
                if (word == NULL) {
                    ls_syslog(
                        LOG_ERR, "%s: %s(%d): %s.", "ls_getconf", fname,
                        lineNum,
                ...
                if (*word == '\0') {
                    ls_syslog(LOG_ERR, "ls_getconf: %s(%d): %s", fname, lineNum,
                              "Both macro and condition name expected after "
                              "#define.");
                    FREEUP(word1);
                    goto Error;
                }
                ...
                if (temp == NULL) {
                    ls_syslog(LOG_ERR, "ls_getconf: %s(%d): If-less elif.",
                              fname, lineNum);
                    goto Error;
                }
                ...
                if (*word == '\0') {
                    ls_syslog(LOG_ERR, "ls_getconf: %s(%d): %s.", fname,
                              lineNum, "Condition name expected after #elif.");
                    goto Error;
                }
                ...
                temp = popStack(blockStack);
                if (temp == NULL) {
                    ls_syslog(LOG_ERR, "ls_getconf: %s(%d): If-less endif.",
                              fname, lineNum);
                    goto Error;
                }
                PUSH_STACK(blockStack, temp);
        ...
        for (;;) {
            ...
            if (word && word[0] == '#') {
                ...
                if (strcasecmp(cp, "define") == 0 ||
                    strcasecmp(cp, "if") == 0 || strcasecmp(cp, "elif") == 0 ||
    ...
    temp = popStack(blockStack);
    if (temp != NULL) {
        ls_syslog(LOG_ERR, "ls_getconf: %s(%d): %s endif.", fname, lineNum,
                  "Missing");
        goto Error;
    }
...
static void freeNode(struct confNode *node)
{
    int i;
...
char *getNextLineC_conf(struct lsConf *conf, int *LineCount, int confFormat)
{
...
static char *readNextLine(struct lsConf *conf, int *lineNum)
{
