...
int get_host_by_name(const char *hostname, struct ll_host *hp)
{
     ...
     * but we keep AF_UNSPEC in comments as a reminder of the future dual-stack path.
     *
    ...
    fill_addrstr((const struct sockaddr *)&hp->sa,
                 hp->salen, hp->addr, sizeof(hp->addr));
