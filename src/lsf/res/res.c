/* $Id: res.c,v 1.12 2007/08/15 22:18:55 tmizan Exp $
...
main(int argc, char **argv)
{
    ...
    if (pathname == NULL) {
        if ((pathname = getenv("LSF_ENVDIR")) == NULL)
            // Bug. Hardcode it for now... anyway we expect
            // modules to be used to start the  system
            pathname = "/etc";
    }
...
initSignals(void)
{
    ...
    Signal_(SIGPIPE, sigHandler);
    Signal_(SIGTTIN, sigHandler);
    Signal_(SIGTTOU, sigHandler);
    Signal_(SIGTSTP, sigHandler);
    Signal_(SIGTERM, term_handler);
    Signal_(SIGXCPU, term_handler);
    Signal_(SIGXFSZ, term_handler);
    Signal_(SIGPROF, term_handler);
    Signal_(SIGABRT, term_handler);
