/* $Id: lim.policy.c,v 1.7 2007/08/15 22:18:54 tmizan Exp $
...
static int findBestHost(register struct resVal *, int, int, char **, int, char,
                        int, int);
...
void placeReq(XDR *xdrs, struct sockaddr_in *from, struct packet_header *reqHdr,
              int s)
{
    ...
    if (!xdr_decisionReq(xdrs, &plReq, reqHdr)) {
        ls_syslog(LOG_ERR, "%s: %s(%s) failed: %m", fname, "xdr_decisionReq",
                  sockAdd2Str_(from));
        ls_syslog(LOG_ERR,
                  "%s: Header: operation=%d sequence=%d version=%d length=%d ",
                  fname, reqHdr->operation, reqHdr->sequence, reqHdr->version,
                  reqHdr->length);
    ...
    if (!validHosts(plReq.preferredHosts, plReq.numPrefs, &clName,
    ...
    if (cc != PARSE_OK ||
        (returnCode = evalResReq(resVal.selectStr, &tclHostData,
    ...
    if (strcmp(plReq.hostType, " ") == 0)
        strcpy(plReq.hostType, (fromHostPtr->hTypeNo >= 0)
                                   ? shortInfo.hostTypes[fromHostPtr->hTypeNo]
                                   : "unknown");
    ...
    if (ncandidates > plReq.numHosts) {
        ncandidates = findBestHost(&resVal, plReq.numHosts, plReq.numPrefs,
                                   plReq.preferredHosts, ncandidates, true,
                                   ignore_res, plReq.options);
    } else {
    ...
    selectBestInstances(ncandidates, plReq.numHosts, plReq.options & LOCALITY,
                        ignore_res);
    ...
    placeReply.placeInfo = (struct placeInfo *) calloc(
        placeReply.numHosts, sizeof(struct placeInfo));
...
Reply1:
    ...
    if (!xdr_encodeMsg(&xdrs2, replyStruct, &replyHdr, xdr_placeReply, 0,
                       NULL)) {
...
static int getOkSites(int num, int retain, int ignore_res)
{
    int i, j, left;
    int tempstatus[2];
...
int getEligibleSites(struct resVal *resValPtr, struct decisionReq *reqPtr,
                     char all, char *fromEligible)
{
    ...
    if (logclass & LC_SCHED)
        ls_syslog(LOG_DEBUG2, "getEligibleSites: Looking at hosts on %s",
                  clPtr->clName);

    ncand = grabHosts(clPtr->hostList, resValPtr, reqPtr, ncand, fromEligible,
                      flags);
    if (all)
    ...
    if (logclass & LC_SCHED)
        ls_syslog(LOG_DEBUG2, "getEligibleSites: Returning <%d> candidates",
                  ncand);
    return ncand;
}

static int grabHosts(struct hostNode *hList, struct resVal *resValPtr,
                     struct decisionReq *reqPtr, int ncand, char *fromEligible,
                     int flags)
{
...
static int getNumInstances(int ncandidates)
{
    int i, numInst;
...
static void potentialOfHost(struct hostNode *hPtr, struct resVal *resValPtr)
{
    int indexpot;
    int i, maxCpus;
    ...
    for (i = 0; i < NBUILTINDEX; i++) {
        if (!(i == R15S || i == R1M || i == R15M || i == MEM || i == SWP ||
              i == TMP))
            continue;
...
static void potentialOfCandidates(int ncandidates, struct resVal *resValPtr)
{
    int i;
...
static void selectBestInstances(int ncandidates, int needed, char locality,
                                int ignore_res)
{
    ...
    if (locality) {
        ...
        cand = bsort(allInfo.numIndx, ncandidates, 0, ncandidates, 0, flags,
                     ignore_res, NORMALIZE);
        if (cand != ncandidates)
            ls_syslog(LOG_ERR,
                      "%s: Internal scheduling error(1cand=%d ncandidates=%d",
                      fname, cand, ncandidates);
...
static int findBestHost(struct resVal *resValPtr, int num, int numPrefs,
                        ...
                        int ignore_res, int rqlOptions)
{
    ...
    for (i = resValPtr->nphase - 1; i >= 0; i--) {
        ...
        if (candidates[ncandidates - 1]->conStatus == true) {
            ...
            cc = 0;
            exld = candidates[ncandidates - 1]->loadIndex[lidx] * nec /
                   (ncandidates * 25.0);
        ...
        if (lidx == R15S || lidx == R1M || lidx == R15M) {
            ...
            cpuf = (candidates[ncandidates - 1]->hModelNo >= 0)
                       ? shortInfo
                             .cpuFactors[candidates[ncandidates - 1]->hModelNo]
                       : 1.0;
...
static int findNPref(int ncandidates, int numPrefs, char **preferredHosts)
{
    register int i, j;
    int nec = 0;
...
static int bsort(int lidx, int ncandidates, int nec, int numHosts,
                 float threshold, char flags, int ignore_res, int rqlOptions)
{
    ...
    if (!(flags & SORT_FINAL)) {
        ...
        while (swap && (i < ncandidates - cutoffs)) {
            ...
            for (j = ncandidates - 2; j >= i; j--) {
                ...
                if (!(flags & SORT_SINDX)) {
                    mkexld(candidates[j], candidates[j + 1], lidx, &exld1,
                           &exld2, coef);
                } else {
                    exld1 = 0.0;
                    exld2 = 0.0;
                ...
                if (NOTORDERED(
                        incr, loadIndexValue(j, lidx, rqlOptions) + exld1,
    ...
    while (swap && (i < cutoffs)) {
        ...
        for (j = ncandidates - 2; j >= i; j--) {
            ...
            if (NOTORDERED(incr, loadIndexValue(j, lidx, rqlOptions) + exld1,
                           loadIndexValue(j + 1, lidx, rqlOptions) + exld2)) {
                swap = true;
                tmp = candidates[j];
                candidates[j] = candidates[j + 1];
...
static int orderByStatus(int j, int ignore_res)
{
    struct hostNode *tmp;
    int *status1, *status2;
...
static void mkexld(struct hostNode *hn1, struct hostNode *hn2, int lidx,
                   float *exld1, float *exld2, float coef)
{
...
static void loadAdj(struct jobXfer *jobXferPtr, struct hostNode **destHostPtr,
                    int num, char child)
{
    ...
    if (lim_udp_sock < 0) {
        ls_syslog(LOG_ERR, "%s: invalid channel(lim_udp_sock=%d)", fname,
                  lim_udp_sock);
        return;
    }
...
void loadadjReq(XDR *xdrs, struct sockaddr_in *from,
...
void updExtraLoad(struct hostNode **destHostPtr, char *resReq, int numHosts)
{
    ...
    if (parseResReq(resReq, &resVal, &allInfo, PR_RUSAGE) != PARSE_OK) {
        ls_syslog(LOG_ERR, "%s: %s(%s) failed: %m", fname, "parseResReq",
                  resReq);
        return;
    }
...
static void jackup(int lidx, struct hostNode *hostPtr, float exval)
{
    ...
    if (lidx == R15S || lidx == R1M || lidx == R15M) {
        ...
        hostPtr->loadIndex[lidx] = normalizeRq(
            hostPtr->uloadIndex[lidx],
            (hostPtr->hModelNo >= 0) ? shortInfo.cpuFactors[hostPtr->hModelNo]
                                     : 1.0,
...
void loadReq(XDR *xdrs, struct sockaddr_in *from, struct packet_header *reqHdr,
             int s)
{
    ...
    if (!validHosts(ldReq.preferredHosts, ldReq.numPrefs, &clName,
                    ldReq.options)) {
        limReplyCode = LIME_UNKWN_HOST;
        ls_syslog(LOG_INFO,
                  ("%s: validHosts( failed for bad cluster/host name requested "
    ...
    if (strcmp(ldReq.hostType, " ") == 0)
        strcpy(ldReq.hostType, (fromHostPtr->hTypeNo >= 0)
                                   ? shortInfo.hostTypes[fromHostPtr->hTypeNo]
                                   : "unknown");
    ...
    if (ldReq.options & OK_ONLY) {
        ncandidates = getOkSites(ncandidates, 0, ignore_res);
        if ((ncandidates == 0) ||
            ((ncandidates < ldReq.numHosts) && (ldReq.options & EXACT))) {
            limReplyCode = LIME_NO_OKHOST;
            goto Reply;
        }
    } else {
    ...
    if (ncandidates > ldReq.numHosts) {
        ...
        findBestHost(&resVal, ncandidates, ldReq.numPrefs, ldReq.preferredHosts,
                     ncandidates, false, ignore_res, ldReq.options);
    ...
    reply.loadMatrix = (struct hostLoad *) malloc(
            ...
            if (indx == R15S || indx == R1M || indx == R15M) {
                if (ldReq.options & NORMALIZE)
                    ...
                    factor = (candidates[i]->hModelNo >= 0)
                                 ? shortInfo.cpuFactors[candidates[i]->hModelNo]
                                 : 1.0;
...
Reply1:
    ...
    if (limReplyCode == LIME_NO_ERR) {
        ...
        bufSize = ALIGNWORD_(MAXLSFNAMELEN * allInfo.numIndx + hlSize +
                             reply.nEntry * (lvecSize + staSize));
    } else {
        replyStruct = (char *) NULL;
        bufSize = 512;
    }
    ...
    if (!xdr_encodeMsg(&xdrs2, replyStruct, &replyHdr, xdr_loadReply, 0,
                       NULL)) {
        ls_syslog(LOG_ERR, "%s: %s failed: %m", fname, "xdr_encodeMsg");
        xdr_destroy(&xdrs2);
        if (limReplyCode == LIME_NO_ERR)
    ...
    if (cc < 0) {
        ls_syslog(LOG_ERR,
                  "%s: Failed in sending lsload reply to %s (len=%d: %m", fname,
...
static int addCandList(struct hostNode *hPtr, int pos)
{
    char *memp;
...
void chkResReq(XDR *xdrs, struct sockaddr_in *from,
               struct packet_header *reqHdr)
{
...
void getTclHostData(struct tclHostData *tclHostData, struct hostNode *hostNode,
                    struct hostNode *fromHostNode, int checkSyntax)
{
    ...
    tclHostData->hostType = (hostNode->hTypeNo >= 0)
                                ? shortInfo.hostTypes[hostNode->hTypeNo]
                                : "unknown";
    tclHostData->hostModel = (hostNode->hModelNo >= 0)
                                 ? shortInfo.hostModels[hostNode->hModelNo]
                                 : "unknown";
    tclHostData->fromHostType = (fromHostNode->hTypeNo >= 0)
                                    ? shortInfo.hostTypes[fromHostNode->hTypeNo]
                                    : "unknown";
    tclHostData->fromHostModel =
        (fromHostNode->hModelNo >= 0)
            ? shortInfo.hostModels[fromHostNode->hModelNo]
            : "unknown";
    tclHostData->cpuFactor = (hostNode->hModelNo >= 0)
                                 ? shortInfo.cpuFactors[hostNode->hModelNo]
                                 : 1.0;
...
static void setBusyIndex(int lidx, struct hostNode *host)
{
    float load;
...
static float loadIndexValue(int hostIdx, int loadIdx, int rqlOptions)
{
    float loadIndex;
    ...
    if (loadIdx == R15S || loadIdx == R1M || loadIdx == R15M) {
        if (rqlOptions & NORMALIZE) {
            ...
            factor = (candidates[hostIdx]->hModelNo >= 0)
                         ? shortInfo.cpuFactors[candidates[hostIdx]->hModelNo]
                         : 1.0;
            loadIndex =
                effectiveRq(candidates[hostIdx]->loadIndex[loadIdx], factor);
