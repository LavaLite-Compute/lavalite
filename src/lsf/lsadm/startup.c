/* $Id: startup.c,v 1.8 2007/08/15 22:18:54 tmizan Exp $
...
getLSFenv(void)
{
    ...
    fprintf(stderr, "%s\n", "Host does not belong to LSF cluster.");
    return (-1);
}
...
void startupAllHosts(int opCode, int confirm)
{
    ...
    if (confirm) {
        if (opCode == LSADM_LIMSTARTUP)
            sprintf(msg,  "Do you really want to start up LIM on all hosts ? [y/n]");
        else if (opCode == LSADM_RESSTARTUP)
            sprintf(msg, "Do you really want to start up RES on all hosts ? [y/n]");
        else if (opCode == BADMIN_HSTARTUP)
            sprintf(msg, "Do you really want to start up slave batch daemon on all hosts ? [y/n] ");
        else {
...
startupLocalHost (int opCode)
{
    ...
    if (opCode == LSADM_LIMSTARTUP)
	sprintf(operation, "Starting up LIM on");
    else if (opCode == LSADM_RESSTARTUP)
	sprintf(operation, "Starting up RES on");
    else if (opCode == BADMIN_HSTARTUP)
	sprintf(operation, "Starting up slave batch daemon on");
    else {
...
startupRemoteHost(char *host, int opCode, int ask)
{
    ...
    if (opCode == LSADM_LIMSTARTUP)
        sprintf(msg, "Start up LIM on");
    else if (opCode == LSADM_RESSTARTUP)
        sprintf(msg, "Start up RES on");
    else if (opCode == BADMIN_HSTARTUP)
        sprintf(msg, "Start up slave batch daemon on");
    else {
...
startup(int argc, char **argv, int opCode)
{
    ...
    if (setStartupUid() < 0) {
	fprintf(stderr, "%s\n",
		"Not authorized to start up as root");
	return (-1);
    }
	...
	fprintf(stderr, "%s\n",
		"Should not start remote lim from slave only or clienthost");
	return (-1);
    }
