/* $Id: lsadmin.c,v 1.9 2007/08/15 22:18:54 tmizan Exp $
...
static char *cmdInfo[] = { "Reconfigure LSF",
        ...
        case 'l':
            ...
            if (opCode == LIM_DEBUG || opCode == RES_DEBUG) {
                if (debug.level < 0 || debug.level > 3) {
                    fprintf(stderr,"Command denied. Valid debug level value is [0-3]\n");
                    return(-1);
                }
            }
            else if (debug.level < 1 || debug.level > 5) {
                fprintf(stderr,"Command denied. Valid timing level value is [1-5]\n");
                return(-1);
            }
            break;
