...
static bool is_master_valid(const struct clusterNode *);
...
static bool is_master_valid(const struct clusterNode *c)
{
    if (!c->masterKnown)
        return false;
...
void errorBack(struct sockaddr_in *from, struct protocol_header *reqHdr,
