...
void rcvConfInfo(XDR *xdrs, struct sockaddr_in *from, struct protocol_header *hdr)
{
...
int probeMasterTcp(struct clusterNode *clPtr)
{
    ...
    rc = send_protocol_header(ch, &hdr);
    if (rc < 0) {
        ls_syslog(LOG_ERR, "%s: failed send_protocol_header() to %s %m", __func__,
