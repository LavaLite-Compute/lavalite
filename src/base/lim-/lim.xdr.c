...
static bool xdr_resPair(XDR *xdrs, struct resPair *resPair, void *ctx)
{
...
bool xdr_loadvector(XDR *xdrs, struct loadVectorStruct *lvp,
...
bool xdr_loadmatrix(XDR *xdrs, int len, struct loadVectorStruct *lmp,
                      struct protocol_header *hdr)
{
    return true;
}

bool xdr_masterReg(XDR *xdrs, struct masterReg *masterRegPtr,
                     struct protocol_header *hdr)
{
    char *sp1;
    char *sp2;
...
bool xdr_masterAnnSLIMConf(XDR *xdrs,
...
bool xdr_statInfo(XDR *xdrs, struct statInfo *sip, struct protocol_header *hdr)
{
    char *sp1, *sp2;
...
bool xdr_lvector(XDR *xdrs, float *li, uint32_t nIndices)
{
    if (nIndices < 0)
        return false;
    if (nIndices == 0)
...
// LavaLite
bool xdr_master_beacon(XDR *xdrs, struct master_beacon *reg)
{
...
bool xdr_wire_load_update(XDR *xdrs, struct wire_load_update *w)
{
    uint32_t n;
    ...
    if (!xdr_uint32_t(xdrs, &w->status0))
        return false;
