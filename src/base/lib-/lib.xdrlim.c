...
static bool xdr_shortLsInfo(XDR *, struct shortLsInfo *, void *);
static void freeUpMemp(char *, int);
static bool xdr_lsResourceInstance(XDR *, struct lsSharedResourceInstance *,
                                     void *);
int sharedResConfigured_ = false;
static bool xdr_placeInfo(XDR *xdrs, struct placeInfo *placeInfo, void *ctx)
{
    char *sp;
...
static bool xdr_hostLoad(XDR *xdrs, struct hostLoad *loadVec, void *ctx)
{
    char *sp;
    int i;
    int *nIndicies = (int *) ctx;
...
bool xdr_decisionReq(XDR *xdrs, struct decisionReq *decisionReqPtr,
                       struct protocol_header *hdr)
{
    ...
    if (!xdr_int(xdrs, &decisionReqPtr->options))
        return false;
    if (!xdr_string(xdrs, &sp1, MAXLSFNAMELEN))
        return false;
    if (!xdr_int(xdrs, &decisionReqPtr->numHosts))
        return false;
    if (!xdr_string(xdrs, &sp2, MAXLINELEN))
        return false;
    if (!xdr_int(xdrs, &decisionReqPtr->numPrefs))
        return false;
    ...
    if (!xdr_array_string(xdrs, decisionReqPtr->preferredHosts, LL_HOSTNAME_MAX,
                          decisionReqPtr->numPrefs)) {
        if (xdrs->x_op == XDR_DECODE)
            free(decisionReqPtr->preferredHosts);
        return false;
    }
...
bool xdr_placeReply(XDR *xdrs, struct placeReply *placeRepPtr,
                      struct protocol_header *hdr)
{
    int i, status = false;
    static char *memp;
    ...
    if (xdrs->x_op == XDR_DECODE) {
        if (memp)
            FREEUP(memp);
        placeRepPtr->placeInfo = (struct placeInfo *) malloc(
            (int) placeRepPtr->numHosts * sizeof(struct placeInfo));
    ...
    for (i = 0; i < placeRepPtr->numHosts; i++) {
        status = xdr_array_element(xdrs, &placeRepPtr->placeInfo[i], NULL,
                                   xdr_placeInfo);
        if (!status) {
            if (xdrs->x_op == XDR_DECODE)
                FREEUP(memp);
...
bool xdr_loadReply(XDR *xdrs, struct loadReply *loadReplyPtr,
                     struct protocol_header *hdr)
{
    char *sp;
    int i, status = true;
    ...
    if (xdrs->x_op == XDR_DECODE) {
        ...
        staSize =
            ALIGNWORD_((1 + GET_INTNUM(loadReplyPtr->nIndex)) * sizeof(int));
    ...
    for (i = 0; i < loadReplyPtr->nEntry; i++) {
        status = xdr_array_element(xdrs, &loadReplyPtr->loadMatrix[i],
                                   &loadReplyPtr->nIndex, xdr_hostLoad);
        if (!status) {
            if (xdrs->x_op == XDR_DECODE)
                FREEUP(memp);
            return false;
...
bool xdr_jobXfer(XDR *xdrs, struct jobXfer *jobXferPtr,
                   struct protocol_header *hdr)
{
    int i, status = false;
    char *sp;
    ...
    if (xdrs->x_op == XDR_DECODE) {
        if (memp)
            FREEUP(memp);
        jobXferPtr->placeInfo = (struct placeInfo *) malloc(
            (int) jobXferPtr->numHosts * sizeof(struct placeInfo));
    ...
    for (i = 0; i < jobXferPtr->numHosts; i++) {
        status = xdr_array_element(xdrs, &jobXferPtr->placeInfo[i], NULL,
                                   xdr_placeInfo);
        if (!status) {
            if (xdrs->x_op == XDR_DECODE)
                FREEUP(memp);
...
bool xdr_hostInfoReply(XDR *xdrs, struct hostInfoReply *hostInfoReply,
                         struct protocol_header *hdr)
{
    int i, status;
    ...
    if (xdrs->x_op == XDR_DECODE) {
        ...
        resSize = (GET_INTNUM(hostInfoReply->shortLsInfo->nRes) +
                   GET_INTNUM(hostInfoReply->nIndex)) *
                  sizeof(int);
        matSize = ALIGNWORD_(hostInfoReply->nHost *
                             (vecSize + resSize + MAXLINELEN + 100));
    ...
    for (i = 0; i < hostInfoReply->nHost; i++) {
        status = xdr_array_element(xdrs, &hostInfoReply->hostMatrix[i],
                                   &hostInfoReply->nIndex, xdr_shortHInfo);
        if (!status) {
            if (xdrs->x_op == XDR_DECODE)
                FREEUP(memp);
            return false;
...
bool xdr_shortHInfo(XDR *xdrs, struct shortHInfo *shortHInfo, void *cntx)
{
    ...
    if (!xdr_string(xdrs, &sp, MAXHOSTNAMELEN) || !xdr_u_int(xdrs, &a) ||
...
static bool xdr_shortLsInfo(XDR *xdrs, struct shortLsInfo *shortLInfo,
                              void *ctx)
{
    int i;
    ...
    if (xdrs->x_op == XDR_DECODE) {
        ...
        memp = malloc(
            (shortLInfo->nRes + shortLInfo->nTypes + shortLInfo->nModels) *
                MAXLSFNAMELEN +
...
bool xdr_limLock(XDR *xdrs, struct limLock *limLockPtr,
                   struct protocol_header *hdr)
{
    char *sp;
...
static bool xdr_resItem(XDR *xdrs, struct resItem *resItem, void *ctx)
{
    char *sp;
    char *sp1;
...
bool xdr_lsInfo(XDR *xdrs, struct lsInfo *lsInfoPtr,
                  struct protocol_header *hdr)
{
    int i;
    char *sp = (char *) 0;
    ...
    if (xdrs->x_op == XDR_DECODE) {
        FREEUP(memp);
        lsInfoPtr->resTable =
            (struct resItem *) malloc(lsInfoPtr->nRes * sizeof(struct resItem));
    ...
    for (i = 0; i < lsInfoPtr->nRes; i++) {
        if (!xdr_array_element(xdrs, &lsInfoPtr->resTable[i], NULL,
                               xdr_resItem)) {
            if (xdrs->x_op == XDR_DECODE)
                FREEUP(memp);
            return false;
...
bool xdr_masterInfo(XDR *xdrs, struct masterInfo *mInfoPtr,
                      struct protocol_header *hdr)
{
    char *sp;
...
bool xdr_clusterInfoReq(XDR *xdrs, struct clusterInfoReq *clusterInfoReq,
                          struct protocol_header *hdr)
{
    ...
    if (clusterInfoReq->listsize && xdrs->x_op == XDR_DECODE) {
        clusterInfoReq->clusters =
            (char **) calloc(clusterInfoReq->listsize, sizeof(char *));
    ...
    if (!xdr_array_string(xdrs, clusterInfoReq->clusters, MAXLSFNAMELEN,
                          clusterInfoReq->listsize)) {
        if (xdrs->x_op == XDR_DECODE) {
...
bool xdr_clusterInfoReply(XDR *xdrs,
                            ...
                            struct protocol_header *hdr)
{
    ...
    if (!xdr_array_element(xdrs, clusterInfoReply->shortLsInfo, NULL,
                           xdr_shortLsInfo))
        return false;
    ...
    for (i = 0; i < clusterInfoReply->nClus; i++) {
        bool status = 0;
        status = xdr_array_element(xdrs, &clusterInfoReply->clusterMatrix[i],
                                   NULL, xdr_shortCInfo);
        if (!status) {
            if (xdrs->x_op == XDR_DECODE) {
                nClus = 0;
...
static void freeUpMemp(char *memp, int nClus)
{
    int i, j;
...
bool xdr_shortCInfo(XDR *xdrs, struct shortCInfo *clustInfoPtr, void *ctx)
{
    char *sp1;
    char *sp2;
    char *sp3;
    ...
    if (xdrs->x_op == XDR_DECODE && clustInfoPtr->nRes) {
        clustInfoPtr->resBitMaps =
            (int *) malloc(GET_INTNUM(clustInfoPtr->nRes) * sizeof(int));
    ...
    if (xdrs->x_op == XDR_DECODE && clustInfoPtr->nTypes) {
        clustInfoPtr->hostTypeBitMaps =
            (int *) malloc(GET_INTNUM(clustInfoPtr->nTypes) * sizeof(int));
    ...
    if (xdrs->x_op == XDR_DECODE && clustInfoPtr->nModels) {
        clustInfoPtr->hostModelBitMaps =
            (int *) malloc(GET_INTNUM(clustInfoPtr->nModels) * sizeof(int));
...
bool xdr_cInfo(XDR *xdrs, struct cInfo *cInfo, struct protocol_header *hdr)
{
    char *sp1, *sp2, *sp3;
    int i;
    ...
    if (!(xdr_string(xdrs, &sp1, MAXLSFNAMELEN) &&
          xdr_string(xdrs, &sp2, MAXHOSTNAMELEN) &&
          xdr_string(xdrs, &sp3, MAXLSFNAMELEN) &&
          xdr_int(xdrs, &cInfo->status) && xdr_int(xdrs, &cInfo->numServers) &&
          xdr_int(xdrs, &cInfo->numClients) &&
          xdr_int(xdrs, &cInfo->managerId) && xdr_int(xdrs, &cInfo->resClass) &&
          xdr_int(xdrs, &cInfo->typeClass) &&
          xdr_int(xdrs, &cInfo->modelClass) && xdr_int(xdrs, &cInfo->numIndx) &&
    ...
    if (xdrs->x_op == XDR_DECODE && cInfo->nRes) {
        cInfo->resBitMaps =
            (int *) malloc(GET_INTNUM(cInfo->nRes) * sizeof(int));
    ...
    if (cInfo->numIndx > 0 && xdrs->x_op == XDR_DECODE) {
        cInfo->loadIndxNames = (char **) calloc(cInfo->numIndx, sizeof(char *));
    ...
    if (cInfo->numIndx > 0) {
        if (!xdr_array_string(xdrs, cInfo->loadIndxNames, LL_NAME_MAX,
                              cInfo->numIndx)) {
    ...
    if (xdrs->x_op == XDR_DECODE && cInfo->nTypes) {
        cInfo->hostTypeBitMaps =
            (int *) malloc(GET_INTNUM(cInfo->nTypes) * sizeof(int));
    ...
    if (xdrs->x_op == XDR_DECODE && cInfo->nModels) {
        cInfo->hostModelBitMaps =
            (int *) malloc(GET_INTNUM(cInfo->nModels) * sizeof(int));
...
bool xdr_resourceInfoReq(XDR *xdrs, struct resourceInfoReq *resourceInfoReq,
                           struct protocol_header *hdr)
{
    int i;
    ...
    if (xdrs->x_op == XDR_DECODE && resourceInfoReq->numResourceNames > 0) {
        if ((resourceInfoReq->resourceNames =
                 calloc((size_t) resourceInfoReq->numResourceNames,
                        sizeof(char *))) == NULL) {
            lserrno = LSE_MALLOC;
            return false;
        }
...
static bool xdr_lsResourceInfo(XDR *xdrs,
                                 ...
                                 struct protocol_header *hdr)
{
    ...
    for (int i = 0; i < lsResourceInfo->nInstances; i++) {
        status = xdr_array_element(xdrs, &lsResourceInfo->instances[i], NULL,
...
bool xdr_resourceInfoReply(XDR *xdrs,
                             ...
                             void *)
{
    ...
    if (xdrs->x_op == XDR_DECODE && resourceInfoReply->numResources > 0) {
        if ((resourceInfoReply->resources =
    ...
    for (i = 0; i < resourceInfoReply->numResources; i++) {
        status = xdr_array_element(xdrs, &resourceInfoReply->resources[i], NULL,
                                   xdr_lsResourceInfo);
        if (!status) {
...
static bool xdr_lsResourceInstance(XDR *xdrs,
                                     ...
                                     void *ctx)
{
    ...
    if (xdrs->x_op == XDR_DECODE && instance->nHosts > 0) {
        if ((instance->hostList =
                 (char **) malloc(instance->nHosts * sizeof(char *))) == NULL) {
            lserrno = LSE_MALLOC;
            return false;
        }
    }
    if (!xdr_array_string(xdrs, instance->hostList, MAXHOSTNAMELEN,
                          instance->nHosts)) {
        if (xdrs->x_op == XDR_DECODE) {
