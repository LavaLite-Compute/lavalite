/* $Id: lsb.conf.c,v 1.18 2007/08/15 22:18:47 tmizan Exp $
...
static char do_Groups(struct groupInfoEnt **, struct lsConf *, char *, int *,
                      int *, int);
...
static char addMember(struct groupInfoEnt *, char *, int, char *, int, char *,
                      int, int);
...
static struct groupInfoEnt *addUnixGrp(struct group *, char *, char *, int,
                                       char *, int);
...
static void checkCpuLimit(char **, float **, int, char *, int *, char *,
                          struct lsInfo *, int);
static int parseDefAndMaxLimits(struct keymap, int *, int *, char *, int *,
                                char *);
...
static int readHvalues_conf(struct keymap *keyList, char *linep,
                            struct lsConf *conf, char *lsfile, int *LineNum,
                            int exact, char *section)
{
    ...
    if (!value) {
        ls_syslog(LOG_ERR,
                  "%s: %s(%d): missing '=' after keyword %s, section %s "
                  "ignoring the line",
        ...
        if (value[0] == '\0') {
            ls_syslog(LOG_ERR,
                      "%s: %s(%d): null value after keyword %s, section %s "
                      "ignoring the line",
                      fname, lsfile, *LineNum, key, section);
            lsberrno = LSBE_CONF_WARNING;
        } else {
            ...
            if (putValue(keyList, key, value) < 0) {
                ls_syslog(LOG_ERR,
                          "%s: %s(%d): bad keyword %s in section %s, ignoring "
                          "the line",
                          fname, lsfile, *LineNum, key, section);
                lsberrno = LSBE_CONF_WARNING;
            }
        ...
        if (isSectionEnd(linep, lsfile, LineNum, section)) {
            ...
            while (keyList[i].key != NULL) {
                if (keyList[i].val == NULL) {
                    ls_syslog(LOG_ERR,
                              "%s: %s(%d): required keyword %s is missing in "
                              "section %s, ignoring the section",
    ...
    ls_syslog(LOG_ERR, NULL, fname, lsfile, *LineNum, section);
    return -1;
}
...
struct paramConf *lsb_readparam(struct lsConf *conf)
{
    ...
    if (pConf) {
        ...
        if ((pConf = (struct paramConf *) malloc(sizeof(struct paramConf))) ==
            NULL) {
            ls_syslog(LOG_ERR, NULL, pname, "malloc", sizeof(struct paramConf));
            lsberrno = LSBE_CONF_FATAL;
            return NULL;
        }
        pConf->param = NULL;
    ...
    if ((pConf->param = (struct parameterInfo *) malloc(
        ...
        ls_syslog(LOG_ERR, NULL, pname, "malloc", sizeof(struct parameterInfo));
        lsberrno = LSBE_CONF_FATAL;
        return NULL;
    }
    ...
    for (;;) {
        if ((cp = getBeginLine_conf(conf, &lineNum)) == NULL) {
            if (paramok == false) {
                ls_syslog(
                    LOG_ERR,
                    "%s: File %s at line %d: No valid parameters are read",
                    pname, fname, lineNum);
                lsberrno = LSBE_CONF_WARNING;
            }
            return pConf;
        ...
        if (!section) {
            ls_syslog(LOG_ERR,
                      "%s: File %s at line %d: Section name expected after "
                      "Begin; ignoring section",
                      pname, fname, lineNum);
            lsberrno = LSBE_CONF_WARNING;
            ...
            ls_syslog(LOG_ERR,
                      "%s: File %s at line %d: Invalid section name <%s>; "
                      "ignoring section",
                      pname, fname, lineNum, section);
            lsberrno = LSBE_CONF_WARNING;
...
static char do_Param(struct lsConf *conf, char *fname, int *lineNum)
{
    char *linep;
    int i, value;
    ...
    if (isSectionEnd(linep, fname, lineNum, "parameters")) {
        ls_syslog(LOG_WARNING, NULL, pname, fname, *lineNum, "parameters");
        lsberrno = LSBE_CONF_WARNING;
        return false;
    }
    ...
    if (strchr(linep, '=') == NULL) {
        ls_syslog(LOG_ERR,
                  "%s: File %s at line %d: Vertical parameters section is not "
                  "implemented yet; use horizontal format; ignoring section",
                  pname, fname, *lineNum);
        lsberrno = LSBE_CONF_WARNING;
    ...
    if (readHvalues_conf(keylist, linep, conf, fname, lineNum, false,
                         "parameters") < 0) {
        ls_syslog(LOG_ERR, "%s: File %s at line %d: Incorrect section; ignored",
                  pname, fname, *lineNum);
        lsberrno = LSBE_CONF_WARNING;
        freekeyval(keylist);
        return false;
        ...
        if (keylist[i].val != NULL && strcmp(keylist[i].val, "")) {
            if (i == 0) {
                ls_syslog(LOG_WARNING,
                          "%s: Ignore LSB_MANAGER value <%s>; use MANAGERS  "
                          "defined in cluster file instead",
                          pname, keylist[i].val);
                lsberrno = LSBE_CONF_WARNING;
            }
                ...
                if (checkSpoolDir(keylist[i].val) == 0) {
                    ...
                    ls_syslog(LOG_ERR, "%s: Invalid JOB_SPOOL_DIR!", pname);
                    lsberrno = LSBE_CONF_WARNING;
                }
            } else if (i == 32) {
                ...
                if (value == INFINIT_INT) {
                    ls_syslog(LOG_ERR, I18N(5459, "\
                                %s: File %s in section Parameters ending at line %d: Value <%s> of %s isn't a positive integer between 1 and %d; ignored"),
                ...
                if ((value = my_atoi(keylist[i].val, INFINIT_INT, -1)) ==
                    INFINIT_INT) {
                    ls_syslog(LOG_ERR,
                              "%s: File %s in section Parameters ending at "
                              "line %d: Value <%s> of %s isn't a non-negative "
                              "integer between 0 and %d; ignored",
                              pname, fname, *lineNum, keylist[i].val,
                              keylist[i].key, INFINIT_INT - 1);
                    lsberrno = LSBE_CONF_WARNING;
                } else if (i == 4)
                ...
                if (strcasecmp(keylist[i].val, "Y") == 0) {
                    ...
                    ls_syslog(LOG_ERR,
                              "%s: File %s in section Parameters ending at "
                              "line %d: unrecognizable value <%s> for the "
                              "keyword DISABLE_UACCT_MAP; assume user level "
                              "account mapping is allowed",
                              pname, fname, *lineNum, keylist[i].val);
                    pConf->param->disableUAcctMap = false;
                }
            } else if (i == 26) {
                ...
                if (ptr == NULL || value == INFINIT_INT ||
                    ...
                    ls_syslog(
                        LOG_ERR,
                        "%s: File %s in section Parameters ending at line %d: "
                        "Value <%s> of %s isn't in format value/time (time in "
                        "minutes) or value is not positive; ignored",
                        pname, fname, *lineNum, keylist[i].val, keylist[i].key);
                    lsberrno = LSBE_CONF_WARNING;
                    continue;
                }
                ...
                if ((value < MAX_JOBID_LOW) || (value > MAX_JOBID_HIGH)) {
                    ls_syslog(LOG_ERR,
                              "%s: File%s in section Parameters ending at line "
                              "%d: maxJobId value %s not in [%d, %d], use "
                              "default value %d;",
                ...
                if (value == INFINIT_INT) {
                    ls_syslog(LOG_ERR,
                              "%s: File %s in section Parameters ending at "
                              "line %d: Value <%s> of %s isn't a positive "
                              "integer between 1 and %d; ignored",
                              pname, fname, *lineNum, keylist[i].val,
                              keylist[i].key, INFINIT_INT - 1);
                    lsberrno = LSBE_CONF_WARNING;
                } else
                    ...
                    case 19:
                        if (value < 1 || value >= LSB_MAX_ARRAY_IDX) {
                            ls_syslog(LOG_ERR,
                                      "%s: File %s in section Parameters "
                                      "ending at line %d: Value <%s> of %s is "
                                      "out of range[1-65534]); ignored",
                                      pname, fname, *lineNum, keylist[i].val,
                                      keylist[i].key);
                            lsberrno = LSBE_CONF_WARNING;
                        } else
                    ...
                    case 25:
                        ...
                        if (value != INFINIT_INT) {
                            ...
                            ls_syslog(LOG_ERR,
                                      "%s: File%s in section Parameters ending "
                                      "at line %d: invalid value <%s> of <%s> "
                                      ": ignored;",
                                      pname, fname, *lineNum, keylist[i].val,
                                      keylist[i].key);
                        }
                    ...
                    default:
                        ls_syslog(LOG_ERR,
                                  "%s: File%s in section Parameters ending at "
                                  "line %d: Impossible cases <%d>.",
                                  pname, fname, *lineNum, i);
                        lsberrno = LSBE_CONF_WARNING;
                        break;
                    }
            }
    ...
    if (pConf->param->maxUserPriority <= 0 &&
        ...
        ls_syslog(LOG_ERR,
                  "%s: File%s in section Parameters : MAX_USER_PRIORITY should "
                  "be defined first so that JOB_PRIORITY_OVER_TIME can be "
                  "used: job priority control disabled",
                  pname, fname);
...
static int my_atoi(char *arg, int upBound, int botBound)
{
    int num;
...
static float my_atof(char *arg, float upBound, float botBound)
{
    float num;
...
static void initParameterInfo(struct parameterInfo *param)
{
    if (param != NULL) {
...
static void freeParameterInfo(struct parameterInfo *param)
{
    if (param != NULL) {
...
int checkSpoolDir(char *pSpoolDir)
{
    char *pTemp = NULL;
...
struct userConf *lsb_readuser(struct lsConf *conf, int options,
                              struct clusterConf *clusterConf)
{
    return (lsb_readuser_ex(conf, options, clusterConf, NULL));
}

struct userConf *lsb_readuser_ex(struct lsConf *conf, int options,
                                 struct clusterConf *clusterConf,
                                 struct sharedConf *sharedConf)
{
    ...
    for (;;) {
        if ((cp = getBeginLine_conf(conf, &lineNum)) == NULL) {
            if (numofugroups) {
                if ((uConf->ugroups = (struct groupInfoEnt *) calloc(
                         numofugroups, sizeof(struct groupInfoEnt))) == NULL) {
                    ls_syslog(LOG_ERR, NULL, pname, "malloc",
        ...
        if (!section) {
            ...
            if (strcasecmp(section, "user") == 0) {
                ...
                if (do_Groups(usergroups, conf, fname, &lineNum, &numofugroups,
                              options) == false) {
                    if (lsberrno == LSBE_NO_MEM) {
                        lsberrno = LSBE_CONF_FATAL;
                        freeWorkUser(true);
...
static char do_Users(struct lsConf *conf, char *fname, int *lineNum,
                     int options)
{
    char *linep;
    char *grpSl = NULL;
    ...
    struct keymap keylist[] = {{"USER_NAME", NULL, 0},
                               {"MAX_JOBS", NULL, 0},
                               {"JL/P", NULL, 0},
                               {NULL, NULL, 0}};
    ...
    if (isSectionEnd(linep, fname, lineNum, "user")) {
        ls_syslog(LOG_WARNING, NULL, pname, fname, *lineNum, "user");
        lsberrno = LSBE_CONF_WARNING;
        return false;
    }
    if (strchr(linep, '=') == NULL) {
        if (!keyMatch(keylist, linep, false)) {
            ls_syslog(LOG_ERR,
                      "%s: File %s at line %d: Keyword line format error for "
                      "User section; ignoring section",
                      pname, fname, *lineNum);
            lsberrno = LSBE_CONF_WARNING;
        ...
        if (keylist[0].position < 0) {
            ls_syslog(LOG_ERR,
                      "%s: File %s at line %d: User name required for User "
                      "section; ignoring section",
                      pname, fname, *lineNum);
            lsberrno = LSBE_CONF_WARNING;
            doSkipSection_conf(conf, lineNum, fname, "user");
            return false;
        ...
        if ((tmpUsers = (struct hTab *) malloc(sizeof(struct hTab))) == NULL ||
            ...
            ls_syslog(LOG_ERR, NULL, pname, "malloc", sizeof(struct hTab));
            lsberrno = LSBE_NO_MEM;
            return false;
        }
        ...
        while ((linep = getNextLineC_conf(conf, lineNum, true)) != NULL) {
            ...
            if (mapValues(keylist, linep) < 0) {
                ls_syslog(LOG_ERR,
                          "%s: File %s at line %d: Values do not match keys in "
                          "User section; ignoring line",
                          pname, fname, *lineNum);
                lsberrno = LSBE_CONF_WARNING;
                continue;
            }
            ...
            if (!new) {
                ls_syslog(LOG_ERR,
                          "%s: File %s at line %d: User name <%s> multiply "
                          "specified; ignoring line",
            ...
            if ((options != CONF_NO_CHECK) && !gp &&
                ...
                if (unixGrp != NULL) {
                    if (options & (CONF_EXPAND | CONF_NO_EXPAND | CONF_CHECK)) {
                        ...
                        if (gp == NULL) {
                            if (lsberrno == LSBE_NO_MEM)
                                return false;
                            ls_syslog(LOG_WARNING,
                                      "%s: File %s at line %d: No valid users "
                                      "defined in Unix group <%s>; ignored",
                                      pname, fname, *lineNum, keylist[0].val);
                            lsberrno = LSBE_CONF_WARNING;
                            continue;
                        }
                    ...
                    ls_syslog(LOG_WARNING,
                              "%s: File %s at line %d: Unknown user <%s>; "
                              "Maybe a windows user or of another domain.",
                              pname, fname, *lineNum, keylist[0].val);
                    lsberrno = LSBE_CONF_WARNING;
                }
            }
            if (hasAt && gp)
                isGroupAt = true;
            if (hasAt && (!(options & (CONF_EXPAND | CONF_NO_EXPAND)) ||
            ...
            if (keylist[1].position >= 0 && keylist[1].val != NULL &&
                ...
                if ((maxjobs = my_atoi(keylist[1].val, INFINIT_INT, -1)) ==
                    INFINIT_INT) {
                    ls_syslog(LOG_ERR,
                              "%s: File %s at line %d: Invalid value <%s> for "
                              "key <%s>; %d is assumed",
                              pname, fname, *lineNum, keylist[1].val,
                              keylist[1].key, INFINIT_INT);
                    lsberrno = LSBE_CONF_WARNING;
                }
            }
            pJobLimit = INFINITY;
            if (keylist[2].position >= 0 && keylist[2].val != NULL &&
                ...
                if ((pJobLimit = my_atof(keylist[2].val, INFINITY, -1.0)) ==
                    ...
                    ls_syslog(LOG_ERR,
                              "%s: File %s at line %d: Invalid value <%s> for "
                              "key %s; %f is assumed",
                              pname, fname, *lineNum, keylist[2].val,
                              keylist[2].key, INFINITY);
                    lsberrno = LSBE_CONF_WARNING;
                }
            }
            if (!isGroupAt && (!(options & (CONF_EXPAND | CONF_NO_EXPAND)) ||
                               options == CONF_NO_CHECK)) {
                h_addEnt_(nonOverridableUsers, keylist[0].val, 0);
                if (!addUser(keylist[0].val, maxjobs, pJobLimit, fname, true) &&
                    lsberrno == LSBE_NO_MEM) {
                    FREEUP(grpSl);
                    lsberrno = LSBE_NO_MEM;
                    goto Error;
                ...
                if ((groupMembers =
                         expandGrp(grpname, &numMembers, USER_GRP)) == NULL) {
                    ls_syslog(LOG_ERR, NULL, fname, "expandGrp");
                    goto Error;
                }
                if (strcmp(groupMembers[0], "all") == 0) {
                    ls_syslog(LOG_ERR,
                              "%s: File %s at line %d: user group <%s> with no "
                              "members is ignored",
                              pname, fname, *lineNum, keylist[0].val);
                    ...
                    for (i = 0; i < numMembers; i++) {
                        if (h_getEnt_(nonOverridableUsers, groupMembers[i]))
                            ...
                            continue;
                        addUser(groupMembers[i], maxjobs, pJobLimit, fname,
                                true);
                    }
                } else {
                    if (isGroupAt)
...
static char do_Groups(struct groupInfoEnt **groups, struct lsConf *conf,
                      char *fname, int *lineNum, int *ngroups, int options)
{
    static char pname[] = "do_Groups";
    struct keymap keylist[] = {
        {"GROUP_NAME", NULL, 0}, {"GROUP_MEMBER", NULL, 0}, {NULL, NULL, 0}};
    char *linep;
    char *wp;
    char *sp;
    char *HUgroups;
    ...
    if (isSectionEnd(linep, fname, lineNum, HUgroups)) {
        ls_syslog(LOG_WARNING, NULL, pname, fname, *lineNum, HUgroups);
        lsberrno = LSBE_CONF_WARNING;
        return false;
    }
    ...
    if (strchr(linep, '=') == NULL) {
        ...
        while ((linep = getNextLineC_conf(conf, lineNum, true)) != NULL) {
            ...
            if (strcmp(keylist[0].val, "all") == 0 ||
                ...
                ls_syslog(
                    LOG_WARNING,
                    "%s: File %s at line %d: Group name <%s> conflicts with "
                    "reserved word <all/default/others>; ignoring line",
                    pname, fname, *lineNum, keylist[0].val);
                lsberrno = LSBE_CONF_WARNING;
                continue;
            }
            ...
            if (options != CONF_NO_CHECK && type == USER_GRP) {
                ...
                if ((pw != NULL) ||
                    ...
                    ls_syslog(LOG_WARNING,
                              "%s: File %s at line %d: Group name <%s> "
                              "conflicts with user login name; ignoring line",
                              pname, fname, *lineNum, keylist[0].val);
                    lsberrno = LSBE_CONF_WARNING;
                    continue;
                }
            ...
            if (options != CONF_NO_CHECK && type == HOST_GRP &&
                ...
                ls_syslog(LOG_WARNING,
                          "%s: File %s at line %d: Group name <%s> conflicts "
                          "with host name; ignoring line",
                          pname, fname, *lineNum, keylist[0].val);
                lsberrno = LSBE_CONF_WARNING;
                continue;
            }
            if (options != CONF_NO_CHECK &&
                ...
                ls_syslog(LOG_WARNING,
                          "%s: File %s at line %d: Group <%s> is multiply "
                          "defined; ignoring line",
                          pname, fname, *lineNum, keylist[0].val);
                lsberrno = LSBE_CONF_WARNING;
                continue;
            }
            ...
            if (*ngroups >= MAX_GROUPS) {
                if (nGrpOverFlow++ == 0)
                    ...
                    ls_syslog(LOG_WARNING,
                              "%s: File %s at line %d: The number of "
                              "configured groups reaches the limit <%d>; "
                              "ignoring the rest of groups defined",
                              pname, fname, *lineNum, MAX_GROUPS);
                lsberrno = LSBE_CONF_WARNING;
                continue;
            }
            ...
            if ((type == HOST_GRP) && *ngroups >= MAX_GROUPS) {
                ls_syslog(LOG_ERR,
                          "%s: File %s at line %d: The number of configured "
                          "host groups reaches the limit <%d>; ignoring the "
                          "rest of groups defined",
                          pname, fname, *lineNum, MAX_GROUPS);
                lsberrno = LSBE_CONF_WARNING;
                continue;
                ...
                ls_syslog(LOG_ERR,
                          "%s: File %s at line %d: The number of configured "
                          "user groups reaches the limit <%d>; ignoring the "
                          "rest of groups defined",
                          pname, fname, *lineNum, MAX_GROUPS);
                lsberrno = LSBE_CONF_WARNING;
                continue;
            ...
            if (type == HOST_GRP && (strchr(keylist[1].val, '~') != NULL &&
                ...
                if (numHosts > 0) {
                    ...
                    ls_syslog(LOG_WARNING,
                              "%s: File %s at line %d: there are no hosts "
                              "found to exclude, replaced with \'%s\'",
                              pname, fname, *lineNum, outHosts);
                } else if (numHosts == -1) {
                    lsberrno = LSBE_NO_MEM;
                    return false;
                } else {
                    if (numHosts == -3) {
                        ls_syslog(LOG_WARNING,
                                  "%s: \'%s\': \'%s\' The result is that all "
                                  "the hosts are to be excluded.",
                                  pname, keylist[0].val, keylist[1].val);
                    }
                    ls_syslog(LOG_WARNING,
                              "%s: File %s at line %d: No valid member in "
                              "group <%s>; ignoring line",
                              pname, fname, *lineNum, keylist[0].val);
            ...
            while (!allFlag && (wp = getNextWord_(&sp)) != NULL) {
                if (!addMember(gp, wp, type, fname, *lineNum, "", options,
                               true) &&
                    lsberrno == LSBE_NO_MEM) {
                    return false;
                }
            ...
            if (allFlag) {
                if (!addMember(gp, "all", type, fname, *lineNum, "", options,
                               true) &&
                    lsberrno == LSBE_NO_MEM) {
                    return false;
                }
            ...
            if (gp->memberList == NULL) {
                ls_syslog(LOG_WARNING,
                          "%s: File %s at line %d: No valid member in group "
                          "<%s>; ignoring line",
                          pname, fname, *lineNum, gp->group);
                lsberrno = LSBE_CONF_WARNING;
                FREEUP(gp->group);
                FREEUP(gp);
                *ngroups -= 1;
...
static int isHostName(char *grpName)
{
    ...
    for (int i = 0; i < numofhosts; i++) {
        if (hosts && hosts[i]->host && equal_host(grpName, hosts[i]->host))
            return true;
    }
...
static struct groupInfoEnt *addGroup(struct groupInfoEnt **groups, char *gname,
                                     int *ngroups, int type)
{
...
static struct groupInfoEnt *addUnixGrp(struct group *unixGrp, char *gname,
                                       char *fname, int lineNum, char *section,
                                       int type)
{
    int i = -1;
    struct groupInfoEnt *gp;
    ...
    if (numofugroups >= MAX_GROUPS) {
        ls_syslog(LOG_ERR,
                  "addUnixGrp: File %s at line %d: numofugroups <%d> is equal "
                  "to or greater than MAX_GROUPS <%d>; ignoring the group <%s>",
                  fname, lineNum, numofugroups, MAX_GROUPS, unixGrp->gr_name);
        return NULL;
    }
    if (gname == NULL)
    ...
    while (unixGrp->gr_mem[++i] != NULL) {
        if ((pw = getpwnam2(unixGrp->gr_mem[i]))) {
            ...
            ls_syslog(LOG_ERR,
                      "addUnixGrp: File %s%s at line %d: Bad user name <%s> in "
                      "group <%s>; ignored",
...
static char addMember(struct groupInfoEnt *gp, char *word, int grouptype,
                      char *fname, int lineNum, char *section, int options,
                      int checkAll)
{
    ...
    if (myWord == NULL) {
        ls_syslog(LOG_ERR, NULL, "addMember", "malloc", strlen(cp) + 1);
        lsberrno = LSBE_NO_MEM;
        return false;
    }

    if (grouptype == USER_GRP && strcmp(myWord, "all") &&
        options != CONF_NO_CHECK) {
        subgrpPtr = getUGrpData(myWord);

        if (gp == subgrpPtr) {
            ls_syslog(LOG_ERR,
                      "%s: File %s at line %d: Recursive member in Unix group "
                      "<%s>; ignored",
                      pname, fname, lineNum, myWord);
            lsberrno = LSBE_CONF_WARNING;
            FREEUP(myWord);
            return false;
        }
        ...
        if (pw != NULL) {
            ...
            if (unixGrp) {
                if (options & CONF_EXPAND) {
                    subgrpPtr =
                        addUnixGrp(unixGrp, myWord, fname, lineNum, section, 1);
                ...
                ls_syslog(
                    LOG_WARNING,
                    "%s: File %s%s at line %d: Unknown user <%s> in group "
                    "<%s>; Maybe a windows user or of another domain.",
                    pname, fname, section, lineNum, myWord, gp->group);
                lsberrno = LSBE_CONF_WARNING;
    ...
    if (grouptype == HOST_GRP && strcmp(myWord, "all") &&
        options != CONF_NO_CHECK) {
        ...
        if (subgrpPtr != NULL) {
            ...
            if (get_host_by_name(myWord, &hp) < 0) {
                ls_syslog(LOG_ERR,
                          "%s: File %s%s at line %d: Bad host/group name <%s> "
                          "in group <%s>; ignored",
                          pname, fname, section, lineNum, myWord, gp->group);
                lsberrno = LSBE_CONF_WARNING;
                FREEUP(myWord);
            ...
            if (getHostData(name) == NULL && numofhosts != 0) {
                ls_syslog(LOG_ERR,
                          "%s: File %s%s at line %d: Host <%s> is not used by "
                          "the batch system; ignored",
                          pname, fname, section, lineNum, name);
                lsberrno = LSBE_CONF_WARNING;
                FREEUP(myWord);
                return false;
            }
            if (isServerHost(name) == false) {
                ls_syslog(LOG_ERR,
                          "%s: File %s%s at line %d: Host <%s> is not a "
                          "server; ignored",
                          pname, fname, section, lineNum, name);
                lsberrno = LSBE_CONF_WARNING;
                FREEUP(myWord);
    ...
    if (returnVal) {
        if (isgrp)
            ls_syslog(LOG_ERR,
                      "%s: File %s%s at line %d: Group <%s> is multiply "
                      "defined in group <%s>; ignored",
                      pname, fname, section, lineNum, myWord, gp->group);
        else
            ls_syslog(LOG_ERR,
                      "%s: File %s%s at line %d: Member <%s> is multiply "
                      "defined in group <%s>; ignored",
                      pname, fname, section, lineNum, myWord, gp->group);
        lsberrno = LSBE_CONF_WARNING;
        FREEUP(myWord);
    ...
    if (gp->memberList == NULL) {
        ...
        if (gp->memberList == NULL) {
            ls_syslog(LOG_ERR, NULL, "addMember", "malloc", strlen(myWord) + 1);
            lsberrno = LSBE_NO_MEM;
            FREEUP(myWord);
            return false;
        }
        ...
        if ((gp->memberList = (char *) myrealloc(gp->memberList,
                                                 len * sizeof(char))) == NULL) {
            ls_syslog(LOG_ERR, NULL, "addMember", "myrealloc",
                      len * sizeof(char));
            lsberrno = LSBE_NO_MEM;
            FREEUP(myWord);
            return false;
...
static struct groupInfoEnt *getUGrpData(char *gname)
{
    return (getGrpData(usergroups, gname, numofugroups));
}

static struct groupInfoEnt *getHGrpData(char *gname)
{
    return (getGrpData(hostgroups, gname, numofhgroups));
}

static struct groupInfoEnt *getGrpData(struct groupInfoEnt **groups, char *name,
                                       int num)
{
    int i;
...
static struct userInfoEnt *getUserData(char *name)
{
    int i;
...
static struct hostInfoEnt *getHostData(char *name)
{
    int i;
...
static struct queueInfoEnt *getQueueData(char *name)
{
    int i;
...
static char searchAll(char *word)
{
    char *sp, *cp;
...
static void initUserInfo(struct userInfoEnt *up)
{
    if (up != NULL) {
        up->user = NULL;
...
static void freeUserInfo(struct userInfoEnt *up)
{
    if (up != NULL)
        FREEUP(up->user);
}

static void initGroupInfo(struct groupInfoEnt *gp)
{
    if (gp != NULL) {
        gp->group = NULL;
        gp->memberList = NULL;
...
static void freeGroupInfo(struct groupInfoEnt *gp)
{
    if (gp != NULL) {
        FREEUP(gp->group);
...
static char addUser(char *username, int maxjobs, float pJobLimit, char *fname,
                    int override)
{
    ...
    if ((up = getUserData(username)) != NULL) {
        if (override) {
            ...
            if (fname) {
                ls_syslog(LOG_ERR,
                          "addUser: %s: User <%s> is multiply defined; "
                          "retaining old definition",
                          fname, username);
                lsberrno = LSBE_CONF_WARNING;
            }
            return false;
        ...
        if (numofusers == usersize) {
            ...
            if ((tmpUsers = (struct userInfoEnt **) myrealloc(
                ...
                ls_syslog(LOG_ERR, NULL, "addUser", "myrealloc",
...
static char isInGrp(char *word, struct groupInfoEnt *gp, int grouptype,
                    int checkAll)
{
    char *tmp, *str;
...
static char **expandGrp(char *word, int *num, int grouptype)
{
    char *tmp, *str;
    ...
    if (gp == NULL) {
        if ((list = (char **) malloc(sizeof(char *))) == NULL) {
            ls_syslog(LOG_ERR, NULL, "expandGrp", "malloc", sizeof(char *));
            lsberrno = LSBE_NO_MEM;
            return NULL;
        }
        if ((list[0] = putstr_(word)) == NULL) {
            ls_syslog(LOG_ERR, NULL, "expandGrp", "malloc", strlen(word) + 1);
            lsberrno = LSBE_NO_MEM;
            return NULL;
        }
        n = 1;
        ...
        if (!strcmp(tmp, "all")) {
            if ((tempList = (char **) myrealloc(list, sizeof(char *))) ==
                NULL) {
                ls_syslog(LOG_ERR, NULL, "expandGrp", "myrealloc",
                          sizeof(char *));
                lsberrno = LSBE_NO_MEM;
                return NULL;
            }
            ...
            if ((list[0] = putstr_("all")) == NULL) {
                ls_syslog(LOG_ERR, NULL, "expandGrp", "malloc",
                          strlen("all") + 1);
                lsberrno = LSBE_NO_MEM;
                return NULL;
            }
            ...
            if (sub_gp == NULL) {
                ...
                if (sub_num) {
                    ...
                    if ((list = (char **) myrealloc(
                        ...
                        ls_syslog(LOG_ERR, NULL, "expandGrp", "realloc",
...
struct hostConf *lsb_readhost(struct lsConf *conf, struct lsInfo *info,
                              int options, struct clusterConf *clusterConf)
{
    ...
    for (;;) {
        if ((cp = getBeginLine_conf(conf, &lineNum)) == NULL) {
            if (!hostok) {
                ls_syslog(LOG_ERR, "%s: Host section missing or invalid.",
                          pname);
                lsberrno = LSBE_CONF_WARNING;
            }
        ...
        if (!section) {
            ...
            if (strcasecmp(section, "host") == 0) {
                ...
                if (do_Groups(hostgroups, conf, fname, &lineNum, &numofhgroups,
                              options))
                    hgroupok = true;
                else if (lsberrno == LSBE_NO_MEM) {
                    lsberrno = LSBE_CONF_FATAL;
...
static char do_Hosts(struct lsConf *conf, char *fname, int *lineNum,
...
#define HKEY_DISPATCH_WINDOW info->numIndx + 5
    ...
    if (isSectionEnd(linep, fname, lineNum, "host")) {
        ls_syslog(LOG_WARNING, NULL, pname, fname, *lineNum, "host");
        lsberrno = LSBE_CONF_WARNING;
        return false;
    }

    if (strchr(linep, '=') != NULL) {
        ls_syslog(LOG_ERR, NULL, pname, fname, *lineNum, "host");
        lsberrno = LSBE_CONF_WARNING;
    ...
    if (!keyMatch(keylist, linep, false)) {
        ls_syslog(LOG_ERR,
                  "%s: File %s at line %d: Keyword line format error for Host "
                  "section; ignoring section",
                  pname, fname, *lineNum);
        lsberrno = LSBE_CONF_WARNING;
        doSkipSection_conf(conf, lineNum, fname, "host");
    ...
    if (keylist[HKEY_HNAME].position < 0) {
        ls_syslog(LOG_ERR,
                  "%s: File %s at line %d: Hostname required for Host section; "
                  "ignoring section",
                  pname, fname, *lineNum);
        lsberrno = LSBE_CONF_WARNING;
        doSkipSection_conf(conf, lineNum, fname, "host");
    ...
    if ((nonOverridableHosts = (struct hTab *) malloc(sizeof(struct hTab))) ==
            NULL ||
    ...
    while ((linep = getNextLineC_conf(conf, lineNum, true)) != NULL) {
        ...
        if (strcmp(keylist[HKEY_HNAME].val, "default") != 0) {
            ...
            if (hp.name[0] == 0 && options != CONF_NO_CHECK) {
                for (i = 0; i < info->nModels; i++) {
                    if (strcmp(keylist[HKEY_HNAME].val, info->hostModels[i]) ==
                        0) {
                        break;
                    }
                }
                if (i == info->nModels) {
                    ...
                    if (i == info->nTypes) {
                        ls_syslog(LOG_ERR,
                                  "%s: File %s at line %d: Invalid "
                                  "host/type/model name <%s>; ignoring line",
                                  pname, fname, *lineNum,
                                  keylist[HKEY_HNAME].val);
                        lsberrno = LSBE_CONF_WARNING;
                        continue;
                    }
                }
                ...
                for (i = 0; i < cConf.numHosts; i++) {
                    ...
                    if ((strcmp(hostInfo->hostType, keylist[HKEY_HNAME].val) ==
                             0 ||
                         strcmp(hostInfo->hostModel, keylist[HKEY_HNAME].val) ==
                             0) &&
                ...
                if (!numSelectedHosts) {
                    ls_syslog(LOG_ERR,
                              "%s: File %s at line %d: no server hosts of "
                              "type/model <%s> are known by LSF; ignoring line",
                              pname, fname, *lineNum, keylist[HKEY_HNAME].val);
                    lsberrno = LSBE_CONF_WARNING;
                    continue;
                }
        ...
        if (!new) {
            ls_syslog(LOG_ERR,
                      "%s: File %s at line %d: Host name <%s> multiply "
                      "specified; ignoring line",
                      pname, fname, *lineNum, keylist[HKEY_HNAME].val);
            lsberrno = LSBE_CONF_WARNING;
            continue;
        ...
        if (keylist[HKEY_MXJ].val != NULL &&
            ...
            if ((host.maxJobs = my_atoi(keylist[HKEY_MXJ].val, INFINIT_INT,
                                        -1)) == INFINIT_INT) {
                ls_syslog(LOG_ERR,
                          "%s: File %s at line %d: Invalid value <%s> for key "
                          "<%s>; %d is assumed",
                          pname, fname, *lineNum, keylist[HKEY_MXJ].val,
                          keylist[HKEY_MXJ].key, INFINIT_INT);
                lsberrno = LSBE_CONF_WARNING;
            }
        }
        ...
        if (keylist[HKEY_RUN_WINDOW].position >= 0) {
            if (strcmp(keylist[HKEY_RUN_WINDOW].val, "")) {
                host.windows = parsewindow(keylist[HKEY_RUN_WINDOW].val, fname,
                                           lineNum, "Host");
        ...
        if (keylist[HKEY_MIG].position >= 0 && keylist[HKEY_MIG].val != NULL &&
            strcmp(keylist[HKEY_MIG].val, "")) {
            if ((host.mig = my_atoi(keylist[HKEY_MIG].val, INFINIT_INT / 60,
                                    -1)) == INFINIT_INT) {
                ls_syslog(LOG_ERR,
                          "%s: File %s at line %d: Invalid value <%s> for key "
                          "<%s>; no MIG threshold is assumed",
                          pname, fname, *lineNum, keylist[HKEY_MIG].val,
                          keylist[HKEY_MIG].key);
                lsberrno = LSBE_CONF_WARNING;
            }
        }
        ...
        getThresh(info, keylist, host.loadSched, host.loadStop, fname, lineNum,
                  " in section Host ending");
        ...
        if (options == CONF_NO_CHECK) {
            ...
            if (i == cConf.numHosts) {
                num = 0;
                ls_syslog(LOG_ERR,
                          "%s: File %s at line %d: Can't find the information "
                          "of host <%s>",
                          pname, fname, *lineNum, hostname);
                lsberrno = LSBE_CONF_WARNING;
                freeHostInfo(&host);
            } else if (cConf.hosts[i].isServer != true) {
                num = 0;
                ls_syslog(LOG_ERR,
                          "%s: File %s at line %d: Host <%s> is not a "
                          "server;ignoring",
                          pname, fname, *lineNum, hostname);
                lsberrno = LSBE_CONF_WARNING;
                freeHostInfo(&host);
...
static void initHostInfo(struct hostInfoEnt *hp)
{
    if (hp != NULL) {
        hp->host = NULL;
        hp->loadSched = NULL;
...
static void freeHostInfo(struct hostInfoEnt *hp)
{
    if (hp != NULL) {
        FREEUP(hp->host);
        FREEUP(hp->loadSched);
...
static void getThresh(struct lsInfo *info, struct keymap *keylist,
                      float loadSched[], float loadStop[], char *fname,
                      int *lineNum, char *section)
{
    ...
    for (i = 0; i < info->numIndx; i++) {
        ...
        if (*keylist[i].val != '\0' &&
            (loadSched[i] = my_atof(keylist[i].val, INFINITY, -INFINITY)) >=
                INFINITY) {
            ls_syslog(LOG_ERR,
                      "%s: File %s%s at line %d: Value <%s> of loadSched <%s> "
                      "isn't a float number between %1.1f and %1.1f; ignored",
                      pname, fname, section, *lineNum, keylist[i].val,
                      keylist[i].key, -INFINITY, INFINITY);
            lsberrno = LSBE_CONF_WARNING;
            if (info->resTable[i].orderType == DECR)
                loadSched[i] = -INFINITY;
        }
        if (*keylist[i].val != '\0' && loadSched[i] < 0 &&
            loadSched[i] > -INFINITY) {
            ls_syslog(LOG_WARNING,
                      "%s: File %s%s at line %d: Warning: Value <%s> of "
                      "loadSched <%s> is not a non-negative number",
                      pname, fname, section, *lineNum, keylist[i].val,
                      keylist[i].key);
            lsberrno = LSBE_CONF_WARNING;
        }
        ...
        if (i == UT) {
            if (loadSched[i] > 1.0 && loadSched[i] < INFINITY) {
                ls_syslog(LOG_INFO,
                          ("%s: File %s %s at line %d: For load index <%s>, "
                           "loadSched <%2.2f> is greater than 1; assumming "
                           "<%5.1f%%>"),
        ...
        if ((loadStop[i] = my_atof(stop, INFINITY, -INFINITY)) == INFINITY) {
            ls_syslog(LOG_ERR,
                      "%s: File %s%s at line %d: Value <%s> of loadStop <%s> "
                      "isn't a float number between %1.1f and %1.1f; ignored",
        ...
        if (loadStop[i] < 0 && loadStop[i] > -INFINITY) {
            ls_syslog(LOG_WARNING,
                      "%s: File %s%s at line %d: Warning: Value <%s> of "
                      "loadStop <%s> is not a non-negative number",
                      pname, fname, section, *lineNum, stop, keylist[i].key);
            lsberrno = LSBE_CONF_WARNING;
        }
        ...
        if (i == UT) {
            if (loadStop[i] > 1.0 && loadSched[i] < INFINITY) {
                ls_syslog(
                    LOG_INFO,
                    ("%s: File %s%s at line %d: For load index <%s>, loadStop "
                     "<%2.2f> is greater than 1; assumming <%5.1f%%>"),
                    pname, fname, section, *lineNum, keylist[i].key,
        ...
        if ((loadSched[i] > loadStop[i]) &&
            ...
            ls_syslog(LOG_ERR,
                      "%s: File %s%s at line %d: For load index <%s>, loadStop "
                      "<%2.2f> is lower than loadSched <%2.2f>; swapped",
                      pname, fname, section, *lineNum, keylist[i].key,
        ...
        if ((loadStop[i] > loadSched[i]) &&
            ...
            ls_syslog(LOG_ERR,
                      "%s: File %s%s at line %d: For load index <%s>, loadStop "
                      "<%2.2f> is higher than loadSched <%2.2f>; swapped",
                      pname, fname, section, *lineNum, keylist[i].key,
                      loadStop[i], loadSched[i]);
            lsberrno = LSBE_CONF_WARNING;
            swap = loadSched[i];
...
static int addHost(struct hostInfoEnt *hp, struct hostInfo *hostInfo,
                   int override)
{
    struct hostInfoEnt *host;
    ...
    if ((hp->host == NULL &&
         (host = getHostData(hostInfo->hostName)) != NULL) ||
    ...
    if (copyHostInfo(host, hp) < 0) {
        ls_syslog(LOG_ERR, NULL, "addHost", "copyHostInfo");
        FREEUP(host);
        return false;
    }
...
static int copyHostInfo(struct hostInfoEnt *toHost,
                        struct hostInfoEnt *fromHost)
{
    int i;
    ...
    if (fromHost->nIdx) {
        if ((toHost->loadSched = (float *) malloc((fromHost->nIdx) *
                                                  sizeof(float *))) == NULL ||
            (toHost->loadStop = (float *) malloc((fromHost->nIdx) *
                                                 sizeof(float *))) == NULL ||
            (fromHost->windows &&
...
static void initThresholds(struct lsInfo *info, float loadSched[],
                           float loadStop[])
{
    int i;
...
static char *parseGroups(char *linep, char *fname, int *lineNum, char *section,
...
#define failReturn(mygp, size)                                                 \
    ...
    if (groupType == USER_GRP && numofugroups >= MAX_GROUPS) {
        ls_syslog(LOG_ERR,
                  "%s: File %s%s at line %d: number of %s group <%d> is equal "
                  "to or greater than MAX_GROUPS <%d>; ignored the group <%s>",
                  pname, fname, section, *lineNum,
                  (groupType == USER_GRP) ? "user" : "host",
                  (groupType == USER_GRP) ? numofugroups : numofhgroups,
                  MAX_GROUPS, linep);
        return NULL;
    }

    if ((mygp = (struct groupInfoEnt *) malloc(sizeof(struct groupInfoEnt))) ==
        NULL) {
    ...
    while ((word = getNextWord_(&linep)) != NULL) {
        ...
        if (returnVal) {
            ls_syslog(LOG_ERR,
                      "%s: File %s%s at line %d: %s group <%s> is multiply "
                      "defined; ignored",
                      pname, fname, section, *lineNum, groupName, myWord);
            lsberrno = LSBE_CONF_WARNING;
            FREEUP(myWord);
            continue;
        }
        ...
        if (groupType == USER_GRP) {
            if (options == CONF_NO_CHECK) {
                if (!addMember(mygp, myWord, USER_GRP, fname, *lineNum, section,
                               options, checkAll) &&
                    lsberrno == LSBE_NO_MEM) {
            ...
            if (unixGrp != NULL) {
                if (options & CONF_EXPAND) {
                    ...
                    if (gp == NULL) {
                        ls_syslog(LOG_WARNING,
                                  "%s: File %s at line %d: No valid users "
                                  "defined in Unix group <%s>; ignoring",
                                  pname, fname, *lineNum, myWord);
                        lsberrno = LSBE_CONF_WARNING;
                        FREEUP(myWord);
                        continue;
                ...
                ls_syslog(
                    LOG_WARNING,
                    "%s: File %s%s at line %d: Unknown user or user group "
                    "<%s>; Maybe a windows user or of another domain",
            ...
            if (groupType == HOST_GRP) {
                ...
                if (length > 2 && myWord[length - 1] >= '0' &&
                    myWord[length - 1] <= '9' && myWord[length - 2] == '+') {
                    if (myWord[length - 3] == '+') {
                        ls_syslog(
                            LOG_ERR,
                            "%s: File %s%s at line %d: Host <%s> is specified "
                            "with bad host preference expression; ignored",
                            pname, fname, section, *lineNum, hostName);
                        FREEUP(myWord);
                        continue;
                    }
                    myWord[length - 2] = '\0';
                    ...
                    while (*cp != '\0') {
                        if (*cp == '+' && !number) {
                            ...
                            ls_syslog(LOG_ERR,
                                      "%s: File %s%s at line %d: Host <%s> is "
                                      "specified with bad host preference "
                                      "expression; ignored",
                                      pname, fname, section, *lineNum,
                                      hostName);
                            FREEUP(myWord);
                    ...
                    *sp = '\0';
                    ...
                    if (parseQFirstHost(myWord, &haveFirst, pname, lineNum,
                                        fname, section)) {
                        continue;
                    }
                }
                ...
                if (!strcmp(myWord, "others")) {
                    if (hasAllOthers == true) {
                        ls_syslog(LOG_ERR,
                                  "%s: File %s%s at line %d: More than one "
                                  "<others> or host group with <all> as its "
                                  "member specified; <%s> is ignored",
                                  pname, fname, section, *lineNum, hostName);
                        FREEUP(myWord);
                        continue;
                    }

                    if (putIntoList(&hostGroup, &len, hostName, NULL) == NULL) {
                        failReturn(mygp, strlen(myWord) + 1);
                    }
                    hasAllOthers = true;
                    FREEUP(myWord);
            ...
            if (isHostName(myWord) == false) {
                ...
                if (groupType == HOST_GRP && strcmp(myWord, "all") != 0 &&
                    ...
                    ls_syslog(LOG_ERR,
                              "%s: File %s%s at line %d: More than one host "
                              "group with <all> as its members or <others> "
                              "specified; <%s> is ignored",
                              pname, fname, section, *lineNum, hostName);
                    lsberrno = LSBE_CONF_WARNING;
                    FREEUP(myWord);
                    continue;
                ...
                if (hp.name[0] == 0) {
                    ls_syslog(LOG_ERR,
                              "%s: File %s%s at line %d: Host name <%s> cannot "
                              "be found; ignored",
                              pname, fname, section, *lineNum, myWord);
                    lsberrno = LSBE_CONF_WARNING;
                    FREEUP(myWord);
                ...
                if (getHostData(myWord) == NULL && numofhosts != 0) {
                    ls_syslog(LOG_ERR,
                              "%s: File %s%s at line %d: Host <%s> is not used "
                              "by the batch system; ignored",
                              pname, fname, section, *lineNum, myWord);
                    lsberrno = LSBE_CONF_WARNING;
                    FREEUP(myWord);
                    continue;
                ...
                if (returnVal) {
                    ls_syslog(LOG_ERR,
                              "%s: File %s%s at line %d: Host name <%s> is "
                              "multiply defined; ignored",
                              pname, fname, section, *lineNum, myWord);
                    lsberrno = LSBE_CONF_WARNING;
                    FREEUP(myWord);
                    continue;
                ...
                if ((options & CONF_NO_EXPAND) == 0) {
                    ...
                    if (strchr(cpWord, '+')) {
                        returnVal =
                            addMember(mygp, myWord, HOST_GRP, fname, *lineNum,
                                      section, options, checkAll);
                    } else {
                        returnVal =
                            addMember(mygp, cpWord, HOST_GRP, fname, *lineNum,
                                      section, options, checkAll);
                    }
                }
                if (!returnVal) {
    ...
    if (groupType != HOST_GRP) {
        ...
        if (hasNone) {
            if (hostGroup[0] != '\000') {
                ls_syslog(LOG_ERR,
                          "%s: file <%s> line <%d> host names <%s> should not "
                          "be specified together with \"none\", ignored",
                          pname, fname, *lineNum, hostGroup);
                lsberrno = LSBE_CONF_WARNING;
            }
...
struct queueConf *lsb_readqueue(struct lsConf *conf, struct lsInfo *info,
                                int options, struct sharedConf *sharedConf)
{
    ...
    if (qConf != NULL) {
        ...
        if ((qConf = (struct queueConf *) malloc(sizeof(struct queueConf))) ==
            NULL) {
            ls_syslog(LOG_ERR, NULL, pname, "malloc", sizeof(struct queueConf));
            lsberrno = LSBE_CONF_FATAL;
            return NULL;
        }
        qConf->numQueues = 0;
    ...
    for (;;) {
        if ((cp = getBeginLine_conf(conf, &lineNum)) == NULL) {
            if (!queueok) {
                ls_syslog(LOG_ERR, "%s: Queue section missing or invalid.",
                          pname);
...
static char do_Queues(struct lsConf *conf, char *fname, int *lineNum,
                      struct lsInfo *info, int options)
{
    static struct keymap *keylist = NULL;
...
#define KEYMAP_SIZE info->numIndx + 47
    ...
    if (isSectionEnd(linep, fname, lineNum, "Queue")) {
        ls_syslog(LOG_WARNING, NULL, pname, fname, *lineNum, "queue");
        lsberrno = LSBE_CONF_WARNING;
        return false;
    }
    ...
    if (strchr(linep, '=') == NULL) {
        ls_syslog(LOG_ERR,
                  "%s: File %s at line %d: Vertical Queue section not "
                  "implented yet; use horizontal format; ignoring section",
                  pname, fname, *lineNum);
        lsberrno = LSBE_CONF_WARNING;
        doSkipSection_conf(conf, lineNum, fname, "Queue");
        return false;
    } else {
        retval = readHvalues_conf(keylist, linep, conf, fname, lineNum, false,
                                  "Queue");
        if (retval < 0) {
            if (retval == -2) {
                lsberrno = LSBE_CONF_WARNING;
                ls_syslog(LOG_ERR,
                          "%s: Parameter error in %s(%d); remaining parameters "
                          "in this section will be either ignored or set to "
                          "default values.",
                          pname, fname, *lineNum);
            } else {
                ls_syslog(LOG_ERR,
                          "%s: File %s at line %d: Incorrect section; ignoring "
                          "this Queue section",
                          pname, fname, *lineNum);
                lsberrno = LSBE_CONF_WARNING;
                freekeyval(keylist);
        ...
        if (keylist[QKEY_NAME].val == NULL) {
            ls_syslog(LOG_ERR,
                      "%s: File %s in section Queue ending at line %d: Queue "
                      "name is not given; ignoring section",
                      pname, fname, *lineNum);
            lsberrno = LSBE_CONF_WARNING;
            freekeyval(keylist);
            return false;
        ...
        if (strcmp(keylist[QKEY_NAME].val, "default") == 0) {
            ls_syslog(
                LOG_ERR,
                "%s: File %s in section Queue ending at line %d: Queue name "
                "<%s> is a reserved word; ignoring the queue section",
                pname, fname, *lineNum, keylist[QKEY_NAME].val);
            lsberrno = LSBE_CONF_WARNING;
            freekeyval(keylist);
            return false;
        }
        ...
        if (getQueueData(keylist[QKEY_NAME].val)) {
            ls_syslog(LOG_ERR,
                      "%s: File %s in section Queue ending at line %d: "
                      "Duplicate queue name <%s>; ignoring section",
                      pname, fname, *lineNum, keylist[QKEY_NAME].val);
            lsberrno = LSBE_CONF_WARNING;
            freekeyval(keylist);
            return false;
        ...
        if (keylist[QKEY_PRIORITY].val != NULL &&
            ...
            if ((queue.priority = my_atoi(keylist[QKEY_PRIORITY].val,
                ...
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "Priority value <%s> isn't a positive integer "
                          "between 1 and %d; ignored",
                          pname, fname, *lineNum, keylist[QKEY_PRIORITY].val,
                          INFINIT_INT - 1);
                lsberrno = LSBE_CONF_WARNING;
            }
        } else {
        ...
        if (keylist[QKEY_NICE].val != NULL &&
            ...
            if (my_atoi(keylist[QKEY_NICE].val, INFINIT_SHORT,
                ...
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "Nice value <%s> must be an integer; ignored",
        ...
        if (keylist[QKEY_UJOB_LIMIT].val != NULL &&
            ...
            if ((queue.userJobLimit =
                     my_atoi(keylist[QKEY_UJOB_LIMIT].val, INFINIT_INT, -1)) ==
                INFINIT_INT) {
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "UJOB_LIMIT value <%s> isn't a non-negative integer "
                          "between 0 and %d; ignored",
                          pname, fname, *lineNum, keylist[QKEY_UJOB_LIMIT].val,
                          INFINIT_INT - 1);
                lsberrno = LSBE_CONF_WARNING;
            }
        }
        ...
        if (keylist[QKEY_PJOB_LIMIT].val != NULL &&
            ...
            if ((queue.procJobLimit = my_atof(keylist[QKEY_PJOB_LIMIT].val,
                                              INFINITY, -1.0)) == INFINITY) {
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "PJOB_LIMIT value <%s> isn't a non-negative integer "
                          "between 0 and %f; ignored",
                          pname, fname, *lineNum, keylist[QKEY_PJOB_LIMIT].val,
                          INFINITY - 1);
                lsberrno = LSBE_CONF_WARNING;
            }
        }
        ...
        if (keylist[QKEY_QJOB_LIMIT].val != NULL &&
            ...
            if ((queue.maxJobs = my_atoi(keylist[QKEY_QJOB_LIMIT].val,
                ...
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "QJOB_LIMIT value <%s> isn't a non-negative integer "
                          "between 0 and %d; ignored",
                          pname, fname, *lineNum, keylist[QKEY_QJOB_LIMIT].val,
                          INFINIT_INT - 1);
                lsberrno = LSBE_CONF_WARNING;
            }
        }
        ...
        if (keylist[QKEY_HJOB_LIMIT].val != NULL &&
            ...
            if ((queue.hostJobLimit =
                     my_atoi(keylist[QKEY_HJOB_LIMIT].val, INFINIT_INT, -1)) ==
                INFINIT_INT) {
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "HJOB_LIMIT value <%s> isn't a non-negative integer "
                          "between 0 and %d; ignored",
                          pname, fname, *lineNum, keylist[QKEY_HJOB_LIMIT].val,
                          INFINIT_INT - 1);
                lsberrno = LSBE_CONF_WARNING;
            }
        }
        ...
        if (keylist[QKEY_RUN_WINDOW].val != NULL &&
            ...
            queue.windows = parsewindow(keylist[QKEY_RUN_WINDOW].val, fname,
                                        lineNum, "Queue");
        ...
        if (keylist[QKEY_DEFAULT_HOST_SPEC].val != NULL &&
            ...
            if (options != CONF_NO_CHECK) {
                if ((cpuFactor = getModelFactor(
                    ...
                    ls_syslog(LOG_ERR,
                              "%s: File %s in section Queue ending at line %d: "
                              "Invalid value <%s> for %s; ignored",
                              pname, fname, *lineNum,
                              keylist[QKEY_DEFAULT_HOST_SPEC].val,
                              keylist[QKEY_DEFAULT_HOST_SPEC].key);
                    lsberrno = LSBE_CONF_WARNING;
                }
            }
        ...
        if (parseCpuAndRunLimit(keylist, &queue, fname, lineNum, pname, info,
                                options) == false &&
            lsberrno == LSBE_NO_MEM) {
            freekeyval(keylist);
            freeQueueInfo(&queue);
        ...
        if (keylist[QKEY_FILELIMIT].val != NULL &&
            ...
            if ((queue.rLimits[LSF_RLIMIT_FSIZE] =
                     my_atoi(keylist[QKEY_FILELIMIT].val, INFINIT_INT, 0)) ==
                INFINIT_INT) {
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "FILELIMIT value <%s> isn't a positive integer "
                          "between 0 and %d; ignored",
                          pname, fname, *lineNum, keylist[QKEY_FILELIMIT].val,
                          INFINIT_INT);
                lsberrno = LSBE_CONF_WARNING;
            }
        }
        ...
        if (keylist[QKEY_DATALIMIT].val != NULL &&
            ...
            parseDefAndMaxLimits(
                keylist[QKEY_DATALIMIT], &queue.defLimits[LSF_RLIMIT_DATA],
                &queue.rLimits[LSF_RLIMIT_DATA], fname, lineNum, pname);
        }

        if (keylist[QKEY_STACKLIMIT].val != NULL &&
            strcmp(keylist[QKEY_STACKLIMIT].val, "")) {
            if ((queue.rLimits[LSF_RLIMIT_STACK] =
                     my_atoi(keylist[QKEY_STACKLIMIT].val, INFINIT_INT, 0)) ==
                INFINIT_INT) {
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "STACKLIMIT value <%s> isn't a positive integer "
                          "between 0 and %d; ignored",
                          pname, fname, *lineNum, keylist[QKEY_STACKLIMIT].val,
                          INFINIT_INT);
                lsberrno = LSBE_CONF_WARNING;
            }
        }
        ...
        if (keylist[QKEY_CORELIMIT].val != NULL &&
            ...
            if ((queue.rLimits[LSF_RLIMIT_CORE] =
                     my_atoi(keylist[QKEY_CORELIMIT].val, INFINIT_INT, -1)) ==
                INFINIT_INT) {
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "CORELIMIT value <%s> isn't a non-negative integer "
                          "between -1 and %d; ignored",
                          pname, fname, *lineNum, keylist[QKEY_CORELIMIT].val,
                          INFINIT_INT);
                lsberrno = LSBE_CONF_WARNING;
            }
        }
        ...
        if (keylist[QKEY_MEMLIMIT].val != NULL &&
            ...
            parseDefAndMaxLimits(
                keylist[QKEY_MEMLIMIT], &queue.defLimits[LSF_RLIMIT_RSS],
                &queue.rLimits[LSF_RLIMIT_RSS], fname, lineNum, pname);
        }

        if (keylist[QKEY_SWAPLIMIT].val != NULL &&
            strcmp(keylist[QKEY_SWAPLIMIT].val, "")) {
            if ((queue.rLimits[LSF_RLIMIT_SWAP] =
                     my_atoi(keylist[QKEY_SWAPLIMIT].val, INFINIT_INT, 0)) ==
                INFINIT_INT) {
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "SWAPLIMIT value <%s> isn't a positive integer "
                          "between 0 and %d; ignored",
        ...
        if (keylist[QKEY_PROCESSLIMIT].val != NULL &&
            ...
            parseDefAndMaxLimits(keylist[QKEY_PROCESSLIMIT],
                                 ...
                                 &queue.rLimits[LSF_RLIMIT_PROCESS], fname,
                                 lineNum, pname);
        }

        if (keylist[QKEY_PROCLIMIT].val != NULL &&
            strcmp(keylist[QKEY_PROCLIMIT].val, "")) {
            if (parseProcLimit(keylist[QKEY_PROCLIMIT].val, &queue, fname,
                               lineNum, pname) == false)
                lsberrno = LSBE_CONF_WARNING;
        }
        ...
        if (keylist[QKEY_USERS].val != NULL &&
            ...
            queue.userList =
                parseGroups(keylist[QKEY_USERS].val, fname, lineNum,
                            " in section  Queue ending", USER_GRP, options);
            if (queue.userList == NULL) {
                if (lsberrno == LSBE_NO_MEM)
                    ls_syslog(LOG_ERR, NULL, pname, "parseGroups");
                else if (numofugroups >= MAX_GROUPS)
                    ls_syslog(LOG_ERR,
                              "%s: File %s in section Queue ending at line %d: "
                              " Number of user group <%d> is equal to or "
                              "greater than MAX_GROUPS <%d>; ignoring the "
                              "queue for <%s>; ignoring the queue",
                              pname, fname, *lineNum, numofugroups, MAX_GROUPS,
                              queue.queue);
                else
                    ls_syslog(LOG_ERR,
                              "%s: File %s in section Queue ending at line %d: "
                              "No valid user or user group specified in USERS "
                              "for <%s>; ignoring the queue",
                              pname, fname, *lineNum, queue.queue);
                freekeyval(keylist);
                freeQueueInfo(&queue);
                return false;
            }
        ...
        if (keylist[QKEY_HOSTS].val != NULL) {
            {
                ...
                while (subString != NULL) {
                    if (strcmp(keylist[QKEY_HOSTS].val, "none") == 0) {
                        ls_syslog(LOG_ERR,
                                  "%s: File %s in section Queue at line %d: "
                                  "\"none\" specified , queue ignored",
                                  pname, fname, *lineNum);
                        lsberrno = LSBE_CONF_WARNING;
                        freekeyval(keylist);
                        freeQueueInfo(&queue);
                ...
                if (strchr(keylist[QKEY_HOSTS].val, '~') != NULL &&
                    ...
                    ls_syslog(LOG_DEBUG,
                              ...
                              "the string is \'%s\'",
                              keylist[QKEY_HOSTS].val);
                    numHosts = resolveBatchNegHosts(keylist[QKEY_HOSTS].val,
                                                    &outHosts, true);
                    if (numHosts > 0) {
                        ls_syslog(LOG_DEBUG,
                                  "resolveBatchNegHosts: for do_Queues "
                        ...
                        if (numHosts == -3) {
                            ls_syslog(LOG_WARNING,
                                      "%s: \'%s\' The result is that all the "
                                      "hosts are to be excluded.",
                                      pname, keylist[QKEY_HOSTS].val);
                        }
                        ls_syslog(LOG_ERR,
                                  "%s: File %s in section Queue ending at line "
                                  "%d: No valid hosts or host group specified "
                                  "in HOSTS for <%s>; ignoring the queue",
                                  pname, fname, *lineNum, queue.queue);
                ...
                queue.hostList =
                    parseGroups(keylist[QKEY_HOSTS].val, fname, lineNum,
                                " in section  Queue ending", HOST_GRP, options);
        ...
        if (keylist[QKEY_CHKPNT].val != NULL &&
            ...
            if (strlen(keylist[QKEY_CHKPNT].val) >= MAXLINELEN) {
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "CHKPNT of the queue <%s> is too long <%s>; ignoring",
                          pname, fname, *lineNum, queue.queue,
                          keylist[QKEY_CHKPNT].val);
                lsberrno = LSBE_CONF_WARNING;
            } else {
                int chkpntPrd = 0;
                ...
                if (chkpntPrd < 0) {
                    ls_syslog(LOG_ERR,
                              "%s: File %s in section Queue ending at line %d: "
                              " options for CHKPNT of the queue <%s> is "
                              "invalid ; ignoring",
                              pname, fname, *lineNum, queue.queue);
                    ls_syslog(LOG_ERR, "%s: invalid checkpoint period", pname);
                    lsberrno = LSBE_CONF_WARNING;
                    freekeyval(keylist);
                    freeQueueInfo(&queue);
                    return false;
        ...
        if (keylist[QKEY_RERUNNABLE].val != NULL) {
            if (strcasecmp(keylist[QKEY_RERUNNABLE].val, "y") == 0 ||
                ...
                if (strcasecmp(keylist[QKEY_RERUNNABLE].val, "n") != 0 &&
                    ...
                    ls_syslog(LOG_ERR,
                              "%s: File %s in section Queue ending at line %d: "
                              " options for RERUNNABLE of the queue <%s> is "
                              "not y|yes|n|no; ignoring",
                              pname, fname, *lineNum, queue.queue,
                              keylist[QKEY_RERUNNABLE].val);
                    lsberrno = LSBE_CONF_WARNING;
                }
                queue.qAttrib &= ~Q_ATTRIB_RERUNNABLE;
            }
        ...
        addBinaryAttributes(fname, lineNum, &queue, &keylist[QKEY_EXCLUSIVE],
                            Q_ATTRIB_EXCLUSIVE, "EXCLUSIVE");

        addBinaryAttributes(fname, lineNum, &queue, &keylist[QKEY_BACKFILL],
        ...
        addBinaryAttributes(
            fname, lineNum, &queue, &keylist[QKEY_ENQUE_INTERACTIVE_AHEAD],
            Q_ATTRIB_ENQUE_INTERACTIVE_AHEAD, "ENQUE_INTERACTIVE_AHEAD");
        ...
        if (keylist[QKEY_INTERACTIVE].val != NULL) {
            ...
            } else if ((strcasecmp(keylist[QKEY_INTERACTIVE].val, "yes") !=
                        0) &&
                       (strcasecmp(keylist[QKEY_INTERACTIVE].val, "y") != 0)) {
                ls_syslog(LOG_ERR, I18N(5311, "\
                            %s: File %s in section Queue ending at line %d: INTERACTIVE value <%s> isn't one of 'Y', 'y', 'N', 'n' or 'ONLY'; ignored"),
                          pname, fname, *lineNum,
        ...
        if (keylist[QKEY_JOB_ACCEPT_INTERVAL].val != NULL &&
            ...
            if ((queue.acceptIntvl =
                ...
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "JOB_ACCEPT_INTERVAL value <%s> isn't an integer "
                          "greater than -1; ignored",
                          pname, fname, *lineNum,
                          keylist[QKEY_JOB_ACCEPT_INTERVAL].val);
                lsberrno = LSBE_CONF_WARNING;
            }
        }
        ...
        if (keylist[QKEY_NEW_JOB_SCHED_DELAY].val != NULL &&
            ...
            if ((queue.schedDelay =
                ...
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "NEW_JOB_SCHED_DELAY value <%s> isn't an integer "
                          "greater than -1; ignored",
                          pname, fname, *lineNum,
                          keylist[QKEY_NEW_JOB_SCHED_DELAY].val);
                lsberrno = LSBE_CONF_WARNING;
            }
        }
        ...
        if (keylist[QKEY_POLICIES].val != NULL) {
            ...
            while ((word = getNextWord_(&sp)) != NULL) {
                if (strcasecmp(word, "EXCLUSIVE") == 0)
                    ...
                    ls_syslog(LOG_ERR,
                              "%s: File %s in section Queue ending at line %d: "
                              "POLICIES value <%s> unrecognizable; ignored",
                              pname, fname, *lineNum, word);
                    lsberrno = LSBE_CONF_WARNING;
                }
            }
        }
        ...
        if (keylist[QKEY_DESCRIPTION].val != NULL) {
            if (strlen(keylist[QKEY_DESCRIPTION].val) > 10 * MAXLINELEN) {
                ls_syslog(
                    LOG_ERR,
                    "%s: File %s in section Queue ending at line %d: Too many "
                    "characters in DESCRIPTION of the queue; truncated",
                    pname, fname, *lineNum);
                lsberrno = LSBE_CONF_WARNING;
        ...
        if (keylist[QKEY_MIG].val != NULL &&
            ...
            if ((queue.mig = my_atoi(keylist[QKEY_MIG].val, INFINIT_INT / 60,
                                     -1)) == INFINIT_INT) {
                ls_syslog(
                    LOG_ERR,
                    "%s: File %s in section Queue ending at line %d: Invalid "
                    "value <%s> for MIG; no MIG threshold is assumed",
        ...
        if (keylist[QKEY_ADMINISTRATORS].val != NULL &&
            ...
            if (queue.admins[0] == '\0') {
                ls_syslog(
                    LOG_ERR,
                    "%s: File %s in section Queue ending at line %d: No valid "
                    "administrators <%s> specified for queue <%s>;ignoring",
                    pname, fname, *lineNum, keylist[QKEY_ADMINISTRATORS].val,
                    queue.queue);
                lsberrno = LSBE_CONF_WARNING;
                FREEUP(queue.admins);
            }
        ...
        if (keylist[QKEY_PRE_EXEC].val != NULL &&
            ...
            if (strlen(keylist[QKEY_PRE_EXEC].val) >= MAXLINELEN) {
                ls_syslog(
                    LOG_ERR,
                    "%s: File %s in section Queue ending at line %d: PRE_EXEC "
                    "of the queue <%s> is too long <%s>; ignoring",
                    pname, fname, *lineNum, queue.queue,
                    keylist[QKEY_PRE_EXEC].val);
                lsberrno = LSBE_CONF_WARNING;
            } else {
        ...
        if (keylist[QKEY_POST_EXEC].val != NULL &&
            ...
            if (strlen(keylist[QKEY_POST_EXEC].val) >= MAXLINELEN) {
                ls_syslog(
                    LOG_ERR,
                    "%s: File %s in section Queue ending at line %d: POST_EXEC "
                    "of the queue <%s> is too long <%s>; ignoring",
                    pname, fname, *lineNum, queue.queue,
                    keylist[QKEY_POST_EXEC].val);
                lsberrno = LSBE_CONF_WARNING;
            } else {
        ...
        if (keylist[QKEY_REQUEUE_EXIT_VALUES].val != NULL &&
            ...
            if (strlen(keylist[QKEY_REQUEUE_EXIT_VALUES].val) >= MAXLINELEN) {
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "REQUEUE_EXIT_VALUES  of the queue <%s> is too long "
                          "<%s>; ignoring",
                          pname, fname, *lineNum, queue.queue,
        ...
        if (keylist[QKEY_JOB_CONTROLS].val != NULL &&
            ...
            if (parseSigActCmd(&queue, keylist[QKEY_JOB_CONTROLS].val, fname,
                               lineNum, "in section Queue ending") < 0) {
                lsberrno = LSBE_CONF_WARNING;
                freekeyval(keylist);
                freeQueueInfo(&queue);
                return false;
        ...
        if (keylist[QKEY_TERMINATE_WHEN].val != NULL &&
            ...
            if (terminateWhen(&queue, keylist[QKEY_TERMINATE_WHEN].val, fname,
                              lineNum, "in section Queue ending") < 0) {
                lsberrno = LSBE_CONF_WARNING;
                freekeyval(keylist);
                freeQueueInfo(&queue);
        ...
        getThresh(info, keylist, queue.loadSched, queue.loadStop, fname,
                  lineNum, " in section Queue ending");
...
static void initQueueInfo(struct queueInfoEnt *qp)
{
    int i;
...
static void freeQueueInfo(struct queueInfoEnt *qp)
{
    if (qp != NULL) {
        FREEUP(qp->queue);
...
char checkRequeEValues(struct queueInfoEnt *qp, char *word, char *fname,
                       int *lineNum)
{
...
#define EXCLUDE_EXIT 1
        ...
        if (isint_(sp) && (exitV = my_atoi(sp, 256, -1)) != INFINIT_INT) {
            ...
            if (found == true) {
                if (fname)
                    ls_syslog(LOG_ERR,
                              "%s: File %s in section Queue ending at line %d: "
                              "requeue exit value <%s> for queue <%s> is "
                              "repeated; ignoring",
                              pname, fname, *lineNum, sp, qp->queue);
                lsberrno = LSBE_CONF_WARNING;
                continue;
            }
            ...
            if (fname)
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "requeue exit values <%s> for queue <%s> is not an "
                          "interger between 0-255; ignored",
                          pname, fname, *lineNum, sp, qp->queue);
            lsberrno = LSBE_CONF_WARNING;
        }
    }
    if (numEValues == 0) {
        if (fname)
            ls_syslog(LOG_ERR,
                      "%s: File %s in section Queue ending at line %d: No "
                      "valid requeue exit values <%s> for queue <%s>; ignoring",
                      pname, fname, *lineNum, cp, qp->queue);
        lsberrno = LSBE_CONF_WARNING;
        return false;
    }
    ...
    if (qp->requeueEValues == NULL) {
        if (fname)
            ls_syslog(LOG_ERR, NULL, pname, "malloc", strlen(exitValues) + 1);
        lsberrno = LSBE_NO_MEM;
        return false;
    }
    return true;
...
static char *a_getNextWord_(char **line)
{
    char *wordp, *word;
...
static char addQueue(struct queueInfoEnt *qp, char *fname, int lineNum)
{
    ...
    if ((queues[numofqueues] = (struct queueInfoEnt *) malloc(
        ...
        ls_syslog(LOG_ERR, NULL, pname, "malloc", sizeof(struct queueInfoEnt));
        lsberrno = LSBE_NO_MEM;
        freeQueueInfo(qp);
        return false;
    }
...
void freeUConf(struct userConf *uConf, int freeAll)
{
    int i;
...
void freeHConf(struct hostConf *hConf, int freeAll)
{
    int i;
...
void freeQConf(struct queueConf *qConf, int freeAll)
{
    int i;
...
static void resetUConf(struct userConf *uConf)
{
    uConf->numUgroups = 0;
    uConf->numUsers = 0;
    uConf->ugroups = NULL;
...
static void checkCpuLimit(char **hostSpec, float **cpuFactor, int useSysDefault,
        ...
        if ((*cpuFactor = getModelFactor(*hostSpec, info)) == NULL) {
            if ((*cpuFactor = getHostFactor(*hostSpec)) == NULL) {
                if (useSysDefault == true) {
                    ls_syslog(LOG_ERR,
                              "%s: File %s in section Queue end at line %d: "
                              "Invalid DEFAULT_HOST_SPEC <%s>; ignored",
                              pname, fname, *lineNum,
                              pConf->param->defaultHostSpec);
                    ...
                    ls_syslog(LOG_ERR,
                              "%s: File %s in section Queue end at line %d: "
                              "Invalid host_spec <%s>; ignored",
                              pname, fname, *lineNum, *hostSpec);
                    lsberrno = LSBE_CONF_WARNING;
                    FREEUP(*hostSpec);
                }
            }
...
static int parseCpuAndRunLimit(struct keymap *keylist, struct queueInfoEnt *qp,
                               char *fname, int *lineNum, char *pname,
                               struct lsInfo *info, int options)
{
    struct keymap key;
    ...
    if (maxLimit != NULL) {
        retValue = parseLimitAndSpec(defaultLimit, &limit, &spec, hostSpec,
                                     key.key, qp, fname, lineNum, pname);
    ...
    retValue = parseLimitAndSpec(maxLimit, &limit, &spec, hostSpec, key.key, qp,
                                 fname, lineNum, pname);
    if (retValue == 0) {
        if (limit >= 0)
    ...
    if (sp && strlen(sp) != 0) {
        ...
        ls_syslog(
            LOG_ERR,
            "%s: File %s in section Queue ending at line %d: CPULIMIT for "
            "queue has extra parameters: %s; These parameters will be ignored.",
            pname, fname, *lineNum, sp);
        lsberrno = LSBE_CONF_WARNING;
    }
    ...
    if (sp && strlen(sp) != 0) {
        ...
        ls_syslog(
            LOG_ERR,
            "%s: File %s in section Queue ending at line %d: RUNLIMIT for "
            "queue has extra parameters: %s; These parameters will be ignored.",
            pname, fname, *lineNum, sp);
        lsberrno = LSBE_CONF_WARNING;
    }
    ...
    if (qp->defLimits[LSF_RLIMIT_CPU] != INFINIT_INT &&
        ...
        ls_syslog(
            LOG_ERR,
            "%s: File %s in section Queue at line %d: The default CPULIMIT %d "
            "should not be greater than the max CPULIMIT %d; ignoring the "
            "default CPULIMIT and using max CPULIMIT also as default CPULIMIT",
            pname, fname, *lineNum, qp->defLimits[LSF_RLIMIT_CPU],
            qp->rLimits[LSF_RLIMIT_CPU]);
        qp->defLimits[LSF_RLIMIT_CPU] = qp->rLimits[LSF_RLIMIT_CPU];
    }
    ...
    if (qp->defLimits[LSF_RLIMIT_RUN] != INFINIT_INT &&
        ...
        ls_syslog(
            LOG_ERR,
            "%s: File %s in section Queue at line %d: The default RUNLIMIT %d "
            "should not be greater than the max RUNLIMIT %d; ignoring the "
            "default RUNLIMIT and using max RUNLIMIT also as default RUNLIMIT",
            pname, fname, *lineNum, qp->defLimits[LSF_RLIMIT_RUN],
            qp->rLimits[LSF_RLIMIT_RUN]);
        qp->defLimits[LSF_RLIMIT_RUN] = qp->rLimits[LSF_RLIMIT_RUN];
    }

    if (hostSpec != NULL) {
        checkCpuLimit(&hostSpec, &cpuFactor, useSysDefault, fname, lineNum,
                      pname, info, options);
    }
    ...
    if (cpuFactor == NULL && options & CONF_RETURN_HOSTSPEC) {
        if (qp->defaultHostSpec != NULL && qp->defaultHostSpec[0]) {
            ...
            if (hostSpec && hostSpec[0]) {
                checkCpuLimit(&hostSpec, &cpuFactor, useSysDefault, fname,
                              lineNum, pname, info, options);
            }
        }
    }

    if (cpuFactor == NULL && options & CONF_RETURN_HOSTSPEC) {
        if (pConf && pConf->param && pConf->param->defaultHostSpec != NULL &&
    ...
    if (hostSpec == NULL && (options & CONF_RETURN_HOSTSPEC) &&
        (options & CONF_CHECK)) {
    ...
    if (hostSpec && ((qp->hostSpec = putstr_(hostSpec)) == NULL)) {
        ls_syslog(LOG_ERR, NULL, "parseCpuAndRunLimit", "malloc",
                  strlen(hostSpec) + 1);
        lsberrno = LSBE_NO_MEM;
        FREEUP(hostSpec);
        cpuFactor = NULL;
...
static int parseProcLimit(char *word, struct queueInfoEnt *qp, char *fname,
    ...
    if (sp == NULL) {
        ...
        for (i = 0; i < 3; i++) {
            ...
            if ((values[i] = my_atoi(curWord, INFINIT_INT, 0)) == INFINIT_INT) {
                ls_syslog(
                    LOG_ERR,
                    "%s: File %s in section Queue ending at line %d: PROCLIMIT "
                    "value <%s> isn't a positive integer; ignored",
        ...
        if (getNextWord_(&sp) != NULL) {
            ls_syslog(LOG_ERR,
                      "%s: File %s in section Queue ending at line %d: "
                      "PROCLIMIT has too many parameters; ignored. "
                      "PROCLIMIT=[minimum [default]] maximum",
                      pname, fname, *lineNum);
            return false;
        }
        ...
        case 2:
            if (values[0] > values[1]) {
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "PROCLIMIT values <%d %d> are not valid; ignored. "
                          "PROCLIMIT values must satisfy the following "
                          "condition: 1 <= minimum <= maximum",
        ...
        case 3:
            if (!(values[0] <= values[1] && values[1] <= values[2])) {
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "PROCLIMIT value <%d %d %d> is not valid; ignored. "
                          "PROCLIMIT values must satisfy the following "
                          "condition: 1 <= minimum <= default <= maximum",
                          pname, fname, *lineNum, values[0], values[1],
                          values[2]);
                return false;
            } else {
...
static int parseLimitAndSpec(char *word, int *limit, char **spec,
                             char *hostSpec, char *param,
                             struct queueInfoEnt *qp, char *fname, int *lineNum,
                             char *pname)
{
    int limitVal = -1;
    char *sp = NULL;
    ...
    *limit = -1;
    ...
    if (*spec && hostSpec && strcmp(*spec, hostSpec) != 0) {
        ls_syslog(
            LOG_ERR,
            "%s: File %s in section Queue at line %d: host_spec for %s is "
            "multiply defined; ignoring <%s> and retaining last host_spec <%s>",
            pname, fname, *lineNum, param, *spec, hostSpec);
        lsberrno = LSBE_CONF_WARNING;
        FREEUP(*spec);
    }
    ...
    if (limitVal == INFINIT_INT) {
        ls_syslog(LOG_ERR,
                  "%s: File %s in section Queue at line %d: Value <%s> of %s "
                  "isn't a positive integer between 0 and %d; ignored.",
                  pname, fname, *lineNum, word, param, INFINIT_INT / 60);
        lsberrno = LSBE_CONF_WARNING;
        sp = NULL;
    } else
        *limit = limitVal * 60;
    ...
    if (sp != NULL) {
        ...
        if (limitVal == INFINIT_INT) {
            ls_syslog(LOG_ERR,
                      "%s: File %s in section Queue at line %d: Value <%s> of "
                      "%s isn't a positive integer between 0 and %d; ignored.",
                      pname, fname, *lineNum, word, param, INFINIT_INT / 60);
            lsberrno = LSBE_CONF_WARNING;
            *limit = -1;
        } else {
...
static float *getModelFactor(char *hostModel, struct lsInfo *info)
{
...
static float *getHostFactor(char *host)
{
    static float cpuFactor;
    const char *officialName;
    int i;
...
static char *parseAdmins(char *admins, int options, char *fname, int *lineNum)
{
    ...
    if ((expandAds = (char *) malloc(MAXLINELEN)) == NULL) {
        ls_syslog(LOG_ERR, NULL, pname, "malloc", MAXLINELEN);
        lsberrno = LSBE_NO_MEM;
        return NULL;
    }
    expandAds = "";
    ...
    while ((word = getNextWord_(&sp)) != NULL) {
        if (strcmp(word, "all") == 0) {
            ...
            if (options & CONF_EXPAND) {
                ...
                int numMembers, i;
                if ((groupMembers =
                         expandGrp(uGrp->group, &numMembers, USER_GRP)) == NULL)
                    goto Error;
                else {
                    for (i = 0; i < numMembers; i++) {
                        if (putIntoList(&expandAds, &len, groupMembers[i],
                                        NULL) == NULL) {
                            FREEUP(groupMembers);
                            goto Error;
                        }
            ...
            ls_syslog(LOG_WARNING,
                      "%s: File %s at line %d: Unknown user or user group name "
                      "<%s>; Maybe a windows user or of another domain.",
                      pname, fname, *lineNum, word);
...
static char *putIntoList(char **list, int *len, char *string, char *listName)
{
    ...
    if (isInList(*list, string) == true) {
        ls_syslog(LOG_ERR, "%s: %s is repeatedly specified %s; ignoring", fname,
                  string, sp);
        FREEUP(sp);
        return (*list);
    }
    if (length <= strlen(*list) + strlen(string) + 2) {
        ...
        if ((temp = (char *) myrealloc(*list, 2 * length)) == NULL) {
            ls_syslog(LOG_ERR, NULL, fname, "myrealloc", 2 * length);
            FREEUP(*list);
            lsberrno = LSBE_NO_MEM;
            return NULL;
        }
...
static int isInList(char *list, char *string)
{
    char *sp, *word;
...
static int setDefaultHost(struct lsInfo *info)
{
    ...
    for (i = 0; i < cConf.numHosts; i++) {
        ...
        if (addHost(&host, &cConf.hosts[i], override) == false) {
            ls_syslog(LOG_ERR, NULL, "setDefaultHost", "addHost");
            return -1;
        }
    }
    return 0;
...
static int setDefaultUser(void)
{
    ...
    if (numofusers && (uConf->users = (struct userInfoEnt *) malloc(
                           numofusers * sizeof(struct userInfoEnt))) == NULL) {
        ls_syslog(LOG_ERR, NULL, "setDefaultUser", "malloc",
                  numofusers * sizeof(struct userInfoEnt));
        lsberrno = LSBE_CONF_FATAL;
        freeWorkUser(true);
        freeUConf(uConf, false);
...
static int handleUserMem(void)
{
    ...
    if (uConf == NULL) {
        if ((uConf = (struct userConf *) malloc(sizeof(struct userConf))) ==
            NULL) {
            ls_syslog(LOG_ERR, NULL, "handleUserMem", "malloc",
...
static int handleHostMem(void)
{
    ...
    if (hConf == NULL) {
        if ((hConf = (struct hostConf *) malloc(sizeof(struct hostConf))) ==
            NULL) {
            ls_syslog(LOG_ERR, NULL, "handleHostMem", "malloc",
                      sizeof(struct hostConf));
            lsberrno = LSBE_CONF_FATAL;
            return -1;
        }
        resetHConf(hConf);
...
static void freeSA(char **list, int num)
{
    int i;
...
int parseSigActCmd(struct queueInfoEnt *qp, char *linep, char *fname,
                   int *lineNum, char *section)
{
    ...
    while (linep != NULL && linep[0] != '\0') {
        if ((actClass = getNextWord1_(&linep)) == NULL) {
            if (fname)
                ls_syslog(LOG_ERR,
                          "%s:File %s %s at line %d: SUSPEND, RESUME or "
        ...
        if (strcmp(actClass, "SUSPEND") == 0)
            ...
            if (fname)
                ls_syslog(LOG_ERR, "%s:File %s %s at line %d: wrong KEYWORD",
                          pname, fname, section, *lineNum);
            lsberrno = LSBE_CONF_WARNING;
            return -2;
        }
        ...
        if (*linep != '[') {
            if (fname)
                ls_syslog(LOG_ERR, "%s:File %s %s at line %d: '[' is missing",
                          pname, fname, section, *lineNum);
            lsberrno = LSBE_CONF_WARNING;
            return -3;
        }
        ...
        *linep++ = '\0';
        ...
        if (actClassValue == 0) {
            ...
            if (strcmp(sigActCmd, "CHKPNT") == 0) {
                if (fname)
                    ls_syslog(LOG_ERR,
                              "%s: File %s %s at line %d: 'CHKPNT' is not "
                              "valid in RESUME",
                              pname, fname, section, *lineNum);
                return -7;
            } else
...
int terminateWhen(struct queueInfoEnt *qp, char *linep, char *fname,
                  int *lineNum, char *section)
{
    static char pname[] = "parseSigActCmd";
        ...
        if ((sigName = getNextWord_(&linep)) != NULL) {
            if (strcmp(sigName, "USER") == 0)
                ...
                if (fname)
                    ls_syslog(
                        LOG_ERR,
                        "%s:File %s %s at line %d: LOAD or WINDOW is missing",
                        pname, fname, section, *lineNum);
                lsberrno = LSBE_CONF_WARNING;
                return -1;
            }
...
static int checkAllOthers(char *word, int *hasAllOthers)
{
    char **grpHosts;
...
static int getReserve(char *reserve, struct queueInfoEnt *qp, char *filename,
                      int lineNum)
{
    ...
    if ((sp = strstr(reserve, "MAX_RESERVE_TIME")) != NULL) {
        ...
        if (*sp == '\0') {
            ls_syslog(LOG_ERR,
                      "%s: File %s in section Queue ending at line %d: "
                      "MAX_RESERVE_TIME is specified without period time; "
                      "ignoring SLOT_RESERVE",
        ...
        if (*sp != '[') {
            ls_syslog(LOG_ERR,
                      "%s: File %s in section Queue ending at line %d: "
                      "MAX_RESERVE_TIME <%s> is specified without '['; "
                      "ignoring SLOT_RESERVE",
                      fname, filename, lineNum, reserve);
            lsberrno = LSBE_CONF_WARNING;
            return -1;
        }
        cp = ++sp;
        ...
        if (*sp == '\0' || (*sp != ']' && *sp != ' ')) {
            ls_syslog(LOG_ERR,
                      "%s: File %s in section Queue ending at line %d: "
                      "MAX_RESERVE_TIME is specified without ']'; ignoring "
                      "SLOT_RESERVE",
                      fname, filename, lineNum);
            lsberrno = LSBE_CONF_WARNING;
            return -1;
        ...
        if (*sp == ' ') {
            ...
            if (*sp != ']') {
                ls_syslog(LOG_ERR,
                          "%s: File %s in section Queue ending at line %d: "
                          "MAX_RESERV_TIME is specified without ']';ignoring "
                          "SLOT_RESERVE",
                          fname, filename, lineNum);
                lsberrno = LSBE_CONF_WARNING;
                return -1;
        ...
        *sp = '\0';
        ...
        if (qp->slotHoldTime == INFINIT_INT) {
            ls_syslog(LOG_ERR,
                      "%s: File %s in section Queue ending at line %d: Value "
                      "<%s> of MAX_RESERV_TIME for queue <%s> isn't an integer "
                      "between 1 and %d; ignored.",
                      fname, filename, lineNum, cp, qp->queue, INFINIT_INT);
            lsberrno = LSBE_CONF_WARNING;
            *sp = ']';
            return -1;
        }
    ...
    if (strstr(reserve, "BACKFILL") != NULL) {
        if (qp->slotHoldTime == INFINIT_INT) {
            ls_syslog(
                LOG_ERR,
                "%s: File %s in section Queue ending at line %d: BACKFILL is "
                "specified without MAX_RESERV_TIME for SLOT_RESERVE; ignoring",
                fname, filename, lineNum);
            lsberrno = LSBE_CONF_WARNING;
            return -1;
        }
...
static int isServerHost(char *hostName)
{
    int i;
...
struct group *copyUnixGrp(struct group *unixGrp)
{
    ...
    if (!((unixGrpCopy->gr_mem) = (char **) calloc(sizeof(char *), count + 1)))
        goto errorCleanup;
...
static void addBinaryAttributes(char *confFile, int *lineNum,
                                struct queueInfoEnt *queue,
    ...
    if (keylist->val != NULL) {
        if ((strcasecmp(keylist->val, "y") == 0) ||
            ...
            if ((strcasecmp(keylist->val, "n") != 0) &&
                ...
                ls_syslog(
                    LOG_ERR,
                    "%s: File %s in section Queue ending at line %d: %s value "
                    "<%s> isn't one of 'Y', 'y', 'n' or 'N'; ignored",
...
static int resolveBatchNegHosts(char *inHosts, char **outHosts, int isQueue)
{
        ...
        if (word[0] == '~') {
            ...
            if (isHostName(word) == false) {
                ...
                if ((strcmp(word, grpMembers[0]) == 0) &&
                    ...
                    ls_syslog(LOG_ERR, "%s: host/group name \"%s\" is ignored.",
                              "resolveBatchNegHosts()", word);
                    lsberrno = LSBE_CONF_WARNING;
                    continue;
                }
                ...
                for (j = 0; j < num; j++) {
                    ...
                    if (((neg_num - cConf.numHosts) % cConf.numHosts) == 0) {
                        outTable =
                            realloc(outTable, (cConf.numHosts + neg_num) *
                                                  sizeof(char *));
                        if (!outTable)
                            goto error_clean_up;
                    }
                ...
                if (((neg_num - cConf.numHosts) % cConf.numHosts) == 0) {
                    outTable = realloc(outTable, (cConf.numHosts + neg_num) *
                                                     sizeof(char *));
                    if (!outTable)
                        goto error_clean_up;
                }
            ...
            if (!strcmp(word, "all")) {
                ...
                if (miniTableSize - inTableSize >= 0) {
                    inTable =
                        realloc(inTable, (cConf.numHosts + miniTableSize) *
                ...
                if (!strcmp(grpMembers[0], "all")) {
                    ...
                    for (j = 0; j < num; j++) {
                        cur_size = fillCell(&inTable[in_num], grpMembers[j],
                                            ptr_level);
                        if (!cur_size) {
                            goto error_clean_up;
                        }
                        ...
                        if (in_num - inTableSize >= 0) {
                            inTable =
                                realloc(inTable, (cConf.numHosts + in_num) *
                                                     sizeof(struct inNames *));
                            if (!inTable) {
                                goto error_clean_up;
                            } else {
                ...
                if (in_num - inTableSize >= 0) {
                    inTable = realloc(inTable, (cConf.numHosts + in_num) *
                                                   sizeof(struct inNames *));
                    if (!inTable) {
                        goto error_clean_up;
                    } else {
    ...
    for (j = 0; j < neg_num; j++) {
        for (k = 0; k < in_num; k++) {
            ...
            if (inTable[k] && inTable[k]->name &&
                equal_host(inTable[k]->name, outTable[j])) {
...
static int expandWordAll(int *size, int *num, struct inNames **inTable,
                         char *ptr_level)
{
    int cur_size = 0;
    int j;
...
static int parseDefAndMaxLimits(struct keymap key, int *defaultVal, int *maxVal,
                                char *fname, int *lineNum, char *pname)
{
    char *sp;
    char *defaultLimit = NULL;
    ...
    if (maxLimit != NULL) {
        if ((*defaultVal = my_atoi(defaultLimit, INFINIT_INT, 0)) ==
            INFINIT_INT) {
            ls_syslog(
                LOG_ERR,
                "%s: File %s in section Queue at line %d: Default value <%s> "
                "of %s isn't a positive integer between 0 and %d; ignored.",
                pname, fname, *lineNum, defaultLimit, key.key, INFINIT_INT);
            lsberrno = LSBE_CONF_WARNING;
        }
    ...
    if ((*maxVal = my_atoi(maxLimit, INFINIT_INT, 0)) == INFINIT_INT) {
        ls_syslog(LOG_ERR,
                  "%s: File %s in section Queue at line %d: Maximum value <%s> "
                  "of %s isn't a positive integer between 0 and %d; ignored.",
                  pname, fname, *lineNum, maxLimit, key.key, INFINIT_INT);
        lsberrno = LSBE_CONF_WARNING;
    }

    FREEUP(defaultLimit);

    if ((*defaultVal != INFINIT_INT) && (*maxVal != INFINIT_INT) &&
        (*defaultVal > *maxVal)) {
        ls_syslog(LOG_ERR,
                  "%s: File %s in section Queue at line %d: The default %s %d "
                  "should not be greater than the max %d; ignoring the default "
                  "and using max value also as default %s",
                  pname, fname, *lineNum, key.key, *defaultVal, *maxVal,
                  key.key);
        *defaultVal = *maxVal;
    }
    return 0;
...
static int parseQFirstHost(char *myWord, int *haveFirst, char *pname,
                           int *lineNum, char *fname, char *section)
{
    struct groupInfoEnt *gp;
    int needCheck = true;
    ...
    if (chkFirstHost(myWord, &needCheck)) {
        if (*haveFirst) {
            ls_syslog(LOG_ERR,
                      "%s: File %s%s ending at line %d : Multiple first "
                      "execution hosts specified:<%s>; ignored.",
                      pname, fname, section, *lineNum, myWord);
            lsberrno = LSBE_CONF_WARNING;
            FREEUP(myWord);
            return 1;
        ...
        if (needCheck) {
            if (!strcmp(myWord, "others")) {
                ls_syslog(LOG_ERR,
                          "%s: File %s%s ending at line %d : \"others\" "
                          "specified as first execution host; ignored.",
                          pname, fname, section, *lineNum);
                lsberrno = LSBE_CONF_WARNING;
                FREEUP(myWord);
                return 1;
            ...
            if (!strcmp(myWord, "all")) {
                ls_syslog(LOG_ERR,
                          "%s: File %s%s ending at line %d : \"all\" specified "
                          "as first execution host; ignored.",
                          pname, fname, section, *lineNum);
                lsberrno = LSBE_CONF_WARNING;
                FREEUP(myWord);
                return 1;
            ...
            if (gp != NULL) {
                ls_syslog(LOG_ERR,
                          "%s: File %s%s ending at line %d : host group <%s> "
                          "specified as first execution host; ignored.",
                          pname, fname, section, *lineNum, myWord);
                lsberrno = LSBE_CONF_WARNING;
                FREEUP(myWord);
            ...
            ls_syslog(LOG_ERR,
                      "%s: File %s%s ending at line %d : Invalid first "
                      "execution host <%s>, not a valid host name; ignored.",
                      pname, fname, section, *lineNum, myWord);
            lsberrno = LSBE_CONF_WARNING;
            FREEUP(myWord);
            return 1;
...
int chkFirstHost(char *host, int *needChk)
{
