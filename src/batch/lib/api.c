...
int32_t llb_priority_job(int64_t job_id, int32_t priority)
{
    size_t bufsz =
        PACKET_HEADER_SIZE + sizeof(struct wire_job_priority) + LL_BUFSIZ_64;
    char *buf = malloc(bufsz);
    if (buf == NULL)
        return -1;

    struct wire_job_priority req;
    memset(&req, 0, sizeof(req));
    req.job_id = job_id;
    ...
    struct protocol_header hdr;
    init_protocol_header(&hdr);
    hdr.operation = BATCH_JOB_PRIORITY;
    hdr.status = MBD_OK;
    ...
    XDR xdrs;
    xdrmem_create(&xdrs, buf, (uint32_t) bufsz, XDR_ENCODE);
    if (!ll_encode_msg(&xdrs, (char *) &req, xdr_wire_job_priority, &hdr)) {
        xdr_destroy(&xdrs);
        free(buf);
        errno = EPROTO;
        return -1;
...
int32_t llb_service_start(const char *name, struct svc_info *out)
{
    ...
    struct protocol_header hdr;
    init_protocol_header(&hdr);
    hdr.operation = BATCH_SERVICE_START;
    hdr.status = MBD_OK;
