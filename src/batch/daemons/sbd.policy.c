/* $Id: sbd.policy.c,v 1.6 2007/08/15 22:18:46 tmizan Exp $
...
static void chkpntEnd(struct jobCard *, int, bool *);
...
void job_checking(void)
{
        ...
        if (jobCard->runTime >
            ...
            if ((jobCard->jobSpecs.terminateActCmd == NULL) ||
                ...
                if (jobCard->runTime >
                    ...
                    ls_syslog(LOG_INFO, "%s: sending warning signal to job=%d",
                              fname, jobCard->jobSpecs.jobId);
    ...
    if (myload == NULL) {
        if (myStatus != NO_LIM)
            ...
            ls_syslog(LOG_INFO, "%s", __func__, "ls_loadofhosts");
    ...
    savedLoad.li =
        (float *) my_malloc(allLsInfo->numIndx * sizeof(float), "job_checking");
...
static void tryChkpntMig(void)
{
    ...
    for (jobCard = jobQueHead->forw; jobCard != jobQueHead; jobCard = nextJob) {
        ...
        if ((jobCard->jobSpecs.jStatus & JOB_STAT_SSUSP) && !migrating &&
            ...
            (now - jobCard->lastChkpntTime > jobCard->migCnt * sbdSleepTime) &&
        ...
        if (!(jobCard->jobSpecs.jStatus & JOB_STAT_MIG) &&
            ...
            jobCard->jobSpecs.actPid == 0 && jobCard->jobSpecs.chkPeriod &&
            now - jobCard->lastChkpntTime > jobCard->jobSpecs.chkPeriod) {
            if (jobSigStart(jobCard, SIG_CHKPNT, 0, jobCard->jobSpecs.chkPeriod,
                            SIGLOG) == 0) {
                sbdlog_newstatus(jobCard);
                continue;
            }
...
static void tryResume(struct hostLoad *myload)
{
    ...
    for (jobCard = jobQueHead->back; jobCard != jobQueHead; jobCard = next) {
        ...
        if (jobCard->jobSpecs.numToHosts == 1) {
            ...
            load = ls_loadofhosts("-", &numh, EFFECTIVE, 0, hostList->names,
                                  hostList->listSize);
...
static void tryStop(char *myhostnm, struct hostLoad *myload)
{
    ...
    for (jobCard = jobQueHead->forw; jobCard != jobQueHead; jobCard = next) {
        ...
        if (jobCard->jobSpecs.numToHosts == 1) {
            ...
            if ((jobCard->jobSpecs.jStatus & JOB_STAT_RUN) &&
                now >= jobCard->jobSpecs.startTime + sbdSleepTime) {
                if (shouldStop(load, jobCard, &reasons, &subreasons, numh,
                               &stopmore)) {
                      ...
                      int *reasons, int *subreasons, int num, int *stopmore)
{
    ...
    *subreasons = 0;
    ...
    for (i = 0; i < jobCard->jobSpecs.numToHosts && (*reasons) == 0; i++) {
        ...
        if (load == NULL) {
            ls_syslog(LOG_ERR,
                      "%s: Can not find load information for host <%s>", fname,
                      jobCard->jobSpecs.toHosts[i]);
            return false;
        }
        ...
        for (j = MEM + 1;
            ...
            if (load->li[j] >= INFINITY || load->li[j] <= -INFINITY ||
                jobCard->jobSpecs.thresholds.loadStop[numLoad][j] >= INFINITY ||
                jobCard->jobSpecs.thresholds.loadStop[numLoad][j] <=
                    -INFINITY) {
                continue;
            }
        ...
        if (!(*reasons) && jobCard->stopCondVal != NULL) {
            ...
            if (returnCode >= 0 &&
                evalResReq(jobCard->stopCondVal->selectStr, &tclHostData,
                           DFT_FROMTYPE) == 1) {
    ...
    if (LS_ISLOCKEDU(load->status) || LS_ISLOCKEDM(load->status)) {
        ...
        if (logclass & (LC_SCHED | LC_EXEC))
            ls_syslog(LOG_DEBUG2,
                      "%s: Should stop job %s; reason=%x, subreasons=%d", fname,
                      lsb_jobid2str(jobCard->jobSpecs.jobId), *reasons,
                      *subreasons);
...
static int shouldStop1(struct hostLoad *loadV)
{
    int rcnt = 0;
    struct jobCard *jp;
...
static int shouldResume(struct hostLoad *loadV, struct jobCard *jp, int num)
{
    ...
    if (logclass & (LC_SCHED | LC_EXEC))
        ls_syslog(
            ...
            fname, lsb_jobid2str(jp->jobSpecs.jobId), jp->jobSpecs.jStatus,
            jp->jobSpecs.reasons, jp->jobSpecs.subreasons, num);
    ...
    loads = (struct hostLoad *) my_malloc(num * sizeof(struct hostLoad), fname);
    if (jp->resumeCondVal != NULL) {
        tclHostData = (struct tclHostData *) my_malloc(
            num * sizeof(struct tclHostData), fname);
        for (i = 0; i < num; i++) {
    ...
    for (j = 0; j < jp->jobSpecs.numToHosts; j++) {
        if (j > 0 &&
            !strcmp(jp->jobSpecs.toHosts[j], jp->jobSpecs.toHosts[j - 1]))
            continue;
        numHosts++;
        found = false;
        for (i = 0; i < num; i++) {
            if (equal_host(jp->jobSpecs.toHosts[j], loadV[i].hostName)) {
                ...
                if (tclHostData != NULL) {
                    if (getTclHostData(&loadV[i], &tclHostData[numHosts],
                                       false) < 0) {
                        break;
                    }
                }
        ...
        if (found != true) {
            ls_syslog(LOG_ERR,
                      "%s: Can not find load information for host <%s> to "
                      "check resume condiftions for job <%s>",
                      fname, jp->jobSpecs.toHosts[j],
    ...
    if (numHosts >= 0) {
        numHosts++;
        resume = checkResumeByLoad(
            jp->jobSpecs.jobId, numHosts, jp->jobSpecs.thresholds, loads,
            &jp->jobSpecs.reasons, &jp->jobSpecs.subreasons,
        ...
        ls_syslog(LOG_ERR,
                  "%s: No valid load information is found for job <%s>", fname,
                  lsb_jobid2str(jp->jobSpecs.jobId));
    }
...
int job_resume(struct jobCard *jp)
{
    ...
    if (logclass & (LC_TRACE | LC_SCHED | LC_EXEC))
        ls_syslog(LOG_DEBUG1, "%s: Resume job %s", fname,
                  lsb_jobid2str(jp->jobSpecs.jobId));
    return 0;
}
...
static int jobResumeAction(struct jobCard *jp, int sigValue, int suspReason)
{
    ...
    if (logclass & (LC_TRACE | LC_SCHED | LC_EXEC))
        ls_syslog(LOG_DEBUG1,
                  "%s: Try to resume job %s with the current reason %d and the "
                  "triggered reason %d;",
...
void jobSuspendAction(struct jobCard *jp, int sigValue, int suspReasons,
                      int suspSubReasons)
{
    ...
    if (logclass & (LC_TRACE | LC_SCHED | LC_EXEC))
        ls_syslog(LOG_DEBUG1,
                  "%s: Suspend job %s; reasons=%x, subresons=%d, sigValue=%d, "
                  "status=%x",
                  fname, lsb_jobid2str(jp->jobSpecs.jobId),
    ...
    cc = jobSigStart(jp, sigValue + jp->jobSpecs.sigMap[-(sigValue)], 0, 0,
                     SIGLOG);
    sbdlog_newstatus(jp);
}
...
static void sigActEnd(struct jobCard *jobCard)
{
    ...
    if (jobCard->jobSpecs.actValue < 0) {
        sprintf(exitFile, "%s/.%s.%s.%s", LSTMPDIR, jobCard->jobSpecs.jobFile,
    ...
    default:
        ls_syslog(LOG_ERR,
                  "sigActEnd: unknown sigValue <%d> for job <%s> at the job "
                  "status <%d> with actPid <%d>",
...
static void chkpntEnd(struct jobCard *jobCard, int w_status, bool *freed)
{
    ...
    if (jobCard->jobSpecs.jStatus & JOB_STAT_MIG) {
        if (w_status == 0) {
            ...
            if (jobCard->cleanupPid == 0) {
                ...
                ls_syslog(LOG_ERR, "%s: Unable to cleanup migrating job <%s>",
...
static int rmJobBufFilesPid(struct jobCard *jp)
{
...
static int cleanupMigJob(struct jobCard *jp)
{
    ...
    if (postJobSetup(jp) == -1) {
        ls_syslog(LOG_ERR, "%s", __func__, lsb_jobid2str(jp->jobSpecs.jobId),
                  "postJobSetup");
        lsb_merr2(
            "Unable to setup the environment for job <%s> to run the queue's "
            "post exec.  Please run <%s> manually if necessary.\n",
            lsb_jobid2str(jp->jobSpecs.jobId), jp->jobSpecs.postCmd);
        exit(-1);
    }
...
static int getTclHostData(struct hostLoad *load,
                          struct tclHostData *tclHostData, int freeMem)
{
    ...
    if (now - lastUpdHostInfo > 10 * 60) {
        if ((temp = ls_gethostinfo("-:server", &num, 0, 0, LOCAL_ONLY)) ==
            NULL) {
        ...
        hostInfo =
            (struct hostInfo *) my_malloc(num * sizeof(struct hostInfo), fname);
        for (i = 0; i < num; i++) {
            copyLsfHostInfo(&hostInfo[i], &temp[i]);

            if (logclass & LC_TRACE) {
                ls_syslog(LOG_DEBUG2,
                          "%s: host <%s> ncpus <%d> maxmem <%u> maxswp <%u> "
                          "maxtmp <%u> ndisk <%d>",
    ...
    tclHostData->resBitMaps =
        getResMaps(hostInfo[i].nRes, hostInfo[i].resources);
    ...
    tclHostData->loadIndex[R15S] =
        (hostInfo[i].cpuFactor != 0.0)
            ? ((load->li[R15S] + 1.0) / hostInfo[i].cpuFactor)
            : load->li[R15S];
    tclHostData->loadIndex[R1M] =
        (hostInfo[i].cpuFactor != 0.0)
            ? ((load->li[R1M] + 1.0) / hostInfo[i].cpuFactor)
            : load->li[R1M];
    tclHostData->loadIndex[R15M] =
        (hostInfo[i].cpuFactor != 0.0)
            ? ((load->li[R15M] + 1.0) / hostInfo[i].cpuFactor)
            : load->li[R15M];
...
static void ruLimits(struct jobCard *jobCard)
{
    struct rlimit rlimit;

    rlimitDecode_(&jobCard->jobSpecs.lsfLimits[LSF_RLIMIT_CPU], &rlimit,
                  LSF_RLIMIT_CPU);
    ...
    rlimitDecode_(&jobCard->jobSpecs.lsfLimits[LSF_RLIMIT_SWAP], &rlimit,
                  LSF_RLIMIT_SWAP);
    ...
    rlimitDecode_(&jobCard->jobSpecs.lsfLimits[LSF_RLIMIT_PROCESS], &rlimit,
                  LSF_RLIMIT_PROCESS);
    ...
    if ((lsbJobMemLimit == 1) ||
        ...
        rlimitDecode_(&jobCard->jobSpecs.lsfLimits[LSF_RLIMIT_RSS], &rlimit,
                      LSF_RLIMIT_RSS);
        if (rlimit.rlim_cur != RLIM_INFINITY) {
            if ((long) (rlimit.rlim_cur / 1024) <
                (long) jobCard->runRusage.mem) {
                if (jobCard->jobSpecs.jStatus & JOB_STAT_KILL) {
                } else {
...
static void initTclHostData(struct tclHostData *tclHostData)
{
    if (tclHostData == NULL)
        return;
