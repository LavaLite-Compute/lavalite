/* $Id: mbd.dep.c,v 1.9 2007/08/15 22:18:45 tmizan Exp $
...
static struct jData **matchJobs(char *, char *, int *, int *, struct jobIdx **,
                                int64_t *, int *);
...
struct dptNode *parseDepCond(char *dependCond, struct lsfAuth *auth,
                             int *replyCode, char **badName, int *jFlags,
                             int flags)
{
    ...
    *replyCode = LSBE_DEPEND_SYNTAX;
    while ((token = getToken(&dependCond, &tokenType)) != NULL) {
        ...
        case DPT_ENDED:
            ...
            if ((jobRec =
                     matchJobs(tempToken, auth->lsfUserName, &numJob, replyCode,
                               &jobIdx, &notExistArrayJob, &opFlag)) == NULL) {
            ...
            if (numJob > 1) {
                nodeList = (struct dptNode **) my_calloc(
                    numJob, sizeof(struct dptNode *), "parseDepCond");
        ...
        case DPT_NAME:
            if (flags & WINDOW_CONF)
                goto Error;
            if ((jobRec =
                     matchJobs(token, auth->lsfUserName, &numJob, replyCode,
                               &jobIdx, &notExistArrayJob, NULL)) == NULL) {
                if (badName && (*badName))
        ...
        case DPT_NUMENDED: {
            ...
            if ((DPT_NUMPEND <= tokenType) && (tokenType <= DPT_NUMENDED)) {
                ...
                if (tempType == DPT_COMMA) {
                    if (!(tempToken = getToken(&dependCond, &tempType)) ||
                        !((tempType >= DPT_GT && tempType <= DPT_NE) ||
                          tempType == DPT_NAME || strcmp(tempToken, "*") == 0))
                        goto Error;
                    if (tempType >= DPT_GT && tempType <= DPT_NE) {
...
int evalDepCond(struct dptNode *node, struct jData *jobRec)
{
    ...
    case DPT_EXIT: {
        ...
        if (jpbw->nodeType == JGRP_NODE_ARRAY) {
            if (node->dptJobIdx && (ptr = node->dptJobIdx->depJobList) &&
...
%s: job %s has one to one dependency but dptJobIdx is NULL",
                                  fname, lsb_jobid2str(jobRec->jobId));
                        node->value = DP_INVALID;
                        return node->value;
                    }

                    listSetIteratorAttach(node->dptJobIdx->depJobList, &iter);
                    for (ptr1 = listSetIteratorBegin(&iter); ptr1 != NULL;
        ...
        while (jpbw) {
            ...
            default:
                ls_syslog(LOG_ERR,
                          "At line %d of file %s, should never be here",
                          __LINE__, __FILE__);
            }
...
void freeDepCond(struct dptNode *dptNode)
{
#define FREEDEP(n)                                                             \
    {                                                                          \
        if (n)                                                                 \
...
void resetDepCond(struct dptNode *dptNode)
{
...
static char *getToken(char **sp, dptType *type)
{
...
static struct dptNode *newNode(dptType nodeType, void *data)
{
    struct dptNode *node;
...
static int mergeNode(int level)
{
...
static struct Stack *initStackDep(void)
{
    ...
    stack->top = -1;
    stack->nodes = (struct dptNode **) my_malloc(5 * sizeof(struct dptNode *),
                                                 "initStackDep");
    stack->size = 5;
    return stack;
}

static int pushStackDep(struct Stack *stack, struct dptNode *node)
{
...
static struct dptNode *popStackDep(struct Stack *stack)
{
    if (stack->top < 0)
        return NULL;
...
static void freeStackDep(struct Stack *stack, int freeNodes)
{
    int i;
...
static struct jData **matchJobs(char *jobp, char *lsfUserName, int *numFoundJob,
                                int *replyCode, struct jobIdx **jobIdx,
                                int64_t *element, int *flag)
{
    ...
    if (idxList) {
        (*jobIdx) =
            (struct jobIdx *) my_malloc(sizeof(struct jobIdx), "matchJobs");
    ...
    *element = 0;
    numJob = 0;
    foundJobRec = (struct jData **) my_calloc(numRec, sizeof(struct jData *),
                                              "matchJobs");
    ...
    if (jobId) {
        ...
        if (foundJobRec[0]) {
            ...
            while (idxList != NULL) {
                ...
                for (i = idxList->start; i <= idxList->end;
                     i += idxList->step) {
                    if ((jpbw = getJobData(LSB_JOBID(jobId, i)))) {
                        (*jobIdx)->depJobList = listSetInsert(
    ...
    for (nPtr = parent->child; nPtr; nPtr = nPtr->right) {
        ...
        if (jpbw->uPtr == uPtr) {
            ...
            if (numJob >= numRec) {
                tempJobRec = (struct jData **) realloc(
                    foundJobRec, 2 * numRec * sizeof(struct jData *));
                if (tempJobRec == NULL) {
...
checkIdx:
    ...
    if (flag != NULL && *flag == ARRAY_DEP_ONE_TO_ONE &&
        ...
        if ((depArrayIdx == NULL || submitIdxList == NULL) ||
            ...
            *replyCode = LSBE_DEP_ARRAY_SIZE;
            ...
            (*jobIdx) = (struct jobIdx *) my_calloc(1, sizeof(struct jobIdx),
                                                    "matchJobs");
            for (i = depArrayIdx->start; i <= depArrayIdx->end;
                 i += depArrayIdx->step) {
                struct jData *jPtr;

                if ((jPtr = getJobData(LSB_JOBID(jobid, i)))) {
                    (*jobIdx)->depJobList = listSetInsert(
                        (long) createjDataRef(jPtr), (*jobIdx)->depJobList);
                }
            }
        }
...
static int opExpr(int opType, int operand1, int operand2)
{
    switch (opType) {
    case DPT_LT:
...
static int getCounterOfDep(int type, int *counts)
{
    ...
    default:
        ...
        ls_syslog(LOG_ERR, "%s: dep Type <%d> out of bound", fname, type);
        return 0;
    }
}

static struct jobIdx *createjobIdxRef(struct jobIdx *jobIdx)
{
    if (jobIdx) {
        jobIdx->numRef++;
    }
...
static void destroyjobIdxRef(struct jobIdx *jobIdx)
{
...
static int evalJobDep(struct dptNode *node, struct jData *jpbw,
                      struct jData *depJob)
{
