/* $Id: bhosts.c,v 1.5 2007/08/15 22:18:43 tmizan Exp $
...
struct indexFmt fmt1[] = {{"r15s", "%6s", "*%4.1", "%5.1", 1.0, 6, "f", "g"},
                          {"r1m", "%6s", "*%4.1", "%5.1", 1.0, 6, "f", "g"},
                          {"r15m", "%6s", "*%4.1", "%5.1", 1.0, 6, "f", "g"},
                          {"ut", "%6s", "*%4.0", "%6.0", 100.0, 6, "f%%",
                           "g%%"},
...
static void prtHostsLong(int numReply, struct hostInfoEnt *hInfo)
{
    ...
    for (i = 0; i < numReply; i++) {
        ...
        else if (hPtr->hStatus &
                 (HOST_STAT_BUSY | HOST_STAT_WIND | HOST_STAT_DISABLED |
                  HOST_STAT_EXCLUSIVE | HOST_STAT_LOCKED |
                  HOST_STAT_LOCKED_MASTER | HOST_STAT_FULL | HOST_STAT_NO_LIM))
        ...
        sprintf(fomt, "%%s%%%dd %%%dd %%%dd %%%dd %%%dd ", HOST_NJOBS_LENGTH,
                HOST_RUN_LENGTH, HOST_SSUSP_LENGTH, HOST_USUSP_LENGTH,
                HOST_RSV_LENGTH);

        printf(fomt, maxJobs, hPtr->numJobs, hPtr->numRUN, hPtr->numSSUSP,
               hPtr->numUSUSP, hPtr->numRESERVE);
        ...
        if (!(hPtr->hStatus & (HOST_STAT_UNAVAIL | HOST_STAT_UNREACH))) {
            ...
            if (lsbSharedResConfigured_) {
                ...
                if (retVal > 0) {
                    int start = 0;
                    int end;
                    while ((end = getDispLastItem(nameTable, start, retVal)) >
                               start &&
                           start < retVal) {
                        printf(" %11s", " ");
                        for (j = start; j < end; j++)
        ...
        if (printThresholds(hPtr->loadSched, hPtr->loadStop, hPtr->busySched,
                            hPtr->busyStop, MIN(hInfo->nIdx, lsInfo->numIndx),
                            lsInfo) < 0)
            continue;
        printf("\n");
...
static void prtHostsShort(int numReply, struct hostInfoEnt *hInfo)
{
    struct hostInfoEnt *hPtr;
    int i;
    char *status, maxJobs[MAX_CHARLEN], userJobLimit[MAX_CHARLEN];
    ...
    for (i = 0; i < numReply; i++) {
        ...
        if (hPtr->hStatus & HOST_STAT_UNAVAIL)
            ...
            status = ("unreach");
        else if (hPtr->hStatus & (HOST_STAT_BUSY | HOST_STAT_WIND |
                                  HOST_STAT_DISABLED | HOST_STAT_EXCLUSIVE |
                                  HOST_STAT_LOCKED | HOST_STAT_LOCKED_MASTER |
...
static void sort_host(int replyNumHosts, struct hostInfoEnt *hInfo)
{
    int i, j, k;
...
static int repeatHost(int currentNum, struct hostInfoEnt *hInfo)
{
    int i;
...
static void prtLoad(struct hostInfoEnt *hPtrs, struct lsInfo *lsInfo)
{
    ...
    if (!fmt) {
        if (!(fmt = (struct indexFmt *) malloc((lsInfo->numIndx + 2) *
                                               sizeof(struct indexFmt)))) {
...
static char *formatHeader(char **dispindex, int start, int end)
{
...
static int makeFields(struct hostInfoEnt *host, char *loadval[],
                      char **dispindex, int option)
{
    int j, id, nf, index;
    char *sp;
        ...
        if (load >= INFINITY)
            ...
            if (strlen(sp) > fmt[id].dispLen) {
                if (load > 1024)
                    ...
                    sprintf(fmtField, "%s%s", firstFmt, fmt[id].normFmt);
                if ((load > 1024) && ((!strcmp(fmt[id].name, "mem")) ||
...
static char **formLINamesList(struct lsInfo *lsInfo)
{
    int i;
    ...
    if (names == NULL)
        if ((names = (char **) malloc((lsInfo->numIndx + 1) *
                                      sizeof(char *))) == NULL) {
            lserrno = LSE_MALLOC;
            ls_perror(NULL);
            exit(-1);
...
static float getLoad(char *dispindex, float *loads, int *index)
{
    int i;
...
static void getCloseString(int hStatus, char **status)
{
...
static void displayShareRes(int argc, char **argv, int index)
{
    ...
    TIMEIT(
        0,
        (lsbResourceInfo = lsb_sharedresourceinfo(resources, &numRes, NULL, 0)),
...
static void prtResourcesShort(int num, struct lsbSharedResourceInfo *info)
{
    struct lsInfo *lsInfo;
    int i, j, k;
    ...
    buf1 = putstr_("RESOURCE");
    buf2 = putstr_("TOTAL");
    buf3 = putstr_("RESERVED");
    buf4 = putstr_("LOCATION");
    ...
    for (i = 0; i < num; i++) {
        for (j = 0; j < info[i].nInstances; j++) {
            for (k = 0; k < lsInfo->nRes; k++) {
                if (strcmp(lsInfo->resTable[k].name, info[i].resourceName) ==
                    0) {
...
static void prtOneInstance(char *resName,
                           struct lsbSharedResourceInstance *instance)
{
#define ONE_LINE 80
    int i, len, currentPos = 52;
    char space52[] = "                                                    ";
    printf("%-20s%10s%15s       ", resName, stripSpaces(instance->totalValue),
...
static int makeShareFields(char *hostname, struct lsInfo *lsInfo,
